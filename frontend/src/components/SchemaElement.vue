...
import { TypeNodeContentDeepType } from "@/state/fragments";
import { ArrowRightIcon } from "@heroicons/vue/20/solid";
import { computed } from "vue";

const props = defineProps<{
  element: FragmentType<typeof TypeNodeContentDeepType>;
  omitName?: boolean;
}>();
const element = computed(() => useFragment(TypeNodeContentDeepType, props.element));
const children = computed(() => element.value?.elements?.map((e) => useFragment(TypeNodeContentDeepType, e)) ?? []);
