...
- thus, it is worth remembering that at the end of that period, most animals were dead, and the surviving species looked rather different from what had been dominant before.  
- it's not hard to see how today's software - and its production process - kinda suck.
- we've got a troubled artisinal process: 
- dependency sprawl
- uncertain timelines
- supply chain attacks
- and if lucky, at the end, we are rewarded with slow, buggy, clunky software!
<!--- very hard to run software locally-->

[that futuristic city](TODO software if we knew what we're doing)

- we're not going to get _great_ software by averaging the past. 
- now that we produce much more software than any one could ever meaningfully review, how could we make it great and as accessible as possible?
...
<!--homoiconic software-->
- to manufacture high quality software with precision, we must finally put the "engineering" into "software engineering"
...
- over the past decade or so, opinionated formatters have standardized
...
- Coding is solved, yet somehow, bugs are not solved yet.
- Well, define "coding", define "solved"?
- if we define coding as 
- if we increasingly work through other tools, why invest in a language?
- it's a fair question: why should we even think about code at all if it can all be written by AI anyway?
...
- code, beautiful though it may be, is in itself entirely inert and useless.
...
- mostly, code is the current way we have of writing down the (alleged) solution to some (perceived) problem that can be addressed in bits.
- software is code solving a problem, and a product packages
...
- in the olden days of 12 months ago, we used to solve problems with artisinal human-directed next-character-prediction.
- now we can often work at a higher level.
...
- but we must still understand and align on some level, or the software sits in some weird liminal space in the sky that serves nobody
- (well over a year of "vibe coding" has shown this pretty conclusively)
...
- non-code higher order programming isn't even new.
...
- in programming, a minimal, simple language like C or even Go is considered "elegant", while others might describe C++ as "ugly"
- it's about using the least parts to get the most done, with deliberate "no" to the last 20%.
- unfortunately, _minimal_ is eventually not quite enough.
...
- if software is going to become even more ubiquotous and ambitious, how do we fill thos elast 20%
