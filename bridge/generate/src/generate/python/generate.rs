...
pub(in crate::generate) fn generate(root: &Path, schema: &Schema) -> Result<()> {
    ...
    write_text(
        root,
        "bridge/python/src/destack/_generated/__init__.py",
        facade,
    )?;
    ...
    write_text(
        root,
        "bridge/python/src/destack/_generated/__init__.pyi",
        stub,
    )?;

    let stub = render_native_stub(schema);
    write_text(root, "bridge/python/src/destack/_native.pyi", stub)?;

    let facade = render_workspace_package_facade();
    write_text(
        root,
        "bridge/python/src/destack/workspace/__init__.py",
        facade,
    )?;
    ...
    write_text(
        root,
        "bridge/python/src/destack/workspace/__init__.pyi",
        stub,
    )?;
    ...
    write_text(
        root,
        "bridge/python/src/destack/workspace/workspace.py",
        facade,
    )?;
    ...
    write_text(
        root,
        "bridge/python/src/destack/workspace/workspace.pyi",
        stub,
    )?;
...
pub(in crate::generate) fn generate_protocol(root: &Path, schema: &Schema) -> Result<()> {
    let protocol_root = root.join("bridge/python/src/destack/protocol");
    if protocol_root.exists() {
        prune_protocol_outputs(&protocol_root)?;
    }

    let generated_protocol_root = root.join("bridge/python/src/destack/_generated/protocol");
