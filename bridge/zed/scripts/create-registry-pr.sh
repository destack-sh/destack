...
DESTACK_ZED_REGISTRY_PUSH_TO="${DESTACK_ZED_REGISTRY_PUSH_TO:-flotothemoon/zed-extensions}"
...
DESTACK_SOURCE_COMMIT="$(git -C "${DESTACK_REPOSITORY_ROOT}" rev-parse "${DESTACK_ZED_SOURCE_REF}^{commit}" 2>/dev/null || true)"
if [ -z "${DESTACK_SOURCE_COMMIT}" ]; then
...
git -C "${DESTACK_REGISTRY_DIRECTORY}/${DESTACK_ZED_EXTENSION_SUBMODULE_PATH}" fetch origin "${DESTACK_SOURCE_COMMIT}" >/dev/null 2>&1 || true

# resolve a publishable commit when the local source ref is not on origin yet
if ! git -C "${DESTACK_REGISTRY_DIRECTORY}/${DESTACK_ZED_EXTENSION_SUBMODULE_PATH}" cat-file -e "${DESTACK_SOURCE_COMMIT}^{commit}" 2>/dev/null; then
    if is_dry_run; then
        DESTACK_SOURCE_COMMIT="$(git -C "${DESTACK_REGISTRY_DIRECTORY}/${DESTACK_ZED_EXTENSION_SUBMODULE_PATH}" rev-parse origin/HEAD^{commit})"
        info "dry run: source commit not found on origin, falling back to ${DESTACK_SOURCE_COMMIT}"
    else
