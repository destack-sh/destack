...
def encode_image(writer: BinaryWriter, value: Image) -> None:
    ...
    if value.kind == "resident":
        ...
        destack._generated.program.native.library.encode_library(writer, value.library)
    elif value.kind == "object":
        writer.write_unsigned(2)
        destack._generated.program.native.object.encode_object(writer, value.object)
    else:
...
def decode_image(reader: BinaryReader) -> Image:
    ...
    if variant == 0:
        ...
        library = destack._generated.program.native.library.decode_library(reader)

        return ImageLibrary(library=library)
    elif variant == 2:
        object = destack._generated.program.native.object.decode_object(reader)
...
def to_json_image(value: Image) -> Json:
    ...
    if value.kind == "resident":
        ...
        return {
            "kind": "library",
            "library": destack._generated.program.native.library.to_json_library(
                value.library
            ),
        }
    elif value.kind == "object":
        return {
            "kind": "object",
            "object": destack._generated.program.native.object.to_json_object(
                value.object
            ),
        }
    else:
...
def from_json_image(value: Json) -> Image:
    ...
    if kind == "resident":
        ...
        return ImageLibrary(
            library=destack._generated.program.native.library.from_json_library(
                json_field(object_, "library")
            )
        )
    elif kind == "object":
        return ImageObject(
            object=destack._generated.program.native.object.from_json_object(
