...
import type { CounterId } from "../metadata/profile.js";
...
import type { TensorConvertMode } from "./tensor.js";
...
import type { VectorConvertMode } from "./vector.js";
...
import { decodeCounterId, encodeCounterId, fromJsonCounterId, toJsonCounterId } from "../metadata/profile.js";
...
import { decodeTensorConvertMode, encodeTensorConvertMode, fromJsonTensorConvertMode, toJsonTensorConvertMode } from "./tensor.js";
...
import { decodeVectorConvertMode, encodeVectorConvertMode, fromJsonVectorConvertMode, toJsonVectorConvertMode } from "./vector.js";
...
export type Instruction =
    ...
    | {
          readonly kind: "frameAllocZeroed";
    ...
    | {
          readonly kind: "frameAllocUninit";
          /** The SSA value to define with the frame allocation pointer. */
          readonly destination: Value;
          /** The type of the value to allocate. */
          readonly layout: LocalNodeId;
...
export const Instruction = {
    ...
    /** Project the function pointer from one function value (function.pointer). */
    functionPointer(destination: Value, function_: Value): Instruction {
        return { kind: "functionPointer", destination, function: function_ };
    },
    ...
    /** Construct a struct from field values. */
    struct(destination: Value, ty: LocalNodeId, fields: ValueSlice): Instruction {
        return { kind: "struct", destination, ty, fields };
    },

    /** Construct a tuple from element values. */
    tuple(destination: Value, ty: LocalNodeId, elements: ValueSlice): Instruction {
        return { kind: "tuple", destination, ty, elements };
    },

    /** Construct an array from element values. */
    array(destination: Value, ty: LocalNodeId, elements: ValueSlice): Instruction {
        return { kind: "array", destination, ty, elements };
    },

    /** Extract a static layout slot from an aggregate value (field.get). */
    fieldGet(destination: Value, aggregate: Value, index: number): Instruction {
        return { kind: "fieldGet", destination, aggregate, index };
    },

    /** Insert a value into a static layout slot (field.set). */
    fieldSet(destination: Value, aggregate: Value, index: number, value: Value): Instruction {
        return { kind: "fieldSet", destination, aggregate, index, value };
    },

    /** Get the address of a static layout slot from an addressable aggregate (field.address). */
    fieldAddr(destination: Value, aggregate: Value, index: number, resultType: LocalNodeId): Instruction {
        return { kind: "fieldAddr", destination, aggregate, index, resultType };
    },

    /** Get the address of an element from an addressable indexed value (element.address). */
    elementAddr(destination: Value, array: Value, index: Value, resultType: LocalNodeId): Instruction {
        return { kind: "elementAddr", destination, array, index, resultType };
    },
    ...
    /** Extract the payload selected by a concrete variant tag. */
    variantPayload(destination: Value, variant: Value, tag: Constant): Instruction {
        return { kind: "variantPayload", destination, variant, tag };
    },
    ...
    /** Convert vector element types with an explicit mode. */
    vectorConvert(destination: Value, mode: VectorConvertMode, vector: Value): Instruction {
    ...
    /** Convert a tensor element type. */
    tensorConvert(destination: Value, mode: TensorConvertMode, tensor: Value): Instruction {
        return { kind: "tensorConvert", destination, mode, tensor };
    },

    /** Call a function directly. */
    call(destination: Value | undefined, function_: LocalNodeId, call: Call): Instruction {
        return { kind: "call", destination, function: function_, call };
    },

    /** Call a virtual method through a virtual dispatch slot. */
    callVirtual(destination: Value | undefined, receiver: Value, class_: LocalNodeId, slot: DispatchSlot, call: Call): Instruction {
        return { kind: "callVirtual", destination, receiver, class: class_, slot, call };
    },

    /** Call through a dynamic dispatch table slot. */
    callDynamic(destination: Value | undefined, receiver: Value, constraint: LocalNodeId, slot: DispatchSlot, call: Call): Instruction {
        return { kind: "callDynamic", destination, receiver, constraint, slot, call };
    },

    /** Call through a function pointer (call.indirect). */
    callIndirect(destination: Value | undefined, callee: Value, call: Call): Instruction {
        return { kind: "callIndirect", destination, callee, call };
    },

    /** Allocate zeroed typed heap storage (`new.zeroed`). */
    newZeroed(destination: Value, layout: LocalNodeId, resultType: LocalNodeId): Instruction {
        return { kind: "newZeroed", destination, layout, resultType };
    },

    /** Allocate uninitialized typed heap storage (`new.uninit`). */
    newUninit(destination: Value, layout: LocalNodeId, resultType: LocalNodeId): Instruction {
        return { kind: "newUninit", destination, layout, resultType };
    },
    ...
    /** Allocate zeroed frame-scoped storage (`frame.alloc.zeroed`). */
    frameAllocZeroed(destination: Value, layout: LocalNodeId, resultType: LocalNodeId): Instruction {
        return { kind: "frameAllocZeroed", destination, layout, resultType };
    },

    /** Allocate uninitialized frame-scoped storage (`frame.alloc.uninit`). */
    frameAllocUninit(destination: Value, layout: LocalNodeId, resultType: LocalNodeId): Instruction {
        return { kind: "frameAllocUninit", destination, layout, resultType };
    },
    ...
    /** Record one profiled runtime value. */
    profileValue(counter: CounterId, value: Value): Instruction {
        return { kind: "profileValue", counter, value };
    },
...
export function encodeInstruction(writer: BinaryWriter, value: Instruction): void {
    switch (value.kind) {
        ...
        case "struct":
            ...
            encodeValueSlice(writer, value.fields);
            return;
        case "tuple":
            writer.writeUnsigned(18);
            encodeValue(writer, value.destination);
            encodeLocalNodeId(writer, value.ty);
            encodeValueSlice(writer, value.elements);
            return;
        case "array":
        ...
        case "elementAddr":
            ...
            encodeValue(writer, value.destination);
            encodeValue(writer, value.array);
        ...
        case "free":
            ...
            encodeValue(writer, value.value);
            return;
        case "frameAllocZeroed":
            writer.writeUnsigned(71);
            encodeValue(writer, value.destination);
            encodeLocalNodeId(writer, value.layout);
            encodeLocalNodeId(writer, value.resultType);
            return;
        case "frameAllocUninit":
...
export function decodeInstruction(reader: BinaryReader): Instruction {
    ...
    switch (variant) {
        ...
        case 17: {
            ...
            const fields = decodeValueSlice(reader);
        ...
        case 18: {
            const destination = decodeValue(reader);
            const ty = decodeLocalNodeId(reader);
            const elements = decodeValueSlice(reader);
        ...
        case 23: {
            const destination = decodeValue(reader);
            const array = decodeValue(reader);
    ...
    switch (value.kind) {
        ...
        case "struct":
            return {
                ...
                fields: toJsonValueSlice(value.fields),
            };
        case "tuple":
            return {
                kind: "tuple",
                destination: toJsonValue(value.destination),
                ty: toJsonLocalNodeId(value.ty),
                elements: toJsonValueSlice(value.elements),
            };
        case "array":
            return {
                kind: "array",
    ...
    switch (kind) {
        ...
        case "struct":
            return {
                ...
                fields: fromJsonValueSlice(jsonField(object, "fields")),
            };
        case "tuple":
            return {
                kind,
                destination: fromJsonValue(jsonField(object, "destination")),
                ty: fromJsonLocalNodeId(jsonField(object, "ty")),
                elements: fromJsonValueSlice(jsonField(object, "elements")),
            };
        case "array":
            return {
                kind,
        ...
        case "elementAddr":
            return {
                kind,
                destination: fromJsonValue(jsonField(object, "destination")),
                array: fromJsonValue(jsonField(object, "array")),
...
/** Kind of type cast. */
export type CastOperator = "bitcast" | "truncate" | "zeroExtend" | "signExtend" | "floatToSignedInt" | "floatToUnsignedInt" | "floatToSignedIntSaturating" | "floatToUnsignedIntSaturating" | "signedIntToFloat" | "unsignedIntToFloat" | "floatTruncate" | "floatExtend" | "floatConvert" | "pointerToInt" | "intToPointer";
