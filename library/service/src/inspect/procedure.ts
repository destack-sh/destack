...
export const PayloadDescription = defineSchema(schema.union([
    schema.object({
...
export const ProcedureDescription = defineSchema(schema.object({
...
function visit(
    ...
    if (ancestors.has(service)) throw new TypeError(`Cyclic service definition: ${name.join(".")}`);
    ...
    if (isContractProcedure(service)) {
        ...
        procedures.push(ProcedureDescription.parse({
            name,
        ...
        // allow the same router at multiple addresses while detecting ancestor cycles
...
function describeSchema(validator: AnySchema): schema.Infer<typeof JsonSchema> {
    if (!(validator instanceof schema.Schema)) throw new TypeError("Expected a Destack schema.");
