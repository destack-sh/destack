...
export const roleBinding = table("role_binding", {
    ...
    check(
        ...
        sql`(${binding.accountMembershipId} IS NOT NULL) + (${binding.groupId} IS NOT NULL) + (${binding.serviceAccountId} IS NOT NULL) + (${binding.accountServiceAccountId} IS NOT NULL) = 1`,
    ),
    check(
        "role_binding_expiry",
