...
export const spaceConfiguration = table("space_configuration", {
    /** The configured space; source management is optional and selects one source. */
    spaceId: identifier("space_id", "space").primaryKey().notNull().references(() => space.id),
    ...
    check(
        "space_configuration_generation",
        sql`${entry.generation} > 0`,
    ),
    check(
...
export const stackRevision = table("stack_revision", {
    ...
    unique("stack_revision_source_generation").on(entry.spaceId, entry.id, entry.sourceGeneration),
