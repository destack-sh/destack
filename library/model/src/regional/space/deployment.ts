...
export const deployment = table("deployment", {
    ...
    state: text("state", { enum: ["prepared", "active", "draining", "retired"] }).notNull().default(
        "prepared",
    ),
    ...
    foreignKey({
        ...
        foreignColumns: [serviceAccount.installationId, serviceAccount.workload, serviceAccount.id],
    }).onDelete("restrict"),
    ...
    check("deployment_state", sql`${entry.state} IN ('prepared', 'active', 'draining', 'retired')`),
    check(
        "deployment_times",
