...
import { Conditions } from "../resource/condition.ts";
...
export const spaceMigration = table("space_migration", {
    ...
    requestedBy: identifier("requested_by", "user").references(() => user.id),
    ...
    foreignKey({
        columns: [migration.accountId, migration.sourceHostId],
        foreignColumns: [hostAccess.accountId, hostAccess.hostId],
    }).onDelete("restrict"),
    foreignKey({
        columns: [migration.accountId, migration.targetHostId],
        foreignColumns: [hostAccess.accountId, hostAccess.hostId],
    }).onDelete("restrict"),
