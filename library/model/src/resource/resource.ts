...
import { reconcileChecks, reconcileColumns } from "./reconcile.ts";
...
export const resource = table("resource", {
    ...
    requestedRegionId: identifier("requested_region_id", "region").references(() => region.id),
    ...
    foreignKey({
        columns: [resource.definitionPackageId, resource.definitionVersion],
        foreignColumns: [release.packageId, release.version],
    }).onDelete("restrict"),
    foreignKey({
        columns: [resource.providerCode, resource.regionId],
        foreignColumns: [region.providerCode, region.id],
    }),
    foreignKey({
        columns: [resource.requestedProviderCode, resource.requestedRegionId],
        foreignColumns: [region.providerCode, region.id],
    }),
    check(
    ...
    ...reconcileChecks("resource", resource),
    ...
    foreignKey({
        columns: [resource.accountId, resource.hostId],
        foreignColumns: [hostAccess.accountId, hostAccess.hostId],
    }).onDelete("restrict"),
    foreignKey({
        columns: [resource.accountId, resource.requestedHostId],
        foreignColumns: [hostAccess.accountId, hostAccess.hostId],
    }).onDelete("restrict"),
    check(
        "resource_provider",
