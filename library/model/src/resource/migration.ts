...
export const resourceMigration = table("resource_migration", {
    ...
    sourceHostId: identifier("source_host_id", "host").references(() => host.id, {
        onDelete: "restrict",
    }),
    ...
    targetHostId: identifier("target_host_id", "host").references(() => host.id, {
        onDelete: "restrict",
    }),
    ...
    uniqueIndex("resource_migration_active").on(migration.resourceId).where(
