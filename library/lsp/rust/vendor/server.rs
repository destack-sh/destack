...
impl<R: Read, W: Write + Send + 'static> Server<R, W> {
	pub fn new(reader: R, writer: W, socket: super::service::Socket) -> Self { Self { reader, writer, socket } }
				...
				Ok(req) => {
					...
					std::thread::spawn(move || {
						...
						match method.as_str() {
							...
							"initialized" => {
								let p: lsp_types::InitializedParams = serde_json::from_value(JsonValue::Null).unwrap_or(lsp_types::InitializedParams {});
