...
impl From<CompilerOptions> for compiler::CompilerOptions {
    fn from(options: CompilerOptions) -> Self {
        compiler::CompilerOptions {
            ...
            retain_comptime_comment_max_length: usize::try_from(
                options.retain_comptime_comment_max_length,
            )
            .unwrap_or(usize::MAX),
...
#[wasm_bindgen(js_name = defaultCompilerOptions)]
pub fn default_compiler_options() -> Result<JsValue, JsValue> {
...
mod tests {
    ...
    /// Keep the compiler options mapping stable between wasm and core options.
    #[test]
    fn test_compiler_options_roundtrip() {
        let core_options = compiler::CompilerOptions::default();
        let wasm_options: CompilerOptions = core_options.clone().into();
        let mapped_core: compiler::CompilerOptions = wasm_options.into();

        assert_eq!(core_options.workers, mapped_core.workers);
        assert_eq!(core_options.follow_imports, mapped_core.follow_imports);
        assert_eq!(core_options.resolve_mode, mapped_core.resolve_mode);
        assert_eq!(
            core_options.default_int_width,
            mapped_core.default_int_width
        );
        assert_eq!(
            core_options.default_float_width,
            mapped_core.default_float_width
        );
        assert_eq!(core_options.emit_dry_run, mapped_core.emit_dry_run);
    }
}
