...
provider "aws" {
  region = local.aws_region_by_destack_region[var.region]
}
...
resource "aws_vpc" "region_vpc" {
  ...
  tags = {
    Name = "destack-${var.env}-${var.region}-region-vpc"
  }
}
...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name                     = "destack-${var.env}-${var.region}-public-subnet-${count.index + 1}"
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name                              = "destack-${var.env}-${var.region}-private-subnet-${count.index + 1}"
...
resource "aws_internet_gateway" "region_vpc" {
  ...
  tags = {
    Name = "destack-${var.env}-${var.region}-internet-gateway"
  }
}
...
resource "aws_eip" "region_vpc" {
  ...
  tags = {
    Name = "destack-${var.env}-${var.region}-eip"
  }
}
...
resource "aws_route_table" "public" {
  ...
  tags = {
    Name = "destack-${var.env}-${var.region}-public-route-table"
  }
}
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name = "destack-${var.env}-${var.region}-private-route-table"
  }
}
...
module "cluster_0" {
  ...
  cluster_name    = "destack-${var.env}-${var.cloud}-${var.region}-0"
  cluster_version = "1.32"
  iam_role_name   = "destack-${var.env}-${var.region}"
  vpc_id          = aws_vpc.region_vpc.id
  ...
  eks_managed_node_groups = {
    "destack-${var.env}-${var.cloud}-${var.region}-system" = {
  ...
  cluster_tags = {
    Name = "destack-${var.env}-${var.region}-0"
  }
}
...
resource "kubernetes_secret" "image_pull_secret" {
  metadata {
    name      = "destack-${var.env}-${var.cloud}-${var.region}-image-pull-secret"
    namespace = "default"
  }
...
resource "kubernetes_secret" "web_certificate_secret" {
  metadata {
    name      = "destack-${var.env}-${var.cloud}-${var.region}-cert-secret"
    namespace = "default"
  }
...
resource "aws_s3_bucket" "destack_files" {
  bucket = "destack-${var.env}-${var.cloud}-${var.region}-files"
  tags = {
    Name = "destack-${var.env}-${var.cloud}-${var.region}-files"
  }
}
# CORS (allow all)
resource "aws_s3_bucket_cors_configuration" "destack_files_cors" {
  bucket = aws_s3_bucket.destack_files.id
