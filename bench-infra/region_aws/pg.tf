...
resource "aws_security_group" "regional_pg_security_group" {
  name        = "bench-${var.env}-${var.region}-regional-pg-security-group"
...
resource "aws_db_subnet_group" "regional_pg_subnet_group" {
  name       = "bench-${var.env}-${var.region}-regional-pg-subnet-group"
...
resource "aws_rds_cluster" "regional_pg_primary" {
  cluster_identifier        = "bench-${var.env}-${var.region}-regional-pg-primary"
  ...
  final_snapshot_identifier = "bench-${var.env}-${var.region}-regional-pg-final-snapshot"
...
resource "aws_rds_cluster_instance" "regional_pg_primary_instance" {
  count                      = 1
  identifier                 = "bench-${var.env}-${var.region}-regional-pg-${count.index}"
