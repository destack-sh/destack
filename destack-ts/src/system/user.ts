...
import { destack, canvas, goToDestack } from "@/system/space";
...
provideCommands<"user">({
  ...
  "user.navigate.goToHome": {
    icon: "fas fa-home",
    title: "Go Home",
    text: "Go back to your Destack.",
    isEnabled: () => {
      return isAuthenticated.value;
    },
    command: async () => {
      if (destack.value?.id == user.value!.destackPtr?.id) {
        toaster.success({
          icon: "fas fa-home",
          title: "Already home",
          text: "You are already on your Destack.",
          override: "user.navigate.goToHome",
        });
      } else {
        await goToDestack({ destack: user.value!.destackPtr! as TypedNodeReferenceData<NodeType.SPACE> });
      }
    },
  },
});
