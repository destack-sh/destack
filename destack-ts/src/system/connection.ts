...
function getScopeFromParams<T extends NodeType>(
  ...
  if ("roots" in params && params.roots.length > 0) return makeScope({ destackId: params.roots[0].destackId });
  if ("bases" in params && (params.bases?.length ?? 0) > 0) return makeScope({ destackId: params.bases![0].destackId });
  // NOTE: scope defaults to current package (not sure if this is right.. probably want to make it explicit)
  return makeScope({ destackId: packagePtr.value?.destackId });
}
...
function makeConnectionOverlayGraph(
  ...
  const sub = connection.txBuffer.subscribeBuffer((event) => {
    ...
    if (edits.length > 0) {
      ...
      //   e.g., we have Destack.memberships loaded but not Thread.memberships since Threads are unloaded, so those Memberships may be missing)
 ...
 * NOTE :Architecture: the graphs and the current destack/pkg/space pointers are not atomically updated,
...
const _activeRemoteDestackIds = computed(() => {
  const destackIds = new Set<string>();
  _activeRemoteConnections.value.map((c) => destackIds.add(c.params.scope.destackId));
  return [...destackIds];
});

export async function sendRemoteKeepAlives() {
  const healthChecks = [];
  for (const destackId of _activeRemoteDestackIds.value) {
    const transport = getGraphTransport({ destackId });
    if (transport == null) continue;
    const healthClient = new HealthClient(transport);
    healthChecks.push(
      healthClient.check({ service: destackId != null ? "symbol.destack.Host" : "symbol.destack.Supervisor" }),
    );
  }
