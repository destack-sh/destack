...
export class Follow extends Entity implements IsOwned {
  ...
  get precededBy(): Follow | null {
    ...
    if (nodePtr != null) {
      return this._session.graph.get(nodePtr.id) as Follow | null;
    }
    return null;
  }
...
export class FollowEvent extends Event {
  ...
  constructor(options: {
    ...
    if (_client === null) {
      throw new Error(`FollowEvent.client is required`);
    }
    this.clientPtr = _client as NodeReference;
    let _clientNonce = options.clientNonce ?? null;
    if (_clientNonce === null) {
      throw new Error(`FollowEvent.clientNonce is required`);
    }
