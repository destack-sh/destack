...
import { NodeType, AgentData, TypeKind, DestackType, FieldType, TypeData } from "@/proto/wire";
...
export function agentToType(
  ...
  if (of == "instance") {
    return makeType({ kind: TypeKind.BASED_NODE, destackType: DestackType.RUN, baseTypePtr: toNodeRef(agent) });
  } else {
