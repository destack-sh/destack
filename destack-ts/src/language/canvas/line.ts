...
export class LineShape extends Shape {
   ...
   */
  get predecessor(): LineShape | null {
  ...
  set points(value: Array<Vector2f>) {
    const oldValue = this.#points;
    if (this._dirty == null) {
      this._dirty = {};
    }
    if (this._dirty["points"] === undefined) {
      this._dirty["points"] = oldValue;
    }
    if (!this._session.dirty[this.id]) {
      this._session.dirty[this.id] = this;
    }
  ...
  constructor(options: {
    ...
    predecessor?: LineShape | NodeReference | null;
    template?: LineShape | NodeReference | null;
