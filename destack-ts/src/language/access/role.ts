...
export abstract class RoleEvent extends Event {
  ...
  declare readonly actorRef: NodeReference;
...
export class RoleAssignedEvent extends RoleEvent {
  ...
  get actor(): Entity | null {
    const nodeRef: NodeReference | null = this.actorRef;
    if (nodeRef != null) {
      return this._session.graph.get(nodeRef) as Entity | null;
    }
    return null;
  }
  readonly actorRef: NodeReference;
  ...
  constructor(options: {
    ...
    this.actorRef = _actor as NodeReference;
  ...
  equals(other: any): boolean {
    ...
    if (!(this.actorRef.id === other.actorRef.id)) {
      return false;
    }
  ...
  hash(): number {
    ...
    h = (h * 31 + hashString(this.actorRef.id)) & 0xffffffff;
