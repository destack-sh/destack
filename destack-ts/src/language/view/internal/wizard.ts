...
export class WizardView extends Node implements InternalView {
  ...
  constructor(options: {
    ...
    if (_materialization === null) {
      throw new Error(`WizardView.materialization is required`);
    }
    ...
    if (_orderKey === null) {
      throw new Error(`WizardView.orderKey is required`);
    }
    this.orderKey = _orderKey;
    let _name = options.name;
    if (_name === null) {
      throw new Error(`WizardView.name is required`);
    }
    this.name = _name;
  ...
  static __packValue__(object: WizardView): { [key: string]: any } {
  ...
  static fromValue(
    ...
    return WizardView.__unpackValue__(objectValue, _session, _supergraph, _graph, _connection);
  }

  toProto(): WizardViewProto {
    return WizardView.__packProto__(this);
  }

  static __packProto__(object: WizardView): WizardViewProto {
    const objectProto: Partial<WizardViewProto> = { metatype: 10650 };
    ...
    return objectProto as WizardViewProto;
  }

  static __unpackProto__(
    objectProto: WizardViewProto,
    _session?: Session | null,
    _supergraph?: Supergraph | null,
    _graph?: any | null,
    _connection?: any | null,
