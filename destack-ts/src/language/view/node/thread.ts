  ...
  implements Spatial, Entity, IsTracked, IsDeletable, IsOrdered, IsTaggable, IsScriptable, IsVisual, View, NodeView
{
  ...
  constructor(options: {
    ...
    this.draftReplyToPtr =
      options.draftReplyTo != null
        ? options.draftReplyTo.metatype == StructType.NODE_REFERENCE
          ? (options.draftReplyTo as NodeReference)
          : (options.draftReplyTo as Node).toRef()
        : null;
    this.scriptPtr =
      options.script != null
