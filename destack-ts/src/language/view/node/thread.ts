import { IsDeletable, SplitView, IsOrdered, CustomView, Dimension, View, Entity, MaterializationType, Canvas, NodeType, QueryConnection, StructType, Space, NodeReference, Graph, Script, User, IsTaggable, Struct, FrameView, Text, StructFrozen, PlaneShape, Spatial, BuiltinObject, EnumType, Scene, Layer, Agent, IsScriptable, Position, Window, activeSession, Message, Node, IsTracked, Supergraph, AnnotationShape, CustomViewDefinition, ACTIVE_SESSION, IsVisual, LabelView, Session, NodeView } from '@/language';
...
export class ThreadView extends Node implements Spatial, Entity, IsTracked, IsDeletable, IsOrdered, IsTaggable, IsScriptable, IsVisual, View, NodeView {
  ...
  constructor(options: {
    ...
    this.draftText = options.draftText ?? null;
    this.draftReplyToPtr = options.draftReplyTo != null ? (options.draftReplyTo.metatype == StructType.NODE_REFERENCE ? (options.draftReplyTo as NodeReference) : (options.draftReplyTo as Node).toRef()) : null;
