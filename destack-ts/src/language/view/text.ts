...
import { Align, Entity, Materialization, Node, NodeType, StructType } from "@destack/language/core";
...
import { AlignProto, MaterializationProto, TextViewProto } from "@destack/proto";
...
export class TextView extends ContentView {
  ...
  constructor(options: {
    ...
    predecessor?: TextView | NodeReference | null;
    template?: TextView | NodeReference | null;
    ...
    if (_space === null) {
      if (this._session === null) {
        throw new Error(`TextView has no session`);
      }
      if (this._session.spacePtr === null) {
        throw new Error(`TextView has no space`);
      }
