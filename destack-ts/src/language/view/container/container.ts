 ...
 */
export class ContainerView extends Node implements View, IsExtensible {
  ...
  constructor(options: {
    ...
    if (_orderKey === null) {
      throw new Error(`ContainerView.orderKey is required`);
    }
    this.orderKey = _orderKey;
    let _name = options.name;
    if (_name === null) {
      throw new Error(`ContainerView.name is required`);
    }
    this.name = _name;
  ...
  __toRef__(): NodeReference {
    return new NodeReference({
      nodeType: NodeType.CONTAINER_VIEW,
      id: this.id,
  ...
  repr(): string {
    ...
    return `<ContainerView '${this.path}' ${propertyReprs.join(" ")}>`;
  }

  toValue(): { [key: string]: any } {
    return ContainerView.__packValue__(this);
  }

  static __packValue__(object: ContainerView): { [key: string]: any } {
  ...
  static fromValue(
    ...
    return ContainerView.__unpackValue__(objectValue, _session, _supergraph, _graph, _connection);
  }

  toProto(): ContainerViewProto {
    return ContainerView.__packProto__(this);
  }

  static __packProto__(object: ContainerView): ContainerViewProto {
    const objectProto: Partial<ContainerViewProto> = { metatype: 10000 };
    ...
    return objectProto as ContainerViewProto;
  }

  static __unpackProto__(
    objectProto: ContainerViewProto,
    _session?: Session | null,
    _supergraph?: Supergraph | null,
    _graph?: any | null,
    _connection?: any | null,
  ...
  static fromProto(
    ...
    return ContainerView.__unpackProto__(objectProto, _session, _supergraph, _graph, _connection);
  }

  static fromProtoString(packedProtoString: string): ContainerView {
    const packedProtoBytes = base64Decode(packedProtoString);
    const packedProto = ContainerViewProto.fromBinary(packedProtoBytes);
