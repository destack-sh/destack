...
import { AlignProto, ContentViewProto } from "@destack/proto";
 ...
 * A content View.
 */
export class ContentView extends Node implements View {
  ...
  constructor(options: {
    ...
    if (_orderKey === null) {
      throw new Error(`ContentView.orderKey is required`);
    }
    this.orderKey = _orderKey;
    let _name = options.name;
    if (_name === null) {
      throw new Error(`ContentView.name is required`);
    }
    this.name = _name;
  ...
  __toRef__(): NodeReference {
    return new NodeReference({
      nodeType: NodeType.CONTENT_VIEW,
      id: this.id,
  ...
  repr(): string {
    ...
    return `<ContentView '${this.path}' ${propertyReprs.join(" ")}>`;
  }

  toValue(): { [key: string]: any } {
    return ContentView.__packValue__(this);
  }

  static __packValue__(object: ContentView): { [key: string]: any } {
  ...
  static fromValue(
    ...
    return ContentView.__unpackValue__(objectValue, _session, _supergraph, _graph, _connection);
  }

  toProto(): ContentViewProto {
    return ContentView.__packProto__(this);
  }

  static __packProto__(object: ContentView): ContentViewProto {
    const objectProto: Partial<ContentViewProto> = { metatype: 10200 };
    ...
    return objectProto as ContentViewProto;
  }

  static __unpackProto__(
    objectProto: ContentViewProto,
    _session?: Session | null,
    _supergraph?: Supergraph | null,
    _graph?: any | null,
    _connection?: any | null,
  ...
  static fromProto(
    ...
    return ContentView.__unpackProto__(objectProto, _session, _supergraph, _graph, _connection);
  }

  static fromProtoString(packedProtoString: string): ContentView {
    const packedProtoBytes = base64Decode(packedProtoString);
    const packedProto = ContentViewProto.fromBinary(packedProtoBytes);
