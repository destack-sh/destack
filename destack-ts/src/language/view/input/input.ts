...
import { InputViewProto } from "@destack/proto";
 ...
 * An input View.
 */
export class InputView extends Node implements View {
  ...
  constructor(options: {
    ...
    if (_orderKey === null) {
      throw new Error(`InputView.orderKey is required`);
    }
    this.orderKey = _orderKey;
    let _name = options.name;
    if (_name === null) {
      throw new Error(`InputView.name is required`);
    }
    this.name = _name;
  ...
  repr(): string {
    ...
    return `<InputView '${this.path}' ${propertyReprs.join(" ")}>`;
  }

  toValue(): { [key: string]: any } {
    return InputView.__packValue__(this);
  }

  static __packValue__(object: InputView): { [key: string]: any } {
  ...
  static fromValue(
    ...
    return InputView.__unpackValue__(objectValue, _session, _supergraph, _graph, _connection);
  }

  toProto(): InputViewProto {
    return InputView.__packProto__(this);
  }

  static __packProto__(object: InputView): InputViewProto {
    const objectProto: Partial<InputViewProto> = { metatype: 10400 };
    ...
    return objectProto as InputViewProto;
  }

  static __unpackProto__(
    objectProto: InputViewProto,
    _session?: Session | null,
    _supergraph?: Supergraph | null,
    _graph?: any | null,
    _connection?: any | null,
  ...
  static fromProto(
    ...
    return InputView.__unpackProto__(objectProto, _session, _supergraph, _graph, _connection);
  }

  static fromProtoString(packedProtoString: string): InputView {
    const packedProtoBytes = base64Decode(packedProtoString);
    const packedProto = InputViewProto.fromBinary(packedProtoBytes);
