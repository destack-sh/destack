...
/** Extract file info from a native File. Like in destack :ExtractFileInfo */
export async function extractFile(
  ...
  const file = makeNode({
    ...
    destackPtr: toNodeRef(destack),
    region: destack.region,
    source: FileSource.DESTACK,
    name,
    type,
    mimeType,
    format,
...
async function doUploadFiles(
  ...
  // get post URLs
  const scope = makeScope({ destackId: uploads[0].destack.id });
...
export function uploadFiles(
  ...
  const uploads: FileUpload[] = contents.map((content) => {
    ...
    const fileIdentity = nodeReference(NodeType.FILE, fileId, { ck: fileId, destackId: options.destack.id });
...
async function doDownloadFiles(downloads: FileDownload[]): Promise<void> {
  ...
  // get getURLs
  const downloadsByDestackId: Record<string, FileDownload[]> = groupByList(downloads, (d) => d.nodePtr.destackId!);
  const downloadPromises = Object.entries(downloadsByDestackId).map(async ([destackId, downloadsForDestack]) => {
    try {
      const scope = makeScope({ destackId });
      const host = await getHostClient({ id: destackId });
      const { response } = await host.downloadFiles({ scope, files: downloadsForDestack.map((d) => d.nodePtr) });
      return response.handles;
    } catch (e) {
      log.error("file.download.error", downloadsForDestack, e);
      for (const download of downloadsForDestack) {
