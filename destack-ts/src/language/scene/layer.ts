import { Script, Canvas, Layout, EnumType, Axis2, Grid, View, StructFrozen, IsOrdered, Team, Align, Agent, Vector2, StructType, QueryConnection, NodeType, Shadow, Distribute, IsScriptable, Supergraph, IsVisual, Session, Node, IsTaggable, ContainerView, IsOwnable, Organization, GridSpan, Fill, Corners, IsDeletable, IsTracked, Role, Dimension, Spatial, Border, Struct, MaterializationType, Graph, User, Insets, Icon, Scene, Entity, Position, Direction, Axis3, IsExtensible, BuiltinObject, Value, NodeReference, Space } from '@/language';
...
export class Layer extends Node implements Spatial, Entity, IsTracked, IsDeletable, IsExtensible, IsOrdered, IsOwnable, IsTaggable, IsScriptable, IsVisual, View, ContainerView {
  readonly id: string;
  get parent(): Scene | Canvas | null | null {
      const nodePtr: NodeReference | null = this.parentPtr;
      if (nodePtr !== null) {
          return this._supergraph.get(nodePtr.id) as Scene | Canvas | null | null;
      }
      return null;
  }
  ;
  ...
  __toRef__(): NodeReference {
    return new NodeReference(NodeType.LAYER, this.id, this.spacePtr?.id ?? null, null, this._supergraph);
  }
