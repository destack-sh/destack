...
import { type TypeIdentity, decodeTypeIdentity, encodeTypeIdentity } from "@/language/core/type";
import { packBuiltinObject, unpackBuiltinObject } from "@/language/core/value";
import { DestackType, NodeType, ObjectType, PrimitiveType, TypeKind } from "@/proto/wire";
import { describe, expect, test } from "vitest";

// the test data & targets are from the backend destack implementation
const TEST_TYPE_IDENTITIES: (Partial<TypeIdentity> & { kind: TypeKind; identityKey: string })[] = [
  ...
  { kind: TypeKind.NODE, destackType: DestackType.USER, isSecret: false, isList: true, identityKey: "N" },
  { kind: TypeKind.BASED_NODE, destackType: DestackType.FIELD, isSecret: false, isList: true, identityKey: "N" },
  { kind: TypeKind.STRUCT, destackType: DestackType.TEXT, isSecret: false, isList: false, identityKey: "sg7C" },
  { kind: TypeKind.ENUM, destackType: DestackType.OBJECT_TYPE, isSecret: true, isList: false, identityKey: "!ej4E" },
  {
    ...
    isSecret: false,
    isList: false,
    identityKey: "oAAAAAAAAAAAAA8//4ZFNS",
  },
  {
