...
import { ActionData, ActionType, DestackType, FieldType, TypeData, TypeKind } from "@/proto/wire";
...
export function actionToType(
  ...
  if (of == "instance") {
    return makeType({ kind: TypeKind.BASED_NODE, destackType: DestackType.RUN, baseTypePtr: toNodeRef(action) });
  } else if (of == "value") {
    if (action.type == ActionType.START) {
      ...
      if (fieldTypes.includes(FieldType.INPUT) || fieldTypes.includes(FieldType.OUTPUT)) {
        ...
        return makeType({
          ...
          baseTypePtr: basePtr,
          destackType: DestackType.ACTION,
