...
import { DestackType, DatabaseData, FieldType, NodeType, ObjectType, TypeData, TypeKind } from "@/proto/wire";
...
export function databaseToType(
  ...
  if (of == "instance") {
    return makeType({ kind: TypeKind.BASED_NODE, destackType: DestackType.RECORD, baseTypePtr: toNodeRef(database) });
  } else {
