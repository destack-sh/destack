  ...
  implements IsOwnable, IsJoinable, IsOrdered, IsStarable, IsFollowable, IsReactable
{
  ...
  get parent(): Space | Folder | null {
    ...
    if (nodeRef != null) {
      return this._session.graph.get(nodeRef) as Space | Folder | null;
    }
    return null;
  }
  ...
  get precededBy(): Folder | null {
    ...
    if (nodeRef != null) {
      return this._session.graph.get(nodeRef) as Folder | null;
    }
    return null;
  }
  ...
  get mainScene(): Scene | null {
    const nodeRef: NodeReference | null = this.mainSceneRef;
    if (nodeRef != null) {
  ...
  set mainScene(node: Scene | null) {
    if (node === null) {
      this.mainSceneRef = null;
    } else {
      this.mainSceneRef = node.toRef();
    }
  }
  /**
   * Folder.mainScene
   */
  get mainSceneRef(): NodeReference | null {
    return this._mainSceneRef;
  }
  set mainSceneRef(value: NodeReference | null) {
    const prop = (this.constructor as NodeClass).__properties__["main_scene"];
    this._session.updateSetProperty(this, prop, value);
    this._mainSceneRef = value;
  }
  _mainSceneRef: NodeReference | null;
  ...
  constructor(options: {
    ...
    this._mainSceneRef = _mainScene as NodeReference | null;
  ...
  equals(other: any): boolean {
    ...
    if (!(this._mainSceneRef?.id === other._mainSceneRef?.id)) {
      return false;
    }
  ...
  hash(): number {
    ...
    if (this._mainSceneRef != null) {
      h = (h * 31 + hashString(this._mainSceneRef.id)) & 0xffffffff;
    }
