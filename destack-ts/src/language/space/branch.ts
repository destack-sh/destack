...
import { BranchProto, MaterializationProto } from "@destack/proto";
 ...
 */
export class Branch extends Entity implements IsOwnable, IsDeletable {
  ...
  constructor(options: {
    ...
    predecessor?: Branch | NodeReference | null;
    ...
    if (_space === null) {
      if (this._session === null) {
        throw new Error(`Branch has no Session`);
      }
      _space = ACTIVE_SPACE.get();
      if (_space === null) {
        throw new Error(`Branch has no Space`);
      }
      _space = _space.toRef();
    }
    if (_space === null) {
