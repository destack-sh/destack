 ...
 * A Destack @handle.
 */
export class Handle extends Entity implements IsGlobal {
  ...
  constructor(options: {
    ...
    predecessor?: Handle | NodeReference | null;
    template?: Handle | NodeReference | null;
  ...
  static fromValue(
    ...
    return Handle.__unpackValue__(objectValue, _session, _supergraph, _graph, _connection);
  }

  toProto(): HandleProto {
    return Handle.__packProto__(this);
  }

  static __packProto__(object: Handle): HandleProto {
    const objectProto: Partial<HandleProto> = { metatype: 10200 };
  ...
  static fromProtoString(packedProtoString: string): Handle {
    const packedProtoBytes = base64Decode(packedProtoString);
    const packedProto = HandleProto.fromBinary(packedProtoBytes);
