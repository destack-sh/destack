...
export class Team extends Entity implements IsGlobal, IsOwner, IsJoinable {
   ...
   */
  get predecessor(): Team | null {
    const nodePtr: NodeReference | null = this.predecessorPtr;
    if (nodePtr !== null) {
      return this._supergraph.get(nodePtr.id) as Team | null;
    }
    return null;
  }
  ...
  constructor(options: {
    ...
    template?: Team | NodeReference | null;
    ...
    if (options.id == null) {
      ...
      if (options.createdAt == null || options.updatedAt == null) {
        throw new Error(`Team.createdAt and Team.updatedAt are required for existing Nodes`);
      }
  ...
  static __packProto__(object: Team): TeamProto {
    const objectProto: Partial<TeamProto> = { metatype: 12100 };
