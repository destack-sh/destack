 ...
 */
export class Service extends Entity implements IsOwnable, IsActor, IsRunnable {
  ...
  constructor(options: {
    ...
    if (_space === null) {
      ...
      if (_space === null) {
        throw new Error(`no active Space for Service`);
      }
      _space = _space.toRef();
    }
    if (_space === null) {
    ...
    if (_branch === null) {
      ...
      if (_branch === null) {
        throw new Error(`no active Branch for Service`);
      }
      _branch = _branch.toRef();
    }
    if (_branch === null) {
      throw new Error(`Service.branch is required`);
    }
    ...
    if (_snapshot === null) {
      ...
      if (_snapshot === null) {
        throw new Error(`no active Snapshot for Service`);
      }
      _snapshot = _snapshot.toRef();
    }
    if (_snapshot === null) {
      throw new Error(`Service.snapshot is required`);
    }
  ...
  toCson(): { [key: string]: any } {
    return Service.__packCson__(this);
  }

  static __packCson__(object: Service): { [key: string]: any } {
  ...
  static fromCson(
    ...
    return Service.__unpackCson__(objectCson, _session, _supergraph, _graph, _connection);
  }
  ...
  static fromProtoString(packedProtoString: string): Service {
    const packedProtoBytes = base64Decode(packedProtoString);
    const packedProto = ServiceProto.fromBinary(packedProtoBytes);
