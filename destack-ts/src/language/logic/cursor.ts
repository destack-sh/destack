...
export class EventCursor extends Cursor {
  ...
  constructor(options: {
    ...
    if (_orderKey === null) {
      throw new Error(`EventCursor.orderKey is required`);
    }
    ...
    if (_event != null && _event.constructor.name != "NodeReference") {
  ...
  static __unpackCson__(
    ...
    const unpackedEventPtr =
      eventPtrValue != undefined
        ? _NodeReference.fromCson(eventPtrValue, _session, _supergraph, _graph, _connection)
        : null;
  ...
  static fromCson(
    ...
    return EventCursor.__unpackCson__(objectCson, _session, _supergraph, _graph, _connection);
  }
...
export class ScreenCursor extends Cursor {
  ...
  constructor(options: {
    ...
    if (_orderKey === null) {
      throw new Error(`ScreenCursor.orderKey is required`);
    }
    this.orderKey = _orderKey;
    let _customValues = options.customValues ?? null;
    if (_customValues === null) {
  ...
  static __unpackCson__(
    ...
    const unpackedPosition =
      positionValue != undefined
        ? _Vector2i.fromCson(positionValue, _session, _supergraph, _graph, _connection)
        : null;
    const activeAtValue = objectCson["110"];
    const unpackedActiveAt =
      activeAtValue != undefined
  ...
  static fromCson(
    ...
    return ScreenCursor.__unpackCson__(objectCson, _session, _supergraph, _graph, _connection);
  }
