...
export function makeNode<T extends NodeType>(
  ...
  if (!options?.omit?.includes("id")) {
    if ("packagePtr" in properties) {
      ...
      } else if (nodeIn.metatype != NodeType.SPACE && (!("packagePtr" in nodeIn) || nodeIn.packagePtr == null)) {
  ...
  if (
    "destackPtr" in properties &&
    nodeIn.metatype != NodeType.SPACE &&
    !Object.prototype.hasOwnProperty.call(node, "destackPtr")
  ) {
    const destackId = node.parentPtr?.destackId ?? (node as any).packagePtr?.destackId;
    if (destackId == null) throw new Error(`missing destackId to make in-destack node ${NodeType[nodeIn.metatype]}`);
    (node as any).destackPtr = nodeReference(NodeType.SPACE, destackId);
  }
...
function _cloneNode<T extends AnyNodeData>(node: T, now: Timestamp): T {
  ...
  if ("destackPtr" in node) (clone as any).destackPtr = node.destackPtr;
...
export function instanceNode<T extends AnyNodeData>(
  ...
  if ("destackPtr" in instancedNode) {
    instancedNode.destackPtr = (options.parent as any).destackPtr;
  }
