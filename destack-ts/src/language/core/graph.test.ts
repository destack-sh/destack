...
const MESSAGE_TYPE_GENERATORS: Record<string, () => any> = {
  ...
  "symbol.destack.NodeReferenceData": () => ({ metatype: ObjectType.NODE_REFERENCE, id: v4(), nodeType: NodeType.USER }),
};
...
export function fabricate<T extends ObjectType>(
  ...
  function fabricateScalarProp(propName: string, field: FieldInfo | undefined, prop: PropertyInfo): any {
    ...
    if (options?.set != null && (options.set as any)[propName] !== undefined) {
      ...
      const destackType = OBJECT_TYPE_BY_MESSAGE_TYPE_NAME[field.T().typeName]!;
      const path = (options?.path ?? []).concat(metatype);
      value = fabricate(destackType, { path });
...
describe("merge nodes", () => {
  const destack = makeNode({ metatype: NodeType.SPACE, name: "test", slug: "test" });
  const pkg = makeNode({ metatype: NodeType.PACKAGE, destackPtr: toNodeRef(destack) });
