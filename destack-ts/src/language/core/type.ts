...
export function describeTypeIdentity(type: TypeIdentity & Partial<AnyNodeData>): string {
  ...
  if (type.destackType != null) typeParts.push(toCamelName(DestackType, type.destackType!));
...
export function typeIdentityEquals(a: TypeIdentity, b: TypeIdentity): boolean {
  if (a.primitiveType != null) {
    ...
    return a.baseTypePtr.id === b.baseTypePtr?.id && a.destackType == b.destackType;
  } else if (a.destackType != null) {
    if (a.destackType != b.destackType) {
      return false;
...
export function getPropertyType(prop: PropertyInfo | PropertyReferenceData): TypeIdentity {
  ...
  if (cached == null) {
    let kind: TypeKind;
    let destackType: DestackType | undefined;
    ...
    if ((prop.referenceNodes?.length ?? 0) > 0) {
      ...
      if (prop.referenceNodes == "any") {
        destackType = undefined;
      } else if ((prop.referenceNodes?.length ?? 0) > 1) {
        destackType = undefined;
        constraint = makeTypeConstraint({ ...constraint });
      } else {
        destackType = prop.referenceNodes![0] as unknown as DestackType;
      }
    } else if (prop.referenceStruct != null) {
      kind = TypeKind.STRUCT;
      destackType = prop.referenceStruct as unknown as DestackType;
    } else if (prop.enumType != null) {
      kind = TypeKind.ENUM;
      destackType = prop.enumType as unknown as DestackType;
...
export function encodeTypeIdentity(type: TypeIdentity): string {
  ...
  if (type.kind == TypeKind.PRIMITIVE) {
    ...
    value = encodeB64VLQ(type.destackType!);
    ...
    value = type.destackType != null ? encodeB64VLQ(type.destackType) : "";
  } else {
...
export function decodeTypeIdentity(key: string): TypeIdentity {
  ...
  if (kind === TypeKind.PRIMITIVE) {
    ...
    return { kind, destackType: decodeB64VLQ(value) as DestackType, isRequired: false, isList, isSecret };
    ...
    return {
      kind,
      destackType: value != "" ? (decodeB64VLQ(value) as DestackType) : undefined,
      isRequired: false,
      isList,
      isSecret,
    };
...
export function getConstrainedTypeName(type: TypeIdentity): string | null {
  const metatypeName = toCamelName(DestackType, type.destackType);
  return metatypeName;
}
...
export function getTypeName(field: Partial<TypeData>): string {
  ...
  if (field.kind == TypeKind.PRIMITIVE) {
    ...
    if (isNodeType(field.destackType)) {
      return getEnumTitle(EnumType.DESTACK_TYPE, field.destackType);
    } else if (field.destackType != null) {
      return getEnumTitle(EnumType.DESTACK_TYPE, field.destackType);
    } else {
    ...
    return field.destackType != null ? `${getEnumTitle(EnumType.DESTACK_TYPE, field.destackType)} Partial` : "Partial";
  } else {
