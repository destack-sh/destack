  ...
  implements IsTaggable, IsArchivable, IsDeletable, IsSourceable
{
   ...
   * The previous Entity this Entity is based on (from another Snapshot).
   */
  get predecessor(): CustomOption | null {
    const nodePtr: NodeReference | null = this.predecessorPtr;
    if (nodePtr !== null) {
   ...
   * CustomOption.group
   */
  get group(): CustomOptionGroup | null {
    const nodePtr: NodeReference | null = this.groupPtr;
    if (nodePtr !== null) {
      return this._supergraph.get(nodePtr.id) as CustomOptionGroup | null;
    }
    return null;
  }
  set group(node: CustomOptionGroup | null) {
    if (node === null) {
      this.groupPtr = null;
    } else {
  ...
  constructor(options: {
    ...
    name: string;
    icon?: Icon | null;
    group?: CustomOptionGroup | NodeReference | null;
    _session?: Session | null;
    _supergraph?: Supergraph | null;
    _graph?: Graph | null;
    _connection?: QueryConnection | null;
  ...
  static __packProto__(object: CustomOption): CustomOptionProto {
    const objectProto: Partial<CustomOptionProto> = { metatype: 120 };
    objectProto.id = String(object.id);
    if (object.parentPtr != null) {
      objectProto.parentPtr = object.parentPtr.toProto();
    }
...
/* ==== DESTACK_GENERATED_START:NODE:121 ==== */
/**
 * A CustomOptionGroup is a group of CustomOptions.
 */
export class CustomOptionGroup extends Entity implements IsArchivable, IsDeletable, IsSourceable {
  static metatype: NodeType = NodeType.CUSTOM_OPTION_GROUP;

  /**
   * CustomOptionGroup.parent
   */
  get parent(): CustomEnumDefinition | null {
    const nodePtr: NodeReference | null = this.parentPtr;
    if (nodePtr !== null) {
   ...
   * The previous Entity this Entity is based on (from another Snapshot).
   */
  get predecessor(): CustomOptionGroup | null {
    const nodePtr: NodeReference | null = this.predecessorPtr;
    if (nodePtr !== null) {
      return this._supergraph.get(nodePtr.id) as CustomOptionGroup | null;
    }
   ...
   * The template this Entity instance is based on (from the template tree).
   */
  get template(): CustomOptionGroup | null {
    const nodePtr: NodeReference | null = this.templatePtr;
    if (nodePtr !== null) {
      return this._supergraph.get(nodePtr.id) as CustomOptionGroup | null;
    }
  ...
  constructor(options: {
    ...
    materialization?: Materialization;
    snapshot?: Snapshot | NodeReference | null;
    predecessor?: CustomOptionGroup | NodeReference | null;
    template?: CustomOptionGroup | NodeReference | null;
    createdAt?: Temporal.ZonedDateTime;
    createdBy?: (Entity & IsSubject) | NodeReference | null;
    updatedAt?: Temporal.ZonedDateTime;
    updatedBy?: (Entity & IsSubject) | NodeReference | null;
    ...
    if (_materialization === null) {
      throw new Error(`CustomOptionGroup.materialization is required`);
    }
    this.materialization = _materialization;
    let _snapshot = options.snapshot ?? null;
    if (_snapshot != null && _snapshot.metatype != StructType.NODE_REFERENCE) {
    ...
    if (_orderKey === null) {
      throw new Error(`CustomOptionGroup.orderKey is required`);
    }
    this.orderKey = _orderKey;
    let _source = options.source ?? null;
    if (_source != null && _source.metatype != StructType.NODE_REFERENCE) {
    ...
    if (options.id == null) {
      ...
      if (options.createdAt == null || options.updatedAt == null) {
        throw new Error(
          `CustomOptionGroup.createdAt and CustomOptionGroup.updatedAt are required for existing Nodes`,
        );
      }
      this.createdAt = options.createdAt;
      this.createdByPtr =
  ...
  __toRef__(): NodeReference {
    const _NodeReference = STRUCT_CLASS_BY_TYPE[StructType.NODE_REFERENCE] as typeof NodeReference;
    return new _NodeReference({
      type: NodeType.CUSTOM_OPTION_GROUP,
      id: this.id,
      spaceId: this.spacePtr?.id ?? null,
      snapshotId: this.snapshotPtr?.id ?? null,
      _session: this._session,
  ...
  static fromValue(
    ...
    return CustomOptionGroup.__unpackValue__(
      objectValue,
      _session,
      _supergraph,
      _graph,
  ...
  toProto(): CustomOptionGroupProto {
    return CustomOptionGroup.__packProto__(this);
  }

  static __packProto__(object: CustomOptionGroup): CustomOptionGroupProto {
    const objectProto: Partial<CustomOptionGroupProto> = { metatype: 121 };
    objectProto.id = String(object.id);
    if (object.parentPtr != null) {
      objectProto.parentPtr = object.parentPtr.toProto();
    }
  ...
  static __unpackProto__(
    objectProto: CustomOptionGroupProto,
    _session?: Session | null,
    _supergraph?: Supergraph | null,
    _graph?: any | null,
    _connection?: any | null,
  ...
  static fromProto(
    ...
    return CustomOptionGroup.__unpackProto__(
      objectProto,
      _session,
      _supergraph,
      _graph,
  ...
  static fromProtoString(packedProtoString: string): CustomOptionGroup {
    const packedProtoBytes = base64Decode(packedProtoString);
    const packedProto = CustomOptionGroupProto.fromBinary(packedProtoBytes);
    return this.fromProto(packedProto);
  }
...
registerNodeClass(NodeType.CUSTOM_OPTION_GROUP, CustomOptionGroup);
/* ==== DESTACK_GENERATED_END:NODE:121 ==== */
