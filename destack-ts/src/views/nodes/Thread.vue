...
import { DESTACK_DESTACK_AGENT_PTR, DESTACK_DESTACK_UBUNTU_DESKTOP_PTR } from "@/language/core/builtin";
...
import { destackPtr, CURRENT_DESTACK_SCOPE, packagePtr } from "@/system/client";
import { SearchConnectionParams, useAutoConnection, useInfiniteSearchConnection } from "@/system/connection";
import { destack, destackConnection, destackGraph, canvas, pagePtr, pkg, space } from "@/system/space";
...
const isEnabled = computed(() => nodePtr.value != null && CURRENT_DESTACK_SCOPE.value?.destackId != null);
...
function submit() {
  ...
  if (nodePtr.value == null) {
    thread = createThread(tx, destackGraph, {
      thread: {
  ...
  if (pagePtr.value != null && !claims.value.some((c) => c.targetPtr?.id == pagePtr.value?.id)) {
    const claim = createClaim(tx, destackGraph, {
      claim: {
        type: ClaimType.WRITE,
        parentPtr: toNodeRef(thread),
        destackPtr: destackPtr.value,
    ...
    if (nodePtr.nodeType == NodeType.PAGE && !claims.value.some((c) => c.targetPtr?.id == nodePtr.id)) {
      const claim = createClaim(threadConnection.tx, threadGraph, {
        claim: {
          ...
          destackPtr: thread.value?.destackPtr,
...
async function addFiles(files: FileList | File[]) {
  Array.from(files).forEach(async (file) => {
    ...
    if (destack.value == null) throw new Error("no destack");
    ...
    const upload = uploadFile(() => destackConnection.tx, file, {
      ...
      >
        ...
        >
          ...
          <li
            ...
            :data-node-destack-id="(message as any).destackPtr?.id"
