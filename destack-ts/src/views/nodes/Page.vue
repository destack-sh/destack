...
import { destack, canvas, inspectionPtr, pkg } from "@/system/space";
...
function addFiles(files: FileList | File[], anchor: "before" | "after" | "inside", target: PageData | BlockData) {
  Array.from(files).forEach(async (file) => {
    ...
    const { block } = createBlock(connection.tx, graph, {
      block: { type: BlockType.NODE, nodePtr: toNodeRef(upload.file.value!) },
      anchor: anchor,
      target: target,
    });
...
function createAndFocusBlock(
  ...
  if (block.nodePtr?.nodeType == NodeType.PAGE) {
