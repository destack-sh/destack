...
const facetIcon = computed(() => {
  ...
  } else if (props.valueType?.destackType != null) {
    const icon =
      NodeTypeOptionInfo[props.valueType.destackType as unknown as NodeType]?.icon ??
      StructTypeOptionInfo[props.valueType.destackType as unknown as StructType]?.icon;
...
const nodePtrs: Ref<NodeReferenceData[]> = computedValue(() => {
  if (
    ...
    (props.valueType.kind != TypeKind.NODE && !isNodeType(props.valueType.destackType))
  ) {
    return []; // no nodes
