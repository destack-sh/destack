...
import { destackPtr } from "@/system/client";
import { canvas, goToDestack } from "@/system/space";
  ...
  try {
    if (stage.value == UserWizardViewStage.SIGN_UP) {
      ...
      try {
        ...
        await goToDestack({ destack: user.destackPtr as TypedNodeReferenceData<NodeType.SPACE> });
      ...
      // if we're outside a Destack and have a Destack, go home
      if (user.destackPtr != null && destackPtr.value == null) {
        await goToDestack({ destack: user.destackPtr as TypedNodeReferenceData<NodeType.SPACE> });
      }
    } else {
      throw new Error(`unexpected registration stage: ${stage.value}`);
...
<template>
  <div class="flex h-full flex-col divide-x divide-gray-200 lg:flex-row">
    ...
    >
      <!-- Header -->
      <div class="mb-3">
        <h2 class="mb-2 text-3xl font-semibold lg:text-4xl">Destack</h2>
        ...
        <div class="mt-5 space-y-2">
          ...
          <div v-if="stage == UserWizardViewStage.SIGN_UP">
            ...
            <Picker
              ...
              is-input
              :value-type="
                makeType({ kind: TypeKind.ENUM, destackType: DestackType.CONTINENT, isList: false, isRequired: true })
              "
            />
          </div>
          <div v-if="stage == UserWizardViewStage.SIGN_UP">
