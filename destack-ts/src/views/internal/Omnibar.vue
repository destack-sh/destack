...
import { destackGraph, canvas, hasLocalPkg } from "@/system/space";
...
const mode = ref<OmnibarMode>("destack");
const query = ref<"">("");
...
const indices = computed(() => {
  ...
  // commands
  if (["destack", "commands"].includes(mode.value)) {
    indices["Commands"] = COMMAND_INDEX;
  }

  // package
  if (packagePtr.value != null && hasLocalPkg.value && ["destack"].includes(mode.value)) {
    indices["Destack"] = graphIndex({
      id: "destack",
      graph: destackGraph,
      metatypes: [NodeType.PAGE, NodeType.THREAD, NodeType.DATABASE, NodeType.FLOW, NodeType.AGENT],
      roots: [destackGraph.getOrError(packagePtr.value)],
      skipDepth: 1,
      // only search deeply if in destack search specifically
      maxDepth: isQueryEmpty.value && mode.value != "destack" ? 1 : undefined,
...
function open(inMode: OmnibarMode = "destack") {
  isActive.value = true;
  clear();
  mode.value = inMode;
  updateCandidates();
      ...
      >
        ...
        >
          ...
          >
            ...
            <!-- Mode -->
            <span v-if="mode != 'destack'" class="font-semibold select-none">
            ...
            <input
              ...
              @keydown.delete="query.length > 0 || (mode = 'destack')"
            />
            <!-- Close -->
