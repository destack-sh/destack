...
export type Operation<I extends object, O extends object> = {
  transport: DestackGrpcWebTransport;
  id: number;
  name: string;
...
export const HUMANIZED_OPERATION_STATUS: { [key: string]: string } = {
  ...
  RESOURCE_EXHAUSTED: "Destack overloaded",
  UNAVAILABLE: "Destack unavailable",
...
export const HUMANIZED_OPERATION_MESSAGE: { [key: string]: string } = {
  ...
  UNAVAILABLE: "Destack could not be reached",
};
...
export type DestackServerStreamingCall<I extends object, O extends object> = ServerStreamingCall<I, O> & {
  operation: Operation<I, O>;
};

export type DestackUnaryCall<I extends object, O extends object> = UnaryCall<I, O> & {
  operation: Operation<I, O>;
};
 ...
 */
class DestackGrpcWebTransport extends GrpcWebFetchTransport {
  uri: string;
  ...
  ): DestackServerStreamingCall<I, O> {
    if (options.retry) throw new Error("serverStreaming does not support retry options");
    const call = super.serverStreaming(method, input, options) as DestackServerStreamingCall<I, O>;
  ...
  unary<I extends object, O extends object>(
    ...
    input: I,
    options: OperationOptions,
  ): DestackUnaryCall<I, O> {
    const call = super.unary(method, input, options) as DestackUnaryCall<I, O>;
    const op = operationsTracker.track({
      transport: this,
      method,
      request: input,
...
const currentMetadataEncoded: Ref<{ [key: string]: any }> = computed(() => {
  ...
  if (metadata.clientType) packed["x-destack-2"] = CLIENT_TYPE.toString();
  if (metadata.clientId) packed["x-destack-3"] = metadata.clientId;
  if (metadata.clientNonce) packed["x-destack-4"] = metadata.clientNonce;
  if (metadata.clientAccessToken) packed["x-destack-5"] = metadata.clientAccessToken;
  if (metadata.badges.length > 0) {
    ...
    packed["x-destack-6"] = btoa(JSON.stringify(packedBadges));
  }
  return packed;
});
...
const _CACHED_HOST_TRANSPORTS: Ref<{ [destackId: string]: DestackGrpcWebTransport }> = shallowRef({});
const _CACHED_HOST_CLIENTS: Ref<{ [destackId: string]: HostClient }> = shallowRef({});

export const supervisorTransport = new DestackGrpcWebTransport(SUPERVISOR_URL);
...
/**
 * Gets the Host for a given Destack (looking up host info via supervisor if not cached)
 * NOTE :Performance: cache resolved hosts across session in local storage?
 */
export async function getHostClient(destack: { id: string }): Promise<HostClient> {
  if ("id" in destack && _CACHED_HOST_CLIENTS.value[destack.id]) {
    return _CACHED_HOST_CLIENTS.value[destack.id];
  }

  // check if there's already a pending resolution for this destack
  const pendingResolution = _PENDING_HOST_RESOLUTIONS.get(destack.id);
  ...
  const resolutionPromise = (async () => {
    ...
    try {
      const { hosts: hostInfos } = await supervisor.resolveHosts({
        destackes: [{ destack: { oneofKind: "id", id: destack.id } }],
      }).response;
      const hostTransport = new DestackGrpcWebTransport(`${hostInfos[0].domain}:${hostInfos[0].grpcWebPort}`);
      const hostClient = new HostClient(hostTransport);

      _CACHED_HOST_CLIENTS.value[destack.id!] = hostClient;
      _CACHED_HOST_TRANSPORTS.value[destack.id!] = hostTransport;
      ...
      log.error("host.resolve.error", { destack, e });
      throw e;
    } finally {
      _PENDING_HOST_RESOLUTIONS.delete(destack.id);
    }
  })();
  _PENDING_HOST_RESOLUTIONS.set(destack.id, resolutionPromise);

  return resolutionPromise;
}

/** Gets a cached transport for the given scope */
export function getGraphTransport(scope?: Partial<GraphScopeData>): DestackGrpcWebTransport | undefined {
  if (scope?.destackId == null) return supervisorTransport;
  else return _CACHED_HOST_TRANSPORTS.value[scope.destackId];
}
...
export async function getGraphClient(scope?: Partial<GraphScopeData>): Promise<HostClient | SupervisorClient> {
  if (scope?.destackId == null) return supervisor;
  else return await getHostClient({ id: scope.destackId });
}

/** Gets the cached Host client for a given scope */
export function getCachedHostClient(scope: GraphScopeData): HostClient {
  if (scope.destackId == null) throw new Error("no destack id");
  const hostClient = _CACHED_HOST_CLIENTS.value[scope.destackId];
  if (hostClient == null) throw new Error(`no cached host client for ${scope.destackId}`);
  return hostClient;
}
...
export function getCachedGraphClient(scope: GraphScopeData): HostClient | SupervisorClient {
  if (scope.destackId == null) return supervisor;
  else return _CACHED_HOST_CLIENTS.value[scope.destackId];
}
