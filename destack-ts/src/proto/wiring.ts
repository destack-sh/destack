...
export function describeScope(scope: GraphScopeData): string {
  ...
  if (scope.destackId) scopeParts.push(`destack=${scope.destackId}`);
...
export function describeNode(node: {
  ...
  if ("destackId" in node) nodeParts.push(`destackId=${node.destackId}`);
  if ("destackCk" in node) nodeParts.push(`destackId=${node.destackCk}`);
...
export function nodeReference<T extends NodeType>(
  nodeType: T,
  id: string,
  meta?: { ck?: string; destackId?: string; baseCk?: string; baseDestackId?: string },
): TypedNodeReferenceData<T> {
  const ptr = { metatype: ObjectType.NODE_REFERENCE, nodeType, ...meta, id, ck: meta?.ck ?? id };
  if (nodeType == NodeType.SPACE && ptr.destackId == null) ptr.destackId = id;
  return ptr;
}
...
export function toNodeRef<T extends NodeType>(node: NodeTypeMapping[T] | null): TypedNodeReferenceData<T> | null {
  ...
  // destackId
  if (node.metatype == ObjectType.DESTACK) {
    ref.destackId = node.id;
  } else if ("packagePtr" in node) {
    ref.destackId = node.packagePtr?.destackId;
  } else if ("destackPtr" in node) {
    ref.destackId = node.destackPtr?.id;
  } else {
    ref.destackId = node.parentPtr?.destackId;
  }
  // base
