...
import { DESTACK_DESTACK_AGENT_ID } from "@/language/core/builtin";
...
export function getTypeIcon(node: Partial<FieldData> | TypeIdentity): IconData | undefined {
  if (node.primitiveType != null) {
    ...
    if (node.destackType == DestackType.FIELD) {
      return makeIcon(BlockTypeOptionInfo[BlockType.CHOICE]!.icon!);
    }
  } else if (node.destackType != null) {
    const icon =
      NodeTypeOptionInfo[node.destackType as unknown as NodeType]?.icon ??
      StructTypeOptionInfo[node.destackType as unknown as StructType]?.icon;
    if (icon != null) return makeIcon(icon);
  }
  return undefined;
}
