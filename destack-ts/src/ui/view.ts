...
export const VIEW_TYPE_BY_DESTACK_TYPE: Partial<Record<DestackType, ViewType>> = {
  [DestackType.ICON]: ViewType.ICON,
  [DestackType.CODE]: ViewType.CODE,
  [DestackType.TEXT]: ViewType.TEXT,
  [DestackType.TEXT_LINE]: ViewType.TEXT_LINE,
  [DestackType.FILE]: ViewType.FILE,
  [DestackType.COLOR]: ViewType.COLOR,
  [DestackType.TYPE]: ViewType.TYPE,
};
...
export function getViewForType(
  ...
  } else if (type.destackType != null || type.kind == TypeKind.NODE) {
    if (VIEW_TYPE_BY_DESTACK_TYPE[type.destackType!] != null) {
      if (type.destackType == DestackType.FILE) {
        // generic file type view
        return { type: ViewType.FILE, valueType, isInline: true };
      }

      // specific destack type view
      return { type: VIEW_TYPE_BY_DESTACK_TYPE[type.destackType!]!, valueType };
    } else if (isEnumType(type.destackType)) {
      // enum type -> picker
      return { type: ViewType.PICKER, valueType };
    } else if (type.kind == TypeKind.NODE || isNodeType(type.destackType)) {
      // node picker
      return { type: ViewType.PICKER, valueType };
    }
