...
export function startDragging(event: DragEvent, data: AnyNodeData | NodeReferenceData | SelectionData): boolean {
  ...
  dt.setData("application/symbol.destack." + uuidt(), JSON.stringify({ ...dragged, trigger: null, nodes: [] }));
...
function updateDragging(event: DragEvent) {
  ...
  //  (this handles the case where something external is dragged into destack-web and we don't get a dragleave/drop/dragend)
  lastDraggedAt.value = DateTime.now();
  setTimeout(() => {
