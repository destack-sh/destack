...
import { isDestackNodeType, isEnumType, isNodeType, isStructType, isUnloadedNodeType } from "@/language/core/const";
...
import { CURRENT_DESTACK_SCOPE } from "@/system/client";
import {
  acquireConnection,
  releaseConnection,
...
import { destackGraph } from "@/system/space";
...
export function makeRemoteSearchParams(options: {
  ...
  const nodeType = valueType.destackType;
  ...
  // params
  const scope = isDestackNodeType(nodeType) ? CURRENT_DESTACK_SCOPE.value : makeScope({});
...
export function useValueSearch(options: {
  ...
  watch(
    ...
    async () => {
      ...
      if (
        ...
        isNodeType(valueType.value?.destackType) &&
        isUnloadedNodeType(valueType.value?.destackType)
      ) {
        ...
        try {
          ...
          const nodeType = valueType.value.destackType as unknown as NodeType;
  ...
  const index: Ref<SearchIndex<any> | null> = computed(() => {
    ...
    } else if (isEnumType(options.valueType.value?.destackType)) {
      // regular enum
      return enumIndex({ id: "enum", enumType: options.valueType.value.destackType });
    } else if (options.valueType.value?.kind == TypeKind.NODE || isNodeType(options.valueType.value?.destackType)) {
      // node from (some) graph
      if (remoteGraphIndex.value != null) {
        // remote graph
        return remoteGraphIndex.value;
      } else if (
        ([TypeKind.NODE, TypeKind.BASED_NODE].includes(valueType.value?.kind!) && valueType.value?.destackType == null) ||
        (isNodeType(valueType.value?.destackType) && isUnloadedNodeType(valueType.value?.destackType))
      ) {
        // remote supergraph
        ...
        // local graph
        const nodeType = valueType.value?.destackType as unknown as NodeType;
        const graph = destackGraph.nodeTypes.has(nodeType) ? destackGraph : destackGraph;
        ...
        if (valueType.value?.destackType != null) {
          metatypes = [nodeType];
    ...
    } else if (options.valueType.value?.destackType == DestackType.TYPE) {
      // some type
      return TYPE_INDEX;
    } else {
      return null;
...
function nodeItemFromNode(
  ...
  for (let i = ancestors.length - 1 - (options.skipDepth ?? 0); i >= 0; i--) {
    ...
    if (ancestor.title != null && ancestor.title.length > 0) {
      // handle destack:package special case
      if (i > 0 && isNode(ancestor.node, NodeType.SPACE) && isNode(ancestors[i - 1].node, NodeType.PACKAGE)) {
        const pkg = ancestors[i - 1].node;
        const destack = ancestor.node;
        if (destack.packagePtr?.id == pkg.id) {
...
export function typeIndex(idx: {
  ...
  function typeItemFromTypeIdentity(value: TypeIdentity): TypeItem | null {
    ...
    } else if (isStructType(value.destackType)) {
      const option = ENUM_OPTIONS_BY_VALUE[EnumType.STRUCT_TYPE][value.destackType];
      if (option != null) return typeItemFromEnumOption(EnumType.STRUCT_TYPE, option);
    } else if (isNodeType(value.destackType)) {
      const option = ENUM_OPTIONS_BY_VALUE[EnumType.NODE_TYPE][value.destackType];
  ...
  function typeItemFromEnumOption(enumType: EnumType, option: EnumOption): TypeItem {
    ...
    if (enumType == EnumType.PRIMITIVE_TYPE) {
      ...
      enumType == EnumType.DESTACK_TYPE ||
      enumType == EnumType.STRUCT_TYPE
    ) {
      item.destackType = option.value as DestackType;
...
export const TYPE_INDEX = typeIndex({ id: "type", graph: destackGraph, skipDepth: 2 });
