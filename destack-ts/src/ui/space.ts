...
import { destackConnection, destackGraph, inspectionPtr, pagePtr, pkg, space, threadPtr } from "@/system/space";
import {
  Command,
  CommandContext,
  declareCommands,
...
declareCommands<"space">({
  ...
  "space.create.page": {
    ...
    shortcuts: ["ctrl+n"],
    command: () => {
      const page = createPage(destackConnection.tx, destackGraph, { anchor: "inside", target: pkg.value!, page: {} });
  ...
  "space.create.thread": {
    ...
    shortcuts: ["ctrl+t"],
    command: () => {
      const thread = createDefaultThread(destackConnection.tx, destackGraph);
