...
from destack.pb2 import ClientData, MachineData
from destack.utils.func import generate_access_token
...
class MachineHandle:
    ...
    async def prepare(self):
        ...
        from destack.system import ACCESS_TOKEN_LENGTH
        ...
        async with session:
            ...
            destack = (
                await Space.select_all().include_descendants(Package).get(slug=self.spec.destack)
            )
            destack._graph.add_types(NodeType.MACHINE, NodeType.CLIENT)
            runtime = first(
                (
                    runtime
            ...
            package = destack.package
            assert package is not None, f"no main package for {destack!r}"
            machine = Machine(
                parent=package,
                name=self.spec.name,
