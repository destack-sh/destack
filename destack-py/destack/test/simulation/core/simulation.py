...
from destack.store.postgres.map import BUILTIN_GLOBAL_SCHEMA, BUILTIN_MAIN_SCHEMA
...
if TYPE_CHECKING:
    from destack.sharding import DatabaseProvider
    from destack.test.simulation.workload import Workload, WorkloadSpec
...
class Simulation:
    ...
    def __init__(
        ...
        from destack.system import pg_engine_from_database
        ...
        self.destackes_by_name: dict[str, SpaceHandle] = {}
        self.destackes_by_id: dict[UUID, SpaceHandle] = {}
    ...
    def get_destack(self, name: str | UUID) -> "SpaceHandle":
        if isinstance(name, UUID):
            destack = self.destackes_by_id.get(name)
        else:
            destack = self.destackes_by_name.get(name)
        if destack is None:
            raise LookupError(
                f"{self!r} has no destack: '{name}' (available: {list(self.destackes_by_name)})"
            )
        return destack
    ...
    def get_space_id(self, name: str) -> UUID:
        destack = self.get_destack(name)
        return destack.space_id
    ...
    async def prepare(self):
        ...
        def add_destack(destack_spec: DestackSpec) -> SpaceHandle:
            """Add a Destack to the simulation."""
            if destack_spec.name in self.destackes_by_name:
                raise ValueError(f"duplicate destack name: {destack_spec.name} in {self!r}")
            destack = SpaceHandle(
                id=f"destack:{destack_spec.name}",
                spec=destack_spec,
                oracle=self.oracle,
                simulation=self,
            )
            self.destackes_by_name[destack_spec.name] = destack
            return destack
        ...
        def add_host(host_spec: HostSpec) -> HostHandle:
            ...
            if host_spec.destack in self.hosts_by_name:
                raise ValueError(f"duplicate Host for Destack: {host_spec.destack} in {self!r}")
            host = HostHandle(
                id=f"host:{host_spec.destack}",
                spec=host_spec,
                oracle=self.oracle,
                simulation=self,
            )
            self.hosts_by_name[host_spec.destack] = host
            add_service(host)
            return host

        def add_workload(workload_spec: "WorkloadSpec") -> "Workload":
            """Add a Workload to the simulation."""
            from destack.test.simulation.workload import get_workload_cls
        ...
        for user in self.spec.users:
            add_user(user)
        for destack in self.spec.destackes:
            add_destack(destack)
        ...
        if self.spec.system:
            ...
            add_destack(DestackSpec(name=SYSTEM_SLUG, owner="system"))
            add_host(HostSpec(destack=SYSTEM_SLUG))
            add_destack(DestackSpec(name=DESTACK_SLUG, owner="system"))
            add_host(HostSpec(destack=DESTACK_SLUG))
    ...
    async def run(self):
        ...
        async with SimulatedChannel(
            ...
            # prepare destackes
            for destack in self.destackes_by_name.values():
                user = self.users_by_name.get(destack.spec.owner)
                await destack.prepare(supervisor_client, user.some_client if user else None)
                self.destackes_by_id[destack.space_id] = destack
...
async def run_simulation(spec: SimulationSpec):
    """Run a Simulation"""
    from destack.test.fixtures import (
        create_test_db,
        get_database,
        get_main_database,
    )
