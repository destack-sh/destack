...
from destack.language import Region
...
if TYPE_CHECKING:
    from destack.system import CreateDestackOptions, HostInfo, SupervisorService
...
class SimulatedHostMap:
    ...
    def get(self, space_id: UUID, region: Region) -> "HostInfo":
        destack = self.simulation.get_destack(space_id)
        host = self.simulation.get_host(destack.name)
...
class SupervisorHandle(ServiceHandle[SupervisorSpec, "SupervisorService", SupervisorClient]):
    ...
    async def start(self) -> "SupervisorService":
        self._service = SupervisorService(
            ...
            create_destack_options=CreateDestackOptions(create_machine_scaler=False),
        )
