...
from destack.test.simulation.core import Simulation, make_remote_session
...
class ClientWorkload[SpecT: ClientWorkloadSpec](Workload[SpecT], abc.ABC):
    ...
    async def prepare(self):
        self.space_id: UUID = self.simulation.get_space_id(self.spec.destack)
        self.client = self.simulation.get_client(self.spec.client)
        self.self_host = self.simulation.get_host(self.spec.destack)
        if self.spec.system:
            self.destack_host = self.simulation.get_host(DESTACK_SLUG)
        else:
            self.destack_host = None
        ...
        async with self.session.active():
            ...
            main_package = self.destack.package
            assert main_package is not None, f"{self!r} has no main package"
            self.main_package = main_package
            self.session.parent = self.destack  # patch in the session parent
            if self.destack_host is not None:
                self.destack_destack = await Space.include_descendants(
                    *LOADED_PACKAGE_NODE_TYPES
                ).get(DESTACK_PTR, live=True)
            else:
                self.destack_destack = None
