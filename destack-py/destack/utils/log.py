...
if structlog is not None:
    from opentelemetry import trace  # must exist if we have structlog
    ...
    _CONSOLE_FORMATTER = structlog.dev.ConsoleRenderer(
        columns=[
            # timestamp (dim)
            ...
            structlog.dev.Column(
                ...
                structlog.dev.LogLevelColumnFormatter(
                    LEVEL_STYLES, reset_style=styles.reset, width=6
                ),
            ),
            # event (very bright)
            structlog.dev.Column(
            ...
            structlog.dev.Column(
                "duration",
                _DurationFormatter(level_styles=LEVEL_STYLES, reset_style=styles.reset, width=10),
            ),
            # logger (bright)
            structlog.dev.Column(
                "logger",
    ...
    def setup_logging(apply_logging: bool = True, apply_structlog: bool = True):
        ...
        if _setup_logging:
            return
        assert structlog is not None, "structlog is not installed"
    ...
    class _FakeLogger:
        ...
        def log(self, level: int, msg: str, *args, **kw):
            print(f"{self.name} {level} {msg} {args} {kw}")  # noqa: T201

        def trace(self, msg: str, *args, **kw):
    ...
    _logger_by_name: dict[str, _FakeLogger] = {}

    def setup_logging(apply_logging: bool = True, apply_structlog: bool = True):
        pass

    def get_logger(name: str) -> "BoundLogger":
        if name not in _logger_by_name:
            _logger_by_name[name] = _FakeLogger(name)
        return _logger_by_name[name]  # type: ignore
