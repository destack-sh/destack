...
export async function run(arguments_: string[]): Promise<void> {
    ...
    if (action === "check" || action === "format") {
        if (selection.length) throw new Error(`${action} takes no deployment arguments.`);
    ...
    if (action !== "init") {
        ...
        if (action === "deploy") {
            if (!confirm(`Apply ${deployment.name}?`)) throw new Error("Deployment cancelled.");
...
function invoke(arguments_: string[], environment: Record<string, string>): void {
    ...
    if (command.signal) throw new Error(`OpenTofu terminated by ${command.signal}.`);
    if (command.status !== 0) throw new Error(`OpenTofu exited with ${command.status}.`);
}
