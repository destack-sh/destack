...
DESTACK_ZED_EXTENSION_PATH="${DESTACK_ZED_EXTENSION_PATH:-platform/zed}"
...
# resolve the version in platform/zed/extension.toml
resolve_manifest_version() {
    local manifest_path="${DESTACK_REPOSITORY_ROOT}/platform/zed/extension.toml"
...
if [ "${DESTACK_MANIFEST_VERSION}" != "${DESTACK_RELEASE_VERSION}" ]; then
    fail "version mismatch: version.txt=${DESTACK_RELEASE_VERSION}, platform/zed/extension.toml=${DESTACK_MANIFEST_VERSION}"
fi
