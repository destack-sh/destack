...
pub struct ReleaseArgs {
    ...
    /// Artifact directory passed to platform preflight recipes.
    #[arg(long, default_value = "cli/install/artifacts")]
...
pub fn run(args: &ReleaseArgs) -> i32 {
    ...
    if !args.is_skip_preflight {
        let preflight_arguments = vec![
            OsString::from("platform/preflight-cli-publish"),
    ...
    // run publish in dry mode by default
    let publish_mode_argument = if args.is_publish_live {
        OsString::from("")
    } else {
        OsString::from("--dry-run")
    };
    let publish_arguments = vec![
        OsString::from("platform/publish"),
        publish_mode_argument.clone(),
    ];
    let publish_exit_code = run_just_command(&publish_arguments);
    ...
    if args.is_publish_zed {
        let zed_publish_arguments = vec![
            OsString::from("platform/publish-zed"),
...
fn validate_repository_root() -> Result<(), String> {
    ...
    // ensure the platform justfile exists
    if !Path::new("platform/justfile").exists() {
        return Err(
            "platform/justfile not found, run this command from the repository root".to_string(),
        );
    }
