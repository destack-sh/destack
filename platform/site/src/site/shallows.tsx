...
/// The depth of the water surface below the top of the shallows, in CSS pixels.
const surface = 56;

/// Float a lone bottle on a strip of flat water that rides the landing page's waves.
export function Shallows(props: { style?: stylex.Styles }) {
    ...
    onSettled(() => {
        ...
        // trace the surface from the shared wave, then fill the water below it
        const draw = (now: number) => {
            const width = host.clientWidth;
            const height = host.clientHeight;
            let line = "";
            for (let x = 0; x <= width + 8; x += 8) {
                line += `${x === 0 ? "M" : "L"}${x} ${(surface + waveAt(x, now / 1000)).toFixed(2)}`;
            }
        ...
        // animate the surface, or draw it once when motion is reduced
    ...
    return (
        <div ref={host} aria-hidden="true" {...stylex.attrs(styles.shallows, props.style)}>
            <Flotsam
                isAdrift={true}
                surfacedAt={0}
                waterline={`${surface}px`}
