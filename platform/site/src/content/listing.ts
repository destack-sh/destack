...
export function renderListing(highlighted: string, { title, href, detail, label = "Code" }: ListingOptions = {}): string {
    ...
    return `<figure class="markdown-code" data-publication-listing>${renderListingCaption({ title, href, detail })}<pre data-publication-body tabindex="0" aria-label="${escapeAttribute(title ?? label)}"><code class="markdown-code-lines" data-publication-lines>${rows}</code></pre></figure>`;
}

/// Render a diagram with the shared listing frame and caption.
export function renderDiagram(source: string, { title, label = "Diagram" }: ListingOptions = {}): string {
    return `<figure class="markdown-code markdown-mermaid" data-publication-listing>${renderListingCaption({ title })}<div data-publication-body data-mermaid tabindex="0" role="region" aria-label="${escapeAttribute(title ?? label)}"><pre><code>${escapeAttribute(source)}</code></pre></div></figure>`;
}
