...
export function renderListing(
    ...
    return `<figure class="markdown-code" data-publication-listing>${
        renderListingCaption({ title, href, detail })
    }<pre data-publication-body tabindex="0" aria-label="${
        escapeAttribute(title ?? label)
...
export function renderDiagram(
    ...
    return `<figure class="markdown-code markdown-mermaid" data-publication-listing>${
        renderListingCaption({ title })
    }<div data-publication-body data-mermaid tabindex="0" role="region" aria-label="${
        escapeAttribute(title ?? label)
