...
export function renderContentList(entries: readonly ContentEntry[]): string {
    return `<ul class="content-list">${entries.map((entry) => {
        const relation = entry.external ? ' target="_blank" rel="noopener noreferrer"' : "";
        const summary = entry.summary ? `<span class="content-entry-summary">${escapeAttribute(entry.summary)}</span>` : "";
        const meta = entry.date
            ? `<time class="content-entry-meta" datetime="${escapeAttribute(entry.date)}">${escapeAttribute(entry.meta ?? formatDate(entry.date))}</time>`
            : entry.meta ? `<span class="content-entry-meta">${escapeAttribute(entry.meta)}</span>` : "";
        ...
        return `<li><a class="content-entry"${entry.date ? ' data-dated="true"' : ""}${entry.external ? ' data-external="true"' : ""} href="${escapeAttribute(entry.href)}"${relation}><${titleTag} class="content-entry-title">${escapeAttribute(entry.title)}</${titleTag}>${meta}${arrow}${summary}</a></li>`;
    }).join("")}</ul>`;
}
