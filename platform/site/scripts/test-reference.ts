...
import { parseReadme, renderPackageDocuments, packageDefinition } from "./reference.ts";
...
try {
    ...
    const reference = {
        ...
        modules: [
            ...
            { module: "relay://nested/index.ds", specifier: "relay/nested/binding", path: "src/nested/index.ds", exports: [] },
            { module: "relay://empty.ds", specifier: "relay/empty", path: "src/empty.ds", exports: [] },
        ],
    };
    const index = { route: "/docs/library/relay/", path: "library/relay/index.md", title: "Relay", order: 0 };
    const { documents, items } = renderPackageDocuments(reference, index, {
        directory, referenceFile: join(directory, "reference.json"), sourceUrl: "https://example.com/",
    });
    assert.deepEqual(documents.map((page) => page.route), [
        "/docs/library/relay/", "/docs/library/relay/nested/binding/", "/docs/library/relay/empty/",
    ]);
    assert.deepEqual(items, []);
    assert.equal(documents[1].html, '<h1 id="binding">Binding</h1><p><a href="/docs/library/relay/#relay">Package</a></p>\n<section class="document-section"><h2 id="exports">Exports</h2></section>');
    ...
    documents.forEach((page, index) => { page.kind = index === 0 ? "chapter" : "module"; });
    const roots = [
        { route: "/docs/", title: "Docs", kind: "chapter" },
        { route: "/docs/library/", title: "Libraries", kind: "chapter" },
    ];
    buildNavigation([...roots, ...documents].sort((left, right) => left.route.localeCompare(right.route)), []);
    assert.deepEqual(documents[1].navigation!.ancestors.map((page) => page.route), [
        "/docs/", "/docs/library/", "/docs/library/relay/",
    ]);
    // the catalog and each module expose the same sibling inventory
    const expectedRoutes = ["/docs/library/relay/", "/docs/library/relay/empty/", "/docs/library/relay/nested/binding/"];
    ...
    const symbol: NavigationPage = { route: `${documents[1].route}symbol/`, title: "Symbol", parentRoute: documents[1].route };
    buildNavigation([...roots, ...documents].sort((left, right) => left.route.localeCompare(right.route)), [symbol]);
    ...
    const nestedPages = renderPackageDocuments({
        ...
        { module: "relay://nested.ds", specifier: "relay/nested", path: "src/nested.ds", exports: [] },
        { module: "relay://deep.ds", specifier: "relay/nested/more/deep", path: "src/deep.ds", exports: [] },
        ],
    }, index, { directory, referenceFile: join(directory, "reference.json"), sourceUrl: "https://example.com/" }).documents;
    nestedPages.forEach((page, index) => { page.kind = index === 0 ? "chapter" : "module"; });
    buildNavigation([...roots, ...nestedPages].sort((left, right) => left.route.localeCompare(right.route)), []);
    ...
    assert.deepEqual(nestedPages[1].navigation!.ancestors.map((page) => page.route), [
        "/docs/", "/docs/library/", "/docs/library/relay/", "/docs/library/relay/nested/",
    ]);
    assert.deepEqual(nestedPages[0].markdown.split("\n").filter((line) => line.startsWith("- [")).map((line) => line.split("]")[0]), [
        "- [relay/nested/binding", "- [relay/empty", "- [relay/nested", "- [relay/nested/more/deep",
    ]);
    ...
    assert.equal(composed.html, '<h1 id="api">API</h1><h2 id="exports">Exports</h2><section class="document-section"><h2 id="exports-2">Exports</h2><section class="document-section"><h3 id="writing">Writing</h3><p>Details.</p></section></section>');
    assert.deepEqual(composed.searchSections.map(({ depth, id, title }) => ({ depth, id, text: title })), composed.headings);
    ...
    const declaration = {
        name: "value", kind: "constant", documentation: "A value.", members: [],
    ...
    const rootOnly = renderPackageDocuments({
        package: { name: "relay" },
        modules: [{ module: "relay://index.ds", specifier: "relay", path: "src/index.ds", exports: [{ name: "value", declarations: [declaration] }] }],
    }, index, { directory, referenceFile: join(directory, "reference.json"), sourceUrl: "https://example.com/" });
    assert.deepEqual(rootOnly.documents.map((page) => page.route), [index.route]);
    assert.equal(rootOnly.documents[0].markdown,
        `# Relay\n\n## Exports\n\n- [value](${rootOnly.items[0].route}) — A value.\n`);
    assert.deepEqual(rootOnly.documents[0].tableOfContents, [{ depth: 2, id: "exports", text: "Exports" }]);

    // comments preserve Unicode text without shifting semantic identifier ranges
    const documented = packageDefinition({
        ...declaration, documentation: "A café value.\n\nMore detail.",
    ...
    for (const token of documented.tokens) {
        assert.equal(Buffer.from(documented.text).subarray(token.start, token.end).toString(), "value");
    }
    assert.equal(packageDefinition({ ...declaration, documentation: undefined }).text, declaration.signature.text);

    const namespaceReference = {
        package: { name: "relay" },
        modules: [{
            module: "relay://api.ds", specifier: "relay/api", path: "src/api.ds", exports: [
        ...
        namespaces: [{
            module: "relay://tools.ds", path: "src/tools.ds", exports: [
