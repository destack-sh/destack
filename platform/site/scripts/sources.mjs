...
export async function writePageSources(pages, publicDirectory) {
    await rm(join(publicDirectory, "docs"), { force: true, recursive: true });
    await rm(join(publicDirectory, "blog"), { force: true, recursive: true });

    for (const page of pages) {
        await writeSource(publicDirectory, page.markdownRoute, page.markdown);
        await writeSource(publicDirectory, page.textRoute, plainTextFor(page.markdown));
    }
}
...
async function writeSource(publicDirectory, route, source) {
    ...
    await writeFile(file, `${source.trim()}\n`);
}
