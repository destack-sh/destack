...
function renderLintCatalog(reference: LintReference, index: DocumentationPage) {
    ...
    const select = (name: string, title: string, options: string[][]) =>
        `<select name="${name}" aria-label="${title}" disabled><option value="">${title}</option>${
    ...
    const fix = select("fixability", "Fix", [["automatic", "Automatic"], [
        "suggestion",
        "Suggested",
    ], ["none", "No fix"]]);
...
function renderLintRule(rule: LintRule, reported: string, accepted: string, order: number) {
    ...
    const html = `<div class="lint-rule">${metadata}<div class="lint-description"><p>${
        escapeHtml(rule.summary)
    }</p>${explanation}</div><section class="lint-examples">${examples}</section>${references}</div>`;
...
function renderLintExample(title: string, highlighted: string) {
    return `<section class="lint-example">${
        renderListing(highlighted, { title, label: title })
    }</section>`;
}
...
function renderLintMarkdownSource(rule: LintRule) {
    const provenance = rule.provenance.length === 0
        ? ""
        : `\n\n## Inspired by\n\n${
            rule.provenance.map((entry) => `- [${entry.rule}](${entry.url})`).join("\n")
        }`;
    const sourceRoute =
        `https://github.com/destack-sh/destack/blob/main/${rule.source.path}#L${rule.source.line}`;
