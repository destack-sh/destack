...
def _compile_value(context: DatabaseContext, arguments: list[Any], value: Value) -> str:
    ...
    value_packed = pack_node_value(value.type, value.value)
    arguments.append(value_packed)
    return f"${len(arguments)}"


def _compile_select(context: DatabaseContext, arguments: list[Any], select: Select) -> str:
    """Compile a Select into a SQL SELECT clause."""
    raise NotImplementedError(select)


def _compile_attribute(
    context: DatabaseContext, arguments: list[Any], attribute: AttributeReference
) -> str:
...
def _compile_condition(context: DatabaseContext, arguments: list[Any], condition: Condition) -> str:
    """Compile a Condition into a SQL WHERE clause."""
    # logical
    if condition.type == ConditionalType.NOT:
        return f"NOT ({_compile_expression(context, arguments, condition.left)})"
    elif condition.type == ConditionalType.AND:
        assert condition.right is not None, f"no right for {condition!r}"
        return f"({_compile_expression(context, arguments, condition.left)} AND {_compile_expression(context, arguments, condition.right)})"
    elif condition.type == ConditionalType.OR:
        assert condition.right is not None, f"no right for {condition!r}"
        return f"({_compile_expression(context, arguments, condition.left)} OR {_compile_expression(context, arguments, condition.right)})"
    # comparison
    elif condition.type == ConditionalType.EQUALS:
        assert condition.right is not None, f"no right for {condition!r}"
        return f"{_compile_expression(context, arguments, condition.left)} = {_compile_expression(context, arguments, condition.right)}"
    ...
    # string
        ...
        assert condition.right is not None, f"no right for {condition!r}"
        return f"{_compile_expression(context, arguments, condition.left)} LIKE {_compile_expression(context, arguments, condition.right)}"
    ...
    # collections
        ...
        assert condition.right is not None, f"no right for {condition!r}"
        return f"{_compile_expression(context, arguments, condition.left)} IN ({_compile_expression(context, arguments, condition.right)})"
    elif condition.type == ConditionalType.NOT_IN:
        assert condition.right is not None, f"no right for {condition!r}"
        return f"{_compile_expression(context, arguments, condition.left)} NOT IN ({_compile_expression(context, arguments, condition.right)})"
    # existence
    elif condition.type == ConditionalType.EXISTS:
        return f"EXISTS ({_compile_expression(context, arguments, condition.left)})"
    elif condition.type == ConditionalType.NOT_EXISTS:
        return f"NOT EXISTS ({_compile_expression(context, arguments, condition.left)})"
    else:
        assert_never(condition.type)


def _compile_sort(context: DatabaseContext, arguments: list[Any], sort: Sequence[Sort]) -> str:
    """Compile a Sort into a SQL ORDER BY clause."""
    raise NotImplementedError(sort)


def _compile_function(context: DatabaseContext, arguments: list[Any], function: Function) -> str:
    """Compile a Function into a SQL expression."""
    raise NotImplementedError(function)


def _compile_aggregation(
    context: DatabaseContext, arguments: list[Any], aggregation: Aggregation
) -> str:
    """Compile an Aggregation into a SQL expression."""
    raise NotImplementedError(aggregation)


def _compile_expression(context: DatabaseContext, arguments: list[Any], expr: Expression) -> str:
