...
impl WatchCoordinator {
    ...
    pub fn next_batch(&self) -> Option<WatchBatch> {
        ...
        record_item(&mut batch, first_item);
        ...
        loop {
            ...
            if batch_len(&batch) >= self.policy.max_batch_size {
                break;
            }
...
fn batch_len(batch: &WatchBatch) -> usize {
    batch.events.len() + batch.status.len()
}

/// Record a watch item into the batch.
fn record_item(batch: &mut WatchBatch, item: WatchItem) {
    // record the incoming watch item
    match item {
