...
#[cfg(test)]
mod tests {
    use super::{BufferedTransport, Transport, loopback_transport_pair};
    use std::sync::Arc;

    #[test]
    fn test_buffered_transport_roundtrip() {
        // create loopback transports
        let (client_inner, server_inner) = loopback_transport_pair(4);
        let client = BufferedTransport::new(Arc::new(client_inner), 4);
        let server = BufferedTransport::new(Arc::new(server_inner), 4);

        // send payload through buffered transport
        client.send(b"ping").expect("send");
        let payload = server.recv().expect("recv");
    ...
    #[test]
    fn test_buffered_transport_close() {
        // create loopback transports
        let (client_inner, server_inner) = loopback_transport_pair(4);
        let client = BufferedTransport::new(Arc::new(client_inner), 4);
        let server = BufferedTransport::new(Arc::new(server_inner), 4);

        // close client and assert send fails
        client.close();
