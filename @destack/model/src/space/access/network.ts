...
/** Outbound network restrictions scoped to accounts, spaces, installations or workloads. */
export const networkPolicy = table(
    "network_policy",
    {
    ...
    (policy) => [
        ...
        uniqueIndex("network_policy_account")
            .on(policy.accountId)
            .where(sql`${policy.scope} = 'account'`),
        uniqueIndex("network_policy_space")
            .on(policy.spaceId)
            .where(sql`${policy.scope} = 'space'`),
        check(
            "network_policy_scope",
            sql`
        (${policy.scope} = 'account' AND ${policy.accountId} IS NOT NULL AND ${policy.spaceId} IS NULL AND ${policy.installationId} IS NULL AND ${policy.workload} IS NULL) OR
        (${policy.scope} = 'space' AND ${policy.accountId} IS NULL AND ${policy.spaceId} IS NOT NULL AND ${policy.installationId} IS NULL AND ${policy.workload} IS NULL) OR
        (${policy.scope} = 'installation' AND ${policy.accountId} IS NULL AND ${policy.spaceId} IS NOT NULL AND ${policy.installationId} IS NOT NULL AND ${policy.workload} IS NULL) OR
        (${policy.scope} = 'workload' AND ${policy.accountId} IS NULL AND ${policy.spaceId} IS NOT NULL AND ${policy.installationId} IS NOT NULL AND ${policy.workload} IS NOT NULL AND length(${policy.workload}) > 0)
    `,
        ),
