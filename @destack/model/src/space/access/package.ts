...
/** Account or space package restrictions. */
export const packagePolicy = table(
    "package_policy",
    {
    ...
    (policy) => [
        ...
        uniqueIndex("package_policy_account")
            .on(policy.accountId)
            .where(sql`${policy.scope} = 'account'`),
        uniqueIndex("package_policy_space")
            .on(policy.spaceId)
            .where(sql`${policy.scope} = 'space'`),
        check(
            "package_policy_scope",
            sql`
        (${policy.scope} = 'account' AND ${policy.accountId} IS NOT NULL AND ${policy.spaceId} IS NULL) OR
        (${policy.scope} = 'space' AND ${policy.accountId} IS NULL AND ${policy.spaceId} IS NOT NULL)
    `,
        ),
