    ...
    return [
        ...
        uniqueIndex(`${name}_source`)
            .on(
                ...
                dialectSQL({
                    sqlite: sql`coalesce(json_extract(${columns.source}, '$.spaceId'), json_extract(${columns.source}, '$.installationId'))`,
                    postgresql: sql`coalesce((${columns.source}::jsonb ->> 'spaceId'), (${columns.source}::jsonb ->> 'installationId'))`,
                }),
                dialectSQL({
