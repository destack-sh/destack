...
export const repository = table(
    "repository",
    {
        ...
        /** The region administering this repository. */
    ...
    (entry) => [
        unique("repository_name").on(entry.accountId, entry.name),
        unique("repository_account").on(entry.accountId, entry.id),
        foreignKey({
            columns: [entry.regionId, entry.residency],
            foreignColumns: [region.id, region.residency],
        }).onDelete("restrict"),
        ...reconciliationChecks("repository", entry),
        check(
