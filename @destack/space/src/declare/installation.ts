...
export const SpaceInstallation = defineSchema(
    schema.object({
        ...
        resources: schema.record(PackageId, schema.record(ResourceName, SpaceResourceBinding)),
        /** Secret selections keyed by immutable package ID and declaration name. */
        secrets: schema.record(PackageId, schema.record(ResourceName, SpaceSecretBinding)),
