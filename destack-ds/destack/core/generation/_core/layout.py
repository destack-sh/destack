...
class ObjectSizer(ABC):
    ...
    def size_property(
        ...
        """
        Get the estimated size of a specific Property on an Object.
        """
        raise NotImplementedError
...
class KompaktObjectSizer(ObjectSizer):
    ...
    def size_type(
        ...
        # map: length (4 bytes) + entries
        elif type.cardinality == TypeCardinality.MAP:
            # unknown length; empty map has length prefix (4 bytes)
            size = ObjectSize(4, None)
        else:
    ...
    def size_type_scalar(
        ...
        # enum: smallest varint up to 5 bytes
            ...
            max_bytes = ceil(log2(max(enum_cls)) / 8)
