...
<template>
  <div class="flex h-[32px] flex-row items-baseline gap-x-1.5 rounded-full border py-1 pr-3 pl-2 text-sm select-none">
    <!-- Runnable -->
    <NodeReference :node-ptr="node?.actionPtr" hide-metadata size="sm" class="flex-shrink-0" />
    <!-- Title/arguments -->
    <TextLine
      v-if="node?.title"
      :model-value="node?.title"
      force-line-type="inherit"
      class="max-w-full truncate text-gray-500 select-none"
      truncate
      colorless
      is-minimal
    />
    <!-- Status -->
    <ProcessStatus v-if="node?.status" :node="node" icon="dot" class="ml-0.5 flex-shrink-0" />
  </div>
</template>
