...
"""),
    ...
    Receive1.connect(LinkType.SELECT, Code1)
    Code1.connect(LinkType.SELECT, Complete1)
    runtime.page().extend(Channel1, Flow1)
    await runtime.commit()
...
async def test_pause_resume_run(simulation: Simulation, runtime: RuntimeLambdaWorkload):
    ...
    Start.connect(LinkType.SELECT, Action1)
    Action1.connect(LinkType.SELECT, Complete)
