...
async def test_run_flow_browser_go_to_url(simulation: Simulation, runtime: RuntimeLambdaWorkload):
    ...
    Start.connect(LinkType.SELECT, target=GoToUrl)
    GoToUrl.connect(LinkType.SELECT, target=Wait)
    Wait.connect(LinkType.SELECT, target=Observe)
    Observe.connect(LinkType.SELECT, target=Complete)
