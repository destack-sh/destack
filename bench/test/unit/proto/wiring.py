...
def test_roundtrip_node_reference_bytes():
    ...
    node_ref_data = node_ref._to_proto()
    assert node_ref._proto is node_ref_data  # cached
    assert node_ref._to_proto() is node_ref_data  # cached
    ...
    assert unpacked_node_ref._to_proto() is unpacked_node_ref_data  # cached
...
def test_roundtrip_builtin_object_bytes(obj: BuiltinObject[AnyObjectData], session: Session):
    packed_obj_data: AnyObjectData = obj._to_proto()
