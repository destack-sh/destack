...
def test_roundtrip_scalar_value(session: Session, package: Package) -> None:
    ...
    assert unpack_value(block.variables_packed, type_info, wrap_primitive=True) == 7

    # set at runtime
    block.variables = 42
    assert block.variables == 42
    assert unpack_value(block.variables_packed, type_info, wrap_primitive=True) == 42
...
def test_roundtrip_nested_value(session: Session, package: Package):
    ...
    unpacked_value = unpack_value(value_packed, class1_type, wrap_primitive=True)
