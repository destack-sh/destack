...
class PlaywrightServer:
    ...
    def __init__(self):
        ...
        self._connection_by_browser_id: dict[UUID, BrowserConnection] = {}
        self._next_port = 9222

    @tracer.start_as_current_span("playwright_server.init")
    ...
    @tracer.start_as_current_span("playwright_server.start_browser")
    async def start_browser(self, browser: Browser) -> BrowserConnection:
        ...
        if browser.id in self._connection_by_browser_id:
            return self._connection_by_browser_id[browser.id]
        ...
        self._connection_by_browser_id[browser.id] = connection

        return connection

    @tracer.start_as_current_span("playwright_server.stop_browser")
    async def stop_browser(self, browser: Browser):
        ...
        self._connection_by_browser_id.pop(browser.id, None)
        logger.debug("playwright_server.stop", browser=browser)

    @tracer.start_as_current_span("playwright_server.close")
    async def close(self):
        ...
        self._connection_by_browser_id.clear()
