# This migration was automatically generated on 2024.05.04. Edit as needed.
import psycopg
...
async def upgrade_global(cur: psycopg.AsyncCursor):
    ...
    await cur.execute(
        'ALTER TABLE "bench_bench" ADD COLUMN "published_branch_id" uuid REFERENCES bench_branch ON DELETE SET NULL'
    )
    await cur.execute(
    ...
    await cur.execute(
        'ALTER TABLE "bench_environment" ADD COLUMN "search_store_id" uuid REFERENCES bench_store ON DELETE SET NULL'
    )
    await cur.execute(
        'ALTER TABLE "bench_environment" ADD COLUMN "analytics_store_id" uuid REFERENCES bench_store ON DELETE SET NULL'
    )
    await cur.execute(
        'ALTER TABLE "bench_environment" ADD COLUMN "drive_id" uuid NOT NULL REFERENCES bench_drive ON DELETE SET NULL'
    )
    await cur.execute(
        'ALTER TABLE "bench_environment" ADD COLUMN "cache_id" uuid REFERENCES bench_cache ON DELETE SET NULL'
    )
    await cur.execute(
    ...
    await cur.execute(
        'ALTER TABLE "bench_notice" ADD COLUMN "parent_trigger_id" uuid REFERENCES bench_trigger ON DELETE CASCADE'
    )
    await cur.execute(
    ...
    await cur.execute(
        'ALTER TABLE "bench_notice" ADD CONSTRAINT "bench_notice_bench_check_one_parent" CHECK ((parent_block_id IS NOT NULL) OR (parent_field_id IS NOT NULL) OR (parent_step_id IS NOT NULL) OR (parent_view_id IS NOT NULL) OR (parent_trigger_id IS NOT NULL))'
    )
    ...
    await cur.execute(
        'ALTER TABLE "bench_trigger" ADD CONSTRAINT "bench_trigger_bench_check_one_parent" CHECK ((parent_block_id IS NOT NULL))'
    )
    ...
    await cur.execute(
        'CREATE INDEX "bench_step_bench_idx_package_deleted_at" ON bench_step USING BTREE (deleted_at, package_id)'
    )
    await cur.execute(
        'CREATE INDEX "bench_step_bench_idx_package_archived_at" ON bench_step USING BTREE (archived_at, package_id)'
    )
    await cur.execute(
    ...
    # bench_cache
    await cur.execute(
        'ALTER TABLE "bench_cache" ADD COLUMN "parent_bench_id" uuid REFERENCES bench_bench ON DELETE CASCADE'
    )
    await cur.execute(
    ...
    await cur.execute(
        'ALTER TABLE "bench_cache" ADD CONSTRAINT "bench_cache_bench_check_one_parent" CHECK ((parent_bench_id IS NOT NULL))'
    )
...
async def upgrade_local(cur: psycopg.AsyncCursor):
    ...
    await cur.execute(
        'ALTER TABLE "bench_run" ADD CONSTRAINT "bench_run_bench_check_one_parent" CHECK ((parent_session_id IS NOT NULL) OR (parent_run_id IS NOT NULL))'
    )

    # bench_pause
    await cur.execute(
        'ALTER TABLE "bench_pause" ADD COLUMN "parent_run_id" uuid REFERENCES bench_run ON DELETE CASCADE'
    )
    await cur.execute(
    ...
    await cur.execute(
        'ALTER TABLE "bench_pause" ADD CONSTRAINT "bench_pause_bench_check_one_parent" CHECK ((parent_run_id IS NOT NULL))'
    )
