# This migration was automatically generated on 2024.02.21. Edit as needed.
import psycopg
...
async def upgrade_global(cur: psycopg.AsyncCursor):
    ...
    CREATE TABLE bench_link (
        ...
        created_by_run_ck uuid,
        updated_by_run_ck uuid,
        reference_dependency_ck uuid,
        reference_upgrade_ck uuid,
    ...
    CREATE TABLE bench_dependency (
        ...
        dependency_scopes_block_ck uuid[] NOT NULL
    )
    """
    )
    ...
    await cur.execute(
        "CREATE INDEX bench_invite_bench_idx_deleted_at ON bench_invite USING BTREE (deleted_at)"
    )
    await cur.execute(
        "CREATE INDEX bench_invite_bench_idx_archived_at ON bench_invite USING BTREE (archived_at)"
    )
    await cur.execute(
        """
        ALTER TABLE bench_invite    
    ...
    await cur.execute(
        """
        ALTER TABLE bench_handle    
        ADD CONSTRAINT bench_handle_bench_slug_is_slug CHECK (((slug)::text ~ \'^[a-z0-9-]+$\'::text)),
...
async def upgrade_local(cur: psycopg.AsyncCursor):
    ...
    await cur.execute(
        "CREATE INDEX bench_signal_bench_idx_object_block_ck ON bench_signal USING BTREE (object_block_ck)"
    )
    await cur.execute(
        "CREATE INDEX bench_signal_bench_idx_sender_block_ck ON bench_signal USING BTREE (sender_block_ck)"
    )
    await cur.execute(
    ...
    await cur.execute(
        "CREATE INDEX bench_run_bench_idx_root_run_ck ON bench_run USING BTREE (root_run_ck)"
    )
    await cur.execute(
