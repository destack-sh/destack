...
from bench.language.const import NodeType, TypedNodeReference
...
def test_parse_text():
    spans = [
        ...
        TextMention.from_reference(TypedNodeReference(NodeType.FIELD, uuid4())),
        TextPlain(text=", it's "),
        TextMention.from_reference(TypedNodeReference(NodeType.STATEMENT, uuid4()), path=".xyz"),
        TextPlain(text="!"),
    ]
...
def test_patch_text():
    ...
    spans = [
        ...
        TextMention.from_reference(TypedNodeReference(NodeType.FIELD, uuid4())),
        TextPlain(text=", it's "),
        TextMention.from_reference(
            TypedNodeReference(NodeType.STATEMENT, statement_ck), path=".xyz"
        ),
        TextPlain(text="!"),
    ]
