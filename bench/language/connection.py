...
from bench.language.const import ReadType, get_active_session
...
class RemoteGetConnection[NodeT: Node, NodeDataT: AnyNodeData](Connection[NodeT, NodeDataT, NodeT]):
    ...
    def __init__(
        ...
        *,
        ...
        assert query._read_type == ReadType.GET, f"not a get query: {query!r}"
        self._remote = remote
        self._scope = scope
...
class RemoteConnector(QueryConnector):
    ...
    async def get[NodeT: Node, NodeDataT: AnyNodeData](
        ...
        if query._read_type != ReadType.GET:
            raise ValueError(f"not a get query: {query!r}")
