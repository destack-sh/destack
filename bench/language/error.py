...
class ErrorType(enum.Enum):
    ...
    MISSING_REFERENCE = 61, "missing reference"
    UNDEFINED_LOCAL_REFERENCE = 62, "undefined reference {path}"
    UNDEFINED_EXTERNAL_REFERENCE = 63, "undefined external reference {path} in module {module}"
    EXTERNAL_LOOKUP_FAILED = 64, "failed to lookup reference {path} in module {module}: {error}"
    REFERENCE_TYPE_MISMATCH = 65, "reference {resolved} is not of type {resolved}"
    AMBIGUOUS_DEFINITION = 66, "multiple definitions for {path}"
    AMBIGUOUS_REQUIREMENT = 67, "multiple requirements for {name}"
    UNEXPECTED_PARENT = 68, "unexpected parent {parent}"
    EXPECTED_PARENT = 69, "expected a parent"
    EXPECTED_PROPER_CHILDREN = 70, "expected proper children"
    UNEXPECTED_CHILDREN = 71, "unexpected children"
    EXPECTED_PARAMETERS = 72, "expected parameters of type {type}"
    UNEXPECTED_PARAMETERS = 73, "unexpected parameters"
    EXPECTED_ARGUMENTS = 74, "expected arguments of type {type}"
