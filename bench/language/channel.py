...
class ChannelError(BenchError):
    def __init__(
        self,
        medium: Union["StoreEngine", "StoreChannel", Any],
...
class StoreEngine(abc.ABC, Generic[NodeT, NodeDataT]):
    """A store engine provides Channels to connect to and query a Store-like thing."""
    ...
    async def connect(self, session: "Session") -> "StoreChannel":
        """Opens the store engine for a session."""
        raise NotImplementedError


class StoreChannel(abc.ABC, Generic[NodeT, NodeDataT]):
    """A channel to a specific store for a session."""
    ...
    async def fetch(
        ...
        raise ChannelIncapableError(self, query, reason="fetch unsupported")
    ...
    async def aggregate(self, query: "QueryBuilder[NodeT, NodeDataT]") -> AggregateResult:
        ...
        raise ChannelIncapableError(self, query, reason="aggregate unsupported")
    ...
    async def flush(self, edits: list[EditData] | tuple[EditData, ...]) -> FlushResult:
        ...
        """
        raise ChannelIncapableError(self, reason="flush unsupported")
    ...
    async def commit(self, edits: list[EditData] | tuple[EditData, ...]) -> FlushResult:
        ...
        If this is a primary store, must return the accepted revisions for every edit (in order).
        """
        raise ChannelIncapableError(self, reason="commit unsupported")
...
class RemoteChannel(StoreChannel[NodeT, NodeDataT]):
    ...
    async def fetch(
        ...
        trace.get_current_span().set_attribute("channel", repr(self))
...
class PostgresChannel(StoreChannel[NodeT, NodeDataT], Generic[NodeT, NodeDataT]):
    """A channel to a Postgres store."""
...
class InMemoryEngine(StoreEngine[NodeT, NodeDataT], Generic[NodeT, NodeDataT]):
    ...
    async def connect(self, session: "Session"):
        return InMemoryChannel(self, session)


class InMemoryChannel(StoreChannel[NodeT, NodeDataT], Generic[NodeT, NodeDataT]):
...
class SplitChannel(StoreChannel[NodeT, NodeDataT], Generic[NodeT, NodeDataT]):
    ...
    async def fetch(
        ...
        initial_result = await initial_channel.fetch(initial_query, options)
        ...
        if remaining_ancestors:
            ...
            for parent_type, parents in actual_roots_parents_by_type.items():
                ...
                ancestor_result = await ancestor_channel.fetch(ancestor_query, options)
    ...
    async def aggregate(self, query: "QueryBuilder[NodeT, NodeDataT]") -> AggregateResult:
        ...
        return await channel.aggregate(query)
