...
async def _compile_task(state: CompilationState, task: Task) -> None:
    ...
    target_code.blank()
    for field in chain(task_t.input.children, task_t.output.children):
        # explain task subtypes (only struct and enum for now, not recursive yet)
        if field.type == TypeTag.STRUCT:
            ...
            target_code.blank()
        elif field.type == TypeTag.ENUM:
            target_code.emit(f"enum {field.source_reference}:\n")
            for member in field.members:
                target_code.emit(f'{member.value} = ("{member.name}", "{member.description}")')
            target_code.blank()
    ...
    # task input fields
    for field in task_t.input.children:
