...
provider "aws" {
  region = local.aws_region_by_destack_region[local.global_region]
}
...
resource "aws_vpc" "global_vpc" {
  ...
  tags = {
    Name = "destack-${var.env}-global-vpc"
  }
}
...
resource "aws_subnet" "global_public" {
  ...
  tags = {
    Name = "destack-${var.env}-global-public-subnet-${count.index + 1}"
  }
}
resource "aws_subnet" "global_private" {
  ...
  tags = {
    Name = "destack-${var.env}-global-private-subnet-${count.index + 1}"
  }
}
...
resource "aws_internet_gateway" "global_vpc" {
  ...
  tags = {
    Name = "destack-${var.env}-global-internet-gateway"
  }
}
...
resource "aws_route_table" "global_public" {
  ...
  tags = {
    Name = "destack-${var.env}-global-public-route-table"
  }
}
...
module "region_aws_eu_frankfurt" {
  ...
  cell_provider               = local.cell_provider
