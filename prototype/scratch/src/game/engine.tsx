...
import { handleBallPaddleCollision, integrateBall, isBallLost, resolveBallBrickCollision } from "./physics";
...
export function Breakout({ className }: Props) {
  ...
  const inputRef = useRef<InputState>({ left: false, right: false, launch: false, pauseToggle: false, rewind: false });

  useEffect(() => {
    const handleKey = (e: KeyboardEvent, down: boolean) => {
      if (e.key === "ArrowLeft" || e.key === "a") inputRef.current.left = down;
      if (e.key === "ArrowRight" || e.key === "d") inputRef.current.right = down;
      if (e.key === " " || e.key === "ArrowUp" || e.key === "w") inputRef.current.launch = down;
    ...
    setState(prev => {
      ...
      while (accumulator >= FIXED_DT_S && steps < MAX_STEPS_PER_FRAME) {
            ...
            if (resolveBallBrickCollision(ball, brick)) {
              ...
              if (brick.hp <= 0) {
                ...
                if (rand < 0.15) {
                  let r2: number;
                  [rng, r2] = rngNext(rng);
                  const kinds = ["widen", "shrink", "slow", "fast", "life"] as const;
                  const idx = Math.floor(r2 * kinds.length);
                  const kind = kinds[idx] ?? "widen";
                  falling.push({ kind, x: brick.x + brick.w / 2, y: brick.y + brick.h / 2, vy: 160 });
                }
              }
            }
          }
        ...
        for (const f of falling) {
          ...
          if (f.y >= paddle.y && Math.abs(f.x - (paddle.x + paddle.w / 2)) <= paddle.w / 2) {
            audioManager.play("power");
            if (f.kind === "widen") paddle.w = Math.min(paddle.w + 30, PADDLE_BASE_WIDTH + 80);
            if (f.kind === "shrink") paddle.w = Math.max(paddle.w - 30, PADDLE_BASE_WIDTH - 40);
        ...
        if (remaining.length === 0) {
          ...
          if (lives <= 0) {
            ...
            return { ...prev, score, bestScore, highestLevel, lives: 0, isGameOver: true, lastUpdateMs: now };
          }
          const ball: Ball = {
            x: VIRTUAL_WIDTH / 2,
            y: paddle.y - BALL_RADIUS - 1,
        ...
        if (shakeMsRemaining > 0) shakeMsRemaining = Math.max(0, shakeMsRemaining - dt * 1000);
  ...
  const draw = useCallback(() => {
    ...
    if (state.isGameOver) {
      ...
      ctx.fillStyle = COLORS.foreground;
      ctx.fillText("Press Space to restart", VIRTUAL_WIDTH / 2 - 110, VIRTUAL_HEIGHT / 2 + 28);
    }
  }, [state]);
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      setState(s => ({ ...s, paddle: { ...s.paddle, x: Math.max(0, Math.min(px - s.paddle.w / 2, VIRTUAL_WIDTH - s.paddle.w)) } }));
    };
  ...
  return (
    <div className={className}>
      <canvas ref={canvasRef} width={VIRTUAL_WIDTH} height={VIRTUAL_HEIGHT} style={{ borderRadius: 8, boxShadow: "0 8px 24px rgba(0,0,0,0.35)" }} />
    </div>
  );
}
