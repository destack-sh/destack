...
pub fn get_semantic_tokens(text: &str) -> Vec<lsp::SemanticToken> {
    ...
    let iter = destack_parser::lexer::tokenize(text).peekable();
...
pub fn get_token_type_at_position(
...
) -> Option<destack_parser::lexer::token::TokenType> {
    ...
    let iter = destack_parser::lexer::tokenize(text);
...
fn map_token(slice: &str, kind: destack_parser::lexer::token::TokenType) -> Option<(u32, usize)> {
    use destack_parser::lexer::token::TokenType as K;
    ...
    let ty_index = match kind {
        ...
        K::Literal { r#type, .. } => match r#type {
            destack_parser::lexer::token::LiteralTokenType::Integer { .. }
            | destack_parser::lexer::token::LiteralTokenType::Float { .. } => 3, // NUMBER

            destack_parser::lexer::token::LiteralTokenType::Character { .. }
            | destack_parser::lexer::token::LiteralTokenType::Byte { .. }
            | destack_parser::lexer::token::LiteralTokenType::String { .. }
            | destack_parser::lexer::token::LiteralTokenType::ByteString { .. }
            | destack_parser::lexer::token::LiteralTokenType::RawString { .. }
            | destack_parser::lexer::token::LiteralTokenType::RawByteString { .. } => 2, // STRING
        },
