...
export function encodeDecoratorApplication(
    writer: BinaryWriter,
    value: DecoratorApplication,
): void {
...
export function decodeDecoratorSegment(reader: BinaryReader): DecoratorSegment {
    ...
    const applications = (() => {
        ...
        const items2: Array<DecoratorApplication> = [];
        for (let index = 0; index < length2; index += 1) {
            items2.push(decodeDecoratorApplication(reader));
        }
        return items2;
    })();
    const applicationsByOwner = (() => {
        const length3 = reader.readNumber();
        const items3 = new Map<GlobalNodeIdAny, ReadonlyArray<LocalDecoratorId>>();
        for (let index = 0; index < length3; index += 1) {
            items3.set(
                decodeGlobalNodeIdAny(reader),
                (() => {
                    const length5 = reader.readNumber();
                    const items5: Array<LocalDecoratorId> = [];
                    for (let index = 0; index < length5; index += 1) {
                        items5.push(decodeLocalDecoratorId(reader));
                    }
                    return items5;
                })(),
            );
    ...
    const uses = (() => {
        ...
        const items4: Array<DecoratorUse> = [];
        for (let index = 0; index < length4; index += 1) {
            items4.push(decodeDecoratorUse(reader));
        }
        return items4;
    })();
...
export function toJsonDecoratorSegment(value: DecoratorSegment): Json {
    return {
        ...
        applicationsByOwner: Array.from(value.applicationsByOwner.entries()).map(
            ([key0, item0]) =>
                [
                    toJsonGlobalNodeIdAny(key0),
                    item0.map((item1) => toJsonLocalDecoratorId(item1)),
                ] as const,
        ),
...
export function fromJsonDecoratorSegment(value: Json): DecoratorSegment {
    ...
    return {
        ...
        applications: jsonArray(jsonField(object, "applications")).map((item0) =>
            fromJsonDecoratorApplication(item0),
        ),
        applicationsByOwner: new Map(
            jsonArray(jsonField(object, "applicationsByOwner")).map((entry) => {
                ...
                return [
                    ...
                    jsonArray(item0).map((item1) => fromJsonLocalDecoratorId(item1)),
                ] as const;
            }),
        ),
...
export const DecoratorSelection = {
    ...
    newtype(
        ...
        arguments_: ReadonlyArray<ArgumentBinding>,
    ): DecoratorSelection {
        return { kind: "newtype", key, backing, arguments: arguments_ };
    },
    ...
    switch (variant) {
        ...
        case 1: {
            const interfaces = (() => {
                ...
                const items0: Array<AutoInterface> = [];
                for (let index = 0; index < length0; index += 1) {
                    items0.push(decodeAutoInterface(reader));
                }
                return items0;
            })();
...
export function fromJsonDecoratorSelection(value: Json): DecoratorSelection {
    ...
    switch (kind) {
        ...
        case "derive":
            return {
                kind,
                interfaces: jsonArray(jsonField(object, "interfaces")).map((item0) =>
                    fromJsonAutoInterface(item0),
                ),
            };
    }
