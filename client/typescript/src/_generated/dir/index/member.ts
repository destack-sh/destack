...
import type { ImplementationEdge } from "../table/member.js";
...
import {
    decodeImplementationEdge,
    encodeImplementationEdge,
    fromJsonImplementationEdge,
    toJsonImplementationEdge,
...
export function decodeMemberIndex(reader: BinaryReader): MemberIndex {
    const entries = (() => {
        ...
        const items0: Array<MemberEntry> = [];
        for (let index = 0; index < length0; index += 1) {
            items0.push(decodeMemberEntry(reader));
        }
        return items0;
    })();
    const implementations = (() => {
        const length1 = reader.readNumber();
        const items1: Array<ImplementationEdge> = [];
        for (let index = 0; index < length1; index += 1) {
            items1.push(decodeImplementationEdge(reader));
        }
        return items1;
    })();
    const bySymbol = (() => {
...
export function fromJsonMemberIndex(value: Json): MemberIndex {
    ...
    return {
        ...
        implementations: jsonArray(jsonField(object, "implementations")).map((item0) =>
            fromJsonImplementationEdge(item0),
        ),
        bySymbol: jsonArray(jsonField(object, "bySymbol")).map((item0) => jsonInteger(item0)),
        byImplementation: jsonArray(jsonField(object, "byImplementation")).map((item0) =>
            jsonInteger(item0),
        ),
    };
}
