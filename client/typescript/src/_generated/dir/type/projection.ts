...
import {
    decodeFieldResolution,
    encodeFieldResolution,
    fromJsonFieldResolution,
    toJsonFieldResolution,
} from "./resolution.js";
import {
    decodeMemberAccess,
    encodeMemberAccess,
    fromJsonMemberAccess,
    toJsonMemberAccess,
} from "./resolution.js";
import {
    decodeSubscript,
...
export const Projection = {
    ...
    discriminant(
        union: GlobalTypeId,
        key: StaticKey,
        cases: ReadonlyArray<DiscriminantCase>,
        ty: GlobalTypeId,
    ): Projection {
    ...
    switch (variant) {
        ...
        case 5: {
            const fields = (() => {
                ...
                const items0: Array<ObjectRestField> = [];
                for (let index = 0; index < length0; index += 1) {
                    items0.push(decodeObjectRestField(reader));
                }
                return items0;
            })();
            const ty = decodeGlobalTypeId(reader);
        ...
        case 9: {
            ...
            const cases = (() => {
                ...
                const items2: Array<DiscriminantCase> = [];
                for (let index = 0; index < length2; index += 1) {
                    items2.push(decodeDiscriminantCase(reader));
                }
                return items2;
            })();
            const ty = decodeGlobalTypeId(reader);
        ...
        case "discriminant":
            return {
                ...
                cases: jsonArray(jsonField(object, "cases")).map((item0) =>
                    fromJsonDiscriminantCase(item0),
                ),
                ty: fromJsonGlobalTypeId(jsonField(object, "ty")),
            };
        case "newtypePayload":
