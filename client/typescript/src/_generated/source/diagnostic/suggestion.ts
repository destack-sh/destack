...
import {
    decodeDiagnosticLabel,
    encodeDiagnosticLabel,
    fromJsonDiagnosticLabel,
    toJsonDiagnosticLabel,
} from "./label.js";
...
export function encodeDiagnosticSuggestion(
    writer: BinaryWriter,
    value: DiagnosticSuggestion,
): void {
...
export function decodeDiagnosticSuggestion(reader: BinaryReader): DiagnosticSuggestion {
    ...
    const labels = (() => {
        ...
        const items1: Array<DiagnosticLabel> = [];
        for (let index = 0; index < length1; index += 1) {
            items1.push(decodeDiagnosticLabel(reader));
        }
        return items1;
    })();
...
export function fromJsonDiagnosticSuggestion(value: Json): DiagnosticSuggestion {
    ...
    return {
        ...
        labels: jsonArray(jsonField(object, "labels")).map((item0) =>
            fromJsonDiagnosticLabel(item0),
        ),
