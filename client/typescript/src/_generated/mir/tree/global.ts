...
export function toJsonGlobal(value: Global): Json {
    return {
        ...
        ...(value.initializer === undefined
            ? {}
            : { initializer: toJsonGlobalInitializer(value.initializer) }),
    };
}
...
export function fromJsonGlobal(value: Json): Global {
    ...
    return {
        ...
        initializer: jsonOptional(object, "initializer", (value) =>
            fromJsonGlobalInitializer(value),
        ),
    };
}
    ...
    switch (variant) {
        ...
        case 5: {
            const aggregate = (() => {
                ...
                const items0: Array<GlobalInitializer> = [];
                for (let index = 0; index < length0; index += 1) {
                    items0.push(decodeGlobalInitializer(reader));
                }
                return items0;
            })();
...
export function fromJsonGlobalInitializer(value: Json): GlobalInitializer {
    ...
    switch (kind) {
        ...
        case "aggregate":
            return {
                kind,
                aggregate: jsonArray(jsonField(object, "aggregate")).map((item0) =>
                    fromJsonGlobalInitializer(item0),
                ),
            };
        case "string":
            return {
...
export type Linkage = "local" | "export" | "import" | "shared";
