...
import type { GenericParameter } from "./type.js";
import {
    decodeStringId,
    encodeStringId,
    fromJsonStringId,
...
import {
    decodeFunctionParameter,
    encodeFunctionParameter,
    fromJsonFunctionParameter,
    toJsonFunctionParameter,
} from "./parameter.js";
...
export function encodeFunction(writer: BinaryWriter, value: Function): void {
    ...
    for (const item2 of value.arguments) {
        encodeGenericArgument(writer, item2);
    }
    writer.writeOption(value.template, (value3) => {
    ...
    for (const item7 of value.parameters) {
        encodeFunctionParameter(writer, item7);
    }
    writer.writeUnsigned(value.lifetimes.length);
    for (const item8 of value.lifetimes) {
        encodeLifetimeParameter(writer, item8);
    }
    encodeLocalNodeId(writer, value.returnType);
    writer.writeOption(value.environment, (value10) => {
        encodeLocalNodeId(writer, value10);
    });
    writer.writeOption(value.binding, (value11) => {
        encodeBinding(writer, value11);
    });
    writer.writeOption(value.body, (value12) => {
        encodeFunctionBody(writer, value12);
    });
}
...
export function decodeFunction(reader: BinaryReader): Function {
    ...
    const arguments_ = (() => {
        ...
        const items2: Array<GenericArgument> = [];
        for (let index = 0; index < length2; index += 1) {
            items2.push(decodeGenericArgument(reader));
        }
        return items2;
    })();
    const template = reader.readOption(() => decodeLocalNodeId(reader));
    ...
    const parameters = (() => {
        ...
        const items7: Array<FunctionParameter> = [];
        for (let index = 0; index < length7; index += 1) {
            items7.push(decodeFunctionParameter(reader));
        }
        return items7;
    })();
    const lifetimes = (() => {
        const length8 = reader.readNumber();
        const items8: Array<LifetimeParameter> = [];
        for (let index = 0; index < length8; index += 1) {
            items8.push(decodeLifetimeParameter(reader));
        }
        return items8;
    })();
...
export function toJsonFunction(value: Function): Json {
    return {
        ...
        ...(value.template === undefined ? {} : { template: toJsonLocalNodeId(value.template) }),
...
export function fromJsonFunction(value: Json): Function {
    ...
    return {
        ...
        template: jsonOptional(object, "template", (value) => fromJsonLocalNodeId(value)),
        ...
        parameters: jsonArray(jsonField(object, "parameters")).map((item0) =>
            fromJsonFunctionParameter(item0),
        ),
...
export function decodeFunctionBody(reader: BinaryReader): FunctionBody {
    ...
    const valueTypes = (() => {
        ...
        const items3: Array<LocalNodeId | undefined> = [];
        for (let index = 0; index < length3; index += 1) {
            items3.push(reader.readOption(() => decodeLocalNodeId(reader)));
        }
        return items3;
    })();
...
export function toJsonFunctionBody(value: FunctionBody): Json {
    return {
        ...
        valueTypes: value.valueTypes.map((item0) =>
            item0 === undefined ? null : toJsonLocalNodeId(item0),
        ),
...
export function fromJsonFunctionBody(value: Json): FunctionBody {
    ...
    return {
        ...
        valueTypes: jsonArray(jsonField(object, "valueTypes")).map((item0) =>
            item0 === null ? undefined : fromJsonLocalNodeId(item0),
        ),
...
export function decodeInstructionIndex(reader: BinaryReader): InstructionIndex {
    const locations = (() => {
        ...
        const items0: Array<readonly [LocalNodeId, InstructionLocation]> = [];
        for (let index = 0; index < length0; index += 1) {
            items0.push([decodeLocalNodeId(reader), decodeInstructionLocation(reader)] as const);
        }
        return items0;
    })();
...
export function toJsonInstructionIndex(value: InstructionIndex): Json {
    return {
        locations: value.locations.map((item0) => [
            toJsonLocalNodeId(item0[0]),
            toJsonInstructionLocation(item0[1]),
        ]),
    };
}
...
export function fromJsonInstructionIndex(value: Json): InstructionIndex {
    ...
    return {
        locations: jsonArray(jsonField(object, "locations")).map((item0) =>
            (() => {
                ...
                return [
                    fromJsonLocalNodeId(items[0]),
                    fromJsonInstructionLocation(items[1]),
                ] as const;
            })(),
        ),
    };
