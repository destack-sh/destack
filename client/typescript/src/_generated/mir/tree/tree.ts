...
import { decodeTypeDeclaration, encodeTypeDeclaration, fromJsonTypeDeclaration, toJsonTypeDeclaration } from "./type.js";
...
export function decodeTree(reader: BinaryReader): Tree {
    ...
    const types = (() => { const length21 = reader.readNumber(); const items21: Array<TypeEntry> = []; for (let index = 0; index < length21; index += 1) { items21.push(decodeTypeEntry(reader)); } return items21; })();
    const typeDeclarations = (() => { const length22 = reader.readNumber(); const items22: Array<TypeDeclaration> = []; for (let index = 0; index < length22; index += 1) { items22.push(decodeTypeDeclaration(reader)); } return items22; })();
...
export function fromJsonTree(value: Json): Tree {
    ...
    return {
        ...
        typeDeclarations: jsonArray(jsonField(object, "typeDeclarations")).map((item0) => fromJsonTypeDeclaration(item0)),
