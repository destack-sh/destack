...
export function decodeOutlineResponse(reader: BinaryReader): OutlineResponse {
    const symbols = (() => {
        ...
        const items0: Array<OutlineSymbol> = [];
        for (let index = 0; index < length0; index += 1) {
            items0.push(decodeOutlineSymbol(reader));
        }
        return items0;
    })();
...
export function decodeOutlineSymbol(reader: BinaryReader): OutlineSymbol {
    ...
    const children = (() => {
        ...
        const items5: Array<OutlineSymbol> = [];
        for (let index = 0; index < length5; index += 1) {
            items5.push(decodeOutlineSymbol(reader));
        }
        return items5;
    })();
