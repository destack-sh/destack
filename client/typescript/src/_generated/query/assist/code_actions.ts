...
import {
    decodeDiagnosticReference,
    encodeDiagnosticReference,
    fromJsonDiagnosticReference,
    toJsonDiagnosticReference,
} from "../../source/diagnostic/diagnostic.js";
import {
    decodeApplicability,
    encodeApplicability,
    fromJsonApplicability,
    toJsonApplicability,
} from "../../source/diagnostic/suggestion.js";
import {
    decodePatchSet,
    encodePatchSet,
    fromJsonPatchSet,
...
export function decodeCodeAction(reader: BinaryReader): CodeAction {
    ...
    const diagnostics = (() => {
        ...
        const items5: Array<DiagnosticReference> = [];
        for (let index = 0; index < length5; index += 1) {
            items5.push(decodeDiagnosticReference(reader));
        }
        return items5;
    })();
...
export function fromJsonCodeAction(value: Json): CodeAction {
    ...
    return {
        ...
        applicability: jsonOptional(object, "applicability", (value) =>
            fromJsonApplicability(value),
        ),
        isPreferred: jsonBool(jsonField(object, "isPreferred")),
        diagnostics: jsonArray(jsonField(object, "diagnostics")).map((item0) =>
            fromJsonDiagnosticReference(item0),
        ),
    };
}
...
export function decodeCodeActionContext(reader: BinaryReader): CodeActionContext {
    const only = (() => {
        ...
        const items0: Array<CodeActionKind> = [];
        for (let index = 0; index < length0; index += 1) {
            items0.push(decodeCodeActionKind(reader));
        }
        return items0;
    })();
    const diagnostics = reader.readOption(() =>
        (() => {
            const length2 = reader.readNumber();
            const items2: Array<DiagnosticReference> = [];
            for (let index = 0; index < length2; index += 1) {
                items2.push(decodeDiagnosticReference(reader));
            }
            return items2;
        })(),
    );
...
export function fromJsonCodeActionContext(value: Json): CodeActionContext {
    ...
    return {
        ...
        diagnostics: jsonOptional(object, "diagnostics", (value) =>
            jsonArray(value).map((item0) => fromJsonDiagnosticReference(item0)),
        ),
    };
}
...
export function decodeCodeActionsResponse(reader: BinaryReader): CodeActionsResponse {
    const actions = (() => {
        const length0 = reader.readNumber();
        const items0: Array<CodeAction> = [];
        for (let index = 0; index < length0; index += 1) {
            items0.push(decodeCodeAction(reader));
        }
        return items0;
    })();
