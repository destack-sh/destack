...
def encode_code(writer: BinaryWriter, value: Code) -> None:
    """Encode one Code."""
    destack._generated.program.vm.code.function.encode_function_table(
        writer, value.functions
    )
    destack._generated.program.vm.code.table.encode_side_table(writer, value.side_table)
    destack._generated.program.vm.code.state.encode_resume_table(writer, value.resume)


def decode_code(reader: BinaryReader) -> Code:
    """Decode one Code."""
    functions = destack._generated.program.vm.code.function.decode_function_table(
        reader
    )
    side_table = destack._generated.program.vm.code.table.decode_side_table(reader)
    resume = destack._generated.program.vm.code.state.decode_resume_table(reader)
...
def to_json_code(value: Code) -> Json:
    ...
    return {
        "functions": destack._generated.program.vm.code.function.to_json_function_table(
            value.functions
        ),
        "sideTable": destack._generated.program.vm.code.table.to_json_side_table(
            value.side_table
        ),
        "resume": destack._generated.program.vm.code.state.to_json_resume_table(
            value.resume
        ),
    }
...
def from_json_code(value: Json) -> Code:
    ...
    return Code(
        functions=destack._generated.program.vm.code.function.from_json_function_table(
            json_field(object_, "functions")
        ),
        side_table=destack._generated.program.vm.code.table.from_json_side_table(
            json_field(object_, "sideTable")
        ),
        resume=destack._generated.program.vm.code.state.from_json_resume_table(
