...
class LayoutMetadata(LayoutMetadataImpl):
    ...
    def encode(self, writer: BinaryWriter) -> None:
        """Encode this value."""
        encode_layout_metadata(writer, self)

    @classmethod
    def decode(cls, reader: BinaryReader) -> LayoutMetadata:
        """Decode one LayoutMetadata."""
        return decode_layout_metadata(reader)

    def to_json(self) -> Json:
        """Return this value as JSON."""
        return to_json_layout_metadata(self)

    @classmethod
    def from_json(cls, value: Json) -> LayoutMetadata:
        """Return one LayoutMetadata from one JSON value."""
        return from_json_layout_metadata(value)


def encode_layout_metadata(writer: BinaryWriter, value: LayoutMetadata) -> None:
    """Encode one LayoutMetadata."""
    encode_layout_table(writer, value.layout_table)
    entries_value_layout_by_type_0 = []
    for (
        key_value_layout_by_type_0,
        item_value_layout_by_type_0,
    ) in value.layout_by_type.items():

        def write_key_value_layout_by_type_0(writer: BinaryWriter) -> None:
            destack._generated.mir.tree.node.encode_local_node_id(
                writer, key_value_layout_by_type_0
            )

        key_bytes = nested_bytes(write_key_value_layout_by_type_0)
        entries_value_layout_by_type_0.append(
            (key_value_layout_by_type_0, item_value_layout_by_type_0, key_bytes)
        )
    entries_value_layout_by_type_0.sort(key=lambda entry: entry[2])
    writer.write_unsigned(len(entries_value_layout_by_type_0))
    for entry_value_layout_by_type_0 in entries_value_layout_by_type_0:
        destack._generated.mir.tree.node.encode_local_node_id(
            writer, entry_value_layout_by_type_0[0]
        )
        encode_layout_id(writer, entry_value_layout_by_type_0[1])


def decode_layout_metadata(reader: BinaryReader) -> LayoutMetadata:
    """Decode one LayoutMetadata."""
    layout_table = decode_layout_table(reader)
    layout_by_type = {
...
def to_json_layout_metadata(value: LayoutMetadata) -> Json:
    ...
    return {
        "layoutTable": to_json_layout_table(value.layout_table),
        "layoutByType": [
            [
                destack._generated.mir.tree.node.to_json_local_node_id(key_0),
                to_json_layout_id(item_0),
            ]
            for key_0, item_0 in value.layout_by_type.items()
        ],
    }
...
def from_json_layout_metadata(value: Json) -> LayoutMetadata:
    ...
    return LayoutMetadata(
        layout_table=from_json_layout_table(json_field(object_, "layoutTable")),
        layout_by_type={
            destack._generated.mir.tree.node.from_json_local_node_id(
                key_0
            ): from_json_layout_id(item_0)
            for key_0, item_0 in json_array(json_field(object_, "layoutByType"))
        },
    )
...
def encode_layout(writer: BinaryWriter, value: Layout) -> None:
    ...
    destack._generated.mir.metadata.trace.encode_trace_map(writer, value.trace_map)
...
def decode_layout(reader: BinaryReader) -> Layout:
    ...
    trace_map = destack._generated.mir.metadata.trace.decode_trace_map(reader)
...
def to_json_layout(value: Layout) -> Json:
    ...
    return {
        ...
        "traceMap": destack._generated.mir.metadata.trace.to_json_trace_map(
            value.trace_map
        ),
    }
...
def from_json_layout(value: Json) -> Layout:
    ...
    return Layout(
        ...
        trace_map=destack._generated.mir.metadata.trace.from_json_trace_map(
