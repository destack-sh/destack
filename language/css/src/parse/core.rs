...
pub(crate) fn rule_block_prelude(source: &str) -> ComponentValueList {
    source
        .split_once('{')
        .map(|(prelude, _)| Parser::parse_component_value_list(prelude.trim_end()))
        .unwrap_or_else(|| {
            Parser::parse_component_value_list(source.trim_end_matches(';').trim_end())
        })
}

/// Serialize one rule with default printer options.
pub(crate) fn serialize_rule(rule: &lightning::CssRule<'_>) -> String {
    serialize_value(rule)
}

/// Serialize one CSS value with default printer options.
pub(crate) fn serialize_value<T: lightning::ToCss>(value: &T) -> String {
    value
...
pub(crate) fn span_for_rule(source: &str, file: &File, rule: &lightning::CssRule<'_>) -> Span {
