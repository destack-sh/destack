...
pub(crate) use crate::format::analysis::{
    ...
    argument_is_collection_literal, argument_is_inline_closure_cast_object,
    argument_is_interpolated_template_literal, argument_is_simple_with_options,
    call_arguments_are_multiline_span, call_arguments_have_boundary_comments,
    call_arguments_preserve_blank_line_between, call_has_leading_block_callback_with_simple_tail,
...
fn build_call_argument_expansion_profiles(
    ...
    if dynamic_arguments.len() == 1 {
        return build_single_argument_expansion_profiles(
            context,
            call_node_id,
            dynamic_arguments,
            has_call_infix_annotations,
    ...
    build_multi_argument_expansion_profiles(
        context,
        call_node_id,
        dynamic_arguments,
        has_call_infix_annotations,
...
/// Build expansion profiles for one multi-argument call.
fn build_multi_argument_expansion_profiles(
    context: &DestackFormatContext<'_>,
    call_node_id: LocalNodeId<Expression>,
    dynamic_arguments: &[LocalNodeId<Argument>],
...
/// Build expansion profiles for one single-argument call.
fn build_single_argument_expansion_profiles(
    context: &DestackFormatContext<'_>,
    call_node_id: LocalNodeId<Expression>,
    dynamic_arguments: &[LocalNodeId<Argument>],
    has_call_infix_annotations: bool,
    ...
    let ensure_argument_annotation_facts = context.ensure_argument_annotation_facts(argument_id);
