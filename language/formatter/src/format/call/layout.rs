...
fn format_default_call_argument_list<'ast>(
    ...
    if can_use_plain_default_fast_path {
        f.context()
            .increment_counter("call.arguments.path.list_default_plain_fast", 1);
...
fn call_arguments_use_no_annotation_multi_argument_fast_path(
    ...
    if !call_argument_layout_facts_are_simple_multi_unannotated(layout_facts) {
        return false;
    }
    ...
    if !layout_facts.trailing_collection_argument {
        return true;
    }
...
fn format_no_annotation_multi_argument_fast_path<'ast>(
    ...
    if expression_source_len(f.context(), call_node_id) <= line_width {
        f.context()
            .increment_counter("call.arguments.path.no_annotation_inline_fast", 1);
        write_inline_call_argument_list(f, dynamic_arguments, all_plain_call_arguments)
    } else {
        f.context()
            .increment_counter("call.arguments.path.no_annotation_grouped_fast", 1);
        let mut list = list_like("(", ")", ",", dynamic_arguments);
...
fn call_arguments_use_forced_hug_last_inline_fast_path(
    ...
    dynamic_arguments.len() > 1 && !has_boundary_comments && layout_facts.force_hug_last_inline
}
...
fn format_single_call_argument_with_group<'ast>(
    ...
    if use_single_simple_argument_fast_path {
        f.context()
            .increment_counter("call.arguments.single_simple.fast_path", 1);
        f.context()
            .increment_counter("call.arguments.path.single_simple_fast_path", 1);
        write_single_call_argument_inline_wrapped(f, argument_id)?;
        return Ok(());
    }
...
fn format_call_arguments_with_group<'ast>(
    ...
    if use_forced_hug_last_inline_fast_path {
        f.context()
            .increment_counter("call.arguments.path.hug_last_forced", 1);
        write_inline_call_argument_list(f, dynamic_arguments, all_plain_call_arguments)?;
        return Ok(());
    }
