...
use crate::format::expression::{
    ...
    hard_line_break, indent, should_drop_parenthesized, soft_line_break_or_space, space,
...
fn write_first_leading_pipe_union_operand<'ast>(
    ...
    if f.context().has_prefix_annotation(expression) {
        if should_indent_operands {
            write!(
                f,
                [indent(&format_with(
                    |f: &mut DestackFormatter<'ast, '_>| {
                        write!(f, [f.context().any_prefix_annotations(expression)])?;
                        if prefer_space_before_first_pipe {
                            write!(f, [token("|"), space()])?;
                        } else {
...
/// Format leading-pipe union operands with shared annotation and comment layout.
pub(crate) fn format_leading_pipe_union<'ast>(
    ...
    let prefer_space_before_first_pipe =
        has_block_prefix_ancestor || root_prefers_inline_first_pipe;
    ...
    write!(
        ...
        [group(&format_with(|f: &mut DestackFormatter<'ast, '_>| {
            ...
            for (index, operand) in operands.iter().enumerate() {
                if index == 0 {
                    write_first_leading_pipe_union_operand(
                        f,
                        should_indent_operands,
                        prefer_space_before_first_pipe,
                        operand.expression,
                    )?;
                } else {
...
pub(crate) fn format_binary_expression<'ast>(
    ...
    // hugged union formatting
    if is_type_union
        && (should_hug_nullable_union_type(f.context(), &operands)
