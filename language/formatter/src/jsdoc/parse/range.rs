/// Find a leading tag type range.
pub(super) fn find_type_range(s: &str) -> Option<(usize, usize)> {
...
/// Find a leading tag name range.
pub(super) fn find_type_name_range(s: &str) -> Option<(usize, usize)> {
...
/// Find an inline token range.
pub(super) fn find_token_range(s: &str) -> Option<(usize, usize)> {
    let mut start = None;
    for (idx, ch) in s.char_indices() {
...
t9b: number;
    ...
    fn extract_type_name_range() {
        for (actual, expect) in [
            ...
            // unmatched square brackets have no name range
            ("[n12[]\nc12", None),
            ("[n12\nc12", None),
        ] {
            assert_eq!(
                find_type_name_range(actual).map(|(s, e)| &actual[s..e]),
                expect
            );
        }
    }
