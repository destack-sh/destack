...
/// Expand one `WalkSelections` derive invocation.
...
fn expand_input(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    Ok(quote! {
        impl #impl_generics destack_dir::WalkSelections for #ident #type_generics #where_clause {
            fn for_each_selection(&self, visit: &mut dyn FnMut(&destack_dir::Selection)) {
                #body
            }
        }
    })
...
/// Build the walk body for one Rust item.
fn body(data: &Data) -> syn::Result<TokenStream2> {
    match data {
        Data::Struct(data) => {
            let walks = data.fields.iter().enumerate().map(|(index, field)| {
        ...
        Data::Enum(data) => {
            let arms = data.variants.iter().map(|variant| {
                ...
                let walks = names.iter().map(|name| walk(quote!(#name)));

                quote!(Self::#ident #bindings => { #(#walks)* })
            });
        ...
        Data::Union(data) => Err(syn::Error::new(
            ...
            "Destack selection walks do not support unions",
        )),
    }
}
...
/// Walk one field place.
fn walk(place: TokenStream2) -> TokenStream2 {
    quote!(destack_dir::WalkSelections::for_each_selection(#place, visit);)
}
