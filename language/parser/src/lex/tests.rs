...
fn lex_source(
    ...
    Vec<destack_ast::TokenSpan>,
    Vec<destack_ast::TokenSpan>,
    destack_ast::TokenSpan,
) {
...
fn lex_source_with_tree_literals(
    ...
    let mut token_stream = TokenStream::new(Arc::new(file), language);
    let mut index = 0;
    while !token_stream.is_finished() {
        token_stream.ensure_token(index);
        let token_type = token_stream.tokens().get(index).map(|token| token.token.ty);
        let Some(token_type) = token_type else {
            break;
        };
        if token_type == TokenType::LessThan
            && token_stream.allow_tree_literals()
            && !token_stream.in_tree_literal()
        {
            token_stream.enter_tree_opening_tag();
        }
    ...
    token_stream.lex_to_end();
    let eof_token = token_stream.eof_token();
    (
        token_stream.tokens().to_vec(),
        token_stream.side_tokens().to_vec(),
        eof_token,
    )
}

fn lex_stream_tokens(input: &str, language: LanguageType) -> (Vec<Token>, Vec<Token>) {
    let file = File::from_text(
        FileId::new(0),
        "<string>".to_string(),
        Uri::from_string("<string>"),
    ...
    let mut stream = TokenStream::new(Arc::new(file), language);
    stream.lex_to_end();

    let semantic_tokens = stream.tokens().iter().map(|token| token.token).collect();
    let side_tokens = stream
        .side_tokens()
        .iter()
        .map(|token| token.token)
        .collect();
...
#[test]
fn test_token_stream_lex_regex_literal_after_if_header_without_space_javascript() {
    let (semantic_tokens, side_tokens) =
        lex_stream_tokens("if(1)/  foo/", LanguageType::JavaScript);
...
#[test]
fn test_token_stream_lex_regex_literal_after_while_header_without_semicolon_javascript() {
    let (semantic_tokens, side_tokens) =
        lex_stream_tokens("while (1) /foo/", LanguageType::JavaScript);
