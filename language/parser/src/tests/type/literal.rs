...
use destack_dir::{
    ...
    Name, NodeType, Parameter, ScalarLiteral, TypeDeclaration, TypeExpression, TypeLiteral,
    TypeMember,
};
  ...
  method: <Expected>(
    ...
    assert_node!(parser.tree, expr_id, Expression::Declaration(decl_id) => {
        assert_node!(parser.tree, *decl_id, Declaration::Type(TypeDeclaration { name, export, place: _, is_ambient, is_nominal, mutability, generic_parameters, where_clauses, value }) => {
            ...
            assert_node!(parser.tree, *value, TypeExpression::Object { members: properties } => {
                assert_eq!(properties.len(), 1);
                assert_node!(parser.tree, properties[0], TypeMember::Field { is_static, is_optional, is_readonly, name, declared_type } => {
                    assert!(!*is_static);
                    assert!(!*is_optional);
                    assert!(!*is_readonly);
    ...
    assert_node!(parser.tree, expression_id, Expression::Declaration(declaration_id) => {
        assert_node!(parser.tree, *declaration_id, Declaration::Type(TypeDeclaration { name, export, place: _, is_ambient, is_nominal, mutability, generic_parameters, where_clauses, value }) => {
            ...
            assert_node!(parser.tree, *value, TypeExpression::Object { members } => {
                ...
                assert_node!(parser.tree, members[0], TypeMember::Field { is_static, is_optional, is_readonly, name, declared_type } => {
                    assert!(!*is_static);
                    assert!(!*is_optional);
                    assert!(!*is_readonly);
                    assert_node!(name, Name::Identifier(name) => {
