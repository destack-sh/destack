...
use core::ptr::{NonNull, addr_of, addr_of_mut, copy, copy_nonoverlapping};
...
/// with respect to `T`.
#[repr(C)]
pub(crate) union RawSmallVec<T, const N: usize> {
...
impl<T, const N: usize> RawSmallVec<T, N> {
    ...
    const fn as_ptr_inline(&self) -> *const T {
        // #Safety: we only expose the pointer and never read the inline storage
        // dereferencing the pointer stays unsafe and requires callers to uphold invariants
    ...
    /// #Safety: the vector must be on the heap
    #[inline]
    ...
    /// #Safety: `new_capacity` must be non zero, greater or equal to the length, and `T` must not be a ZST
        ...
        let new_ptr = unsafe {
            if len == 0 || !was_on_heap {
                ...
                let new_ptr = NonNull::new(new_ptr)
                    .ok_or(CollectionAllocErr::AllocErr { layout: new_layout })?;
...
impl TaggedLen {
    #[inline]
    pub(crate) const fn new(len: usize, on_heap: bool, is_zst: bool) -> Self {
        if is_zst {
    ...
    #[inline]
    #[must_use]
    pub(crate) const fn on_heap(self, is_zst: bool) -> bool {
        if is_zst {
            false
        } else {
            (self.0 & 1_usize) == 1
    ...
    #[inline]
    pub(crate) const fn value(self, is_zst: bool) -> usize {
...
pub struct Drain<'a, T, const N: usize> {
    ...
    //
    // #Safety: members in vec[tail_start..tail_start + tail_len] stay initialized
    // even though the vector length is shorter during iteration
    tail_start: usize,
    tail_len: usize,
...
pub struct IntoIter<T, const N: usize> {
    // #Safety: the tagged end decides whether the data lives on the heap
    // and every item in begin..end stays initialized during iteration
    raw: RawSmallVec<T, N>,
    begin: usize,
    end: TaggedLen,
    _marker: PhantomData<T>,
...
// #Safety: the iterator owns its contents, so sending (or sharing) it matches sharing a `SmallVec<T, N>`
...
impl<T, const N: usize> SmallVec<T, N> {
    ...
    /// # Safety
    ///
    /// Ensure `len <= N` and all elements in `buf[..len]` are initialized.
    #[inline]
...
impl<T, const N: usize> SmallVec<T, N> {
    ...
    /// Set the tag to be on the heap.
    ///
    /// #Safety: ensure the active union member is `self.raw.heap`.
    #[inline]
    unsafe fn set_on_heap(&mut self) {
        self.len = TaggedLen::new(self.len(), true, Self::is_zst());
    }

    /// Set the tag to be inline.
    ///
    /// #Safety: ensure the active union member is `self.raw.inline`.
    #[inline]
    ...
    /// # Safety
    ///
    /// Ensure `new_len <= self.capacity()` and that every element in `..self.len` is initialized.
    #[inline]
    ...
    pub fn into_vec(self) -> Vec<T> {
        ...
        if !self.spilled() {
            ...
            let this = ManuallyDrop::new(self);
            // #Safety: the pointer comes from the global allocator with proper alignment,
            // the allocation matches `cap * size_of::<T>()`, `len <= cap`, the first `len` entries
            // are initialized `T` values, and the allocation fits within `isize::MAX`
            unsafe {
    ...
    /// Create a `SmallVec` directly from the raw components of another `SmallVec`.
    ///
    /// # Safety
    ///
    /// Uphold all invariants below to avoid allocator corruption.
...
impl<T, const N: usize> SmallVec<T, N> {
    ...
    ///
    /// #Safety: ensure `n <= Self::inline_size()`.
    ...
    ///
    /// #Safety: ensure both the vector length and spare capacity are at least `src.len()`.
    ///
    ...
    /// Create a `SmallVec` value based on the contents of `slice`.
    /// This will use the inline storage, not the heap.
    ///
    /// #Safety: ensure `slice.len() <= Self::inline_size()`.
...
impl<T, const N: usize, const M: usize> From<[T; M]> for SmallVec<T, N> {
    fn from(array: [T; M]) -> Self {
        if M > N {
            ...
            // #Safety: the value M <= this.capacity()
            unsafe {
