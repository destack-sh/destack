...
mod tests {
    ...
    /// Visit nested HTML nodes through the standard visitor entry points.
    #[test]
    fn test_walk_root_visits_nested_nodes() {
        ...
        walk_root(&mut visitor, &tree, &document.into_any());

        assert_eq!(visitor.visited, vec![2, 1, 0]);
    }

    /// Stop recursion when one capturing visitor sees one document root.
    #[test]
