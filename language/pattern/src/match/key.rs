...
impl Matcher<'_, '_> {
    ...
    /// Match one key stored on a containing DIR node.
    pub(crate) fn match_node_key(
        &self,
        nodes: &PatternNodes<'_>,
        pattern_owner: dir::LocalNodeIdAny,
        candidate_owner: dir::LocalNodeIdAny,
        ...
        match (pattern, candidate) {
            (dir::Key::Name(pattern), dir::Key::Name(candidate)) => self.match_name(
                nodes,
                pattern_owner,
                candidate_owner,
                pattern,
            ...
            (dir::Key::Expression(pattern), dir::Key::Expression(candidate)) => {
    ...
    /// Match two optional keys stored on containing DIR nodes.
    pub(crate) fn match_optional_node_key(
        &self,
        nodes: &PatternNodes<'_>,
        pattern_owner: dir::LocalNodeIdAny,
        candidate_owner: dir::LocalNodeIdAny,
        pattern: Option<dir::Key>,
        candidate: Option<dir::Key>,
        bindings: &mut Bindings,
    ) -> Result<bool, MatchError> {
        match (pattern, candidate) {
            (Some(pattern), Some(candidate)) => self.match_node_key(
                nodes,
                pattern_owner,
                candidate_owner,
                pattern,
