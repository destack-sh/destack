...
use crate::{ByteRange, FileId, ProvenanceId, Span};
...
struct NodeRange {
    ...
    range: ByteRange,
    /// The provenance of this exact range.
...
impl PositionIndex {
    ...
    fn build(source_index: &SourceIndex) -> Self {
        ...
        for (index, range) in source_index.enclosing_ranges.iter().copied().enumerate() {
            let node_id = source_index.node_id(index);
...
pub struct SourceIndex {
    /// The first node id indexed by the dense enclosing range column.
    first_node_id: u32,
    /// Source file runs for node ranges.
    file_runs: Vec<SourceFileRun>,
    /// Enclosing ranges of all nodes, indexed relative to the first node id.
...
impl SourceIndex {
    ...
    /// Create an empty source index beginning at one node id.
    pub fn with_base(first_node_id: u32, capacity: usize) -> Self {
        Self {
            first_node_id,
            file_runs: Vec::new(),
    ...
    /// Return the number of nodes carrying enclosing source ranges.
    pub fn len(&self) -> usize {
        self.enclosing_ranges.len()
    }

    /// Return whether no node carries an enclosing source range.
    pub fn is_empty(&self) -> bool {
        self.enclosing_ranges.is_empty()
    }

    /// Return whether this index has an enclosing span for a node id.
    #[inline]
    pub fn contains_node(&self, node_id: u32) -> bool {
        self.try_node_index(node_id).is_some()
    }
    ...
    pub fn append_from(&mut self, source_id: u32) {
        let source_index = self.node_index(source_id);
        let target_id = self.node_id(self.enclosing_ranges.len());
        let file = self.file_for_node(source_id);
        let enclosing = self.enclosing_ranges[source_index];
        ...
        for ranges in &mut self.range_columns {
            if let Some(source) = Self::get_node_range(ranges, source_id) {
    ...
    pub fn set(&mut self, node_id: u32, span: Span) {
        ...
        // reject file changes that would invalidate exact source provenance
        if previous_file != file {
            assert!(
                !self.has_side_ranges(node_id),
                "cannot move indexed source ranges to another source file"
            );
        }
    ...
    /// Truncate the index before one next node id.
    #[inline]
    pub fn truncate(&mut self, next_node_id: u32) {
        let retained_node_count = next_node_id
            .checked_sub(self.first_node_id)
            .unwrap_or_else(|| panic!("node id {next_node_id} is before this source index"))
            as usize;
        assert!(
            retained_node_count <= self.enclosing_ranges.len(),
            "node id {next_node_id} is after this source index"
        );
        ...
        let file_run_count = self
            .file_runs
            .partition_point(|run| run.first_node_id < next_node_id);
        self.file_runs.truncate(file_run_count);

        for ranges in &mut self.range_columns {
            Self::prune_node_ranges(ranges, next_node_id);
        }

        let sparse_range_count = self
            .sparse_ranges
            .partition_point(|entry| entry.key.source_id < next_node_id);
    ...
    pub fn set_side(
        ...
        self.set_side_range(node_id, span_type, span.range(), provenance);
    }

    /// Set a file-local side range for one node.
    #[inline]
    pub fn set_side_range(
        &mut self,
        node_id: u32,
        span_type: NodeSpanType,
        range: ByteRange,
        ...
        if let Ok(column) = NodeSpanColumn::try_from(span_type) {
            ...
            Self::set_node_range(ranges, node_id, range, provenance);
        } else {
            let key = NodeSpanKey::new(node_id, span_type);
            self.set_sparse_range(key, range, provenance);
        }
    }
    ...
    pub fn get_side_range(&self, node_id: u32, span_type: NodeSpanType) -> Option<ByteRange> {
        if let Ok(column) = NodeSpanColumn::try_from(span_type) {
            Self::get_node_range(self.range_column(column), node_id).map(|entry| entry.range)
        } else {
    ...
    /// Return the provenance of one indexed source range when present.
    #[inline]
    pub fn provenance(&self, key: NodeSpanKey) -> Option<ProvenanceId> {
        if let Ok(column) = NodeSpanColumn::try_from(key.span_type) {
            Self::get_node_range(self.range_column(column), key.source_id)
                .map(|entry| entry.provenance)
        } else {
            self.get_sparse_range(key).map(|entry| entry.provenance)
        }
    }
    ...
    /// Find the innermost typed node span that fully contains a span.
    ...
    #[inline]
    pub fn set_main(&mut self, node_id: u32, span: Span, provenance: ProvenanceId) {
        self.set_side(node_id, NodeSpanType::Main, span, provenance);
    }
    ...
    pub fn get(&self, node_id: u32) -> Span {
        self.expand_range(node_id, self.enclosing_ranges[self.node_index(node_id)])
    }
    ...
    pub fn spans_at(&self, file: FileId, offset: u32) -> Vec<Span> {
        ...
        for entry in enclosing {
            ...
            for column in NodeSpanColumn::ALL {
                let Some(entry) = Self::get_node_range(self.range_column(column), entry.source_id)
                else {
                    continue;
                };
                let range = entry.range;
                let span = self.expand_range(entry.node_id, range);
    ...
    pub fn get_range(&self, node_id: u32) -> ByteRange {
        self.enclosing_ranges[self.node_index(node_id)]
    }

    /// Get the span for a node by its id when it is present.
    #[inline]
    pub fn try_get(&self, node_id: u32) -> Option<Span> {
        self.try_node_index(node_id)
            .and_then(|index| self.enclosing_ranges.get(index))
            .copied()
    ...
    fn get_node_range(ranges: &[NodeRange], node_id: u32) -> Option<NodeRange> {
        ranges
    ...
    fn set_node_range(
        ...
        match ranges.binary_search_by_key(&node_id, |entry| entry.node_id) {
            Ok(index) => {
                ...
                ranges[index].provenance = provenance;
            }
            Err(index) => ranges.insert(
                index,
                NodeRange {
                    node_id,
                    range,
    ...
    fn prune_node_ranges(ranges: &mut Vec<NodeRange>, next_node_id: u32) {
        let retained_range_count = ranges.partition_point(|entry| entry.node_id < next_node_id);
        ranges.truncate(retained_range_count);
    }

    /// Return one sparse typed range by node span key.
    fn get_sparse_range(&self, key: NodeSpanKey) -> Option<SparseNodeRange> {
        self.sparse_ranges
            .binary_search_by_key(&key, |entry| entry.key)
            .ok()
            .map(|index| self.sparse_ranges[index])
    }

    /// Set one sparse typed range by node span key.
    fn set_sparse_range(&mut self, key: NodeSpanKey, range: ByteRange, provenance: ProvenanceId) {
        if let Some(last) = self.sparse_ranges.last_mut() {
            ...
            if last.key < key {
                self.sparse_ranges.push(SparseNodeRange {
                    key,
                    range,
                    provenance,
                });
        ...
        {
            Ok(index) => {
                self.sparse_ranges[index].range = range;
                self.sparse_ranges[index].provenance = provenance;
            }
            Err(index) => self.sparse_ranges.insert(
                index,
                SparseNodeRange {
                    key,
                    range,
    ...
    /// Return whether one node has indexed side ranges.
    fn has_side_ranges(&self, node_id: u32) -> bool {
        let dense = self.range_columns.iter().any(|ranges| {
            ranges
                .binary_search_by_key(&node_id, |entry| entry.node_id)
                .is_ok()
        });
        let start = self
            .sparse_ranges
            .partition_point(|entry| entry.key.source_id < node_id);
        let sparse = self
            .sparse_ranges
            .get(start)
            .is_some_and(|entry| entry.key.source_id == node_id);

        dense || sparse
    }

    /// Return the dense index of one node id when it belongs to this index.
    #[inline]
    fn try_node_index(&self, node_id: u32) -> Option<usize> {
        let index = node_id.checked_sub(self.first_node_id)? as usize;

        (index < self.enclosing_ranges.len()).then_some(index)
    }

    /// Return the dense index of one node id.
    #[inline]
    fn node_index(&self, node_id: u32) -> usize {
        self.try_node_index(node_id)
            .unwrap_or_else(|| panic!("node id {node_id} is outside this source index"))
    }

    /// Return the node id at one dense index.
    #[inline]
    fn node_id(&self, index: usize) -> u32 {
        Self::node_id_from(self.first_node_id, index)
    }

    /// Return the node id at one dense index and base.
    #[inline]
    fn node_id_from(first_node_id: u32, index: usize) -> u32 {
        let index = u32::try_from(index)
            .unwrap_or_else(|_| panic!("source index exhausted its node id space"));

        first_node_id
            .checked_add(index)
            .unwrap_or_else(|| panic!("source index exhausted its node id space"))
    }
    ...
    fn begin_file_run(&mut self, file: FileId) {
        let node_id = self.node_id(self.enclosing_ranges.len());
    ...
    fn set_file_for_node(&mut self, node_id: u32, file: FileId) {
        ...
        let next_node_id = self.node_id(self.enclosing_ranges.len());
...
mod tests {
    ...
    const PROVENANCE: ProvenanceId = ProvenanceId::new(0);
    ...
    fn test_route_typed_ranges_through_storage_columns() {
        ...
        for (span_type, span) in ranges {
            source_index.set_side(0, span_type, span, PROVENANCE);
        }
    ...
    #[test]
    fn test_truncate_spans_drops_file_runs() {
        let first_file = FileId::new(1);
        let second_file = FileId::new(2);
        let mut source_index = SourceIndex::new();
    ...
    #[test]
    fn test_truncate_spans_truncates_range_columns() {
        ...
        source_index.set_main(0, Span::new(file, 1, 9), PROVENANCE);
        source_index.set_main(2, Span::new(file, 21, 29), PROVENANCE);
        ...
        source_index.set_side(2, NodeSpanType::Head, Span::new(file, 20, 24), PROVENANCE);
        source_index.set_side(
            2,
            NodeSpanType::Region(NodeSpanRegion::Parentheses),
            Span::new(file, 20, 30),
    ...
    fn test_roundtrip_serialized_ranges_keeps_file_identity() {
        ...
        source_index.append(Span::new(first_file, 0, 10));
        source_index.append(Span::new(second_file, 0, 8));
        source_index.set_main(1, Span::new(second_file, 1, 4), PROVENANCE);
        source_index.set_side(
            1,
            NodeSpanType::Head,
            Span::new(second_file, 1, 2),
            PROVENANCE,
        );
        source_index.set_side(
            1,
