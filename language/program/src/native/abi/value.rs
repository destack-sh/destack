...
impl NativeValue {
    ...
    pub fn to_value(self) -> Result<Value, NativeValueError> {
        ...
        match tag {
            ...
            NativeValueTag::Int => {
                let width = self.integer_width(ValueTag::Int)?;

                Ok(Value::int(self.signed_wide(width), width))
            }
            NativeValueTag::UInt => {
                let width = self.integer_width(ValueTag::UInt)?;
            ...
            NativeValueTag::HeapReference => {
                let bits = usize::try_from(self.low).map_err(|_| NativeValueError::OutOfRange {
                    value_type: ValueTag::HeapReference,
                })?;
            ...
            NativeValueTag::SharedHeapReference => {
                let bits = usize::try_from(self.low).map_err(|_| NativeValueError::OutOfRange {
                    value_type: ValueTag::SharedHeapReference,
                })?;
    ...
    fn integer_width(self, value_type: ValueTag) -> Result<u16, NativeValueError> {
