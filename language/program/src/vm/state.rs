...
use crate::{FrameSlotId, FrameStateId};
use destack_mir as mir;

/// VM resume recipes keyed by execution frame state.
...
impl ResumeTable {
    ...
    pub fn next_id(&self) -> FrameStateId {
        FrameStateId(self.states.len() as u32)
    }

    /// Add one resume state.
    pub fn push(&mut self, id: FrameStateId, state: ResumeState) {
    ...
    pub fn state(&self, id: FrameStateId) -> Option<&ResumeState> {
        self.states.get(id.0 as usize)
    }

    /// Return one resume state id by lowered program point.
    pub fn state_id_at(&self, point: ProgramPoint) -> Option<FrameStateId> {
