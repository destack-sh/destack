...
use crate::vm::{Cell, CellLayout, address_space_from_reference, cell_layout_from_address_space};
...
impl Layout {
    ...
    pub fn field(&self, index: u32) -> Option<FieldLayout> {
    ...
    pub fn element(&self) -> Option<ElementLayout> {
        match &self.shape {
...
impl TypeTable {
    ...
    pub fn new(types: HashMap<mir::LocalNodeId<mir::Type>, Layout>) -> Self {
        Self { types }
    }
    ...
    /// Return one compiled layout by execution storage layout id.
    pub fn layout_for_storage_id(&self, layout: StorageLayoutId) -> Option<&Layout> {
        self.layout(Self::type_for_storage_id(layout))
    }

    /// Return the MIR type id encoded by one execution storage layout id.
    pub fn type_for_storage_id(layout: StorageLayoutId) -> mir::LocalNodeId<mir::Type> {
        mir::LocalNodeId::new(layout.0)
    }

    /// Return the execution storage layout id for one MIR type id.
    pub fn storage_layout_id(ty: mir::LocalNodeId<mir::Type>) -> StorageLayoutId {
        StorageLayoutId(ty.id)
    }
