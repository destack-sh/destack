...
use destack_mir::MemoryOrdering;
...
impl From<MemoryOrdering> for AtomicOrder {
    /// Convert a MIR memory ordering into a lowered atomic order.
    #[inline(always)]
    fn from(ordering: MemoryOrdering) -> Self {
        match ordering {
            MemoryOrdering::Relaxed => Self::Relaxed,
            MemoryOrdering::Acquire => Self::Acquire,
            MemoryOrdering::Release => Self::Release,
            MemoryOrdering::AcquireRelease => Self::AcquireRelease,
            MemoryOrdering::SequentiallyConsistent => Self::SequentiallyConsistent,
        }
    }
}
...
impl AtomicOrder {
    ...
    #[inline(always)]
    pub const fn to_std_load(self) -> Result<Ordering, Error> {
        match self {
    ...
    #[inline(always)]
    pub const fn to_std_store(self) -> Result<Ordering, Error> {
        match self {
    ...
    #[inline(always)]
    pub const fn to_std_fence(self) -> Result<Ordering, Error> {
        match self {
    ...
    #[inline(always)]
    pub const fn to_std_compare_exchange_failure(self) -> Result<Ordering, Error> {
        match self {
            Self::Release | Self::AcquireRelease => Err(Error::invalid_instruction()),
            other => Ok(other.to_std()),
        }
    ...
    #[inline(always)]
    pub const fn to_std_update_failure(self) -> Ordering {
        match self {
...
pub struct AtomicReadModifyWriteShape {
    ...
    pub operator: AtomicReadModifyWriteOperator,
}

impl AtomicReadModifyWriteShape {
    /// Create one atomic read-modify-write shape.
    #[inline(always)]
    pub const fn new(shape: AtomicShape, operator: AtomicReadModifyWriteOperator) -> Self {
        Self { shape, operator }
    }
