use destack_mir::{BinaryOperator, FloatType, UnaryOperator};
...
impl BinaryFloat {
    ...
    #[inline(always)]
    pub const fn new(format: FloatType, kernel: BinaryFloatKernel) -> Self {
        Self {
    ...
    #[inline(always)]
    pub fn decode(self) -> Result<(FloatType, BinaryFloatKernel)> {
...
impl UnaryFloat {
    ...
    #[inline(always)]
    pub const fn new(format: FloatType, kernel: UnaryFloatKernel) -> Self {
        Self {
            field: float_format_field(format) | (kernel.field() << FLOAT_KERNEL_SHIFT),
        }
    }
    ...
    #[inline(always)]
    pub fn decode(self) -> Result<(FloatType, UnaryFloatKernel)> {
        let format = float_format_from_field(self.field & FLOAT_FORMAT_MASK)?;
...
impl BinaryFloatKernel {
    ...
    #[inline(always)]
    pub const fn select(operator: BinaryOperator) -> Option<Self> {
        match operator {
...
impl UnaryFloatKernel {
    ...
    #[inline(always)]
    pub const fn select(operator: UnaryOperator) -> Option<Self> {
        match operator {
            UnaryOperator::FloatNegate => Some(Self::Negate),
            _ => None,
        }
    }
...
#[inline(always)]
const fn float_format_field(format: FloatType) -> u32 {
    match format {
        FloatType::Float16 => FLOAT_FORMAT_FLOAT16,
        FloatType::Bfloat16 => FLOAT_FORMAT_BFLOAT16,
        FloatType::Float32 => FLOAT_FORMAT_FLOAT32,
        FloatType::Float64 => FLOAT_FORMAT_FLOAT64,
    }
}

/// Decode one float format.
#[inline(always)]
const fn float_format_from_field(field: u32) -> Result<FloatType> {
    match field {
        FLOAT_FORMAT_FLOAT16 => Ok(FloatType::Float16),
        FLOAT_FORMAT_BFLOAT16 => Ok(FloatType::Bfloat16),
        FLOAT_FORMAT_FLOAT32 => Ok(FloatType::Float32),
        FLOAT_FORMAT_FLOAT64 => Ok(FloatType::Float64),
        _ => Err(Error::invalid_instruction()),
    }
}
