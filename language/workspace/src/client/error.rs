use crate::protocol::{ProtocolCodecError, ProtocolError, QueryPayloadCodecError};
use crate::{PayloadReceiveError, TransportError};
...
pub enum ClientError {
    ...
    /// Artifact payload encoding or decoding error.
    ArtifactPayload(destack_serde::Error),
...
impl std::fmt::Display for ClientError {
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ArtifactPayload(error) => write!(formatter, "artifact payload error: {error}"),
...
impl std::error::Error for ClientError {
    ...
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            Self::ArtifactPayload(error) => Some(error),
