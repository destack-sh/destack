...
use super::Server;
use crate::connection::state::OpenRoot;
use crate::protocol::{
...
impl Server {
    ...
    pub(super) fn handle_open_root(
        ...
        let workspace = self.workspaces.open(&request.workspace)?;
        let root = self.normalize_root(workspace.as_ref(), &request.root)?;
        let entry = OpenRoot {
            workspace: workspace.home().to_path_buf(),
            root: root.clone(),
        };
    ...
    pub(super) fn handle_close_root(
        ...
        self.workspaces.release(&entry.workspace, &entry.root)?;
    ...
    fn open_root(&self, entry: OpenRoot) -> Result<(RootId, bool), ProtocolError> {
        ...
        self.workspaces.acquire(&entry.workspace, &entry.root)?;

        let mut state = self.state.lock();
        if let Some(existing) = state.handle(&entry) {
            let _ = self.workspaces.release(&entry.workspace, &entry.root);
            return Ok((existing, false));
        }
    ...
    pub(super) fn resolve_root(
        ...
        let workspace = self.workspaces.workspace(&entry.workspace).ok_or_else(|| {
