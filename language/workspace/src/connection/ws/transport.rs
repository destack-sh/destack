            ...
            match message.opcode {
                ...
                OPCODE_TEXT => {
                    return Err(TransportError::from(
                        WorkspaceWebSocketError::UnsupportedFrame("text frames are not supported"),
                    ));
                }
                _ => {
                    return Err(TransportError::from(
                        WorkspaceWebSocketError::UnsupportedFrame("unknown opcode"),
                    ));
                }
            }
        }
        ...
        loop {
            ...
            if message_opcode.is_none() {
                if frame.opcode == OPCODE_CONTINUATION {
                    return Err(TransportError::from(
                        WorkspaceWebSocketError::UnsupportedFrame("unexpected continuation frame"),
                    ));
                }
                message_opcode = Some(frame.opcode);
            } else if frame.opcode != OPCODE_CONTINUATION {
                return Err(TransportError::from(
                    WorkspaceWebSocketError::UnsupportedFrame("expected continuation frame"),
                ));
            }
...
impl WebSocketFrame {
    ...
    fn read(stream: &mut TcpStream) -> Result<Self, TransportError> {
        ...
        if !is_masked {
            return Err(TransportError::from(
                WorkspaceWebSocketError::UnsupportedFrame("client frames must be masked"),
            ));
        }
        if Self::is_control_opcode(opcode) && !fin {
            return Err(TransportError::from(
                WorkspaceWebSocketError::UnsupportedFrame("control frames must not be fragmented"),
            ));
        }
        let len = usize::try_from(len).map_err(|_| {
            TransportError::from(WorkspaceWebSocketError::PayloadTooLarge {
                limit: usize::MAX,
                actual: usize::MAX,
            })
        })?;
        if Self::is_control_opcode(opcode) && len > CONTROL_PAYLOAD_LIMIT {
            return Err(TransportError::from(
                WorkspaceWebSocketError::PayloadTooLarge {
...
impl From<WorkspaceWebSocketError> for TransportError {
    /// Convert a WebSocket error into a transport error.
    fn from(error: WorkspaceWebSocketError) -> Self {
        match error {
            WorkspaceWebSocketError::Frame(error) => TransportError::Frame(error),
            WorkspaceWebSocketError::Io(error) => TransportError::Io(error),
            WorkspaceWebSocketError::Closed => TransportError::Closed,
