use super::{PayloadPrepareError, PayloadSendError};
use crate::Transport;
use crate::protocol::{
...
impl PayloadSender {
    /// Create a payload sender for one response.
    ...
    pub(crate) fn prepare(
    ...
    ) -> Result<BinaryPayload, PayloadPrepareError> {
        ...
        let chunk_limit = payload_chunk_bytes(self.limits).map_err(PayloadPrepareError::Codec)?;
        if chunk_limit == 0 {
            return Err(PayloadPrepareError::ChunkLimitTooSmall);
        }
