...
fn read_request(stream: &mut TcpStream) -> Result<Vec<u8>, WebSocketError> {
...
fn parse_request(request: &[u8], path: &str, token: &str) -> Result<String, WebSocketError> {
    ...
    if !path_has_token(parsed.path, path, token) {
        return Err(WebSocketError::Handshake("invalid websocket token"));
    }
