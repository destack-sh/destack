...
pub struct SourceUpdateResult {
    ...
    pub after: Revision,
    /// Workspace updates produced by the source update.
...
impl Workspace {
    ...
    pub fn save_file(&self, path: &Path, change: FileChange) -> Result<UpdateBatch, Error> {
    ...
    /// Apply one file update through the workspace.
    pub fn apply_file(&self, path: &Path, update: FileChange) -> Result<UpdateBatch, Error> {
    ...
    /// Apply one atomic source update through the workspace.
    pub fn apply_source_update(
        &self,
        root: &Path,
        update: SessionSourceUpdate,
    ) -> Result<SourceUpdateResult, Error> {
    ...
    /// Extend one workspace result with one session change result.
    fn extend_with_change_result(
        ...
        // rebuild workspace updates from the committed repository change
    ...
    /// Build one workspace result for a committed session change.
    ...
    /// Convert session file updates into workspace file updates.
    fn file_updates(
        &self,
        session: &Session,
        revision: Revision,
        ...
        // project each session update into workspace protocol shape
    ...
    /// Convert one session file update into a workspace file update.
    fn file_update(
        &self,
        session: &Session,
        revision: Revision,
