...
// each tier is separated by 200 to prevent overlap after length/spread penalties
...
/// Higher score = better match. `matched_indices` contains byte positions
/// for UI highlighting.
pub(super) fn score_completion(candidate: &str, prefix: &str) -> Option<FuzzyMatch> {
    ...
    // camelCase/snake_case word boundary match
