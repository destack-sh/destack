...
pub fn semantic_tokens(session: &Session, file: FileId) -> Vec<SemanticToken> {
    with_query_context_for_file(session, file, |ctx| {
        ...
        for (expression_id, expression) in dir_tree.iter_nodes_of_type::<dir::Expression>() {
            ...
            match expression {
                ...
                | dir::Expression::ModuleReference { target_symbol, .. } => {
                    ...
                    let Some(target_ctx) = session.query_context(&target_guard) else {
                        continue;
                    };
            ...
            let (token_type, modifiers) = match member {
                ...
                dir::Member::Method {
                    ...
                    if matches!(
                        ...
                        dir::FunctionAbstraction::Abstract
        ...
        for (item_id, item) in dir_tree.iter_nodes_of_type::<dir::DependencyItem>() {
            ...
            let token_type = match item {
                ...
                | dir::DependencyItem::Remote { target_symbol, .. } => {
                    let target_module = session.modules.get(target_symbol.module_id);
                    let target_guard = target_module.read();
                    if let Some(target_ctx) = session.query_context(&target_guard) {
                        let target_symbols = target_ctx.symbols();
                        let symbol = target_symbols.get_symbol(target_symbol.local_id);
        ...
        // filter out zero width spans with no visible source text
        tokens.retain(|t| t.span.start < t.span.end);

        // sort tokens deterministically by span and specificity
        tokens.sort_by_key(semantic_token_sort_key);

        // deduplicate overlapping tokens by explicit specificity
...
/// Compute a deterministic sort key for semantic tokens.
fn semantic_token_sort_key(token: &SemanticToken) -> (u32, u32, u8, u8, u32) {
    // compute the token specificity ranks
    let type_rank = semantic_token_type_rank(token.token_type);
    let modifier_rank = token.modifiers.bits().count_ones() as u8;
...
/// Rank token types by specificity.
fn semantic_token_type_rank(token_type: SemanticTokenType) -> u8 {
    match token_type {
        SemanticTokenType::Variable => 0,
        SemanticTokenType::Number | SemanticTokenType::String | SemanticTokenType::Comment => 1,
        SemanticTokenType::Type
        | SemanticTokenType::Namespace
        | SemanticTokenType::Keyword
        | SemanticTokenType::Operator
        | SemanticTokenType::Modifier => 2,
        ...
        | SemanticTokenType::Regexp => 4,
        SemanticTokenType::Parameter => 5,
        SemanticTokenType::TypeParameter => 6,
    }
}

/// Compute a specificity key for overlap resolution.
fn semantic_token_specificity_key(token: &SemanticToken) -> (u8, u8, u32, u32) {
    // prefer higher ranks, then narrower spans, then stable modifier bits
    let type_rank = semantic_token_type_rank(token.token_type);
    let modifier_rank = token.modifiers.bits().count_ones() as u8;
    let span_len = token.span.end.saturating_sub(token.span.start);
    let inverse_span_len = u32::MAX.saturating_sub(span_len);
...
/// Remove duplicate tokens, preferring more specific ones.
fn deduplicate_tokens(tokens: &mut Vec<SemanticToken>) {
    ...
    for read_idx in 1..tokens.len() {
        ...
        if tokens[read_idx].span.start < tokens[write_idx].span.end {
            // prefer the more specific token for the overlapping span
            let current_key = semantic_token_specificity_key(&tokens[write_idx]);
            let next_key = semantic_token_specificity_key(&tokens[read_idx]);
