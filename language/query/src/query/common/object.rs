...
use super::{QueryContext, ScopeAtOffset, expression_scope_at_offset, sorted_enclosing_spans};
...
pub(crate) fn is_inside_object_literal_expression(ctx: &QueryContext<'_>, offset: u32) -> bool {
    // resolve enclosing spans from innermost to outermost
    let enclosing = sorted_enclosing_spans(ctx, offset, offset);
