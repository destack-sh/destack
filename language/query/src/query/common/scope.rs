...
use super::{QueryContext, enclosing_spans_with_previous};
...
pub(crate) fn scope_at_offset(ctx: &QueryContext<'_>, offset: u32) -> Option<ScopeAtOffset> {
    ...
    ast_parent_scope_at_offset(ctx, &enclosing, offset)
}

/// Resolve the nearest enclosing block or owned declaration scope at one offset.
pub(crate) fn block_scope_at_offset(ctx: &QueryContext<'_>, offset: u32) -> Option<ScopeAtOffset> {
    let enclosing = enclosing_spans_with_previous(ctx, offset);
    ...
    // damaged span stacks may still recover through ast parents
    ast_parent_block_scope_at_offset(ctx, &enclosing, offset)
}
