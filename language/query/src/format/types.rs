...
impl<'owner, 'module, 'program> TypeFormatter<'owner, 'module, 'program> {
    ...
    fn property(&self, property: &dir::TypeProperty) -> QueryResult<Option<String>> {
        ...
        let key = formatted!(self.static_key(property.key));

        match property.access {
            dir::PropertyAccess::Read(ty) => {
                let ty = formatted!(self.global(ty));
            ...
            dir::PropertyAccess::ReadWrite { read, write } if read == write => {
                let ty = formatted!(self.global(read));

                Ok(Some(format!("{key}{optional}: {ty}")))
            }
            dir::PropertyAccess::ReadWrite { read, write } => {
                let read = formatted!(self.global(read));
                let write = formatted!(self.global(write));
