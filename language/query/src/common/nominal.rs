...
use super::{QueryContext, is_type_symbol, resolve_expression_symbol};
...
pub(crate) fn resolve_member_access_symbol(
    ...
    let source_id = ctx.tree().get_source(expr_id.id);
    let span_type = Expression::member_source_part(ctx.tree(), expr_id);
    if let Some(target_symbol) = ctx
        .types()
    ...
    if let Some(target_symbol) = recorded_member_resolution(ctx, expr_id) {
        return Some(target_symbol);
    }
...
pub(crate) fn resolve_nominal_symbol_from_type_expression(
    ...
    match expression {
        ...
        Expression::Member { .. } => {
            if let Some(symbol_id) = resolve_member_access_symbol(ctx, expression_id) {
...
fn symbol_is_type_symbol(session: &Session, symbol_id: GlobalSymbolId) -> bool {
    ...
    super::with_dir_resolved_context_for_module(session, module, |ctx| {
