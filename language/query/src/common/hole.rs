...
use super::{
    QueryContext, enclosing_missing_expression, enclosing_spans_at_cursor_boundary,
    previous_significant_token, span_owns_cursor_boundary, token_text,
...
pub(crate) fn expression_slot_position(
    ...
    let source_file = ctx.program.files.get(ctx.file_id);
...
pub(crate) fn current_initializer_binding_names(
    ...
    if let Some(declarator_id) = current_initializer_declarator(ctx, offset) {
...
pub(crate) fn missing_declarator_value_at_cursor(ctx: &QueryContext<'_>, offset: u32) -> bool {
    ...
    let Some(declarator_id) = current_initializer_declarator(ctx, offset) else {
        return false;
    };
...
fn unwrap_statement_ast_expression(
    ...
    // statement wrappers do not own expression slots themselves
...
fn current_initializer_declarator(
    ...
    initializer_declarator_at_cursor_boundary(ctx, offset)
}

/// Resolve the declarator whose initializer span still owns one cursor boundary.
fn initializer_declarator_at_cursor_boundary(
    ctx: &QueryContext<'_>,
    offset: u32,
) -> Option<ast::LocalNodeId<ast::Declarator>> {
    let ast = ctx.ast_context();
    ...
    for enclosing in enclosing_spans_at_cursor_boundary(ctx, offset) {
            ...
            std::iter::once(enclosing.idx).chain(parents.walk_parents_by_id(enclosing.idx))
        {
            ...
            if !span_owns_cursor_boundary(value_span, offset) {
                continue;
            }
