...
impl ExpressionSlotPosition {
    /// Resolve the expression slot position for a parameter child.
    fn from_parameter_child(
        ...
        match parameter {
            ...
            dir::Parameter::Error => panic!("error parameter reached expression slot query"),
        }
    }

    /// Resolve the expression slot position for an argument child.
    fn from_argument_child(
    ...
    /// Resolve the expression slot position for a property child.
    fn from_property_child(
        ...
        match property {
            ...
            dir::Property::Error => panic!("error property reached expression slot query"),
        }
    }

    /// Resolve the expression slot position for a member child.
    fn from_member_child(
        member: &dir::Member,
        expr_id: dir::LocalNodeId<dir::Expression>,
    ) -> Option<Self> {
        match member {
            ...
            dir::Member::Error => panic!("error member reached expression slot query"),
        }
    }

    /// Resolve the expression slot position for an expression child.
    fn from_expression_child(
        ...
        match expression {
            ...
            dir::Expression::Assign { left, right, .. } => {
                if assign_pattern_contains_expression(parsed_tree, *left, expr_id)
                    || *right == expr_id
                {
                    return Some(Self::Value);
                }
...
impl ExpressionSlotOwner {
    /// Create a slot owner from one public slot position.
    fn from_position(position: ExpressionSlotPosition) -> Self {
        match position {
            ExpressionSlotPosition::Constructor => Self::Constructor,
            ExpressionSlotPosition::Value => Self::Value,
            ExpressionSlotPosition::Type => Self::Type,
        }
    }
}
...
impl ModuleQueryContext<'_> {
    ...
    pub(crate) fn current_initializer_binding_names(
        ...
        // initializer holes resolve through the parsed expression hole first
    ...
    pub(crate) fn is_declarator_value_hole_at_cursor(&self, source: &str, offset: u32) -> bool {
        let parsed_tree = self.tree();
    ...
    fn expression_slot_owner(&self, source: &str, offset: u32) -> Option<ExpressionSlotOwner> {
        if let Some(expr_id) = self.expression_hole_at_offset(offset) {
            return expression_hole_owner(self.tree(), self.parents(), expr_id);
        }
...
fn expression_hole_owner(
    ...
    match parsed_tree.get_node_type(parent_id) {
        ...
        dir::NodeType::Parameter => {
            ...
            ExpressionSlotPosition::from_parameter_child(parameter, expr_id)
                .map(ExpressionSlotOwner::from_position)
        }
        dir::NodeType::Argument => {
            let argument = parsed_tree.get(dir::LocalNodeId::<dir::Argument>::new(parent_id));
            ExpressionSlotPosition::from_argument_child(argument, expr_id)
                .map(ExpressionSlotOwner::from_position)
        }
        dir::NodeType::Property => {
            let property = parsed_tree.get(dir::LocalNodeId::<dir::Property>::new(parent_id));
            ExpressionSlotPosition::from_property_child(property, expr_id)
                .map(ExpressionSlotOwner::from_position)
        }
        dir::NodeType::Member => {
            let member = parsed_tree.get(dir::LocalNodeId::<dir::Member>::new(parent_id));
            ExpressionSlotPosition::from_member_child(member, expr_id)
                .map(ExpressionSlotOwner::from_position)
        }
        dir::NodeType::Expression => {
            let parent = parsed_tree.get(dir::LocalNodeId::<dir::Expression>::new(parent_id));
            ExpressionSlotPosition::from_expression_child(parsed_tree, parent, expr_id)
                .map(ExpressionSlotOwner::from_position)
        }
        _ => None,
    }
...
impl ModuleQueryContext<'_> {
    /// Resolve the structural context for an explicit value slot outside parser holes.
    fn open_expression_slot_owner(&self, source: &str, offset: u32) -> Option<ExpressionSlotOwner> {
        ...
        for enclosing in self.enclosing_spans_at_cursor(offset) {
            ...
            if let dir::Expression::Return { value: None } = expr {
                if self.is_cursor_after_expression_keyword(source, offset, expr_span, "return") {
            ...
            if let dir::Expression::Yield { value: None, .. } = expr {
                if self.is_cursor_after_expression_keyword(source, offset, expr_span, "yield") {
                    return Some(ExpressionSlotOwner::Value);
                }
            }
            ...
            if let dir::Expression::New { ty, .. } = expr {
                if matches!(parsed_tree.get(*ty), dir::TypeExpression::Missing)
                    && self.is_cursor_after_expression_keyword(source, offset, expr_span, "new")
                {
...
impl ModuleQueryContext<'_> {
    ...
    fn is_cursor_after_expression_keyword(
        &self,
        source: &str,
        offset: u32,
        expr_span: Span,
...
impl ModuleQueryContext<'_> {
        ...
        for enclosing in self.enclosing_spans_at_cursor(offset) {
                ...
                .chain(parents.walk_parents_by_id(enclosing.source_id))
            {
                ...
                if matches!(parsed_tree.get(value_id), dir::Expression::Missing)
                    && self.is_cursor_after_initializer_assign(declarator_span, offset)
                {
                    return Some(declarator_id);
                }

                let value_span = self.source_index().get(value_id.id);
                if !value_span.owns_cursor(offset) {
                    continue;
                }
    ...
    fn is_cursor_after_initializer_assign(&self, declarator_span: Span, offset: u32) -> bool {
        let Some(token) = self.previous_significant_token(offset) else {
            return false;
        };
