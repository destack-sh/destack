...
impl SelectionRange {
    ...
    /// Create a nested selection range from leaf-to-root spans.
    fn from_chain(chain: &[Span]) -> Self {
        let mut selection = Self::leaf(
            *chain
                .last()
                .unwrap_or_else(|| panic!("empty selection chain")),
        );

        for span in chain.iter().rev().skip(1) {
            selection = Self::with_parent(*span, selection);
        }

        selection
    }

    /// Return the depth of this selection range.
...
impl ModuleQueryContext<'_> {
    /// Return selection ranges for positions in a file.
    ///
    ...
    pub fn selection_ranges(&self, positions: &[u32]) -> Vec<SelectionRange> {
        let mut results = Vec::with_capacity(positions.len());

        // build one selection chain per requested position
        for &offset in positions {
            results.push(self.selection_range_at_offset(offset));
        }
    ...
    fn selection_range_at_offset(&self, offset: u32) -> SelectionRange {
        let enclosing = self
            .source_index()
            .get_enclosing_spans(self.file_id(), offset, offset);
        if enclosing.is_empty() {
            return SelectionRange::leaf(Span::new(self.file_id(), offset, offset));
        }
        ...
        let leaf_span = spans.first().copied().unwrap_or_else(|| {
            panic!("missing selection span after source index returned enclosing spans")
        });
        ...
        SelectionRange::from_chain(&chain)
    }
}
