...
use destack_qir::{
    AnnotationEntry, CallEntry, ExtensionEntry, ImportEntry, MemberEntry, NominalEntry,
    SpecifierEntry, SymbolEntry,
};
...
impl WorkspaceQueryContext<'_> {
    ...
    pub(crate) fn search_import_candidates(
        ...
        // collect import candidates from each indexed module
    ...
    pub(crate) fn search_symbol_candidates(&self, query: &str) -> Vec<(ProfileId, SymbolEntry)> {
        let mut entries = Vec::new();

        // collect declaration symbols from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
                entries.extend(
                    index
    ...
    /// Search workspace member candidates across indexed modules.
    pub(crate) fn search_member_candidates(&self, query: &str) -> Vec<(ProfileId, MemberEntry)> {
        let mut entries = Vec::new();

        // collect source members from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
                entries.extend(
                    index
    ...
    pub(crate) fn members_for_owner(&self, owner_symbol: dir::GlobalSymbolId) -> Vec<MemberEntry> {
        let mut entries = Vec::new();

        // collect owner members from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
                entries.extend(index.index.members.for_owner(owner_symbol));
            }
        }

        entries.sort_by(MemberEntry::cmp_source);
        entries.dedup();
    ...
    pub(crate) fn members_for_declaring(
        &self,
        declaring_symbol: dir::GlobalSymbolId,
    ) -> Vec<MemberEntry> {
        let mut entries = Vec::new();

        // collect declaration members from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
                entries.extend(index.index.members.for_declaring(declaring_symbol));
            }
        }
    ...
    pub(crate) fn search_annotation_candidates(
        ...
        // collect annotations from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
                entries.extend(
                    index
    ...
    pub(crate) fn modules_referencing_symbol(
        ...
        // collect reference memberships from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
    ...
    pub(crate) fn call_candidates_for_callee(
        ...
        // collect call edges from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
        ...
        I: IntoIterator<Item = PathBuf>,
    {
        ...
        // collect specifier rewrite candidates from each indexed module
        for profile in self.indexes() {
            for index in profile.modules() {
                entries.extend(
                    index
