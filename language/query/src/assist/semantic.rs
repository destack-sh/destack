...
impl ModuleQueryContext<'_> {
    ...
    pub fn semantic_tokens(&self) -> Vec<SemanticToken> {
        ...
        // collect local variable bindings (dir::Pattern::Binding)
        ...
        for (member_id, member) in dir_tree.iter_nodes_of_type::<dir::Member>() {
            ...
            let (token_type, modifiers) = match member {
                ...
                dir::Member::Field {
                    ...
                    let is_readonly =
                        *is_readonly || *mutability == Some(dir::Mutability::Immutable);
                    let mods =
                        modifiers_from_member_flags(true, is_readonly, *is_static, *is_abstract);
                    (SemanticTokenType::Property, mods)
                }
                dir::Member::Method {
                    signature,
        ...
        for (item_id, _) in dir_tree.iter_nodes_of_type::<dir::DependencyItem>() {
            ...
            let token_type =
                if let Some(target_symbol) = ctx.dir().dependency_symbol_target(item_id) {
                    let Some(target_ctx) = ctx.module_context(target_symbol.module_id) else {
                        continue;
                    };
                    let target_symbols = target_ctx.dir().symbols();
...
/// Map dir::SymbolKind to SemanticTokenType.
