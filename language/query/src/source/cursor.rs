...
use super::{
    SourceQueryContext, enclosing_spans_at_offsets, enclosing_spans_with_previous,
    next_significant_token, previous_significant_token,
};

/// Collect probe offsets that should share one cursor owner.
fn cursor_probe_offsets(parsed: SourceQueryContext<'_>, offset: u32) -> Vec<u32> {
    ...
    if let Some(token) = next_significant_token(parsed, offset)
...
pub(crate) fn enclosing_spans_at_cursor(
    ...
    let offsets = cursor_probe_offsets(parsed, offset);
...
pub(crate) fn offset_is_in_source_type_side_span(
    parsed: SourceQueryContext<'_>,
    offset: u32,
) -> bool {
    let previous_offset = offset.saturating_sub(1);

    for enclosing in enclosing_spans_with_previous(parsed, offset) {
...
pub(crate) fn enclosing_missing_expression(
    ...
    for enclosing in enclosing_spans_at_cursor(parsed, offset) {
        if parsed.tree().get_node_type(enclosing.idx) != dir::NodeType::Expression {
            continue;
        }
        ...
        if matches!(parsed.tree().get(expr_id), dir::Expression::Missing) {
            return Some(expr_id);
        }
    }
