...
use crate::core::WorkspaceQueryContext;
...
impl WorkspaceQueryContext<'_> {
    ...
    fn file_for_rename(&self, file_id: FileId) -> Option<Arc<File>> {
        let repository = self.repository();
        let file = repository.file(self.revision(), file_id).ok().flatten()?;
...
impl WorkspaceQueryContext<'_> {
    ...
    pub fn rename_files(&self, renames: &[FileRenameEntry]) -> Option<BatchEdit> {
        ...
        let specifier_entries =
            ctx.specifier_candidates_for_rename_paths(rename_map.keys().cloned());
        ...
        for ((profile_id, module_id), entries) in entries_by_module {
            ...
            let Some(file) = ctx.file_for_rename(module.file_id) else {
                continue;
            };
