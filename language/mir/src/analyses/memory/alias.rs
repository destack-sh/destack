...
use crate::{
    Analysis, BlockId, ConstantTable, DefinitionTable, DominatorTable, Function, Instruction,
    KnownBits, LayoutError, LayoutTable, MemoryAddress, MemoryLocation, MemoryPlace, MemoryRegion,
    MemorySize, Mutation, StorageRoot, TargetLayout, Tree, Value, ValueDefinition,
};
...
impl AliasTable {
    ...
    pub fn analyse(
        ...
        for region in regions.iter().flatten() {
            ...
            for offset in &place.indexed_offsets {
                ...
                let ty = tree.get(function.expect_value_type(offset.index));
                ...
                let bits =
                    KnownBits::analyse(offset.index, 0, function, &definitions, target, tree);
                ...
                if let Some((mut minimum, mut maximum)) = range {
                    if (minimum == 0 || maximum == 0)
                        && offset
                            .index
                            .is_known_nonzero(function, &definitions, target, tree)
                    {
                        if minimum == 0 {
                            minimum = 1;
                        }
    ...
    fn compare(
        ...
        // resolve both address definitions
    ...
    pub fn is_invariant(&self, region: &MemoryRegion, block: BlockId) -> bool {
        let region = match region {
            MemoryRegion::Address { location, .. } => {
                if matches!(location.address, MemoryAddress::Dynamic { .. }) {
                    return self.is_available(location.address.value(), block);
                }
                let resolved = self.region(location.address.value());
        ...
        if let MemoryRegion::Place(place) = region {
            if let StorageRoot::Address { value, .. } = place.root
                && !self.is_available(value, block)
            {
                return false;
            }
    ...
    fn build_regions(
        ...
        target: TargetLayout,
        tree: &Tree,
    ) -> Result<Vec<Option<MemoryRegion>>, LayoutError> {
...
struct AliasQuery<'a> {
    ...
    active: SmallVec<[(Value, MemorySize, Value, MemorySize); 8]>,
}
...
impl AliasQuery<'_> {
    ...
    fn alias(
        ...
        let result = self.table.compare(left, right)?;
        if result != AliasResult::MayAlias
        ...
        if self.active.contains(&pair) || self.active.contains(&(pair.2, pair.3, pair.0, pair.1)) {
            return Ok(AliasResult::NoAlias);
        }
        if self.remaining == 0 {
            return Ok(AliasResult::MayAlias);
    ...
    fn compare(
        ...
        let left_value = left.address.value();
        let right_value = right.address.value();
        ...
        if let Some(selection) = table.selections.get(&left_value) {
            ...
            for (index, &value) in selection.values.iter().enumerate() {
                ...
                left.address = value.into();
                if let Some(other) = other.filter(|other| other.condition == selection.condition) {
                    right.address = other.values[index].into();
                }
                let alias = self.alias(&left, &right)?;
        ...
        if let Some(ValueDefinition::BlockParameter { block, .. }) = definition {
            ...
            let region = MemoryRegion::from_address(right_value, None, None, None);
            if !is_paired && !table.is_invariant(&region, block) {
                return Ok(AliasResult::MayAlias);
            }
...
entry:
    v0: ref<int64, borrowed, 'frame, mutable, frame> = local.address l0
    return
}
"#,
        );
...
entry(v0: boolean, v1: ref<int32, borrowed, 'a, mutable, local>, v2: ref<int32, borrowed, 'a, mutable, local>):
    ...
    local.set l0, v3
    local.set l1, v3
    v4: ref<int32, borrowed, 'a, mutable, local> = local.get l0
    v5: ref<int32, borrowed, 'a, mutable, local> = local.get l1
    return
}
"#,
        );
...
entry:
    ...
    v4: uint8 = cast.truncate v3 -> uint8
...
"#,
        ...
        for (size, expected) in [
            ...
            let mut first = MemoryLocation::from_address(Value::new(1));
...
entry(v0: ref<Choice, borrowed, 'a, mutable, local>):
    v1: ptr<int32, mutable> = variant.payload.address v0, 0
    v2: ptr<uint32, mutable> = variant.payload.address v0, 1
    return
}
"#,
        );
...
entry(v0: boolean):
    v1: ptr<int32, mutable> = local.address l0
    v2: ptr<int32, mutable> = local.address l0
...
entry(v0: ptr<Pair, mutable>):
    v1: ptr<int32, mutable> = field.address v0, 1
    return
}
"#,
        );
        let function = program.tree.get(program.entry_function_id());
        let ty = function.pointee_type(Value(0), &program.tree).unwrap();
        let mut analyses = program.function_analyses();
        let error = analyses
            .alias(function, Arc::new(LayoutTable::new()), &program.tree)
            .unwrap_err();
...
"#,
        ...
        let actual = [Value(0), Value(1)].map(|value| {
            ...
            alias.is_invariant(&region, function.block(0))
        });
...
entry(v0: boolean, v1: boolean):
    ...
    v3: ref<int32, borrowed, 'frame, mutable, frame> = local.address l1
...
entry(v0: boolean):
    v1: ref<int64, borrowed, 'frame, mutable, frame> = local.address l0
    v2: ref<int64, borrowed, 'frame, mutable, frame> = local.address l1
    jump loop(v1, v2)
...
entry(v0: boolean):
    v1: ref<[int32; 3], borrowed, 'frame, mutable, frame> = local.address l0
    v2: int64 = 0
    v3: int64 = 1
    v4: ref<int32, borrowed, 'frame, mutable, frame> = element.address v1, v2
    v5: ref<int32, borrowed, 'frame, mutable, frame> = element.address v1, v3
    jump loop(v4, v5)
...
entry(v0: ptr<ptr<Pair, mutable>, mutable>):
    ...
    v3: ptr<int32, mutable> = field.address v1, 0
    v4: ptr<int32, mutable> = field.address v1, 1
    v5: ptr<int32, mutable> = field.address v2, 1
    return
}
"#,
        );
...
entry(v0: ref<[int32; 256], borrowed, 'a, mutable, local>, v1: uint8):
    ...
    v4: uint8 = and v1, v2
    v5: uint8 = add v4, v3
    v6: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v4
    v7: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v5
    v8: uint8 = add v1, v3
    v9: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v8
    v10: uint8 = intrinsic.math.arithmetic.unchecked.add(v4, v3)
    v11: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v10
    return
}
"#,
        );
...
entry(v0: ref<[Pair; 16], borrowed, 'a, mutable, local>, v1: uint64, v2: uint64):
    ...
    v6: ref<int32, borrowed, 'a, mutable, local> = field.address v4, 1
    v7: ref<int32, borrowed, 'a, mutable, local> = field.address v4, 0
    return
}
"#,
        );
...
entry(v0: ptr<[uint8; 1], mutable>):
    ...
    v3: ptr<uint8, mutable> = element.address v0, v1
    v4: ptr<uint8, mutable> = element.address v0, v2
    return
}
"#,
        );
...
entry(v0: ref<[int32; 256], borrowed, 'a, mutable, local>, v1: uint8):
    ...
    v8: uint8 = mul v6, v4
    v9: uint8 = shl v6, v5
    v10: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v6
    v11: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v7
    v12: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v8
    v13: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v9
    v14: uint8 = 0
    v15: ref<int32, borrowed, 'a, mutable, local> = element.address v0, v14
    return
}
"#,
        );
