...
/// A formatted type alias entry.
struct AliasEntry {
    /// The aliased type id.
    type_id: LocalNodeId<Type>,
    /// The alias formatting source.
    kind: AliasEntryKind,
}

/// The source for a formatted type alias entry.
enum AliasEntryKind {
    /// Synthetic alias generated by the formatter.
    Synthetic { name: String },
    /// Explicit alias defined in the MIR tree.
    Explicit { alias_id: LocalNodeId<TypeAlias> },
}

/// Order alias entries so referenced aliases appear first.
fn order_alias_entries(tree: &NodeTree, entries: &[AliasEntry]) -> Vec<usize> {
    if entries.len() <= 1 {
    ...
    fn format(&self, f: &mut MirFormatter<'a, '_>) -> FormatResult<()> {
        ...
        for (type_id, name) in &synthetic_aliases {
            alias_entries.push(AliasEntry {
                type_id: *type_id,
                kind: AliasEntryKind::Synthetic { name: name.clone() },
            });
        }
        for type_alias_id in &type_alias_ids {
            let alias = tree.get(*type_alias_id);
            alias_entries.push(AliasEntry {
                type_id: alias.ty,
                kind: AliasEntryKind::Explicit {
                    alias_id: *type_alias_id,
                },
            });
        }
        let has_type_aliases = !alias_entries.is_empty();
        if has_type_aliases {
