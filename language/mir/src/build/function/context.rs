...
use crate::{Instruction, LocalNodeId, Type, Value};

impl<'a> FunctionBuilder<'a> {
    /// Load the current execution context.
    pub fn context_current(&mut self, result_type: LocalNodeId<Type>) -> Value {
    ...
    pub fn context_replace(&mut self, context: Value, result_type: LocalNodeId<Type>) -> Value {
        let destination = self.allocate_value();
