...
impl<'a, 'b> PreferArrayFilterVisitor<'a, 'b> {
    ...
    fn extract_filter_pattern(
        ...
        let parameter_symbol = self.ctx.symbol_for_node(parameter_id)?;
    ...
    fn if_push_from_callback_body(
        ...
        let if_expression_id = if let dir::Expression::Block(block) = expression {
            ...
            self.unwrap_statement_expression(block.first_expression().unwrap())
        ...
        let if_expression = self.ctx.dir.get(if_expression_id);
    ...
    fn filter_fix(
        ...
        let replacement =
            format!("{receiver_text}.filter(({parameter_name}) => {condition_text})",);
    ...
    fn report(&mut self, expression_id: dir::LocalNodeId<dir::Expression>, pattern: FilterPattern) {
        ...
        .label("use array.filter(...) instead");
