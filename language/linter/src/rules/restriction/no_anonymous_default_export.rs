use destack_ast::{self as ast, Declaration, DependencyMode};
...
"#,
    ...
    // NOTE: `export default { foo: 1 }` test skipped - parser only supports
    // `export default identifier`, not arbitrary expressions (see parser/parse/dependency.rs)
