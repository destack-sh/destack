...
| `no-arguments-order-mismatch` | SonarQube | DIR | ✗ | 🔶 | Suggestion | Disallow arguments that appear swapped based on parameter names |
| `no-array-constructor` | ESLint | DIR | ✗ | 🔶 | Safe | Disallow `new Array()` (confusing behavior) |
...
| `no-division-by-zero` | Destack | DIR | ✗ | 🔶 | None | Disallow division where denominator is not proven non-zero |
| `no-duplicate-case` | ESLint | AST | ✓ | ✅ | None | Disallow duplicate case labels |
| `no-floating-point-equality` | Clippy | DIR | ✓ | 🔶 | Suggestion | Disallow direct `==` comparison of floats |
...
| `no-useless-increment` | SonarQube | DIR | ✗ | 🔶 | Safe | Disallow incrementing a value that is never used afterward |
...
| `no-path-traversal` | Destack | DIR | ✗ | 🔶 | None | Disallow tainted data in file system paths |
| `no-prototype-pollution` | Semgrep | DIR | ✗ | 🔶 | Suggestion | Disallow patterns that may pollute Object.prototype |
| `no-regex-injection` | Destack | DIR | ✗ | 🔶 | None | Disallow tainted data in regular expression patterns |
...
| `prefer-nullish-coalescing` | TS-ESLint | DIR | ✗ | 🔶 | Safe | Prefer `??` over `\|\|` for default values |
...
| `no-banned-import` | ESLint | DIR | ✗ | 🔶 | None | Disallow imports from specified modules |
| `no-bitwise` | ESLint | AST | ✓ | ✅ | None | Disallow bitwise operators |
| `no-circular-dependency` | Import | DIR | ✗ | 🔶 | None | Disallow circular module dependencies |
...
| `no-layer-violation` | SonarQube | DIR | ✗ | 🔶 | None | Disallow imports that cross architectural layer boundaries |
...
| `no-parameter-reassignment` | SonarQube | DIR | ✓ | 🔶 | None | Disallow reassigning function parameters |
