use super::{
    GcKind, GcStats, MarkSet, Promotion, TraceQueue, trace_managed_references_in_reader,
    trace_managed_references_in_reader_range,
};
use crate::heap::sum_bytes;
use crate::managed::{
    LargeEntryId, ManagedLocation, ManagedSpace, ManagedYoungId, MapId, checked_reference_id,
};
...
impl ManagedSpace {
    ...
    pub fn collect_young_references(
        &mut self,
    ...
    fn stage_young_promotion(
        ...
        // otherwise copy the payload directly into one mature large entry
            ...
            let mut pages = arena.allocate_zeroed(entry.byte_len).map_err(|error| {
            ...
            if let Err(error) = arena.copy_bytes_between_page_views(
                ...
                arena.release_page_view(&pages).map_err(|error| {
                    HeapError::ManagedPromotionFailed {
                        reference,
                        error: Box::new(error),
                    }
            ...
            let entry_id = self
                .store_large_entry(entry.byte_len, pages, entry.map_id, layout_id, false)
                .map_err(|error| HeapError::ManagedPromotionFailed {
                    reference,
                    error: Box::new(error),
                })?;
        ...
        for promotion in promotions.iter().rev() {
            ...
            let result = match promotion.target {
                ...
                ManagedLocation::Small(slot) => {
                    ...
                    if should_requeue
                        && let Some(class_index) =
                            self.small.size_classes.class_index_for(size_class)
                    {
    ...
    fn mark_reachable_young_references(
        ...
        while let Some(reference) = pending.pop() {
            ...
            let map_id = self.trace_map_id(reference, location)?;
            let Some(reference_map) = self.map_table.map(map_id) else {
                return Err(HeapError::ManagedTraceFailed {
                    ...
                    error: Box::new(HeapError::InvalidMapId {
                        index: map_id.index(),
                    }),
                });
            };
    ...
    /// Return the reference map id for one traceable managed location.
    fn trace_map_id(
        &self,
        reference: ManagedReference,
