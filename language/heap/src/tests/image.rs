...
use crate::{
    HeapError, HeapImage, HeapOptions, ManagedSpace, RawSpace, ReferenceMap, SharedSpace,
    StorageLayoutId, Value,
};

use super::tests::{TestHeap, test_arena};
...
fn test_roundtrip_shared_memory_image_and_fork() {
    ...
    let mut shared = SharedSpace::with_arena(arena);
...
fn test_roundtrip_managed_space_image() {
    ...
    let first = managed
        .allocate_bytes(&first_bytes, ReferenceMap::empty(), None)
        .expect("managed allocation should succeed");
    let _second = managed
        .allocate_bytes(&second_bytes, ReferenceMap::empty(), None)
        .expect("managed allocation should succeed");
    managed
        .set_layout_id(
            first,
            StorageLayoutId::new(41).expect("storage layout id should be non-zero"),
        )
    ...
    assert_eq!(restored.layout_id(first), Ok(StorageLayoutId::new(41)));
...
fn test_heap_managed_write_detaches_only_touched_allocation() {
    ...
    // mutating one allocation should detach only that allocation
    restored
        .set_managed_bytes(first, 0, &[0xCC])
...
fn test_heap_managed_write_detaches_only_touched_page() {
    ...
    restored
        .set_managed_bytes(reference, 4096, &[0xCC])
        .expect("managed bytes should update");
    let mutated_image = restored.image().expect("heap image should capture");
...
fn test_restore_heap_image_releases_retained_pages_on_failure() {
    ...
    let heap = &mut test_heap.heap;
    heap.allocate_managed_bytes(&[1, 2, 3], ReferenceMap::empty(), None)
        .expect("managed allocation should succeed");
