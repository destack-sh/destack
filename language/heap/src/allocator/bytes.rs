...
impl Allocator {
    /// Return one mutable pointer into one logical page run.
    pub fn page_run_ptr(&self, page_run: &PageRun, byte_offset: usize) -> HeapResult<*mut u8> {
    ...
    pub(crate) fn write_bytes(
        &self,
        page_run: &mut PageRun,
        start: usize,
        source: &[u8],
    ) -> HeapResult<()> {
        let end = self.byte_range_end(page_run, start, source.len())?;
        let page_bytes = self.page_bytes();
        let start_page = start / page_bytes;
        let end_page = end.div_ceil(page_bytes);
        let mut source_offset = 0usize;

        // copy each page-local source slice in order
        for page_index in start_page..end_page {
            // resolve the logical page first
            let Some(page_id) = page_run.page(page_index) else {
                return Err(HeapError::MissingLogicalPage { page_index });
            ...
            // copy the matching caller source slice
