use super::{HeapExtent, HeapPageMapEntry, HeapPlace, HeapState, HeapStorage, LargeBlockId};
use crate::SharedHeapReference;
use crate::allocator::{PageSpan, Slot};

impl HeapStorage {
    /// Return the page-map entry for one logical page.
    fn page_entry(&self, store: &HeapState, page_index: usize) -> Option<HeapPageMapEntry> {
    ...
    fn resolve_small_extent(
        ...
        Some(HeapExtent {
            storage: HeapPlace::SmallSlot(slot),
