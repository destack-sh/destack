...
use super::{
    LargeEntry, LargeEntryImage, ManagedReferenceEntry, ManagedSpace, MapId, MapTable,
    ReferenceMap, SmallSpan, SmallSpanImage, YoungImage, YoungSpace,
};
use crate::CowTable;
use crate::alloc::{Arena, PageView, SizeClassTable};
use crate::gc::{CardSet, GcState, MarkSet, TraceQueue};
use crate::heap::{AllocationTotals, HeapError, HeapOptions, HeapResult};
...
impl ManagedSpace {
    ...
    fn restore_from_image(arena: Arc<Arena>, image: &ManagedSpaceImage) -> Result<Self, HeapError> {
        ...
        let result = (|| {
            ...
            let map_table = Self::restore_map_table(image)?;
    ...
    fn restore_map_table(image: &ManagedSpaceImage) -> Result<MapTable, HeapError> {
        MapTable::from_maps(image.reference_maps().to_vec())
    }
    ...
    fn restore_span(span: &SmallSpanImage, card_bytes: usize) -> HeapResult<SmallSpan> {
        ...
        let map_ids = span
            ...
            .iter()
            .copied()
            .map(|map_id| MapId::from_index(map_id as usize))
            .collect::<HeapResult<Vec<_>>>()?
            .into_boxed_slice();
        ...
        let dirty_card_bytes =
            span.slot_count
                ...
                .ok_or(HeapError::CountOverflow {
                    current: span.slot_count,
                    added: span.size_class,
                })?;
    ...
    fn capture_reference_maps(&self) -> Box<[ReferenceMap]> {
        self.map_table.snapshot().into_boxed_slice()
    }
    ...
    fn capture_span_image(span: &SmallSpan) -> SmallSpanImage {
        SmallSpanImage {
            ...
            map_ids: span
                .map_ids
                .iter()
                .map(|map_id| map_id.index() as u32)
                .collect::<Vec<_>>()
                .into_boxed_slice(),
            layout_ids: span.layout_ids.clone(),
            pages: span.pages,
