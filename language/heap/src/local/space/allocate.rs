...
use super::{
    CardSet, HeapPageMapEntry, HeapSpace, HeapStorage, LargeBlock, LargeBlockId, LocalGcPhase,
    SmallSpan, YoungGcPhase, YoungSpan, YoungSpanBits,
};
...
impl HeapSpace {
    ...
    pub(crate) fn retained_byte_delta(&self, layout: &AllocationPlan<'_>) -> HeapResult<i64> {
        // reject empty managed heap blocks
        if layout.is_empty() {
    ...
    fn publish_young_cache_allocation(
        ...
        self.publish_major_allocation(
            reference,
            HeapStorage::YoungSlot(Slot::new(slot.span_index, slot.slot_index)?),
            trace_map,
        )
    }

    /// Allocate one managed heap block.
    pub fn allocate(
        &mut self,
        layout: &AllocationPlan<'_>,
        payload: Payload<'_>,
    ...
    /// Reserve one managed heap block in young space.
    ...
    /// Allocate one mature managed heap block from one block plan.
    ...
    fn allocate_mature(
        ...
        if let Some(small) = layout.class.small() {
            ...
            Ok((HeapStorage::MatureSlot(slot), class.size_class))
        }
        // otherwise allocate one dedicated large block
        else {
            ...
            Ok((HeapStorage::LargeBlock(block_id), layout.byte_len))
        }
    }
