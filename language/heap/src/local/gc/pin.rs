...
impl PinSet {
    ...
    #[cfg(test)]
    pub(crate) fn pin(&mut self, reference: ManagedReference) -> crate::HeapResult<()> {
        let Some(entry) = self.entries.get_mut(&reference) else {
            self.active_count = self.active_count.checked_add(1).ok_or(
                crate::HeapError::ManagedPinActiveCountOverflow {
        ...
        let count =
            entry
                ...
                .get()
                .checked_add(1)
                .ok_or(crate::HeapError::ManagedPinCountOverflow {
                    reference,
        ...
        let Some(count) = NonZeroU32::new(count) else {
            return Err(crate::HeapError::ManagedPinCountOverflow { reference, count });
        };
    ...
    #[cfg(test)]
    pub(crate) fn unpin(&mut self, reference: ManagedReference) -> crate::HeapResult<()> {
        let Some(entry) = self.entries.get(&reference) else {
            return Err(crate::HeapError::ManagedPinMissing { reference });
        };
        let count = entry.count.get();
        self.active_count = self.active_count.checked_sub(1).ok_or(
            crate::HeapError::ManagedPinActiveCountUnderflow {
                reference,
                active_count: self.active_count,
            },
        )?;
