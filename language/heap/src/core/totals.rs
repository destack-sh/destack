...
impl HeapDomain {
    ...
    fn invalid_error(self, totals: AllocationTotals, freed_bytes: u64) -> HeapError {
        match self {
            Self::Managed | Self::Raw | Self::Shared => HeapError::InvalidUsage {
                domain: self,
    ...
    fn overflow_error(self, _totals: AllocationTotals, _added_bytes: u64) -> HeapError {
        match self {
            Self::Managed => HeapError::InvariantOverflow {
                context: "managed allocation totals",
            },
            Self::Raw => HeapError::InvariantOverflow {
                context: "raw allocation totals",
            },
            Self::Shared => HeapError::InvariantOverflow {
                context: "shared allocation totals",
            },
            Self::Total => unreachable!("allocation totals do not track the total domain"),
        }
    }
