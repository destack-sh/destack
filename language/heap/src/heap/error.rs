...
use crate::alloc::PageId;
use crate::value::{ManagedReference, RawPointer, SharedPointer};
...
impl HeapDomain {
    /// Return the usage subject for this heap domain.
...
pub enum HeapError {
    ...
    /// The arena segment directory cannot grow far enough for one entry.
    ArenaSegmentDirectoryExhausted {
    ...
    /// The heap-domain usage counters cannot represent one additional allocation.
    UsageOverflow {
        /// The heap domain whose counters overflowed.
        domain: HeapDomain,
        /// The live entry count before the failed update.
        allocated_count: usize,
        /// The live byte count before the failed update.
        allocated_bytes: u64,
        /// The bytes being added.
        added_bytes: u64,
    },
    /// One shared pointer did not resolve to one live entry.
    InvalidSharedPointer {
        /// The invalid shared pointer.
        pointer: SharedPointer,
    },
    /// One shared pointer id cannot be represented by shared pointers.
    InvalidSharedPointerId {
        /// The invalid shared pointer id.
        id: u64,
    },
    /// One exact byte sum overflowed `u64`.
    ByteCountOverflow {
        /// The left-hand byte count.
        left: u64,
        /// The right-hand byte count.
        right: u64,
    },
    /// One byte delta would move outside the representable `u64` range.
    InvalidByteDelta {
        /// The current byte count.
        current: u64,
        /// The requested signed byte delta.
        delta: i64,
    },
    /// One exact count overflowed `usize`.
    CountOverflow {
        /// The current count.
        current: usize,
    ...
    /// One managed young-space generation counter overflowed `u32`.
    ManagedYoungGenerationOverflow {
        /// The current young-space generation.
        generation: u32,
    },
    /// One GC cycle counter overflowed `u64`.
    GcCycleCountOverflow {
        /// The completed GC cycle count before the failed increment.
...
impl Display for HeapError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ArenaSegmentDirectoryExhausted {
                required_segments,
                max_segments,
            } => {
                write!(
                    formatter,
                    "arena segment directory exhausted: required {required_segments} segments with maximum {max_segments}"
                )
            }
            ...
            Self::UsageOverflow {
                ...
                write!(
                    formatter,
                    "{subject} usage overflow: count {allocated_count}, bytes {allocated_bytes}, added bytes {added_bytes}"
                )
            }
            Self::InvalidSharedPointer { pointer } => {
                write!(
                    formatter,
                    "invalid shared pointer: id={} offset={}",
                    pointer.id(),
                    pointer.byte_offset(),
                )
            }
            Self::InvalidSharedPointerId { id } => {
                write!(formatter, "invalid shared pointer id: {id}")
            }
            Self::ByteCountOverflow { left, right } => {
                write!(formatter, "byte count overflow: {left} + {right}")
            }
            Self::InvalidByteDelta { current, delta } => {
                write!(
                    formatter,
                    "invalid byte delta: current {current}, delta {delta}"
                )
            }
            Self::CountOverflow { current, added } => {
                write!(formatter, "count overflow: {current} + {added}")
            }
            Self::ManagedYoungGenerationOverflow { generation } => {
                write!(formatter, "managed young generation overflow: {generation}")
            }
            Self::GcCycleCountOverflow { completed_cycles } => {
                write!(formatter, "gc cycle count overflow: {completed_cycles}")
            }
