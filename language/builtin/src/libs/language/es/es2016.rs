// generated by builtin/generate.py
// run `just generate-builtin-libs` to regenerate

use crate::libs::source::BuiltinLib;
use crate::{builtin_lib_sources};
use crate::libs::language::symbols::{ES2016_DECLARED_SYMBOLS, ES2016_EMPTY_DECLARED_SYMBOLS};

builtin_lib_sources!(
    [
        (LIB_ES_ES2016_ARRAY_INCLUDE_D_TS, "language", "es/es2016", "array.include.d.ts"),
        (LIB_ES_ES2016_INDEX_D_TS, "language", "es/es2016", "index.d.ts"),
        (LIB_ES_ES2016_INTL_D_TS, "language", "es/es2016", "intl.d.ts"),
        (LIB_ES_ES2016_FULL_D_TS, "language", "es/es2016", "full.d.ts"),
    ]
);

pub const LIB_ES2016: BuiltinLib = BuiltinLib::language(
    "es2016",
    &[
        LIB_ES_ES2016_ARRAY_INCLUDE_D_TS,
        LIB_ES_ES2016_INDEX_D_TS,
        LIB_ES_ES2016_INTL_D_TS,
    ],
    &[
    ],
)
.ambient()
.with_declared_symbols(ES2016_DECLARED_SYMBOLS)
;

pub const LIB_ES2016_ARRAY_INCLUDE: BuiltinLib = BuiltinLib::language(
    "es2016.array.include",
    &[
        LIB_ES_ES2016_ARRAY_INCLUDE_D_TS,
    ],
    &[
    ],
)
.ambient()
.with_declared_symbols(ES2016_EMPTY_DECLARED_SYMBOLS)
;

pub const LIB_ES2016_FULL: BuiltinLib = BuiltinLib::language(
    "es2016.full",
    &[
        LIB_ES_ES2016_FULL_D_TS,
    ],
    &[
    ],
)
.ambient()
.with_declared_symbols(ES2016_DECLARED_SYMBOLS)
;

pub const LIB_ES2016_INTL: BuiltinLib = BuiltinLib::language(
    "es2016.intl",
    &[
        LIB_ES_ES2016_INTL_D_TS,
    ],
    &[
    ],
)
.ambient()
.with_declared_symbols(ES2016_EMPTY_DECLARED_SYMBOLS)
;
