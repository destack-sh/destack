...
Widened `let` scalar inputs should infer `T = string` for generic `${T}` positions.
...
Const ternary unions should preserve span unions in generic `${T}` argument inference.
...
Renamed re-exports should preserve template argument inference precision.
...
Export-star forwarding should preserve narrow template rejection for widened mutable inputs.
...
Namespace imports should preserve generic `${T}` inference for const literals.
