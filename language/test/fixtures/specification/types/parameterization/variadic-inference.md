# Variadic Inference Pipelines

## partial application

### partial application preserves tail tuple order

> Variadic partial application should infer the remaining tail tuple in original parameter order.
...
> A partially applied function should reject calls that provide the inferred tail arguments in the wrong order.
...
> Rest inference from const tuple inputs should keep literal element types instead of widening to primitives.
...
> Rest inference from mutable arrays should widen scalar literals to their mutable primitive counterparts.
