...
> Excluding from any yields any.

```ds libs=es5
type AnyLetters = Exclude<any, "b">;

declare let value: AnyLetters;
let ok: number = value;
```
...
> NonNullable preserves any.

```ds libs=es5
type AnyValue = NonNullable<any>;

declare let value: AnyValue;
let ok: string = value;
```
...
> Extracting from any yields any.

```ds libs=es5
type AnyLetters = Extract<any, "a">;
