...
> Defaults for trailing type parameters should resolve from concrete inferences established earlier in the list.
...
> Supplying an explicit middle argument should override that link while later defaults keep chaining from the new value.
...
> Even with explicit trailing arguments, constraints implied by earlier inferred defaults should still be enforced.
...
> `keyof` defaults should be computed from the inferred object argument and produce the corresponding key union.
...
> Providing a concrete key argument should narrow indexed access below the broader default key union.
...
> Explicitly fixing early generics should still allow later parameters to resolve through their declared defaults.
