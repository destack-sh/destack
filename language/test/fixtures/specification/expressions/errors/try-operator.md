...
The Try operator and nullish coalescing.
...
extension<T, E> of FancyTry<T, E> implements Try<T, E> {
    ...
    static fromFailure(error: E): FancyTry<T, E> {
        FancyTry { value: { kind: "failure", error } }
    }
}
...
extension<T, E> of AliasTry<T, E> implements Try<T, E> {
    ...
    static fromFailure(error: E): AliasTry<T, E> {
...
### try unwrap requires fromFailure

> The ? operator requires Try.fromFailure to be implemented.
...
extension<T, E> of LooseTry<T, E> {
    ...
    static fromFailure(error: E): LooseTry<T, E> {
        LooseTry { value: { kind: "failure", error } }
    }
}
...
extension<T, E> of BadTry<T, E> implements Try<T, E> {
    ...
    static fromFailure(error: E): BadTry<T, E> {
        BadTry { value: { error } }
    }
}
...
extension<T, E> of WrongTry<T, E> implements Try<T, E> {
    ...
    static fromFailure(error: E): WrongTry<T, E> {
...
### try coalesce allows missing fromFailure

> The ?? operator does not require Try.fromFailure.
