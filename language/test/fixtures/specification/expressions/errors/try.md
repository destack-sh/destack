...
Applying `try` to `Result` unions preserves both success and error alternatives through propagation.
...
A single `try` peels exactly one `Result` layer and keeps any inner `Result` untouched.
...
When `try` is followed by `??`, the success branch type remains intact while fallback type is joined.
...
### try propagation requires fromFailure on custom Try types

Custom `Try` types must provide `fromFailure` so propagated errors can be reconstructed.
...
extension<T, E> of Custom<T, E> implements Try {
    type Value = T;
    type Error = E;
