...
Guard expressions should narrow the matched value inside the guarded arm.

## Guard narrowing

### match guard narrows with is

> Guard expressions using `is` should narrow the scrutinee inside the guarded arm.
...
> Guard expressions using `in` should narrow object unions inside the guarded arm.
...
> Guard narrowing should not leak into later arms that do not match the guard.
...
> Typeof guards should narrow the match value in the guarded arm.
...
> Boolean guards should narrow in the guarded arm when the left side narrows the value.
