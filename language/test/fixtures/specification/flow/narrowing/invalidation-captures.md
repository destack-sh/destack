...
Calling a nested function that mutates a captured local must invalidate the caller's previous local narrowing.
...
A mutation through an object alias must invalidate discriminant-based member availability on the original binding.
...
Mutations performed by returned helper closures must still invalidate previously narrowed discriminant members.
...
Closures created before a loop mutation should observe the joined post-loop type, not the pre-loop narrow.
...
A loop iteration that writes a captured local must invalidate narrows established earlier in that branch.
...
Nested helper writes to dotted properties must invalidate earlier dotted-name narrowing at the call site.
