...
/** Rule metadata. `meta` property of `Rule`. */
export interface RuleMeta {
     ...
     *   the parts of the program that determine how the code looks rather than how it executes.
    ...
    /**
     * Type of fixes that the rule provides. Must be `'code'` or `'whitespace'` if the rule provides
     * fixes.
     */
    fixable?: "code" | "whitespace";
    /**
     * Specifies whether rule can return suggestions. Must be `true` if the rule provides
     * suggestions.
     *
     * @default false
     */
    hasSuggestions?: boolean;
    /** Shape of options for the rule. Mandatory if the rule has options. */
    ...
    /**
     * Information about available replacements for the rule. This may be an empty array to
     * explicitly state there is no replacement.
     *
...
/**
 * Rule documentation. `docs` property of `RuleMeta`.
 *
...
/** Info about deprecation of a rule, and possible replacements. `deprecated` property of `RuleMeta`. */
...
export interface RuleDeprecatedInfo {
    ...
    /**
     * Version (as semver string) likely to remove the rule. e.g. the next major version.
     *
...
export interface RuleReplacedByInfo {
     ...
     *
     * The `name` property should be the package name, and should be: - `"oxlint"` if the
     * replacement is an Oxlint core rule. - `"eslint"` if the replacement is an ESLint core rule.
     *
     * This property should be omitted if the replacement rule is in the same plugin.
     */
    plugin?: RuleReplacedByExternalSpecifier;
    /**
     * Name of replacement rule. May be omitted if the plugin only contains a single rule, or has
...
export interface RuleReplacedByExternalSpecifier {
    /** For a plugin, the package name. For a rule, the rule name. */
    name?: string;
