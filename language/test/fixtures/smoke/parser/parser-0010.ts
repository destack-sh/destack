...
export async function compile({
  ...
  file,
  options,
}: CompilerOptions): Promise<BabelCore.BabelFileResult> {
  ...
  const plugins =
    options != null ? [[BabelPluginReactCompiler, options]] : [[BabelPluginReactCompiler]];
  ...
  if (result?.code == null) {
    throw new Error(`Expected BabelPluginReactCompiler to compile successfully, got ${result}`);
  }
  try {
