import { hasDomain, isThunk, omit, printable, throwParseError, type requireKeys } from "@ark/util";
...
export class OptionalNode extends BaseProp<"optional"> {
  ...
  override get rawIn(): OptionalNode {
    ...
    return this.$.node("optional", omit(baseIn.inner, { default: true }) as never, {
      prereduced: true,
    });
  }
  ...
  expression: string = this.hasDefault()
...
export const computeDefaultValueMorph = (
  ...
  const precomputedMorphedDefault = value.includesTransform
    ? value.assert(defaultInput)
    : defaultInput;
