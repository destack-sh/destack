...
pub(super) fn large_signature(scale: usize, _width: usize) -> String {
...
/// Generate one ambient function with many parameters.
pub(super) fn large_ambient_signature(scale: usize, _width: usize) -> String {
    let parameters = (0..scale)
        .map(|index| format!("value{index}: number"))
        .collect::<Vec<_>>()
        .join(", ");

    format!("export declare function largeFunction({parameters}): number;\n")
}
