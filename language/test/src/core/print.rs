use std::io::Write;
use std::time::Duration;

use super::{Case, CaseResult, RunSummary};

/// ANSI color helpers.
pub mod color {
    pub const BOLD: &str = "\x1b[1m";
    pub const GREEN: &str = "\x1b[32m";
    pub const RED: &str = "\x1b[31m";
    pub const YELLOW: &str = "\x1b[33m";
    pub const CYAN: &str = "\x1b[36m";
    pub const DIM: &str = "\x1b[2m";
    pub const RESET: &str = "\x1b[0m";

    /// Apply bold green.
    pub fn green(s: &str) -> String {
        format!("{BOLD}{GREEN}{s}{RESET}")
    }

    /// Apply bold red.
    pub fn red(s: &str) -> String {
        format!("{BOLD}{RED}{s}{RESET}")
    }

    /// Apply bold yellow.
    pub fn yellow(s: &str) -> String {
        format!("{BOLD}{YELLOW}{s}{RESET}")
    }

    /// Apply bold cyan.
    pub fn cyan(s: &str) -> String {
        format!("{BOLD}{CYAN}{s}{RESET}")
    }

    /// Apply dim.
    pub fn dim(s: &str) -> String {
        format!("{DIM}{s}{RESET}")
    }

    /// Apply bold.
    pub fn bold(s: &str) -> String {
        format!("{BOLD}{s}{RESET}")
    }
}

/// Print one test result with colors.
pub fn print_result(case: &Case, result: &CaseResult, duration: Duration, _verbose: bool) {
    let (_status_plain, status) = match result {
        CaseResult::Passed => ("ok".to_string(), color::green("ok")),
        CaseResult::Failed { .. } => ("FAILED".to_string(), color::red("FAILED")),
        CaseResult::Skipped { .. } => ("skipped".to_string(), color::yellow("skipped")),
        CaseResult::Suite { passed, failed, .. } => {
            if *failed == 0 {
                let plain = format!("suite ok ({passed} passed)");
                let colored = color::green(&plain);
                (plain, colored)
            } else {
                let plain = format!("suite ({passed} passed, {failed} failed)");
                let colored = color::yellow(&plain);
                (plain, colored)
            }
        }
    };

    let duration_plain = if duration.as_millis() > 100 {
        format!(" ({:.2}s)", duration.as_secs_f64())
    } else {
        String::new()
    };
    let duration_str = if duration_plain.is_empty() {
        String::new()
    } else {
        color::dim(&duration_plain)
    };

    println!("test {} ... {}{}", case.full_name(), status, duration_str);
    let _ = std::io::stdout().flush();

    if let CaseResult::Failed { message } = result {
        if message.is_empty() {
            return;
        }

        let indent = "       ";

        println!("{indent}{}", color::red("failure details:"));
        for line in message.lines() {
            if line.is_empty() {
                println!("{indent}");
            } else {
                println!("{indent}  {line}");
            }
        }
        println!();
    }
}

/// Print the overall test summary.
pub fn print_summary(summary: &RunSummary, duration: Duration) {
    let passed = summary.passed();
    let failed = summary.failed();
    let skipped = summary.skipped();

    let status = if failed == 0 {
        color::green("ok")
    } else {
        color::red("FAILED")
    };

    // build parts with colors
    let mut parts = Vec::new();
    if passed > 0 {
        parts.push(color::green(&format!("{passed} passed")));
    }
    if failed > 0 {
        parts.push(color::red(&format!("{failed} failed")));
    }
    if skipped > 0 {
        parts.push(color::yellow(&format!("{skipped} skipped")));
    }

    let counts = parts.join("; ");
    let time = color::dim(&format!("finished in {:.2}s", duration.as_secs_f64()));

    println!();
    println!("test result: {status}. {counts}; {time}");
}

/// Print the list of failed tests.
pub fn print_failures(cases: &[(Case, CaseResult)]) {
    let failures: Vec<_> = cases
        .iter()
        .filter(|(_, result)| result.is_failed())
        .collect();
    if failures.is_empty() {
        return;
    }

    // just list failed test names
    println!();
    println!("{}:", color::red("failures"));
    for (case, _) in &failures {
        println!("    {}", case.full_name());
    }
}

/// Print the discovered test list without running.
pub fn print_test_list(cases: &[Case]) {
    for case in cases {
        let suffix = if case.is_skipped {
            format!(": {} {}", color::yellow("test"), color::dim("(skipped)"))
        } else {
            format!(": {}", color::cyan("test"))
        };
        println!("{}{}", case.full_name(), suffix);
    }

    println!();
    let total = color::bold(&format!("{}", cases.len()));
    let skipped_count = cases.iter().filter(|case| case.is_skipped).count();
    if skipped_count > 0 {
        let skipped = color::yellow(&format!("{skipped_count} skipped"));
        println!("{total} tests ({skipped})");
    } else {
        println!("{total} tests");
    }
}
