...
// pinned version of Biome formatter fixtures
const BIOME_VERSION: &str = "1.x";
const BIOME_COMMIT: &str = "9f1b3b0"; // short sha

/// Biome formatter conformance suite.
#[derive(Debug, Clone)]
...
impl BiomeSuite {
    ...
    fn discover_in_dir(&self, dir: &Path, tests: &mut Vec<Test>) {
        ...
        for entry in entries {
            ...
            if let Some(expected_path) = sibling_with_suffix(&path, ".prettier-snap")
                && expected_path.is_file()
                && let Ok(relative_expected) = expected_path.strip_prefix(&self.root)
            {
                test = test.with_expected_output(ExpectedOutput::PlainFile(
...
impl ConformanceSuite for BiomeSuite {
    ...
    fn download_instructions(&self) -> String {
        format!(
            "To download Biome formatter fixtures (version {BIOME_VERSION}, commit {BIOME_COMMIT}):\n\
             \n\
               just language/install-formatter-conformance\n\
             \n\
             Or manually:\n\
               ./language/test/fixtures/formatter/conformance/biome-fetch.sh\n"
        )
    }
}

/// Run Biome formatter conformance tests.
