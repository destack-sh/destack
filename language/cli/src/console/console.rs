...
pub fn prompt_input(prompt: &str) -> ConsoleResult<String> {
    write(prompt);
    let mut buffer = String::new();
    io::stdin()
        .read_line(&mut buffer)
        .map_err(|error| ConsoleError::message(format!("failed to read input: {error}")))?;
    let trimmed = buffer.trim_end_matches(['\n', '\r']).to_string();
    Ok(trimmed)
}

/// Ask the user for confirmation, returning the default on empty input.
pub fn prompt_yes_no(question: &str, default: bool) -> ConsoleResult<bool> {
    loop {
...
pub fn page_with_less(content: &str) -> ConsoleResult<()> {
    let mut child = Command::new("less")
        ...
        .stdin(Stdio::piped())
        .spawn()
        .map_err(|error| ConsoleError::message(format!("failed to spawn less: {error}")))?;

    if let Some(stdin) = child.stdin.as_mut() {
        stdin.write_all(content.as_bytes()).map_err(|error| {
            ConsoleError::message(format!("failed to write to less stdin: {error}"))
        })?;
    }

    let status = child
        .wait()
        .map_err(|error| ConsoleError::message(format!("failed to wait for less: {error}")))?;
    if !status.success() {
...
pub fn page_or_print(content: &str) -> ConsoleResult<()> {
