...
impl Printer<'_, '_, '_> {
    ...
    pub(super) fn operation(&self, operation: &dir::TypeOperation) -> DocResult<String> {
        match operation {
    ...
    fn mapped(&self, mapped: dir::MappedType) -> DocResult<String> {
    ...
    fn infer(&self, infer: dir::InferType) -> DocResult<String> {
        let name = match (infer.name, infer.symbol) {
            ...
            (None, None) => return Err(DocError::invalid("inferred type name")),
        };
    ...
    fn template_literal(&self, template: dir::TemplateLiteralType) -> DocResult<String> {
        let strings = self.types().strings(template.strings);
        let spans = self.types().type_ids(template.spans);
        if strings.len() != spans.len() + 1 {
            return Err(DocError::invalid("template literal"));
        }
