...
private func performUpdateContact(
    ...
    // lookup
    guard let unifiedContact = try? contactStore.unifiedContact(
        withIdentifier: id,
        keysToFetch: contactKeys(fullContactQuery())
    ) else {
...
private func runtimeHostContact(
    ...
    let phones = query.includePhones ? contact.phoneNumbers.enumerated().map { index, value in
    ...
    let emails = query.includeEmails ? contact.emailAddresses.enumerated().map { index, value in
    ...
    let addresses: [RuntimeHostContactAddress] = query.includeAddresses ? contact.postalAddresses.map { value in
    ...
    let organization = query.includeOrganization
