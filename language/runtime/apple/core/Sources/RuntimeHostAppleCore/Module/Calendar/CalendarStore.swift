...
public final class CalendarStoreRequests: CalendarRequests {
    ...
    public func listCalendarEvents(
        ...
        guard let calendars = resolveQueryCalendars(
...
private func runtimeHostCalendarEvent(
    ...
    RuntimeHostCalendarEvent(
        ...
        recurrenceMasterId: event.calendarItemExternalIdentifier == event.eventIdentifier ? nil : event.calendarItemExternalIdentifier,
...
private func eventKitRecurrenceRule(
    ...
    return EKRecurrenceRule(
        ...
        daysOfTheWeek: daysOfTheWeek.isEmpty ? (fallbackDaysOfWeek.isEmpty ? nil : fallbackDaysOfWeek) : daysOfTheWeek,
        ...
        weeksOfTheYear: rule.byWeekNumbers.isEmpty ? nil : rule.byWeekNumbers.map(NSNumber.init(value:)),
        daysOfTheYear: rule.byYearDays.isEmpty ? nil : rule.byYearDays.map(NSNumber.init(value:)),
        setPositions: rule.bySetPositions.isEmpty ? nil : rule.bySetPositions.map(NSNumber.init(value:)),
        end: recurrenceEnd
    )
}
...
private func colorArgb(
    ...
    let blue = UInt32((components[safe: 2] ?? components[safe: 1] ?? components[safe: 0] ?? 0) * 255.0)
