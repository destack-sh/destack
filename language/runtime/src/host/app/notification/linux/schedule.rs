...
use crate::host::app::notification::storage::{
    DesktopScheduledNotificationRecord, notification_scheduler_key,
    notification_systemd_service_path, notification_systemd_timer_path, read_notification_records,
    remove_notification_record, write_notification_record,
};
use crate::host::app::notification::time::{
    calendar_date_trigger_unix_ns, notification_time_zone_suffix,
};
use crate::host::app::notification::wrapper::{
    systemd_quote_argument, write_notification_wrapper_script,
};
use crate::host::app::systemd::SystemdManager;
...
pub(super) fn cancel_scheduled_notification(
    ...
    let service_path = notification_systemd_service_path(id)?;
    let timer_path = notification_systemd_timer_path(id)?;
    let wrapper_path = super::super::storage::notification_wrapper_script_path(context, id)?;
    let timer_unit_name = format!("{key}.timer");
