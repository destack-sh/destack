...
struct UnixNotificationResponseState {
    ...
    runtimes: FxHashMap<HostRuntimeId, FxHashMap<String, u32>>,
...
impl UnixNotificationResponseService {
    ...
    fn register_notification(
        ...
        let runtime_notifications = state.runtimes.entry(host_runtime_id).or_default();
    ...
    fn remove_notification(&self, host_runtime_id: HostRuntimeId, id: &str) -> Option<u32> {
        let mut state = self.state.lock();
        remove_notification_by_runtime_id(&mut state, host_runtime_id, id)
    }

    /// Remove every active Unix notification route for one runtime.
    fn unregister_runtime(&self, host_runtime_id: HostRuntimeId) {
        let mut state = self.state.lock();
        remove_notification_runtime(&mut state, host_runtime_id);
    }
}
...
pub(super) fn register_active_notification(
    ...
    service.register_notification(host_runtime_id, platform, id, request, server_id);
...
pub(super) fn remove_active_notification(
    ...
    Ok(service.remove_notification(host_runtime_id, id))
}
...
fn handle_notification_response(
    ...
    runtime::remove_posted_notification(notification.host_runtime_id, &notification.id);
...
fn remove_notification_by_server_id(
    ...
    if let Some(runtime_notifications) = state.runtimes.get_mut(&notification.host_runtime_id) {
        runtime_notifications.remove(&notification.id);
        remove_runtime = runtime_notifications.is_empty();
    }

    if remove_runtime {
        state.runtimes.remove(&notification.host_runtime_id);
    }
...
fn remove_notification_runtime(
    ...
    let Some(runtime_notifications) = state.runtimes.remove(&host_runtime_id) else {
        return;
    };
