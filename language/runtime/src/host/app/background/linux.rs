...
use super::runtime::{desktop_background_test_mode_enabled, enqueue_test_background_launch};
use super::storage::{
...
use super::wrapper::{systemd_quote_argument, write_background_wrapper_script};

/// Return whether the active host exposes one Linux background scheduler.
pub(super) fn background_status() -> RuntimeResult<BackgroundStatusValue> {
...
pub(super) fn unregister_background_task(
    context: &HostRequestContext,
    identifier: &str,
) -> RuntimeResult<()> {
...
pub(super) fn trigger_background_task(identifier: &str) -> RuntimeResult<bool> {
    if desktop_background_test_mode_enabled() {
