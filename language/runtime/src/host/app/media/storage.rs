...
use super::roots::{desktop_media_roots, import_root_for_kind};

/// The media import operation name.
pub(super) const HOST_MEDIA_IMPORT_OPERATION: &str = "destack.os.media.importPath";

/// The media delete operation name.
pub(super) const HOST_MEDIA_DELETE_OPERATION: &str = "destack.os.media.delete";

/// Import one local path into the selected desktop media root.
pub(super) fn import_media_asset(
    platform: Platform,
    kind: MediaAssetKind,
    path: OsPath,
...
pub(super) fn delete_media_assets(platform: Platform, ids: &[String]) -> RuntimeResult<u32> {
    let mut paths = Vec::with_capacity(ids.len());

    // validate the full delete set before removing anything
    for id in ids {
        let path = pathbuf_from_file_uri(id, "id")?;
        let path = validated_media_path(platform, &path, HOST_MEDIA_DELETE_OPERATION)?;
        let metadata = stdfs::metadata(&path).map_err(|error| {
            io_operation_error(
                HOST_MEDIA_DELETE_OPERATION,
...
/// Validate that one decoded media path belongs to the desktop media roots.
pub(super) fn validated_media_path(
    platform: Platform,
    path: &Path,
    ...
    // only allow identifiers rooted in one declared desktop media directory
    for root in roots {
    ...
    Err(invalid_argument(
        "id",
        "media identifier is not rooted in one desktop media library",
    ))
}
