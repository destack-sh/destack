...
/// Solaris host adapter implementation.
#[derive(Debug)]
...
impl SolarisHostAdapter {
    /// Create one Solaris host adapter.
    pub(crate) fn new() -> Self {
        ...
        let registration = register_host_bridge(HostPlatform::Solaris, &bridge);
        let services = build_solaris_services(bridge.state_store());
...
/// Build Solaris host service trait surfaces from one shared state object.
fn build_solaris_services(state_store: &Arc<HostStateStore>) -> HostServices {
