use super::{
    ...
    destack_host_ios_media_watch_close, destack_host_ios_media_watch_open,
};
...
/// Submit one iOS media watch-open request.
fn submit_media_watch_open(
    ...
    let payload_length = callback_payload_length(payload.len(), operation)?;
    let status = unsafe {
        destack_host_ios_media_watch_open(
            runtime_id,
...
/// Submit one iOS media watch-close request.
fn submit_media_watch_close(
    ...
    let status =
        unsafe { destack_host_ios_media_watch_close(runtime_id, NativeStringRef::from(watch_id)) };
    decode_callback_host_status(status, operation)
}
