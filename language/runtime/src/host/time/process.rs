...
#[cfg(target_os = "macos")]
use crate::host::unix::apple_process_monotonic_nanos;
#[cfg(all(unix, not(target_os = "macos")))]
use crate::host::unix::unix_process_monotonic_nanos;
#[cfg(windows)]
use crate::host::windows::qpc_process_monotonic_nanos;
...
pub(crate) fn monotonic_now_ns() -> u64 {
    ...
    #[cfg(target_os = "macos")]
    {
        match apple_process_monotonic_nanos() {
            Some(nanos) => nanos,
    ...
    #[cfg(all(unix, not(target_os = "macos")))]
    {
        match unix_process_monotonic_nanos() {
            Some(nanos) => nanos,
            None => fallback_process_monotonic_nanos(),
        }
    }
    ...
    #[cfg(windows)]
    {
        match qpc_process_monotonic_nanos() {
            Some(nanos) => nanos,
            None => fallback_process_monotonic_nanos(),
        }
    }
