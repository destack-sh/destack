...
use crate::host::macos::request::background::{
    ...
    unregister_background_task as unregister_macos_background_task,
};
    ...
    match request {
        ...
        HostRequest::OsBackgroundComplete {
            ...
            if matches!(result, BackgroundTaskResultValue::Retry) {
                trigger_macos_background_task(context, &execution.identifier)?;
            ...
            // once: remove the durable registration after the execution settles
...
/// Return the current macOS background scheduler status.
fn background_status() -> RuntimeResult<BackgroundStatusValue> {
    macos_background_status()
}
...
fn background_register(
    ...
    register_macos_background_task(context, options)?;
    ...
    if let Err(error) = write_result {
        if let Err(cleanup_error) = unregister_macos_background_task(context, &options.identifier) {
            tracing::warn!(
                ?cleanup_error,
                identifier = options.identifier,
                "failed to roll back macOS background registration after metadata write failure"
            );
        }
...
fn background_trigger_test(context: &HostRequestContext, identifier: &str) -> RuntimeResult<bool> {
    ...
    if record.is_some() {
        return trigger_macos_background_task(context, identifier);
    }
