use super::{
    ...
    destack_host_android_notification_pending_cancel,
    destack_host_android_notification_pending_cancel_all,
    destack_host_android_notification_pending_list, destack_host_android_notification_post,
...
pub(crate) fn submit_notification_request(
    ...
    match request {
        HostRequest::OsNotificationRequestPermission => {
            ...
            let status = unsafe {
                destack_host_android_notification_request_permission(runtime_id, &mut state)
            };
        ...
        HostRequest::OsNotificationPendingList => {
            let descriptors =
                read_buffered_callback_host_json::<Vec<NotificationScheduledDescriptorValue>>(
                    request.operation_name(),
                    "pending notifications",
                    |output, output_written| unsafe {
                        destack_host_android_notification_pending_list(
                            runtime_id,
                            output,
                            output_written,
                        )
        ...
        HostRequest::OsNotificationPendingCancelAll => {
            let status =
                unsafe { destack_host_android_notification_pending_cancel_all(runtime_id) };
            decode_callback_host_status(status, request.operation_name())?;
...
/// Submit one buffered Android notification request that returns one id string.
