...
use crate::host::core::{HostQueue, HostSessionHandle, HostSessionId, HostSessionRegistry};

/// Return the active Android host queue for this process.
pub(crate) fn android_host_queue(
    session_handle: HostSessionHandle,
) -> RuntimeResult<Arc<HostQueue>> {
    let host_session_id = HostSessionId::from_handle(session_handle);

    HostSessionRegistry::queue_for_session(host_session_id, Platform::Android)
}
