// generated by generate-bindings: do not edit

use crate::platform::abi::{NativeStringSlice};
use crate::platform::abi::NativeStringRef;
use crate::host::android::abi::bindings::invoke_android_binding_callback;

/// The host callback for `can_open_url`.
pub(crate) type AndroidHostIntentCanOpenUrlCallback =
    unsafe extern "C" fn(session_handle: u64, url: NativeStringRef, is_supported: *mut bool) -> u32;

/// The host callback for `open_url`.
pub(crate) type AndroidHostIntentOpenUrlCallback =
    unsafe extern "C" fn(session_handle: u64, url: NativeStringRef) -> u32;

/// The host callback for `open_path`.
pub(crate) type AndroidHostIntentOpenPathCallback =
    unsafe extern "C" fn(session_handle: u64, path: NativeStringRef) -> u32;

/// The host callback for `share_text`.
pub(crate) type AndroidHostIntentShareTextCallback =
    unsafe extern "C" fn(session_handle: u64, text: NativeStringRef, has_mime_type: bool, mime_type: NativeStringRef) -> u32;

/// The host callback for `share_paths`.
pub(crate) type AndroidHostIntentSharePathsCallback =
    unsafe extern "C" fn(session_handle: u64, paths: NativeStringSlice, has_mime_type: bool, mime_type: NativeStringRef) -> u32;

/// Callback table for Android host intent request interop.
#[derive(Clone, Copy, Debug, Default)]
#[repr(C)]
pub(crate) struct AndroidHostIntentCallbacks {
    /// The callback for `can_open_url`.
    pub can_open_url: Option<AndroidHostIntentCanOpenUrlCallback>,
    /// The callback for `open_url`.
    pub open_url: Option<AndroidHostIntentOpenUrlCallback>,
    /// The callback for `open_path`.
    pub open_path: Option<AndroidHostIntentOpenPathCallback>,
    /// The callback for `share_text`.
    pub share_text: Option<AndroidHostIntentShareTextCallback>,
    /// The callback for `share_paths`.
    pub share_paths: Option<AndroidHostIntentSharePathsCallback>,
}

/// Resolve and invoke one Android host intent callback.
pub(super) fn call_android_intent_callback<T: Copy>(
    session_handle: u64,
    resolve: impl FnOnce(&AndroidHostIntentCallbacks) -> Option<T>,
    invoke: impl FnOnce(T) -> u32,
) -> u32 {
    invoke_android_binding_callback(session_handle, |bindings| resolve(&bindings.intent), invoke)
}
