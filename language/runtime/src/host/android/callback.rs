...
pub fn android_notify_window_available(runtime_id: u64) -> RuntimeResult<()> {
    let bridge = android_host_bridge(runtime_id)?;
...
pub fn android_notify_window_terminated(runtime_id: u64) -> RuntimeResult<()> {
    let bridge = android_host_bridge(runtime_id)?;
...
pub fn android_notify_window_focus_changed(runtime_id: u64, is_focused: bool) -> RuntimeResult<()> {
    let bridge = android_host_bridge(runtime_id)?;
