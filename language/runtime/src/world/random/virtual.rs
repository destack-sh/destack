...
use super::{RandomImage, RandomStreamId, WorkerRandomStreamKey};
...
impl VirtualRandom {
    ...
    /// Capture one deterministic random image.
    pub(crate) fn image(&self) -> RandomImage {
        ...
        RandomImage {
            ...
            default_stream,
            next_stream_id: self.next_stream_id.load(Ordering::Relaxed),
            streams,
            worker_streams,
        }
    }

    /// Restore one deterministic random image.
    pub(crate) fn restore(&self, image: &RandomImage) -> Result<(), StreamStateDecodeError> {
        // reset deterministic state
        self.state.store(image.root_seed, Ordering::Relaxed);
        self.next_stream_id
            .store(image.next_stream_id, Ordering::Relaxed);
        self.streams.lock().clear();
        self.worker_streams.lock().clear();

        // restore default stream state
        self.import_stream_state_bytes(RandomStreamId::DEFAULT, &image.default_stream)?;

        // restore materialized stream states
        for (stream_id, bytes) in &image.streams {
        ...
        for (key, stream_id) in &image.worker_streams {
