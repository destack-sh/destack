...
impl Random {
    ...
    pub(crate) fn restore(&self, image: &RandomImage) -> RuntimeResult<()> {
        self.virtual_random.restore(image).map_err(|error| {
            RuntimeError::Internal {
                message: format!("random image restore failed: {error:?}"),
            }
            .boxed()
        })?;
