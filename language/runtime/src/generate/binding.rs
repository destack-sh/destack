...
pub(crate) fn render_abi_types(domain: &str, types: &DomainAbiTypes) -> String {
    ...
    for (struct_name, binding_type) in structs {
        ...
        output.push_str(&format!("pub struct {struct_name}Replay {{\n"));
        for field in fields {
            let field_name = to_snake_case(&field.name);
...
impl<'a> DomainWriter<'a> {
    ...
    fn write_header(&mut self) {
        ...
        if usage.needs_decode || usage.uses_replay_payload || usage.needs_native_out {
            self.output
...
pub(super) fn native_type_for_binding(domain: &str, binding_type: &BindingType) -> String {
    match binding_type {
        ...
        | BindingType::Enum {
            ...
            if type_domain == "error" && name == "PlatformError" {
                "crate::platform::error::PlatformError".to_string()
            } else {
