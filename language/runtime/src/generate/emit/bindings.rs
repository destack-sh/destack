...
fn render_binding_scope(scope: CatalogBindingScope) -> String {
    match scope {
...
fn render_binding_blocking(blocking: CatalogBindingBlocking) -> String {
    match blocking {
...
pub(super) fn vm_set_name_for_domain(domain: &str) -> String {
...
pub(super) fn native_set_name_for_domain(domain: &str) -> String {
...
/// Compute one deterministic tagged union variant tag.
pub(super) fn tagged_union_variant_tag(union_name: &str, variant_name: &str) -> u32 {
    // fnv1a-64 over one canonical key
    ...
    // reserve zero as invalid and map to [1, u32::MAX]
...
fn tagged_union_abi_path(domain: &str, type_domain: &str, name: &str, abi: &str) -> String {
    let alias = if type_domain == domain {
        platform_domain_alias(domain)
    } else {
        platform_domain_alias(type_domain)
...
fn tagged_union_vm_path(domain: &str, type_domain: &str, name: &str) -> String {
    let vm_name = format!("{name}Vm");
    named_type_path(domain, type_domain, vm_name.as_str())
}
...
pub(super) fn native_type_for_binding(domain: &str, binding_type: &BindingType) -> String {
    match binding_type {
        ...
        BindingType::Optional(inner) => {
            format!("Option<{}>", native_type_for_binding(domain, inner))
        }
        BindingType::Newtype {
            name,
            domain: type_domain,
...
fn abi_struct_field_type(domain: &str, binding_type: &BindingType) -> String {
    match binding_type {
        ...
        BindingType::Optional(inner) => format!("Option<{}>", abi_struct_field_type(domain, inner)),
        BindingType::Newtype {
            name,
            domain: type_domain,
            inner,
        ...
        BindingType::TaggedUnion {
            ...
            if binding_type_requires_abi(binding_type) {
                tagged_union_abi_path(domain, type_domain, name, "A")
            } else {
                named_type_path(domain, type_domain, name)
            }
        }
...
fn abi_newtype_inner_type(domain: &str, binding_type: &BindingType) -> String {
    match binding_type {
        ...
        BindingType::Float(64) => "f64".to_string(),
        BindingType::Optional(inner) => {
            format!("Option<{}>", abi_newtype_inner_type(domain, inner))
        }
        BindingType::Newtype {
            name,
            domain: type_domain,
...
fn render_encode_expr(domain: &str, binding_type: &BindingType, value_expr: &str) -> String {
    match binding_type {
        ...
        BindingType::TaggedUnion {
            ...
            let union_type = tagged_union_vm_path(domain, union_domain, name);
            let mut arms = Vec::new();
            for variant in variants {
                let tag = tagged_union_variant_tag(name, variant.name.as_str());
                let payload_expr = render_encode_expr(domain, &variant.binding_type, "value");
                arms.push(format!(
...
pub(super) fn render_replay_encode_lines(
    ...
    match binding_type {
        ...
        BindingType::Optional(inner) => {
            ...
            lines.extend(
                render_replay_encode_lines(domain, inner, &inner_name, "value")
                    .into_iter()
        ...
        BindingType::TaggedUnion {
            ...
            let source_union = if binding_type_requires_abi(binding_type) {
                tagged_union_vm_path(domain, union_domain, union_name)
            } else {
                named_type_path(domain, union_domain, union_name)
            };
            let target_union = if binding_type_requires_abi(binding_type) {
                let replay_name = replay_tagged_union_name(union_name);
                named_type_path(domain, union_domain, replay_name.as_str())
            } else {
                named_type_path(domain, union_domain, union_name)
            };
            lines.push(format!("let {name} = match {value_expr} {{"));
            for variant in variants {
                let inner_name = format!("{name}_{}", to_snake_case(variant.name.as_str()));
                lines.push(format!("    {source_union}::{}(value) => {{", variant.name));
                lines.extend(
                    render_replay_encode_lines(domain, &variant.binding_type, &inner_name, "value")
                        .into_iter()
                        .map(|line| format!("        {line}")),
                );
                lines.push(format!(
...
fn binding_type_is_copy(binding_type: &BindingType) -> bool {
    match binding_type {
        ...
        | BindingType::Slice(_)
        | BindingType::Array(_) => true,
        BindingType::Optional(inner) => binding_type_is_copy(inner),
        ...
        BindingType::TaggedUnion { variants, .. } => variants
            .iter()
            .all(|variant| binding_type_is_copy(&variant.binding_type)),
...
fn binding_type_is_copy_for_replay(binding_type: &BindingType) -> bool {
    match binding_type {
        ...
        BindingType::Optional(inner) => binding_type_is_copy_for_replay(inner),
        ...
        BindingType::TaggedUnion { variants, .. } => variants
            .iter()
            .all(|variant| binding_type_is_copy_for_replay(&variant.binding_type)),
        BindingType::String
        | BindingType::StringSlice
        | BindingType::Slice(_)
...
pub(super) fn binding_type_is_copy_for_handle(binding_type: &BindingType) -> bool {
    match binding_type {
        ...
        BindingType::Optional(inner) => binding_type_is_copy_for_handle(inner),
        ...
        BindingType::TaggedUnion { variants, .. } => variants
            .iter()
            .all(|variant| binding_type_is_copy_for_handle(&variant.binding_type)),
...
fn binding_type_is_byte_buffer(binding_type: &BindingType) -> bool {
    match binding_type {
        ...
        BindingType::Optional(inner) => binding_type_is_byte_buffer(inner),
        _ => false,
    }
}
...
pub(super) fn render_native_replay_encode_lines(
    ...
    match binding_type {
        ...
        BindingType::Optional(inner) => {
            ...
            lines.push(format!("let {name} = if let Some(value) = {value_expr} {{"));
            lines.extend(
                render_native_replay_encode_lines(domain, inner, &inner_name, "value")
                    .into_iter()
                    .map(|line| format!("    {line}")),
            );
            lines.push(format!("    Some({inner_name})"));
        ...
        BindingType::TaggedUnion {
            ...
            let mut lines = Vec::new();
            let source_union = named_type_path(domain, union_domain, union_name);
            let target_union = if binding_type_requires_abi(binding_type) {
                let replay_name = replay_tagged_union_name(union_name);
                named_type_path(domain, union_domain, replay_name.as_str())
            } else {
            ...
            for variant in variants {
                ...
                lines.push(format!("    {source_union}::{}(value) => {{", variant.name));
                lines.extend(
                    render_native_replay_encode_lines(
                        domain,
                        &variant.binding_type,
                        &inner_name,
                        "value",
...
fn render_native_replay_decode_lines(
    ...
    match binding_type {
        ...
        BindingType::Optional(inner) => {
            ...
            lines.push(format!("let {name} = if let Some(value) = {value_expr} {{"));
            lines.extend(
                render_native_replay_decode_lines(domain, inner, &inner_name, "value")
                    .into_iter()
                    .map(|line| format!("    {line}")),
            );
            lines.push(format!("    Some({inner_name})"));
        ...
        BindingType::TaggedUnion {
            ...
            let target_union = named_type_path(domain, union_domain, union_name);
            lines.push(format!("let {name} = match {value_expr} {{"));
            for variant in variants {
                let inner_name = format!("{name}_{}", to_snake_case(variant.name.as_str()));
                lines.push(format!("    {source_union}::{}(value) => {{", variant.name));
                lines.extend(
                    render_native_replay_decode_lines(
                        domain,
                        &variant.binding_type,
                        &inner_name,
                        "value",
...
pub(super) fn render_replay_to_vm_binding_lines(
    ...
    match binding_type {
        ...
        BindingType::Optional(inner) => {
            ...
            lines.push(format!("let {name} = if let Some(value) = {value_expr} {{"));
            lines.extend(
                render_replay_to_vm_binding_lines(domain, inner, &inner_name, "value")
                    .into_iter()
                    .map(|line| format!("    {line}")),
            );
            lines.push(format!("    Some({inner_name})"));
...
pub(super) fn vm_type_for_binding(domain: &str, binding_type: &BindingType) -> String {
    match binding_type {
        ...
        BindingType::Optional(inner) => format!("Option<{}>", vm_type_for_binding(domain, inner)),
        BindingType::Newtype {
            name,
            domain: type_domain,
            inner,
        ...
        BindingType::TaggedUnion {
            ...
            domain: type_domain,
            ..
        } => tagged_union_vm_path(domain, type_domain, name),
    }
}
...
pub(super) fn replay_type_for_binding(domain: &str, binding_type: &BindingType) -> String {
    match binding_type {
        ...
        BindingType::Optional(inner) => {
            format!("Option<{}>", replay_type_for_binding(domain, inner))
        }
        BindingType::Newtype {
            name,
            domain: type_domain,
        ...
        BindingType::TaggedUnion {
            ...
            if binding_type_requires_abi(binding_type) {
                let replay_name = replay_tagged_union_name(name);
                named_type_path(domain, type_domain, replay_name.as_str())
            } else {
                named_type_path(domain, type_domain, name)
            }
...
fn replay_tagged_union_name(name: &str) -> String {
    format!("{name}ReplayRecord")
}
...
fn render_decode_value_lines(
    ...
    match binding_type {
        ...
        BindingType::TaggedUnion {
            ...
            let union_type = tagged_union_vm_path(domain, union_domain, union_name);
            vec![format!(
