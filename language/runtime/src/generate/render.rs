...
use crate::model::{
    ...
    BindingType, EffectClass, LogKind, ReplayPolicy,
};
...
fn push_header(output: &mut String, domain: &str, bindings: &BindingCatalogEntry) {
    ...
    let type_domains = collect_type_domains(domain, bindings);
    let uses_log_kind = bindings.values().any(|entry| entry.log_kind.is_some());
    ...
    if uses_log_kind {
        binding_imports.push("LogKind");
    }
    if uses_replay_policy {
...
fn push_descriptor_consts(output: &mut String, consts: &[BindingConst<'_>]) {
    for binding in consts {
        ...
        let (ctor, args) =
            descriptor_ctor_for_binding(binding.entry.effect_class, binding.entry.log_kind);
        output.push_str(&format!(
...
/// Resolve the descriptor constructor for an effect class and log kind.
fn descriptor_ctor_for_binding(
    ...
    match effect_class {
        ...
        EffectClass::External { replay } => {
            ...
            let log_arg = match log_kind {
                Some(kind) => format!("Some({})", log_kind_value(kind)),
                None => "None".to_string(),
            };
            ("external", vec![replay.to_string(), log_arg])
        }
    }
}

/// Render a log kind literal for descriptor constructors.
fn log_kind_value(log_kind: LogKind) -> &'static str {
    match log_kind {
        LogKind::Time => "LogKind::Time",
        LogKind::Random => "LogKind::Random",
        LogKind::Scheduler => "LogKind::Scheduler",
    }
}
