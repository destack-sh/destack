...
pub(crate) fn destack_crypto_certificate_delete(
    ...
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.certificate.delete is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_certificate_export(
    ...
    let _ = (handle, format);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.certificate.export is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_certificate_import(
    ...
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.certificate.import is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_certificate_metadata(
    ...
    let _ = handle;
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.certificate.metadata is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_certificate_verify(
    ...
    let _ = request;
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.certificate.verify is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_decrypt(
    ...
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.decrypt is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_delete(
    ...
    let _ = handle;
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.delete is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_encrypt(
    ...
    let _ = (handle, scheme, argument_payload);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.encrypt is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_export_public(
    ...
    let _ = (handle, format);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.exportPublic is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_generate(
    ...
    let _ = (store, spec);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.generate is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_import(
    ...
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.import is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_metadata(
    ...
    let _ = handle;
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.metadata is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_sign(
    ...
    let _ = (handle, scheme, argument_payload);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.sign is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_key_verify(
    ...
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.key.verify is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_store_close(
    ...
    let _ = handle;
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.store.close is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_store_list_certificates(
    ...
    let _ = (handle, query);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.store.listCertificates is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_store_list_keys(
    ...
    let _ = (handle, query);
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.store.listKeys is not available in the VM yet",
    ))
    .boxed())
}
...
pub(crate) fn destack_crypto_store_open(
    ...
    let _ = options;
    Err(RuntimeError::from(PlatformError::not_supported(
        "destack.crypto.store.open is not available in the VM yet",
    ))
    .boxed())
}
