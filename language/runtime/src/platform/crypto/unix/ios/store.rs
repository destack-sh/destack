...
/// Snapshot payload magic for encrypted iOS host key-store blobs.
const IOS_HOST_SNAPSHOT_MAGIC: &[u8; 8] = b"DSCKEY01";
/// Snapshot payload version for encrypted iOS host key-store blobs.
const IOS_HOST_SNAPSHOT_VERSION: u8 = 1;
/// AES-GCM nonce size for encrypted host snapshots.
const IOS_HOST_SNAPSHOT_NONCE_BYTES: usize = 12;
/// AES-GCM tag size for encrypted host snapshots.
const IOS_HOST_SNAPSHOT_TAG_BYTES: usize = 16;
/// AES-256 key size for encrypted host snapshots.
const IOS_HOST_SNAPSHOT_KEY_BYTES: usize = 32;
/// Authenticated context bound into snapshot encryption and decryption.
const IOS_HOST_SNAPSHOT_AAD: &[u8] = b"destack.crypto.ios.snapshot.v1";
    ...
    let snapshot_payload = match fs::read(&path) {
        ...
        Err(error) => {
            ...
            return Err(permission_denied(
                operation,
                format!("failed to read ios host keystore snapshot {path:?}: {error}"),
            ));
        }
    };
...
pub(crate) fn store_host_key_snapshot_bytes(
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            permission_denied(
                operation,
                format!("failed to create ios host keystore path {parent:?}: {error}"),
            )
        })?;
    }
    ...
    fs::write(&path, encrypted_payload).map_err(|error| {
        permission_denied(
            operation,
            format!("failed to write ios host keystore snapshot {path:?}: {error}"),
        )
    })?;
    ...
    if matches!(kind, CryptoStoreKind::User | CryptoStoreKind::Machine) {
        ...
        fs::set_permissions(&path, permissions).map_err(|error| {
            permission_denied(
                operation,
                format!("failed to set ios host keystore permissions for {path:?}: {error}"),
            )
        })?;
    }
...
fn load_or_create_snapshot_encryption_key(
    snapshot_path: &Path,
    operation: &'static str,
) -> RuntimeResult<[u8; IOS_HOST_SNAPSHOT_KEY_BYTES]> {
    ...
    match fs::read(&key_path) {
        ...
        Err(error) => {
            return Err(permission_denied(
                operation,
                format!("failed to read ios host keystore key {key_path:?}: {error}"),
            ));
        }
    }

    // generate one new random key and persist it with strict permissions
    let mut key = [0u8; IOS_HOST_SNAPSHOT_KEY_BYTES];
    ...
    if let Some(parent) = key_path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            permission_denied(
                operation,
                format!("failed to create ios host keystore key path {parent:?}: {error}"),
            )
        })?;
    }
    ...
    fs::write(&key_path, key).map_err(|error| {
        permission_denied(
            operation,
            format!("failed to write ios host keystore key {key_path:?}: {error}"),
        )
    })?;
    ...
    #[cfg(target_family = "unix")]
    {
        ...
        fs::set_permissions(&key_path, permissions).map_err(|error| {
            permission_denied(
                operation,
                format!("failed to set ios host keystore key permissions {key_path:?}: {error}"),
            )
        })?;
    }
...
fn decode_snapshot_encryption_key(
    ...
    if key_bytes.len() != IOS_HOST_SNAPSHOT_KEY_BYTES {
        return Err(invalid_data(
            operation,
            "ios host keystore encryption key has one invalid length",
        ));
    }
...
fn encode_encrypted_snapshot_payload(
    clear_payload: &[u8],
    encryption_key: &[u8; IOS_HOST_SNAPSHOT_KEY_BYTES],
    operation: &'static str,
) -> RuntimeResult<Vec<u8>> {
    // generate one random nonce for this snapshot write
    let mut nonce = [0u8; IOS_HOST_SNAPSHOT_NONCE_BYTES];
    ...
    let mut tag = [0u8; IOS_HOST_SNAPSHOT_TAG_BYTES];
    let ciphertext = encrypt_aead(
        cipher,
        encryption_key,
        Some(&nonce),
        IOS_HOST_SNAPSHOT_AAD,
        clear_payload,
        &mut tag,
    )
    ...
    let mut payload = Vec::with_capacity(
        IOS_HOST_SNAPSHOT_MAGIC.len()
            + 1
            + IOS_HOST_SNAPSHOT_NONCE_BYTES
            + IOS_HOST_SNAPSHOT_TAG_BYTES
            + ciphertext.len(),
    );
    payload.extend_from_slice(IOS_HOST_SNAPSHOT_MAGIC);
    payload.push(IOS_HOST_SNAPSHOT_VERSION);
...
fn decode_snapshot_payload(
    ...
    if payload.len() < IOS_HOST_SNAPSHOT_MAGIC.len()
        || &payload[..IOS_HOST_SNAPSHOT_MAGIC.len()] != IOS_HOST_SNAPSHOT_MAGIC
    {
        return Ok(payload.to_vec());
    }

    // validate envelope lengths and version
    let envelope_header_len = IOS_HOST_SNAPSHOT_MAGIC.len()
        + 1
        + IOS_HOST_SNAPSHOT_NONCE_BYTES
        + IOS_HOST_SNAPSHOT_TAG_BYTES;
    if payload.len() < envelope_header_len {
        return Err(invalid_data(
            operation,
            "ios host keystore snapshot envelope is truncated",
        ));
    }
    let version_index = IOS_HOST_SNAPSHOT_MAGIC.len();
    let version = payload[version_index];
    if version != IOS_HOST_SNAPSHOT_VERSION {
        return Err(invalid_data(
            operation,
            format!("ios host keystore snapshot uses one unsupported version {version}"),
        ));
    }

    // load encryption key and decrypt ciphertext payload
    let nonce_start = version_index + 1;
    let nonce_end = nonce_start + IOS_HOST_SNAPSHOT_NONCE_BYTES;
    let tag_end = nonce_end + IOS_HOST_SNAPSHOT_TAG_BYTES;
