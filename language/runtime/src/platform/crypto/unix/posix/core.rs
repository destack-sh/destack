...
pub(super) fn configured_system_certificate_files(context: &BindingCallContext) -> Vec<PathBuf> {
    ...
    DEFAULT_UNIX_SYSTEM_CERTIFICATE_FILES
        .iter()
        .map(PathBuf::from)
        .collect()
}
...
pub(super) fn keystore_path(
    ...
    if kind == CryptoStoreKind::User {
        ...
        return Some(Path::new(&home_directory).join(UNIX_USER_KEYSTORE_RELATIVE_PATH));
    }
    ...
    if kind == CryptoStoreKind::Machine {
        ...
        return Some(PathBuf::from(UNIX_MACHINE_KEYSTORE_ABSOLUTE_PATH));
    }
