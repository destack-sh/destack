...
fn test_process_limits_set_get_restore_roundtrip() {
    ...
    with_harness_context(|mut context| {
        ...
        let original = context.get_limit(resource)?;
        ...
        let set_result: RuntimeResult<()> = (|| {
            if target != original {
                context.set_limit(resource, target)?;
                observed_after_set = Some(context.get_limit(resource)?);
            }

            Ok(())
        })();

        context.set_limit(resource, original)?;
        // restored limit should match the original snapshot
        let restored = context.get_limit(resource)?;
...
fn test_process_limits_reject_invalid_soft_greater_than_hard() {
    ...
    with_harness_context(|mut context| {
        assert_platform_error_code(
            context.set_limit(resource, invalid_limit),
...
fn test_process_cgroup_and_job_bindings_report_expected_errors() {
    with_harness_context(|mut context| {
        ...
        assert_platform_error_codes(
            context.cgroup_get_limit(missing_path, cgroup_resource),
            &[
        ...
        assert_platform_error_codes(
            context.cgroup_join(missing_path),
            &[
                PlatformErrorCode::ProcessNotFound,
                PlatformErrorCode::NotSupported,
            ],
        ...
        assert_platform_error_codes(
            context.cgroup_set_limit(
                missing_path,
                cgroup_resource,
                ProcessLimit { soft: 1, hard: 1 },
            ),
            &[
                PlatformErrorCode::ProcessNotFound,
                PlatformErrorCode::NotSupported,
        ...
        assert_platform_error_code(
            context.job_assign("destack-test-job", &[pid]),
            PlatformErrorCode::NotSupported,
        )?;
        assert_platform_error_code(
            context.job_set_limit(
                "destack-test-job",
                cgroup_resource,
                ProcessLimit { soft: 1, hard: 1 },
            ),
...
fn test_process_scheduler_and_affinity_validation() {
    with_harness_context(|mut context| {
        ...
        let affinity = context.get_affinity(pid);
        match affinity {
            Ok(affinity) => assert!(!affinity.is_empty()),
            Err(error) => {
        ...
        assert_platform_error_codes(
            context.set_affinity(pid, &[u32::MAX]),
            &[
                PlatformErrorCode::InvalidArgumentValue,
                PlatformErrorCode::NotSupported,
            ],
        ...
        let scheduler = context.get_scheduler(pid);
        match scheduler {
            Ok(_) => {}
            Err(error) => {
        ...
        let previous = context.umask(0o022)?;
        let restored = context.umask(previous)?;
