...
pub(crate) fn midi_output_port_list(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .jack_service("destack.midi.output.port.list")?;
...
pub(crate) fn midi_output_port_open(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .jack_service("destack.midi.output.port.open")?;
    ...
    let port = register_output_port(&client, "output", "destack.midi.output.port.open")?;
    ...
    if let Err(error) = activate_client(&client, "destack.midi.output.port.open") {
        release_output_callback_context(callback_context_token);
        return Err(error);
    }

    // destination connection
    let local_port_name = port_name(&client, port, "destack.midi.output.port.open")?;
...
pub(crate) fn midi_output_virtual_create(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .jack_service("destack.midi.output.virtual.create")?;
    let pending_records = Arc::new(parking_lot::Mutex::new(std::collections::VecDeque::new()));
    let terminal_error = Arc::new(parking_lot::Mutex::new(None));
    ...
    let port = register_output_port(&client, &name, "destack.midi.output.virtual.create")?;
    let context = Arc::new(JackOutputCallbackContext {
        library: client.library.clone(),
        port,
        pending_records: pending_records.clone(),
    ...
    if let Err(error) = activate_client(&client, "destack.midi.output.virtual.create") {
        release_output_callback_context(callback_context_token);
        return Err(error);
    }

    let backend_port_name = port_name(&client, port, "destack.midi.output.virtual.create")?;
    let descriptor =
