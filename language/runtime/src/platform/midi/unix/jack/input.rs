...
pub(crate) fn midi_input_port_list(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .jack_service("destack.midi.input.port.list")?;
...
pub(crate) fn midi_input_port_open(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .jack_service("destack.midi.input.port.open")?;
    ...
    let port = register_input_port(&client, "input", "destack.midi.input.port.open")?;
    ...
    if let Err(error) = activate_client(&client, "destack.midi.input.port.open") {
        release_input_callback_context(callback_context_token);
        return Err(error);
    }

    // source connection
    let local_port_name = port_name(&client, port, "destack.midi.input.port.open")?;
...
pub(crate) fn midi_input_virtual_create(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .jack_service("destack.midi.input.virtual.create")?;
    let queue = Arc::new(BoundedQueue::new(input_queue_capacity(
        options.queue_capacity,
    )));
    let terminal_error = Arc::new(parking_lot::Mutex::new(None));
    ...
    let open_epoch_ns = binding_timestamp_now();
    let open_jack_time_usecs = unsafe { (client.library.api.jack_get_time)() };
    let port = register_input_port(&client, &name, "destack.midi.input.virtual.create")?;
    let context = Arc::new(JackInputCallbackContext {
        library: client.library.clone(),
        client: client.raw,
        port,
    ...
    if let Err(error) = activate_client(&client, "destack.midi.input.virtual.create") {
        release_input_callback_context(callback_context_token);
        return Err(error);
    }

    let backend_port_name = port_name(&client, port, "destack.midi.input.virtual.create")?;
    let descriptor =
