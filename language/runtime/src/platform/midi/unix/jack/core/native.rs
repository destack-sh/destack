...
use crate::platform::midi::core::binding_timestamp_now;
...
pub(crate) fn query_topology_snapshot(operation: &'static str) -> RuntimeResult<JackTopologyState> {
    ...
    let input_rows = query_direction_snapshot(
        &client,
        crate::platform::midi::MidiPortDirection::Input,
        operation,
    )?;
    let output_rows = query_direction_snapshot(
        &client,
        crate::platform::midi::MidiPortDirection::Output,
        operation,
    )?;
...
fn query_direction_snapshot(
    client: &JackClientHandle,
    direction: crate::platform::midi::MidiPortDirection,
    operation: &'static str,
) -> RuntimeResult<BTreeMap<String, JackEndpointInfo>> {
    let flags = match direction {
        crate::platform::midi::MidiPortDirection::Input => JACK_PORT_IS_OUTPUT,
        crate::platform::midi::MidiPortDirection::Output => JACK_PORT_IS_INPUT,
    };
