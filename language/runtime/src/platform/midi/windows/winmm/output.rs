...
pub(crate) fn midi_output_port_list(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .winmm_service("destack.midi.output.port.list")?;
    service.refresh_topology("destack.midi.output.port.list")?;
...
pub(crate) fn midi_output_port_open(
    ...
    let service = binding
        ...
        .platform_state
        .midi
        .winmm_service("destack.midi.output.port.open")?;
    service.refresh_topology("destack.midi.output.port.open")?;
...
pub(crate) fn midi_output_write(
    ...
    for record in &records {
        ...
        if uses_short_message_path(record) {
            write_short_message(session.handle, record, "destack.midi.output.write")?;
