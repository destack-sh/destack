...
/// Advertise the real ALSA capability surface on Linux.
#[test]
fn test_midi_alsa_backend_row_advertises_real_host_capabilities() {
    with_harness_context(|mut context| {
        ...
        assert_eq!(
            ...
            0,
            "ALSA should not advertise scheduled output support",
        );
        assert_eq!(
            capability_flags.0 & MIDI_BACKEND_CAP_RECEIVE_TIMESTAMPS.0,
            0,
            "ALSA should not advertise receive timestamps yet",
        );
        assert_eq!(
...
fn test_midi_alsa_virtual_destination_roundtrips_midi1_records() {
    with_harness_context(|mut context| {
        ...
        let (_received_at_ns, source_id, data_format, protocol, data) =
