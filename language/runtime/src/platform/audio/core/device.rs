...
use crate::platform::audio::{
    AudioBackend, AudioChannelLayout, AudioDeviceCapabilityFlags, AudioDeviceDescriptor,
...
use super::codec::all_sample_format_mask;
use super::constants::{
    BACKEND_OPEN_ALSA_NO_RESAMPLE, BACKEND_OPEN_JACK_NO_AUTOCONNECT,
    ...
    DEVICE_CAPABILITY_STREAM_MUTE, DEVICE_CAPABILITY_STREAM_VOLUME,
    DEVICE_LIST_INCLUDE_DISCONNECTED, DEVICE_LIST_INCLUDE_LOOPBACK, DEVICE_LIST_INCLUDE_RAW,
    DEVICE_OPEN_RAW, DIRECTION_MASK_CAPTURE, DIRECTION_MASK_DUPLEX, DIRECTION_MASK_LOOPBACK,
    DIRECTION_MASK_PLAYBACK, KNOWN_DEVICE_LIST_FLAGS_MASK, KNOWN_DEVICE_OPEN_FLAGS_MASK,
    KNOWN_SUPPORTED_EVENT_SUBSCRIPTION_FLAGS_MASK, MIN_STREAM_PERIOD_FRAMES,
    NULL_DEVICE_MAX_PERIOD_FRAMES, NULL_DEVICE_MAX_SAMPLE_RATE, NULL_DEVICE_MIN_SAMPLE_RATE,
    NULL_DEVICE_PREFERRED_CHANNEL_MASK, NULL_DEVICE_PREFERRED_PERIOD_FRAMES,
    NULL_DEVICE_PREFERRED_SAMPLE_RATE, NULL_DEVICE_SUPPORTED_CHANNEL_MASK, SHARE_MODE_SHARED_BIT,
    ...
    SUPPORTED_STREAM_CLOCK_WALL, SUPPORTED_STREAM_FLAG_EXPLICIT_SAMPLE_FORMAT,
    SUPPORTED_STREAM_FLAG_MINIMIZE_LATENCY, SUPPORTED_STREAM_FLAG_NO_AUTO_CONVERT,
    SUPPORTED_STREAM_FLAG_NON_INTERLEAVED, SUPPORTED_STREAM_FLAG_REPORT_XRUN,
    ...
    SUPPORTED_STREAM_REQUIREMENT_SCHEDULED_WRITE,
};
use super::model::{AudioDeviceHostState, HostDeviceDescriptor};
...
pub(crate) fn supported_backend_stream_flags(backend: AudioBackend) -> AudioSupportedStreamFlags {
    let mut flags =
        SUPPORTED_STREAM_FLAG_REPORT_XRUN.0 | SUPPORTED_STREAM_FLAG_EXPLICIT_SAMPLE_FORMAT.0;

    if backend == AudioBackend::Asio {
        flags |= SUPPORTED_STREAM_FLAG_NON_INTERLEAVED.0;
    }

    if backend == AudioBackend::AAudio || backend == AudioBackend::Alsa {
        flags |= SUPPORTED_STREAM_FLAG_MINIMIZE_LATENCY.0;
    }

    if backend == AudioBackend::Alsa {
        flags |= SUPPORTED_STREAM_FLAG_NO_AUTO_CONVERT.0;
    }
