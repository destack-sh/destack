...
use super::constants::{
    PULSEAUDIO_APPLICATION_NAME, PULSEAUDIO_DEFAULT_DEVICE_NAME,
    PULSEAUDIO_STREAM_DIRECTION_CAPTURE, PULSEAUDIO_STREAM_DIRECTION_PLAYBACK,
    PULSEAUDIO_STREAM_NAME,
};
use super::core::{
    PulseAudioLibrary, c_string, pulseaudio_error, pulseaudio_not_supported,
...
unsafe impl Send for PulseSimpleHandle {}

/// One PulseAudio stream runtime payload.
#[derive(Debug)]
...
impl audio_core::AudioHostStreamOps for PulseHostStreamOps {
...
pub(super) fn open_stream(
    ...
    let parsed = parse_pulseaudio_stable_id(&device_info.id)?;
    validate_pulseaudio_stable_id_direction(&parsed, device_info.direction)?;

    let library = require_pulseaudio_library("destack.audio.stream.open")?;
    let sample_format = pulseaudio_sample_format(config.format).ok_or_else(|| {
        pulseaudio_not_supported(
            "destack.audio.stream.open",
            format!(
                "sample format {:?} is not supported by PulseAudio",
                config.format
            ),
        )
    })?;
    ...
    let channels_u8 = u8::try_from(channels).map_err(|_| {
        RuntimeError::from(PlatformError::invalid_argument_value(
            "config.channels",
            "config.channels exceeds PulseAudio channel limits",
        ))
        .boxed()
    })?;
    ...
    let playback = if needs_playback {
        let device_name = if parsed.playback_name.is_empty() {
            PULSEAUDIO_DEFAULT_DEVICE_NAME
        } else {
...
fn open_simple_stream(
...
) -> RuntimeResult<PulseSimpleHandle> {
    let application_name = c_string(PULSEAUDIO_APPLICATION_NAME, "applicationName")?;
    let stream_name = c_string(PULSEAUDIO_STREAM_NAME, "streamName")?;
    ...
    if stream.is_null() {
        return Err(pulseaudio_error(
            operation,
            Some(error),
            "failed to open PulseAudio stream lane",
        ));
    }
...
fn flush_runtime_lanes(runtime: &PulseStreamRuntime, operation: &'static str) -> RuntimeResult<()> {
    if let Some(playback) = runtime.playback.as_ref() {
        ...
        if !pulseaudio_succeeded(status) {
            return Err(pulseaudio_error(
                operation,
                Some(error),
                "failed to flush PulseAudio playback lane",
            ));
        }
    }
    ...
    if let Some(capture) = runtime.capture.as_ref() {
        ...
        if !pulseaudio_succeeded(status) {
            return Err(pulseaudio_error(
                operation,
                Some(error),
                "failed to flush PulseAudio capture lane",
            ));
        }
    }
...
fn drain_runtime_playback(
    ...
    Err(pulseaudio_error(
        operation,
        Some(error),
        "failed to drain PulseAudio playback lane",
    ))
}
