...
use super::constants::{
    PULSEAUDIO_MAX_CHANNELS, PULSEAUDIO_MAX_PERIOD_FRAMES, PULSEAUDIO_MAX_SAMPLE_RATE,
    PULSEAUDIO_MIN_PERIOD_FRAMES, PULSEAUDIO_MIN_SAMPLE_RATE, PULSEAUDIO_PREFERRED_PERIOD_FRAMES,
...
pub(super) fn enumerate_devices() -> RuntimeResult<Vec<audio_core::HostDeviceDescriptor>> {
    let _ = require_pulseaudio_library("destack.audio.device.list")?;
...
fn playback_descriptor(name: &str, is_default: bool) -> audio_core::HostDeviceDescriptor {
    ...
    audio_core::HostDeviceDescriptor {
        ...
        capability_flags,
        preferred_sample_rate: PULSEAUDIO_PREFERRED_SAMPLE_RATE,
        min_sample_rate: PULSEAUDIO_MIN_SAMPLE_RATE,
        max_sample_rate: PULSEAUDIO_MAX_SAMPLE_RATE,
        preferred_period_frames: PULSEAUDIO_PREFERRED_PERIOD_FRAMES,
        min_channels: 1,
        max_channels: PULSEAUDIO_MAX_CHANNELS,
        preferred_layout,
        preferred_channel_mask: preferred_mask,
        supported_channel_mask: supported_mask,
        min_period_frames: PULSEAUDIO_MIN_PERIOD_FRAMES,
        max_period_frames: PULSEAUDIO_MAX_PERIOD_FRAMES,
        format_mask,
...
fn descriptor_profile() -> (audio_types::AudioChannelLayout, u64, u64, u32) {
    ...
    let supported_mask = channel_mask(PULSEAUDIO_MAX_CHANNELS);
    let format_mask = pulseaudio_format_mask();
