...
#[cfg(any(unix, windows))]
#[test]
fn test_tty_termios_roundtrip_or_not_supported() {
    with_harness_context(|mut context| {
        // open one pty pair or skip when host backend is unavailable
        ...
        // read one termios snapshot and reapply it when available
        let attributes = assert_ok_or_expected_error(
            context.destack_tty_termios_get_attributes(pair.worker),
            &[PlatformErrorCode::NotSupported],
        )?;
...
#[cfg(any(unix, windows))]
#[test]
fn test_tty_termios_set_attributes_speed_overflow_host_dependent_or_not_supported() {
    with_harness_context(|mut context| {
        // open one pty pair or skip when host backend is unavailable
        let pair = assert_ok_or_expected_error(
            context.destack_tty_pty_open(24, 80, 0),
        ...
        // read one baseline termios payload or skip when unsupported
        let attributes = assert_ok_or_expected_error(
            context.destack_tty_termios_get_attributes(pair.worker),
            &[PlatformErrorCode::NotSupported],
        )?;
...
#[cfg(any(unix, windows))]
#[test]
fn test_tty_termios_set_process_group_rejects_zero_or_not_supported() {
    with_harness_context(|mut context| {
        // open one pty pair or skip when host backend is unavailable
        let pair = assert_ok_or_expected_error(
            context.destack_tty_pty_open(24, 80, 0),
...
#[cfg(any(unix, windows))]
#[test]
fn test_tty_termios_send_break_rejects_duration_overflow_or_not_supported() {
    with_harness_context(|mut context| {
        // open one pty pair or skip when host backend is unavailable
        let pair = assert_ok_or_expected_error(
            context.destack_tty_pty_open(24, 80, 0),
...
#[cfg(any(unix, windows))]
#[test]
fn test_tty_termios_repeated_roundtrip_stress_or_not_supported() {
    with_harness_context(|mut context| {
        // open one pty pair or skip when host backend is unavailable
        let pair = assert_ok_or_expected_error(
            context.destack_tty_pty_open(24, 80, 0),
