...
pub(super) fn uring_open(
    ...
    #[cfg(target_os = "linux")]
    {
        // reject empty ring entry counts
        ...
        // build one ring builder from supported setup flags
        ...
        // reject unknown or unsupported setup flags
        if remaining != 0 {
            return Err(RuntimeError::from(PlatformError::invalid_argument_value(
                "parameters.flags",
        ...
        // open one concrete io_uring ring and store it as one runtime resource
        let ring = builder
...
pub(super) fn uring_register_files(
    ...
    #[cfg(target_os = "linux")]
    {
        // decode one file-id slice payload
...
pub(super) fn uring_register_buffers(
    ...
    #[cfg(target_os = "linux")]
    {
        // decode one address and length lane payload
        ...
        // build one iovec lane for fixed-buffer registration
...
pub(super) fn uring_unregister_buffers(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // unregister one fixed-buffer set
        state
            .ring
            .submitter()
            .unregister_buffers()
