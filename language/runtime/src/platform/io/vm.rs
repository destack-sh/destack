...
use crate::platform::core::{
    NativeBindingCodec, VmDecodeCodec, VmEncodeCodec,
    allocate_vm_read_buffer as allocate_read_buffer, call_out,
    store_bytes_from_vm as bytes_from_vm, store_os_path_from_vm as path_from_vm,
    store_values_from_vm, write_vm_read_buffer as write_read_buffer,
};
use crate::platform::io::{
    CompletionEvent, CompletionEventVm, CompletionOperation, CompletionOperationVm,
    DescriptorControlCommand, DescriptorControlFlags, DescriptorRequest, DescriptorRequestVm,
    DescriptorResultVm, EventToken, PollBackend, PollEventVm, PollInterest, TimerFdClock,
    TimerFdFlags, TimerFdSetFlags, TimerFdSpecVm, UringFeaturesVm, UringParametersVm,
    core as core_io,
};
...
pub(crate) fn destack_io_completion_submit(
    ...
    // decode the vm aggregate into one binding-owned operation
    let operation = VmDecodeCodec::new(context).decode(operation)?;
    let operation: CompletionOperation = NativeBindingCodec::new(binding).encode(operation);
...
pub(crate) fn destack_io_completion_submit_batch(
    ...
    // copy the compact operation words into binding-owned storage
...
pub(crate) fn destack_io_completion_wait(
    ...
    // fetch one staged completion batch from the shared runtime queue
    let events: Vec<CompletionEvent> =
        core_io::completion_wait(binding, handle, timeoutns, maxevents)?;
...
pub(crate) fn destack_io_control_fcntl(
    ...
    call_out(|out| unsafe {
        host::destack_io_control_fcntl(binding, out, handle, command, argument, flags)
    })
}
...
pub(crate) fn destack_io_control_ioctl(
    ...
    // decode the vm request payload into binding-owned storage
    let request = VmDecodeCodec::new(context).decode(request)?;
    let request: DescriptorRequest = NativeBindingCodec::new(binding).encode(request);

    // execute the host request and encode the result back into vm storage
    let result =
        call_out(|out| unsafe { host::destack_io_control_ioctl(binding, out, handle, request) })?;
    let result = unsafe { NativeBindingCodec::new(binding).decode(result)? };

    VmEncodeCodec::new(context).encode::<DescriptorResultVm>(result)
}
...
pub(crate) fn destack_io_device_close(
    ...
    unsafe { host::destack_io_device_close(binding, handle) }
}
...
pub(crate) fn destack_io_device_control(
    ...
    // execute the host request and encode the result back into vm storage
    let result =
        call_out(|out| unsafe { host::destack_io_device_control(binding, out, handle, request) })?;
    let result = unsafe { NativeBindingCodec::new(binding).decode(result)? };
...
pub(crate) fn destack_io_device_open(
    ...
    // decode the vm path into binding-owned storage
    let path = path_from_vm(binding, context, path)?;

    call_out(|out| unsafe { host::destack_io_device_open(binding, out, path, flags, mode) })
}
...
pub(crate) fn destack_io_device_read(
    ...
    // allocate one binding-owned read buffer and write it back after the host read
    let native_buffer = allocate_read_buffer(binding, buffer);
    let read = call_out(|out| unsafe {
        host::destack_io_device_read(binding, out, handle, native_buffer)
    })?;
...
pub(crate) fn destack_io_device_write(
    ...
    // copy the vm bytes into binding-owned storage for the host write path
    let buffer = bytes_from_vm(binding, context, buffer)?;

    call_out(|out| unsafe { host::destack_io_device_write(binding, out, handle, buffer) })
}
