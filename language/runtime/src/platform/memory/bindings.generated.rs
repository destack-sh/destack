...
use crate::platform::memory::{MemoryRange, MemoryRangeVm, ProtectedMemoryRange, ProtectedMemoryRangeVm};
...
use crate::platform::memory::runtime::{native as platform_runtime_native, vm as platform_runtime_vm};
use crate::platform::memory::simulated::{native as platform_simulated_native, vm as platform_simulated_vm};
...
pub const MEMORY_LOCK_LOCK: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const MEMORY_PROTECT_SET_WRITE_XOR_EXECUTE: BindingDescriptor = BindingDescriptor::deterministic_with_requires_and_behavior(
...
pub const MEMORY_NATIVE_BINDINGS: NativeBindingSet = NativeBindingSet {
    name: "memory",
    bindings: &[
        NativeBinding::new(MEMORY_ADVISE_ADVISE, "destack.memory.advise.advise", destack_memory_advise_advise as *const ()),
        ...
        NativeBinding::new(MEMORY_LOCK_LOCK, "destack.memory.lock.lock", destack_memory_lock_lock as *const ()),
        ...
        NativeBinding::new(MEMORY_PROTECT_EXECUTE, "destack.memory.protect.execute", destack_memory_protect_execute as *const ()),
        ...
        NativeBinding::new(MEMORY_PROTECT_PROTECT, "destack.memory.protect.protect", destack_memory_protect_protect as *const ()),
        NativeBinding::new(MEMORY_PROTECT_REMAP, "destack.memory.protect.remap", destack_memory_protect_remap as *const ()),
        NativeBinding::new(MEMORY_PROTECT_SET_WRITE_XOR_EXECUTE, "destack.memory.protect.setWriteXorExecute", destack_memory_protect_set_write_xor_execute as *const ()),
    ],
};
...
fn destack_memory_advise_advise_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_advise(context, address, length, advice) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_advise(context, address, length, advice) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_advise_discard_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_discard(context, address, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_discard(context, address, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_advise_huge_page_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_huge_page(context, address, length, enabled) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_huge_page(context, address, length, enabled) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_lock_lock_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_lock(context, address, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_lock(context, address, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_lock_unlock_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_unlock(context, address, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_unlock(context, address, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_map_commit_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_commit(context, address, length, flags) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_commit(context, address, length, flags) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_map_decommit_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_decommit(context, address, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_decommit(context, address, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_map_numa_bind_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_numa_bind(context, address, length, policy, nodemask) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_numa_bind(context, address, length, policy, nodemask) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_map_release_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_release(context, address, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_release(context, address, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_map_reserve_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_reserve(context, out, length, flags) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_reserve(context, out, length, flags) },
        },
        |result| {
            if let Ok(()) = result {
...
fn destack_memory_protect_execute_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        MEMORY_PROTECT_EXECUTE,
        context.replay_payload_for(MEMORY_PROTECT_EXECUTE)?,
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_protect_execute(context, address, length, enabled) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_protect_execute(context, address, length, enabled) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_protect_flush_instruction_cache_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_flush_instruction_cache(context, address, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_flush_instruction_cache(context, address, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    MemoryProtectFlushInstructionCacheReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_memory_protect_protect_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_protect(context, address, length, protection) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_protect(context, address, length, protection) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_memory_protect_remap_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_memory_remap(context, out, address, oldlength, newlength, flags) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_memory_remap(context, out, address, oldlength, newlength, flags) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
pub unsafe extern "C" fn destack_memory_lock_lock(
    address: u64,
    length: u64,
) -> RuntimeStatus {
    native_call(|context| {
...
pub unsafe extern "C" fn destack_memory_protect_set_write_xor_execute(
    ...
    native_call(|context| {
        ...
        { context.check_policy(MEMORY_PROTECT_SET_WRITE_XOR_EXECUTE)?; unsafe { platform_runtime_native::destack_memory_set_write_xor_execute(context, enabled) } }
    })
}
...
fn destack_memory_advise_advise_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_advise(runtime, context, address, length, advice),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_advise(runtime, context, address, length, advice),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_advise_discard_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_discard(runtime, context, address, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_discard(runtime, context, address, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_advise_huge_page_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_huge_page(runtime, context, address, length, enabled),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_huge_page(runtime, context, address, length, enabled),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_lock_lock_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_lock(runtime, context, address, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_lock(runtime, context, address, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_lock_unlock_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_unlock(runtime, context, address, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_unlock(runtime, context, address, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_map_commit_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_commit(runtime, context, address, length, flags),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_commit(runtime, context, address, length, flags),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_map_decommit_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_decommit(runtime, context, address, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_decommit(runtime, context, address, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_map_numa_bind_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_numa_bind(runtime, context, address, length, policy, nodemask),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_numa_bind(runtime, context, address, length, policy, nodemask),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_map_release_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_release(runtime, context, address, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_release(runtime, context, address, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_map_reserve_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_reserve(runtime, context, length, flags),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_reserve(runtime, context, length, flags),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_protect_execute_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        MEMORY_PROTECT_EXECUTE,
        runtime.replay_payload_for(MEMORY_PROTECT_EXECUTE)?,
        context,
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_protect_execute(runtime, context, address, length, enabled),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_protect_execute(runtime, context, address, length, enabled),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_protect_flush_instruction_cache_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_flush_instruction_cache(runtime, context, address, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_flush_instruction_cache(runtime, context, address, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_protect_protect_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_protect(runtime, context, address, length, protection),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_protect(runtime, context, address, length, protection),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_memory_protect_remap_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_memory_remap(runtime, context, address, oldlength, newlength, flags),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_memory_remap(runtime, context, address, oldlength, newlength, flags),
            }
        },
        |context, result| {
            let _ = &context;
...
pub fn register_memory_vm_bindings(
    ...
    {
        binding!(registry, isolate, MEMORY_ADVISE_ADVISE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (address, length, advice) = decode_destack_memory_advise_advise_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(MEMORY_ADVISE_ADVISE)?;
                destack_memory_advise_advise_vm_replay(runtime, context, world, address, length, advice)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, MEMORY_ADVISE_DISCARD, move |context, args| {
            with_runtime_call_context(|runtime| {
                ...
                destack_memory_advise_discard_vm_replay(runtime, context, world, address, length)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, MEMORY_ADVISE_HUGE_PAGE, move |context, args| {
            with_runtime_call_context(|runtime| {
                ...
                destack_memory_advise_huge_page_vm_replay(runtime, context, world, address, length, enabled)
            })
            .map_err(Into::into)
        });
    }
    ...
    }
    {
        binding!(registry, isolate, MEMORY_MAP_COMMIT, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (address, length, flags) = decode_destack_memory_map_commit_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(MEMORY_MAP_COMMIT)?;
                destack_memory_map_commit_vm_replay(runtime, context, world, address, length, flags)
            })
            .map_err(Into::into)
        });
    }
    ...
    }
    {
        binding!(registry, isolate, MEMORY_MAP_NUMA_BIND, move |context, args| {
            with_runtime_call_context(|runtime| {
                ...
                destack_memory_map_numa_bind_vm_replay(runtime, context, world, address, length, policy, nodemask)
            })
            .map_err(Into::into)
        });
    }
    ...
    {
        binding!(registry, isolate, MEMORY_PROTECT_EXECUTE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (address, length, enabled) = decode_destack_memory_protect_execute_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(MEMORY_PROTECT_EXECUTE)?;
                destack_memory_protect_execute_vm_replay(runtime, context, world, address, length, enabled)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, MEMORY_PROTECT_FLUSH_INSTRUCTION_CACHE, move |context, args| {
            with_runtime_call_context(|runtime| {
                ...
                // execute binding
                let world = runtime.check_and_resolve_world(MEMORY_PROTECT_FLUSH_INSTRUCTION_CACHE)?;
                destack_memory_protect_flush_instruction_cache_vm_replay(runtime, context, world, address, length)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, MEMORY_PROTECT_PROTECT, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (address, length, protection) = decode_destack_memory_protect_protect_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(MEMORY_PROTECT_PROTECT)?;
                destack_memory_protect_protect_vm_replay(runtime, context, world, address, length, protection)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, MEMORY_PROTECT_REMAP, move |context, args| {
            with_runtime_call_context(|runtime| {
                ...
                destack_memory_protect_remap_vm_replay(runtime, context, world, address, oldlength, newlength, flags)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, MEMORY_PROTECT_SET_WRITE_XOR_EXECUTE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (enabled,) = decode_destack_memory_protect_set_write_xor_execute_args(context, args)?;

                // execute binding
                let result = { runtime.check_policy(MEMORY_PROTECT_SET_WRITE_XOR_EXECUTE)?; platform_runtime_vm::destack_memory_set_write_xor_execute(runtime, context, enabled) };
