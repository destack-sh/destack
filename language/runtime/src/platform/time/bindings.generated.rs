...
fn destack_time_clock_info_replay(
    ...
    context.replay().run_binding(
        TIME_CLOCK_INFO,
        || unsafe { platform_native::destack_time_clock_info(context, out, clock) },
        |result| {
            if let Ok(()) = result {
...
fn destack_time_clock_now_ns_replay(
    ...
    context.replay().run_binding(
        TIME_CLOCK_NOW_NS,
        || unsafe { platform_native::destack_time_now_ns(context, out, clock) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
fn destack_time_clock_process_cpu_ns_replay(
    ...
    context.replay().run_binding(
        TIME_CLOCK_PROCESS_CPU_NS,
        || unsafe { platform_native::destack_time_process_cpu_ns(context, out) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
fn destack_time_clock_thread_cpu_ns_replay(
    ...
    context.replay().run_binding(
        TIME_CLOCK_THREAD_CPU_NS,
        || unsafe { platform_native::destack_time_thread_cpu_ns(context, out) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
pub unsafe extern "C" fn destack_time_clock_info(
    ...
    native_call(|context| {
        ...
        destack_time_clock_info_replay(context, out, clock)
    })
}
...
fn destack_time_clock_info_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        TIME_CLOCK_INFO,
        context,
        |context| platform_vm::destack_time_clock_info(runtime, context, clock),
        |context, result| {
            let _ = &context;
...
fn destack_time_clock_now_ns_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        TIME_CLOCK_NOW_NS,
        context,
        |context| platform_vm::destack_time_now_ns(runtime, context, clock),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_time_clock_process_cpu_ns_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        TIME_CLOCK_PROCESS_CPU_NS,
        context,
        |context| platform_vm::destack_time_process_cpu_ns(runtime, context),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_time_clock_thread_cpu_ns_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        TIME_CLOCK_THREAD_CPU_NS,
        context,
        |context| platform_vm::destack_time_thread_cpu_ns(runtime, context),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_time_sleep_on_ns_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        TIME_SLEEP_ON_NS,
        context,
        |context| platform_vm::destack_time_sleep_on_ns(runtime, context, duration, clock),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_time_sleep_until_on_ns_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        TIME_SLEEP_UNTIL_ON_NS,
        context,
        |context| platform_vm::destack_time_sleep_until_on_ns(runtime, context, deadline, clock),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_recorded = ();
...
pub fn register_time_vm_bindings(registry: &mut BindingRegistry, isolate: &mut Isolate) {
    {
        binding!(registry, isolate, TIME_CLOCK_INFO, move |context, args| {
            with_runtime_call_context(|runtime| {
                ...
                // execute binding
                destack_time_clock_info_vm_replay(runtime, context, clock)
            })
            .map_err(Into::into)
        });
    }
