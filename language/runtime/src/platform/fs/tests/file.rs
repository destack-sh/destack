...
use crate::platform::fs::{
    FileMode, FileOffset, OpenFlags, destack_fs_close, destack_fs_fsync, destack_fs_ftruncate,
    destack_fs_mkdir_bytes, destack_fs_open_bytes, destack_fs_read, destack_fs_readv,
    destack_fs_rmdir_bytes, destack_fs_truncate_bytes, destack_fs_unlink_bytes, destack_fs_write,
    destack_fs_writev,
};
...
fn test_fs_truncate_and_fsync() {
    with_native_harness(|harness| {
        ...
        harness.with_context(|| {
            ...
            let status = unsafe { destack_fs_fsync(handle) };
            runtime.assert_status_ok(status, "fsync");

            let status = unsafe { destack_fs_ftruncate(handle, FileOffset(5)) };
        ...
        harness.with_context(|| {
            ...
            let (_bytes, file) = path_bytes(&file_path);
            let status = unsafe { destack_fs_truncate_bytes(file, FileOffset(0)) };
...
fn test_fs_readv_writev() {
    with_native_harness(|harness| {
        ...
        harness.with_context(|| {
            ...
            let mut out = 0u64;
            let status = unsafe { destack_fs_writev(&mut out, handle, buffers, FileOffset(0)) };
        ...
        let read_back = harness.with_context(|| {
            ...
            let mut out = 0u64;
            let status = unsafe { destack_fs_readv(&mut out, handle, buffers, FileOffset(0)) };
