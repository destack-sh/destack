...
use crate::platform::debug::{InspectorEndpoint, InspectorEndpointValue, InspectorEndpointVm, InspectorendpointReplayRecord, ProfileKind, TraceLevel};
...
fn encode_destack_debug_inspector_endpoint_result(
    ...
    result.and_then(|value| { let field_0: RuntimeResult<vm::Value> = Ok(value.url.value()); let field_1: RuntimeResult<vm::Value> = Ok(vm::Value::uint(value.process_id as u64, 32)); context.allocate_aggregate(vec![field_0?, field_1?]).map_err(Box::<RuntimeError>::from) })
}
...
fn decode_destack_debug_profile_start_args(
    ...
    let kind = match kind_raw { 1i32 => ProfileKind::Cpu, 2i32 => ProfileKind::Heap, 3i32 => ProfileKind::Event , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("kind", "unknown ProfileKind value")).boxed()), };
    Ok((kind,))
}
...
fn decode_destack_debug_trace_start_args(
    ...
    let level = match level_raw { 1i32 => TraceLevel::Error, 2i32 => TraceLevel::Warn, 3i32 => TraceLevel::Info, 4i32 => TraceLevel::Debug , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("level", "unknown TraceLevel value")).boxed()), };
...
pub(crate) const DEBUG_CORE_BREAK_NOW: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) const DEBUG_CORE_MARK: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) const DEBUG_INSPECTOR_STOP: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) const DEBUG_PROFILE_START: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) const DEBUG_PROFILE_STOP: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) const DEBUG_TRACE_STOP: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) unsafe extern "C" fn destack_debug_core_break_now(
) -> RuntimeStatus {
    native_call(|context| {
...
pub(crate) unsafe extern "C" fn destack_debug_core_mark(
    label: NativeStringRef,
) -> RuntimeStatus {
    native_call(|context| {
        let _ = &label;
...
fn destack_debug_inspector_endpoint_vm_replay(
    ...
    let result = binding.trace().run_binding(
        ...
        |context, payload| {
            ...
            match payload.result {
                Ok(value) => {
                    let vm_result_url = context.string_handle(value.url.as_str()).map_err(Box::<RuntimeError>::from)?;
...
pub(crate) fn register_debug_vm_bindings(
