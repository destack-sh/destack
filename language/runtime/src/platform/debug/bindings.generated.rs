...
use crate::platform::debug::{InspectorEndpoint, InspectorEndpointReplayRecord, InspectorEndpointVm, ProfileKind, TraceLevel};
...
use crate::platform::debug::runtime::{native as platform_runtime_native, vm as platform_runtime_vm};
...
fn encode_destack_debug_inspector_endpoint_result(
    ...
    result.map(|value| { let field_0 = value.url.value(); let field_1 = vm::Value::uint(value.pid as u64, 32); context.allocate_aggregate(vec![field_0, field_1]) })
}
...
struct DebugInspectorEndpointReplay {
    /// Replay result payload.
    pub result: Result<InspectorEndpointReplayRecord, PlatformError>,
}
...
struct DebugInspectorStartReplay {
    /// Replay result payload.
    pub result: Result<resource::InspectorHandle, PlatformError>,
}
...
struct DebugProfileStartReplay {
    /// Replay result payload.
    pub result: Result<resource::ProfileHandle, PlatformError>,
}
...
struct DebugTraceStartReplay {
    /// Replay result payload.
    pub result: Result<resource::TraceHandle, PlatformError>,
}
...
fn destack_debug_inspector_endpoint_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || unsafe { platform_runtime_native::destack_debug_inspector_endpoint(context, out, handle) },
        |result| {
            if let Ok(()) = result {
...
fn destack_debug_inspector_start_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || unsafe { platform_runtime_native::destack_debug_inspector_start(context, out, host, port) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() { return Err(RuntimeError::from(PlatformError::null_pointer("out")).boxed()); }
...
fn destack_debug_trace_emit_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || unsafe { platform_runtime_native::destack_debug_trace_emit(context, category, name, payloadjson) },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_debug_trace_start_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || unsafe { platform_runtime_native::destack_debug_trace_start(context, out, level, destination) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() { return Err(RuntimeError::from(PlatformError::null_pointer("out")).boxed()); }
...
fn destack_debug_trace_emit_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        context,
        |context| platform_runtime_vm::destack_debug_trace_emit(runtime, context, category, name, payloadjson),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_debug_trace_start_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        context,
        |context| platform_runtime_vm::destack_debug_trace_start(runtime, context, level, destination),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
