...
use crate::platform::debug::{
    InspectorEndpoint, InspectorEndpointReplay, InspectorEndpointVm, ProfileKind, TraceLevel,
};
...
fn encode_destack_debug_inspector_endpoint_result(
    ...
    result.map(|value| {
        ...
        let field_1 = vm::Value::uint(value.pid as u64, 32);
...
struct DebugInspectorEndpointReplay {
    /// Replay result payload.
    pub result: Result<InspectorEndpointReplay, PlatformError>,
}
...
#[inline]
fn destack_debug_core_break_now_replay(context: &RuntimeCallContext) -> RuntimeResult<()> {
...
fn destack_debug_inspector_endpoint_replay(
    ...
    context.replay().run_binding(
        DEBUG_INSPECTOR_ENDPOINT,
        || unsafe { platform_native::destack_debug_inspector_endpoint(context, out, handle) },
        |result| {
            if let Ok(()) = result {
...
fn destack_debug_inspector_start_replay(
    ...
    context.replay().run_binding(
        DEBUG_INSPECTOR_START,
        || unsafe { platform_native::destack_debug_inspector_start(context, out, host, port) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
fn destack_debug_profile_snapshot_replay(
    ...
    context.replay().run_binding(
        DEBUG_PROFILE_SNAPSHOT,
        || unsafe { platform_native::destack_debug_profile_snapshot(context, out, handle) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
fn destack_debug_profile_start_replay(
    ...
    context.replay().run_binding(
        DEBUG_PROFILE_START,
        || unsafe { platform_native::destack_debug_profile_start(context, out, kind) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
fn destack_debug_trace_emit_replay(
    ...
    context.replay().run_binding(
        DEBUG_TRACE_EMIT,
        || unsafe {
            platform_native::destack_debug_trace_emit(context, category, name, payloadjson)
        },
        |result| {
            if let Ok(()) = result {
                let result_replay = ();
...
fn destack_debug_trace_start_replay(
    ...
    context.replay().run_binding(
        DEBUG_TRACE_START,
        || unsafe { platform_native::destack_debug_trace_start(context, out, level, destination) },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
                    if out.is_null() {
...
fn destack_debug_core_break_now_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_CORE_BREAK_NOW,
        context,
        |context| platform_vm::destack_debug_break_now(runtime, context),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_replay = ();
...
fn destack_debug_core_mark_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_CORE_MARK,
        context,
        |context| platform_vm::destack_debug_mark(runtime, context, label),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_replay = ();
...
fn destack_debug_inspector_endpoint_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_INSPECTOR_ENDPOINT,
        context,
        |context| platform_vm::destack_debug_inspector_endpoint(runtime, context, handle),
        |context, result| {
            let _ = &context;
...
fn destack_debug_inspector_start_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_INSPECTOR_START,
        context,
        |context| platform_vm::destack_debug_inspector_start(runtime, context, host, port),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_debug_inspector_stop_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_INSPECTOR_STOP,
        context,
        |context| platform_vm::destack_debug_inspector_stop(runtime, context, handle),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_replay = ();
...
fn destack_debug_profile_snapshot_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_PROFILE_SNAPSHOT,
        context,
        |context| platform_vm::destack_debug_profile_snapshot(runtime, context, handle),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_debug_profile_start_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_PROFILE_START,
        context,
        |context| platform_vm::destack_debug_profile_start(runtime, context, kind),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_debug_profile_stop_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_PROFILE_STOP,
        context,
        |context| platform_vm::destack_debug_profile_stop(runtime, context, handle),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_replay = ();
...
fn destack_debug_trace_emit_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        ...
        context,
        |context| {
            platform_vm::destack_debug_trace_emit(runtime, context, category, name, payloadjson)
        },
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
...
fn destack_debug_trace_start_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_TRACE_START,
        context,
        |context| platform_vm::destack_debug_trace_start(runtime, context, level, destination),
        |context, result| {
            let _ = &context;
            if let Ok(value) = result {
                let result_value = *value;
...
fn destack_debug_trace_stop_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context(
        DEBUG_TRACE_STOP,
        context,
        |context| platform_vm::destack_debug_trace_stop(runtime, context, handle),
        |context, result| {
            let _ = &context;
            if let Ok(()) = result {
                let result_replay = ();
