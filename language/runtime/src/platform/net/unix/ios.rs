...
/// Configure TCP keepalive delay on apple targets.
pub(crate) fn set_keepalive_delay(fd: RawFd, delay_seconds: u32) -> RuntimeResult<()> {
    set_socket_u32(fd, libc::IPPROTO_TCP, libc::TCP_KEEPALIVE, delay_seconds)
}

/// Configure SO_REUSEPORT on apple targets.
...
/// Return flags for send on apple targets.
