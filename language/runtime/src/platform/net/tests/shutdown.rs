...
use crate::platform::net::{
    SocketShutdown, destack_net_accept, destack_net_close, destack_net_close_listener,
    destack_net_connect, destack_net_listen, destack_net_shutdown, destack_net_write,
};
...
fn test_net_shutdown() {
    with_native_harness(|harness| {
        ...
        let listener = harness.with_context(|| {
            ...
            let status = unsafe { destack_net_listen(&mut handle, host, 0, 16) };
        ...
        harness.with_context(|| {
            let status = unsafe { destack_net_shutdown(client, SocketShutdown::ReadWrite) };
            runtime.assert_status_ok(status, "shutdown");
        });
        ...
        harness.with_context(|| {
            ...
            runtime.assert_status_err(status, "write after shutdown");
        });
