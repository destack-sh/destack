...
use super::{
    SERIAL_WATCH_QUEUE_CAPACITY, UnixSerialWatchRecord, UnixSerialWatchResource,
...
/// Finalizer that unregisters one unix serial watch and closes its queue.
pub(super) struct UnixSerialWatchFinalizer {
    /// Shared unix serial topology service.
    pub(super) service: Arc<UnixSerialWatchService>,
...
impl ResourceFinalizer for UnixSerialWatchFinalizer {
...
/// Resolve one typed unix serial-watch resource from the resource table.
fn watch_resource(
    binding: &BindingCallContext,
    handle: resource::SerialWatchHandle,
    operation: &'static str,
) -> RuntimeResult<Arc<UnixSerialWatchResource>> {
    serial_watch_payload(binding, handle, operation)
}

/// Build one binding-visible serial watch event from one queued unix watch record.
fn event_from_record(
    binding: &BindingCallContext,
    resource: &UnixSerialWatchResource,
    record: UnixSerialWatchRecord,
) -> SerialWatchEvent {
    match record {
        UnixSerialWatchRecord::Instance(info) => attached_watch_event(
            binding,
            &resource.event_state,
            serial_descriptor_from_info(binding, &info),
        ),
        UnixSerialWatchRecord::Detached(info) => detached_watch_event(
            binding,
            &resource.event_state,
            serial_descriptor_from_info(binding, &info),
        ),
...
/// Open one unix serial topology watch stream.
pub(crate) unsafe fn destack_device_serial_watch_open(
    ...
    let service = binding
        ...
        .platform_state
        .device
        .unix_serial_watch_service("destack.device.serial.watchOpen")?;
    ...
    let resource = Arc::new(UnixSerialWatchResource {
    ...
    for info in known_ports.values() {
        resource
            .event_queue
            .push_drop_oldest(UnixSerialWatchRecord::Instance(info.clone()));
    }
...
/// Close one unix serial topology watch stream.
...
/// Read one blocking unix serial topology event.
pub(crate) unsafe fn destack_device_serial_watch_read(
    ...
    let timeout = std::time::Duration::from_nanos(timeoutns);
...
/// Poll one unix serial topology event without blocking.
