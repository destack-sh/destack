        ...
        ) in descriptors
        {
            ...
            if capability_flags.0 & MIDI_BACKEND_CAP_VIRTUAL_INPUT.0 == 0 {
                assert_platform_error_codes(
                    context.destack_device_midi_input_virtual_create(options),
                    &[PlatformErrorCode::NotSupported],
                )?;
            ...
            // created descriptor
            let handle = context.destack_device_midi_input_virtual_create(options)?;
    ...
    with_harness_context(|mut context| {
        ...
        ) in descriptors
        {
            ...
            if capability_flags.0 & MIDI_BACKEND_CAP_VIRTUAL_OUTPUT.0 == 0 {
                assert_platform_error_codes(
                    context.destack_device_midi_output_virtual_create(options),
                    &[PlatformErrorCode::NotSupported],
                )?;
