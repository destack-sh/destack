...
fn test_midi_jack_virtual_destination_roundtrips_midi1_records() {
    with_harness_context(|mut context| {
        ...
        let output_handle = context.destack_device_midi_output_port_open(id, open_options)?;
        ...
        let written_count = context.destack_device_midi_output_write(output_handle, records)?;
        assert_eq!(written_count, 1);

        let record = context.destack_device_midi_input_read(input_handle, 1_000_000_000)?;
