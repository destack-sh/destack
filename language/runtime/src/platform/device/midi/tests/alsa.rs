...
fn test_midi_alsa_native_event_feed_reports_virtual_source_additions_and_removals() {
    with_harness_context(|mut context| {
        ...
        let output_handle = context.destack_device_midi_output_virtual_create(create_options)?;
        let descriptor = context.destack_device_midi_output_port_descriptor(output_handle)?;
        ...
        let input_ports = context.destack_device_midi_input_port_list(list_options)?;
...
fn test_midi_alsa_virtual_destination_roundtrips_midi1_records() {
    with_harness_context(|mut context| {
        ...
        let descriptor = context.destack_device_midi_input_port_descriptor(input_handle)?;
        let (
            input_id,
            _backend_id,
            _name,
        ...
        let list_options =
            harness_port_list_options_with_flags(&mut context, MIDI_PORT_LIST_INCLUDE_VIRTUAL);
        let output_ports = context.destack_device_midi_output_port_list(list_options)?;
        ...
        let output_handle = context.destack_device_midi_output_port_open(id, options)?;
        ...
        let written = context.destack_device_midi_output_write(output_handle, records)?;
        assert_eq!(written, 1, "virtual ALSA writes should report one record");

        // transport read
        let received = context.destack_device_midi_input_read(input_handle, 1_000_000_000)?;
...
fn test_midi_alsa_scheduled_virtual_destination_delivery_waits_for_the_deadline() {
    with_harness_context(|mut context| {
        ...
        assert_platform_error_codes(
            context.destack_device_midi_input_try_read(input_handle),
