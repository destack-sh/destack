...
pub(crate) fn midi_input_port_open(
    ...
    let queue_capacity = u32::try_from(queue_capacity).map_err(|_| {
        core_platform::invalid_argument(
            "queueCapacity",
            "destack.device.midi.input.port.open: queueCapacity exceeds the Android host ABI width",
        )
    })?;
...
pub(crate) fn midi_input_virtual_create(
    ...
    let queue_capacity = u32::try_from(queue_capacity).map_err(|_| {
        core_platform::invalid_argument(
            "queueCapacity",
            "destack.device.midi.input.virtual.create: queueCapacity exceeds the Android host ABI width",
        )
    })?;
