...
use super::service::{
    WinRtNativeEventRegistry, WinRtService, register_native_event_session,
...
pub(crate) fn midi_event_open(
    ...
    if options.delivery_mode != MidiEventDeliveryMode::PollOnly {
        ...
        let poll_task =
            register_poll_event_session(&service, &session, session.lock().poll_interval)?;
