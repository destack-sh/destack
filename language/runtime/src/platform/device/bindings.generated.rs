...
use crate::platform::device::simulated::{native as platform_simulated_native, vm as platform_simulated_vm};
...
fn decode_destack_device_control_control_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::DeviceHandle, DeviceControlOperationVm, VmSlice<u8>, VmSlice<u8>)> {
    ...
    let operation = {
        if operation_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("operation", "DeviceControlOperation")).boxed()); }
        let slots = context.aggregate_slots(operation_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 2 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("operation", "expected 2 fields")).boxed()); }
...
pub const DEVICE_IO_CLOSE: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const DEVICE_NATIVE_BINDINGS: NativeBindingSet = NativeBindingSet {
    name: "device",
    bindings: &[
        NativeBinding::new(DEVICE_CONTROL_CONTROL, "destack.device.control.control", destack_device_control_control as *const ()),
        NativeBinding::new(DEVICE_IO_CLOSE, "destack.device.io.close", destack_device_io_close as *const ()),
        NativeBinding::new(DEVICE_IO_OPEN, "destack.device.io.open", destack_device_io_open as *const ()),
        NativeBinding::new(DEVICE_IO_READ, "destack.device.io.read", destack_device_io_read as *const ()),
        NativeBinding::new(DEVICE_IO_WRITE, "destack.device.io.write", destack_device_io_write as *const ()),
    ],
};
...
fn destack_device_control_control_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_device_control(context, out, handle, operation, input, output) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_device_control(context, out, handle, operation, input, output) },
        },
        |result| {
            if let Ok(()) = result {
...
fn destack_device_io_close_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            ...
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_device_close(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_device_io_open_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_device_open(context, out, path, flags, mode) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_device_open(context, out, path, flags, mode) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_device_io_read_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_device_read(context, out, handle, buffer) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_device_read(context, out, handle, buffer) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_device_io_write_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_device_write(context, out, handle, buffer) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_device_write(context, out, handle, buffer) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
pub unsafe extern "C" fn destack_device_io_close(
    handle: resource::DeviceHandle,
) -> RuntimeStatus {
    native_call(|context| {
        let _ = &handle;
...
fn destack_device_control_control_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_device_control(runtime, context, handle, operation, input, output),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_device_control(runtime, context, handle, operation, input, output),
            }
        },
        |context, result| {
            let _ = &context;
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_device_close(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_device_io_open_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_device_open(runtime, context, path, flags, mode),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_device_open(runtime, context, path, flags, mode),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_device_io_read_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_device_read(runtime, context, handle, buffer),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_device_read(runtime, context, handle, buffer),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_device_io_write_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_device_write(runtime, context, handle, buffer),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_device_write(runtime, context, handle, buffer),
            }
        },
        |context, result| {
            let _ = &context;
...
pub fn register_device_vm_bindings(
    registry: &mut BindingRegistry,
    isolate: &mut Isolate,
) {
    {
        binding!(registry, isolate, DEVICE_CONTROL_CONTROL, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, operation, input, output) = decode_destack_device_control_control_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(DEVICE_CONTROL_CONTROL)?;
                destack_device_control_control_vm_replay(runtime, context, world, handle, operation, input, output)
            })
            .map_err(Into::into)
        });
    }
...
pub fn install_device_vm_bindings(
    registry: &mut BindingRegistry,
    isolate: &mut Isolate,
) {
