...
use crate::platform::device::{
    ...
    CameraExposureCompensationRange, CameraExposureTimeRange, CameraFloatControlRange,
    CameraFocusDistanceRange, CameraSensorIsoRange, CameraWhiteBalanceRange,
};
...
pub(crate) unsafe fn destack_device_camera_stream_control_capabilities(
    ...
    let control_modes = dispatch_device(
        &resource,
        "destack.device.camera.stream.controlCapabilities",
        |device| Ok(camera_control_modes(device)),
    )?;
    let mut capabilities = camera_control_capabilities(&control_modes);
