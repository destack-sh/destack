...
use super::roots::{media_kind_for_path, media_roots};

/// The media list operation name.
pub(crate) const MEDIA_LIST_OPERATION: &str = "destack.os.media.list";

/// The media read operation name.
pub(crate) const MEDIA_READ_OPERATION: &str = "destack.os.media.read";

/// Build one media descriptor from one file path.
pub(crate) fn media_descriptor_from_path(
    platform: Platform,
    path: &Path,
    metadata: &Metadata,
...
/// Return one media class from root ownership.
