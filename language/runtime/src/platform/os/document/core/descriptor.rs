...
pub(crate) const DOCUMENT_PICK_OPERATION: &str = "destack.os.document.pick";
...
fn document_pick_path_error(
    ...
    RuntimeError::from(PlatformError::generic(
        code,
        format!(
            "{DOCUMENT_PICK_OPERATION}: {message} {}: {error}",
            path.display()
        ),
    ))
    .boxed()
