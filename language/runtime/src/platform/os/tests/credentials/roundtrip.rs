...
fn test_credentials_roundtrip() {
    with_harness_context(|mut context| {
        ...
        // verify unsupported targets return notSupported for write
        if !credentials_roundtrip_supported_on_target() {
            ...
            let result = context.destack_os_credentials_write(write_options);
        ...
        let access_group_value = string_value(&mut context, "");
        ...
        let query = credential_query_value(&mut context, &service, &account, "", false);
        ...
        let replacement_query = credential_query_value(&mut context, &service, &account, "", false);
