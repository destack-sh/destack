...
fn require_test_suspend_hook() -> RuntimeResult<LinuxSuspendHook> {
    ...
    let Some(hook) = hook else {
        return Err(core_platform::invalid_state(format!(
            "{OS_POWER_SUSPEND_OPERATION} tests must install a suspend hook before calling suspend"
        )));
    };
    ...
    let entries = match std::fs::read_dir(LINUX_POWER_SUPPLY_PATH) {
        ...
        Err(error) => {
            ...
            return Err(core_platform::io_operation_error(
                ...
                None,
                format!("read {LINUX_POWER_SUPPLY_PATH} failed: {error}"),
            ));
        }
    };
