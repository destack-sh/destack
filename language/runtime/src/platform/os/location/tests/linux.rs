...
/// Shared Linux location-test mutex that serializes the process-global hook slot.
static TEST_LOCATION_MUTEX: OnceLock<Mutex<()>> = OnceLock::new();

/// Shared captured Linux location watch id from the active watch-open hook.
static TEST_LOCATION_WATCH_ID: OnceLock<Mutex<Option<String>>> = OnceLock::new();

/// Reset the installed location hooks after one Linux test case.
struct LocationHookGuard;

impl Drop for LocationHookGuard {
    /// Clear the active location hooks after one test case.
    fn drop(&mut self) {
        set_linux_location_test_hooks(LinuxLocationHooks::default());
...
/// Verify Linux location routes last-known reads and watch events through runtime state.
#[test]
fn test_location_linux_routes_last_known_and_watch_streams() {
    let _guard = TEST_LOCATION_MUTEX
        .get_or_init(|| Mutex::new(()))
        .lock()
        .unwrap_or_else(|error| error.into_inner());

    set_linux_location_test_hooks(LinuxLocationHooks {
    ...
    let watch_id = TEST_LOCATION_WATCH_ID
        ...
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .expect("Linux location watch hook should capture one watch id");
    assert!(watch_id.starts_with("location-watch-"));
}

/// Report that Linux location services are available in tests.
fn test_location_services_enabled_hook() -> RuntimeResult<bool> {
    Ok(true)
}

/// Report one cached Linux location sample in tests.
