...
/// Shared macOS location-test mutex that serializes the process-global hook slot.
static TEST_LOCATION_MUTEX: OnceLock<Mutex<()>> = OnceLock::new();

/// Shared captured macOS location watch id from the active watch-open hook.
static TEST_LOCATION_WATCH_ID: OnceLock<Mutex<Option<String>>> = OnceLock::new();

/// Reset the installed location hooks after one macOS test case.
...
/// Verify macOS location routes real host reads and watch events through runtime state.
#[test]
fn test_location_macos_routes_last_known_and_watch_streams() {
    ...
    let watch_id = TEST_LOCATION_WATCH_ID
        ...
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .expect("macOS location watch hook should capture one watch id");
    assert!(watch_id.starts_with("location-watch-"));
}

/// Report that macOS location services are available in tests.
fn test_location_services_enabled_hook() -> RuntimeResult<bool> {
    Ok(true)
}

/// Report one cached macOS location sample in tests.
