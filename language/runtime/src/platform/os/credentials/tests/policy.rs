...
mod shared {
    ...
    fn test_credentials_delete_and_contains_access_group_policy_on_linux_and_windows() {
        with_harness_context(|mut context| {
            ...
            let access_group = optional_string_value(&mut context, Some("group.identifier"));
            let delete_result =
                context.destack_os_credentials_delete(service, account, access_group);
...
mod linux_only {
    use crate::platform::os::credentials::tests::tests::{
        credential_query_value, credential_write_options_value,
    };
    use crate::platform::os::tests::with_harness_context;
    use crate::platform::os::{CredentialAccessibility, CredentialAuthenticationPolicy};
    use crate::tests::platform::assert_not_supported_error;

    /// Verify unsupported accessibility write-policy lanes report notSupported on Linux.
    #[test]
    fn test_credentials_write_accessibility_policy_on_linux() {
        with_harness_context(|mut context| {
    ...
    fn test_credentials_read_policy_lanes_on_linux() {
        with_harness_context(|mut context| {
            ...
            let query =
                credential_query_value(&mut context, "destack.policy", "read-auth", None, true);
