...
pub(crate) fn publish_delivered_notification(
    ...
    publish_notification_event(host_session_id, platform, event)
}

/// Publish one interacted notification event into the runtime host queue.
#[cfg(any(target_os = "macos", windows, all(unix, not(target_os = "macos"))))]
pub(crate) fn publish_interacted_notification(
    host_session_id: HostSessionId,
    platform: Platform,
    id: String,
    request: NotificationRequestValue,
...
#[cfg(any(target_os = "macos", windows, all(unix, not(target_os = "macos"))))]
pub(crate) fn publish_dismissed_notification(
    host_session_id: HostSessionId,
    platform: Platform,
    id: String,
    request: NotificationRequestValue,
...
#[cfg(any(target_os = "macos", windows, all(unix, not(target_os = "macos"))))]
pub(crate) fn next_notification_sequence(
    host_session_id: HostSessionId,
    platform: Platform,
) -> u64 {
