...
use crate::runtime::bindings::{BindingAffinity, BindingDescriptor, BindingBlocking, BindingRegistry, NativeBinding, NativeBindingSet, BindingScope, native_call, BindingReplayKind, BindingReplayPolicy};
...
use crate::platform::resource::{native as platform_runtime_native, vm as platform_runtime_vm};
...
fn decode_destack_resource_id_transfer_args(
    ...
    let ownership = match ownership_raw { 1i32 => ResourceOwnership::Borrowed, 2i32 => ResourceOwnership::Owned , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("ownership", "unknown ResourceOwnership value")).boxed()), };
    Ok((id, ownership))
}
...
pub(crate) const RESOURCE_ID_CLOSE: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub(crate) const RESOURCE_ID_KIND: BindingDescriptor = BindingDescriptor::deterministic_with_requires_and_behavior(
...
pub(crate) const RESOURCE_ID_REMOVE: BindingDescriptor = BindingDescriptor::deterministic_with_requires_and_behavior(
...
pub(crate) const RESOURCE_NATIVE_BINDINGS: NativeBindingSet = NativeBindingSet {
    name: "resource",
    bindings: &[
        NativeBinding::new(RESOURCE_ID_CLOSE, "destack.resource.id.close", destack_resource_id_close as *const ()),
        NativeBinding::new(RESOURCE_ID_KIND, "destack.resource.id.kind", destack_resource_id_kind as *const ()),
        NativeBinding::new(RESOURCE_ID_REMOVE, "destack.resource.id.remove", destack_resource_id_remove as *const ()),
        NativeBinding::new(RESOURCE_ID_TRANSFER, "destack.resource.id.transfer", destack_resource_id_transfer as *const ()),
    ],
};
...
pub(crate) unsafe extern "C" fn destack_resource_id_close(
    id: ResourceId,
) -> RuntimeStatus {
    native_call(|context| {
        let _ = &id;
...
pub(crate) unsafe extern "C" fn destack_resource_id_remove(
    id: ResourceId,
) -> RuntimeStatus {
    native_call(|context| {
        let _ = &id;
...
pub(crate) fn register_resource_vm_bindings(
    registry: &mut BindingRegistry,
    isolate: &mut Isolate,
) {
    {
        binding!(registry, isolate, RESOURCE_ID_CLOSE, move |context, args| {
    ...
    {
        binding!(registry, isolate, RESOURCE_ID_REMOVE, move |context, args| {
            with_binding_call_context(|binding| {
                // decode args
    ...
    {
        binding!(registry, isolate, RESOURCE_ID_TRANSFER, move |context, args| {
            with_binding_call_context(|binding| {
                // decode args
                ...
                // execute binding
                let result = {
                        let _binding_hook_guard = binding.on_before_binding(RESOURCE_ID_TRANSFER)?;
                        platform_runtime_vm::destack_resource_transfer(binding, context, id, ownership)
                    };
