...
mod tests {
    ...
    fn test_insert_remove_and_finalize() {
        ...
        let resource_id = table.insert(world_state, entry, None);
        assert!(table.contains(resource_id));

        // remove and finalize the entry
        let removed = table.remove_and_finalize(world_state, resource_id, None);
        assert!(removed);
        assert_eq!(hits.load(Ordering::SeqCst), 1);
        assert!(!table.contains(resource_id));

        // removing again should return false
        let removed_again = table.remove_and_finalize(world_state, resource_id, None);
    ...
    fn test_snapshot_restore_requires_external_rebinding() {
        ...
        let _ = table.insert(world_state, entry, None);
