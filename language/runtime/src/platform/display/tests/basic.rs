use super::{
    ...
    decode_monitor_modes, decode_window_descriptor, default_window_options, harness_string,
    with_harness_context,
};
use crate::platform::diagnostic::PlatformErrorCode;
use crate::platform::display::{
    DisplayEventKind, DisplayMode, DisplayModeVm, WindowEventKind, WindowVisibility,
};
...
fn test_display_monitor_surface_works_end_to_end() {
    with_harness_context(|mut context| {
        let monitor_list = context.destack_display_monitor_list()?;
        ...
        let display = context.destack_display_monitor_open(display_id_value)?;
        ...
        let event_stream = context.destack_display_monitor_event_open()?;
        ...
        context.destack_display_monitor_set_mode(
            display,
            requested_mode,
        )?;
        ...
        match event {
            HarnessValue::Native(event) => {
                assert_eq!(event.kind, DisplayEventKind::ModeChanged);
            }
        ...
        let primary = context.destack_display_monitor_primary()?;
...
fn test_display_window_surface_works_end_to_end() {
    with_harness_context(|mut context| {
        ...
        let event_stream = context.destack_display_window_event_open()?;
        ...
        match event {
            HarnessValue::Native(event) => {
                assert!(
                    event.kind == WindowEventKind::RefreshRequested
                        || event.kind == WindowEventKind::Created
                );
            }
            HarnessValue::Vm(event) => {
                assert!(
