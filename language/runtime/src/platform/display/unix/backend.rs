...
use crate::platform::display::{
    DisplayBackend, DisplayDescriptor, DisplayEvent, DisplayMode, DisplayMonitorEventOpenOptions,
...
pub(crate) unsafe fn destack_display_window_vsync_wait(
    ...
    let backend = core::resolve_default_backend("destack.display.window.vsyncWait")?;
    dispatch_backend!(
        backend,
        "destack.display.window.vsyncWait",
        window_vsync_wait(context, window, timeoutns)
    )
}
