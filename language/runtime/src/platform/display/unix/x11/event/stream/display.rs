...
use crate::platform::display::unix::x11::event::codec::display_event_from_record;
use crate::platform::display::unix::x11::event::publish::seed_monitor_event_stream;
...
fn next_monitor_event(
    ...
    if take_monitor_overflow_error(resolved_stream) {
        return Err(x11_core::overflow_error(operation));
    }
...
pub(crate) unsafe fn monitor_event_open(
    ...
    let resource_id = binding.worker().resources.insert(
        ...
        ResourceEntry::new(ResourceKind::Display)
            .with_label(x11_core::DISPLAY_EVENT_RESOURCE_LABEL)
