...
use crate::platform::display::unix::x11::model::DisplayDescriptorSnapshot;
...
impl DisplayEventRecordKind {
    ...
    pub(crate) fn kind_mask(&self) -> u32 {
        ...
        match self {
            DisplayEventRecordKind::Added { .. } => x11_core::DISPLAY_MONITOR_EVENT_KIND_ADDED,
            DisplayEventRecordKind::Removed { .. } => x11_core::DISPLAY_MONITOR_EVENT_KIND_REMOVED,
            DisplayEventRecordKind::PrimaryChanged { .. } => {
                x11_core::DISPLAY_MONITOR_EVENT_KIND_PRIMARY_CHANGED
            }
            DisplayEventRecordKind::DescriptorChanged { .. } => {
                x11_core::DISPLAY_MONITOR_EVENT_KIND_DESCRIPTOR_CHANGED
            }
            DisplayEventRecordKind::ModeChanged { .. } => {
                x11_core::DISPLAY_MONITOR_EVENT_KIND_MODE_CHANGED
            }
        }
    }
...
impl MonitorEventFilterState {
    ...
    pub(crate) fn from_open_options(
        ...
        let kind_mask = x11_core::monitor_kind_mask(filter.kind_mask);
        x11_core::validate_monitor_event_kind_mask(kind_mask, "options.filter.kindMask")?;
...
#[cfg(test)]
mod tests {
    use crate::platform::display::unix::x11::event::codec::monitor_topology_records;
    use crate::platform::display::unix::x11::model::MonitorSnapshot;
    use crate::platform::display::{DisplayMode, DisplayOrientation, DisplaySupportStatus};

    use super::{DisplayDescriptorSnapshot, DisplayEventRecordKind, x11_core};
    ...
    fn test_monitor_topology_records_emit_descriptor_and_mode_changes() {
        ...
        assert!(records.iter().any(|record| matches!(
            ...
            DisplayEventRecordKind::DescriptorChanged { ref current, changed_mask, .. }
                if current.id == "x11-output-1" && (changed_mask & x11_core::DISPLAY_CHANGED_MASK_BOUNDS) != 0
        )));
        assert!(records.iter().any(|record| matches!(
            record.kind,
