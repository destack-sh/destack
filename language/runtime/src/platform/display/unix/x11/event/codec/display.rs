...
use crate::platform::display::unix::x11::event::{DisplayEventRecord, DisplayEventRecordKind};
...
pub(crate) fn descriptor_changed_mask(
    ...
    // record name changes
    if name_changed {
        changed_mask |= x11_core::DISPLAY_CHANGED_MASK_NAME;
    }

    // record primary-display changes
    if previous.primary != next.primary {
        changed_mask |= x11_core::DISPLAY_CHANGED_MASK_PRIMARY;
    }
    ...
    if bounds_changed {
        changed_mask |= x11_core::DISPLAY_CHANGED_MASK_BOUNDS;
    }
    ...
    if work_area_changed {
        changed_mask |= x11_core::DISPLAY_CHANGED_MASK_WORKAREA;
    }

    // record scale-factor changes
    if previous.scale_factor_milli != next.scale_factor_milli {
        changed_mask |= x11_core::DISPLAY_CHANGED_MASK_SCALE;
    }

    // record orientation changes
    if previous.orientation != next.orientation {
        changed_mask |= x11_core::DISPLAY_CHANGED_MASK_ORIENTATION;
    }
