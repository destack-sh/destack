...
impl WindowEventRecordKind {
    ...
    pub(crate) fn kind_mask(&self) -> u64 {
        ...
        match self {
            WindowEventRecordKind::Created { .. } => wayland_core::WINDOW_EVENT_KIND_CREATED,
            WindowEventRecordKind::CloseRequested { .. } => {
                wayland_core::WINDOW_EVENT_KIND_CLOSE_REQUESTED
            }
            WindowEventRecordKind::Destroyed { .. } => wayland_core::WINDOW_EVENT_KIND_DESTROYED,
            WindowEventRecordKind::RefreshRequested { .. } => {
                wayland_core::WINDOW_EVENT_KIND_REFRESH_REQUESTED
            }
            WindowEventRecordKind::VisibilityChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_VISIBILITY_CHANGED
            }
            WindowEventRecordKind::OcclusionChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_OCCLUSION_CHANGED
            }
            WindowEventRecordKind::PositionChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_POSITION_CHANGED
            }
            WindowEventRecordKind::SizeChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_SIZE_CHANGED
            }
            WindowEventRecordKind::ScaleFactorChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_SCALE_FACTOR_CHANGED
            }
            WindowEventRecordKind::FocusChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_FOCUS_CHANGED
            }
            WindowEventRecordKind::ModeChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_MODE_CHANGED
            }
            WindowEventRecordKind::DisplayChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_DISPLAY_CHANGED
            }
            WindowEventRecordKind::ThemeChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_THEME_CHANGED
            }
            WindowEventRecordKind::ChromeChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_CHROME_CHANGED
            }
            WindowEventRecordKind::TaskbarVisibilityChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_TASKBAR_VISIBILITY_CHANGED
            }
            WindowEventRecordKind::SafeAreaChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_SAFE_AREA_CHANGED
            }
            WindowEventRecordKind::OpacityChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_OPACITY_CHANGED
            }
            WindowEventRecordKind::ParentChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_PARENT_CHANGED
            }
            WindowEventRecordKind::TransientChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_TRANSIENT_CHANGED
            }
            WindowEventRecordKind::ModalChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_MODAL_CHANGED
            }
            WindowEventRecordKind::MousePassthroughChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_MOUSE_PASSTHROUGH_CHANGED
            }
            WindowEventRecordKind::AspectRatioChanged { .. } => {
                wayland_core::WINDOW_EVENT_KIND_ASPECT_RATIO_CHANGED
            }
            WindowEventRecordKind::DropStarted { .. } => {
                wayland_core::WINDOW_EVENT_KIND_DROP_STARTED
            }
            WindowEventRecordKind::FileHovered { .. } => {
                wayland_core::WINDOW_EVENT_KIND_FILE_HOVERED
            }
            WindowEventRecordKind::DropCancelled { .. } => {
                wayland_core::WINDOW_EVENT_KIND_DROP_CANCELLED
            }
            WindowEventRecordKind::DropCompleted { .. } => {
                wayland_core::WINDOW_EVENT_KIND_DROP_COMPLETED
            }
            WindowEventRecordKind::FileHoverLeft { .. } => {
                wayland_core::WINDOW_EVENT_KIND_FILE_HOVER_LEFT
            }
            WindowEventRecordKind::FileDropped { .. } => {
                wayland_core::WINDOW_EVENT_KIND_FILE_DROPPED
            }
            WindowEventRecordKind::TextDropped { .. } => {
                wayland_core::WINDOW_EVENT_KIND_TEXT_DROPPED
            }
        }
    }
...
impl WindowEventFilterState {
    ...
    pub(crate) fn from_open_options(options: WindowEventOpenOptions) -> RuntimeResult<Self> {
        ...
        let kind_mask = wayland_core::window_kind_mask(filter.kind_mask);
        wayland_core::validate_window_event_kind_mask(kind_mask, "options.filter.kindMask")?;
