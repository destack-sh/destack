...
impl DisplayEventRecordKind {
    ...
    pub(crate) fn kind_mask(&self) -> u32 {
        ...
        match self {
            DisplayEventRecordKind::Added { .. } => appkit_core::DISPLAY_MONITOR_EVENT_KIND_ADDED,
            DisplayEventRecordKind::Removed { .. } => {
                appkit_core::DISPLAY_MONITOR_EVENT_KIND_REMOVED
            }
            DisplayEventRecordKind::PrimaryChanged { .. } => {
                appkit_core::DISPLAY_MONITOR_EVENT_KIND_PRIMARY_CHANGED
            }
            DisplayEventRecordKind::DescriptorChanged { .. } => {
                appkit_core::DISPLAY_MONITOR_EVENT_KIND_DESCRIPTOR_CHANGED
            }
            DisplayEventRecordKind::ModeChanged { .. } => {
                appkit_core::DISPLAY_MONITOR_EVENT_KIND_MODE_CHANGED
            }
        }
    }
...
impl MonitorEventFilterState {
    ...
    pub(crate) fn from_open_options(
        ...
        let kind_mask = appkit_core::monitor_kind_mask(filter.kind_mask);
        appkit_core::validate_monitor_event_kind_mask(kind_mask, "options.filter.kindMask")?;
...
mod tests {
    ...
    fn test_monitor_topology_records_emit_removed_added_mode_and_primary() {
        let previous = vec![snapshot(
            descriptor("AppKit-output-1", true, 1920, 1080),
            mode(1920, 1080, 60_000),
        )];
        let next = vec![snapshot(
            descriptor("AppKit-output-2", true, 2560, 1440),
        ...
        assert!(records.iter().any(|record| matches!(
            record.kind,
            DisplayEventRecordKind::Removed { ref id, .. } if id == "AppKit-output-1"
        )));
        assert!(records.iter().any(|record| matches!(
            record.kind,
            DisplayEventRecordKind::Added { ref descriptor } if descriptor.id == "AppKit-output-2"
        )));
        assert!(records.iter().any(|record| matches!(
            record.kind,
            DisplayEventRecordKind::ModeChanged { ref id, current: mode, .. } if id == "AppKit-output-2" && mode.refresh_milli_hz == 144_000
        )));
        assert!(records.iter().any(|record| matches!(
            record.kind,
            DisplayEventRecordKind::PrimaryChanged { current_id: Some(ref id), .. } if id == "AppKit-output-2"
        )));
    }
    ...
    fn test_monitor_topology_records_emit_descriptor_and_mode_changes() {
        ...
        let next = vec![snapshot(
            descriptor("AppKit-output-1", true, 2560, 1440),
        ...
        assert!(records.iter().any(|record| matches!(
            ...
            DisplayEventRecordKind::DescriptorChanged { ref current, changed_mask, .. }
                if current.id == "AppKit-output-1"
                    && (changed_mask & appkit_core::DISPLAY_CHANGED_MASK_BOUNDS) != 0
        )));
        assert!(records.iter().any(|record| matches!(
            record.kind,
            DisplayEventRecordKind::ModeChanged { ref id, current: mode, .. }
                if id == "AppKit-output-1" && mode.refresh_milli_hz == 120_000
        )));
    }
}
