...
use crate::platform::gpu::{GpuAdapterInfo, GpuAdapterInfoReplayRecord, GpuAdapterInfoVm, GpuAdapterRequest, GpuAdapterRequestVm, GpuAdapterType, GpuBackend, GpuBufferCopy, GpuBufferCopyLayout, GpuBufferCopyLayoutVm, GpuBufferCopyVm, GpuBufferOptions, GpuBufferOptionsVm, GpuCommandEncoderOptions, GpuCommandEncoderOptionsVm, GpuComputePipelineOptions, GpuComputePipelineOptionsVm, GpuDeviceInfo, GpuDeviceInfoVm, GpuDeviceLimits, GpuDeviceLimitsVm, GpuDeviceOptions, GpuDeviceOptionsVm, GpuExtent3D, GpuExtent3DVm, GpuMappedMemory, GpuMappedMemoryVm, GpuPowerPreference, GpuPresentOptions, GpuPresentOptionsVm, GpuRenderPassOptions, GpuRenderPassOptionsVm, GpuRenderPipelineOptions, GpuRenderPipelineOptionsVm, GpuSamplerOptions, GpuSamplerOptionsVm, GpuShaderOptions, GpuShaderOptionsVm, GpuShaderStage, GpuSubmitOptions, GpuSubmitOptionsVm, GpuSurfaceOptions, GpuSurfaceOptionsVm, GpuTextureCopy, GpuTextureCopyVm, GpuTextureOptions, GpuTextureOptionsVm};
...
use crate::platform::gpu::simulated::{native as platform_simulated_native, vm as platform_simulated_vm};
...
fn encode_destack_gpu_adapter_info_result(
    ...
    result.map(|value| { let field_0 = value.id.value(); let field_1 = value.name.value(); let field_2 = vm::Value::uint(value.backend as u8 as u64, 8); let field_3 = vm::Value::uint(value.adapter_type as u8 as u64, 8); let field_4 = vm::Value::uint(value.vendor_id as u64, 32); let field_5 = vm::Value::uint(value.device_id as u64, 32); let field_6 = vm::Value::uint(value.feature_mask, 64); let field_7 = vm::Value::uint(value.max_bind_groups as u64, 32); let field_8 = vm::Value::uint(value.max_texture_dimension2_d as u64, 32); let field_9 = vm::Value::uint(value.max_storage_buffer_binding_size, 64); context.allocate_aggregate(vec![field_0, field_1, field_2, field_3, field_4, field_5, field_6, field_7, field_8, field_9]) })
}
...
fn decode_destack_gpu_adapter_list_args(
    ...
    let request = {
        if request_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("request", "GpuAdapterRequest")).boxed()); }
        let slots = context.aggregate_slots(request_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("request", "expected 3 fields")).boxed()); }
        let request_backend = decode_uint32(slots[0], "request_backend", "backend")?;
        let request_power_preference_raw = decode_uint8(slots[1], "request_power_preference_raw", "powerPreference")?;
        let request_power_preference = match request_power_preference_raw { 1u8 => GpuPowerPreference::LowPower, 2u8 => GpuPowerPreference::HighPerformance, 3u8 => GpuPowerPreference::None , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("request_power_preference", "unknown GpuPowerPreference value")).boxed()), };
        let request_allow_fallback = decode_bool(slots[2], "request_allow_fallback", "allowFallback")?;
        GpuAdapterRequestVm {
...
fn decode_destack_gpu_command_bind_compute_pipeline_args(
    ...
    let handle_inner_inner = decode_uint64(handle_value, "handle_inner_inner", "GpuCommandListHandle")?;
    let handle_inner = resource::ResourceId(handle_inner_inner);
    let handle = resource::GpuCommandListHandle(handle_inner);
    let pipeline_value = arg_value(args, 1, "pipeline", "GpuPipelineHandle")?;
    let pipeline_inner_inner = decode_uint64(pipeline_value, "pipeline_inner_inner", "GpuPipelineHandle")?;
...
fn decode_destack_gpu_command_copy_buffer_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuCommandListHandle, resource::GpuBufferHandle, u64, resource::GpuBufferHandle, u64, u64)> {
    // ignore unused context
    let _ = context;
...
fn decode_destack_gpu_command_copy_buffer_to_texture_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuCommandListHandle, GpuBufferCopyVm, GpuTextureCopyVm, GpuExtent3DVm)> {
    let handle_value = arg_value(args, 0, "handle", "GpuCommandListHandle")?;
    let handle_inner_inner = decode_uint64(handle_value, "handle_inner_inner", "GpuCommandListHandle")?;
    let handle_inner = resource::ResourceId(handle_inner_inner);
    let handle = resource::GpuCommandListHandle(handle_inner);
    ...
    let source = {
        if source_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("source", "GpuBufferCopy")).boxed()); }
        let slots = context.aggregate_slots(source_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 2 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("source", "expected 2 fields")).boxed()); }
        let source_buffer_inner_inner = decode_uint64(slots[0], "source_buffer_inner_inner", "buffer")?;
        ...
        let source_layout = {
            if slots[1].tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("source_layout", "layout")).boxed()); }
            let slots = context.aggregate_slots(slots[1]).map_err(|error| RuntimeError::from(error).boxed())?;
            if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("source_layout", "expected 3 fields")).boxed()); }
            let source_layout_offset = decode_uint64(slots[0], "source_layout_offset", "offset")?;
            let source_layout_bytes_per_row = decode_uint32(slots[1], "source_layout_bytes_per_row", "bytesPerRow")?;
            let source_layout_rows_per_image = decode_uint32(slots[2], "source_layout_rows_per_image", "rowsPerImage")?;
    ...
    let destination = {
        if destination_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("destination", "GpuTextureCopy")).boxed()); }
        let slots = context.aggregate_slots(destination_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 6 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("destination", "expected 6 fields")).boxed()); }
        let destination_texture_inner_inner = decode_uint64(slots[0], "destination_texture_inner_inner", "texture")?;
    ...
    let size = {
        if size_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("size", "GpuExtent3D")).boxed()); }
        let slots = context.aggregate_slots(size_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("size", "expected 3 fields")).boxed()); }
        let size_width = decode_uint32(slots[0], "size_width", "width")?;
        let size_height = decode_uint32(slots[1], "size_height", "height")?;
        let size_depth_or_array_layers = decode_uint32(slots[2], "size_depth_or_array_layers", "depthOrArrayLayers")?;
        GpuExtent3DVm {
            width: size_width,
            height: size_height,
...
fn decode_destack_gpu_command_copy_texture_to_buffer_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuCommandListHandle, GpuTextureCopyVm, GpuBufferCopyVm, GpuExtent3DVm)> {
    let handle_value = arg_value(args, 0, "handle", "GpuCommandListHandle")?;
    let handle_inner_inner = decode_uint64(handle_value, "handle_inner_inner", "GpuCommandListHandle")?;
    let handle_inner = resource::ResourceId(handle_inner_inner);
    let handle = resource::GpuCommandListHandle(handle_inner);
    ...
    let source = {
        if source_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("source", "GpuTextureCopy")).boxed()); }
        let slots = context.aggregate_slots(source_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 6 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("source", "expected 6 fields")).boxed()); }
        let source_texture_inner_inner = decode_uint64(slots[0], "source_texture_inner_inner", "texture")?;
    ...
    let destination = {
        if destination_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("destination", "GpuBufferCopy")).boxed()); }
        let slots = context.aggregate_slots(destination_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 2 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("destination", "expected 2 fields")).boxed()); }
        let destination_buffer_inner_inner = decode_uint64(slots[0], "destination_buffer_inner_inner", "buffer")?;
        ...
        let destination_layout = {
            if slots[1].tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("destination_layout", "layout")).boxed()); }
            let slots = context.aggregate_slots(slots[1]).map_err(|error| RuntimeError::from(error).boxed())?;
            if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("destination_layout", "expected 3 fields")).boxed()); }
            let destination_layout_offset = decode_uint64(slots[0], "destination_layout_offset", "offset")?;
            let destination_layout_bytes_per_row = decode_uint32(slots[1], "destination_layout_bytes_per_row", "bytesPerRow")?;
            let destination_layout_rows_per_image = decode_uint32(slots[2], "destination_layout_rows_per_image", "rowsPerImage")?;
            GpuBufferCopyLayoutVm {
...
fn decode_destack_gpu_command_copy_texture_to_texture_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuCommandListHandle, GpuTextureCopyVm, GpuTextureCopyVm, GpuExtent3DVm)> {
    let handle_value = arg_value(args, 0, "handle", "GpuCommandListHandle")?;
    let handle_inner_inner = decode_uint64(handle_value, "handle_inner_inner", "GpuCommandListHandle")?;
    let handle_inner = resource::ResourceId(handle_inner_inner);
    let handle = resource::GpuCommandListHandle(handle_inner);
...
fn decode_destack_gpu_command_draw_args(
    ...
    Ok((handle, vertexcount, instancecount, firstvertex, firstinstance))
}
...
fn decode_destack_gpu_command_draw_indexed_args(
    ...
    let firstinstance = decode_uint32(firstinstance_value, "firstinstance", "uint32")?;
    Ok((handle, indexcount, instancecount, firstindex, basevertex, firstinstance))
}
...
fn decode_destack_gpu_command_encoder_open_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuCommandEncoderOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 1 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 1 fields")).boxed()); }
...
fn decode_destack_gpu_command_queue_submit_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuQueueHandle, VmSlice<resource::GpuCommandListHandle>, GpuSubmitOptionsVm)> {
    ...
    let commandlists = decode_slice::<resource::GpuCommandListHandle>(context, commandlists_value, "commandlists", "Slice<GpuCommandListHandle>")?;
    let options_value = arg_value(args, 2, "options", "GpuSubmitOptions")?;
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuSubmitOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
...
fn decode_destack_gpu_command_queue_write_buffer_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuQueueHandle, resource::GpuBufferHandle, u64, VmSlice<u8>, u64, u64)> {
    let queue_value = arg_value(args, 0, "queue", "GpuQueueHandle")?;
    let queue_inner_inner = decode_uint64(queue_value, "queue_inner_inner", "GpuQueueHandle")?;
    let queue_inner = resource::ResourceId(queue_inner_inner);
    let queue = resource::GpuQueueHandle(queue_inner);
...
fn decode_destack_gpu_command_queue_write_texture_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuQueueHandle, GpuTextureCopyVm, VmSlice<u8>, GpuBufferCopyLayoutVm, GpuExtent3DVm)> {
    let queue_value = arg_value(args, 0, "queue", "GpuQueueHandle")?;
    let queue_inner_inner = decode_uint64(queue_value, "queue_inner_inner", "GpuQueueHandle")?;
    let queue_inner = resource::ResourceId(queue_inner_inner);
    let queue = resource::GpuQueueHandle(queue_inner);
    ...
    let layout = {
        if layout_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("layout", "GpuBufferCopyLayout")).boxed()); }
        let slots = context.aggregate_slots(layout_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("layout", "expected 3 fields")).boxed()); }
        ...
        let layout_rows_per_image = decode_uint32(slots[2], "layout_rows_per_image", "rowsPerImage")?;
        GpuBufferCopyLayoutVm {
...
fn decode_destack_gpu_command_render_pass_begin_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuRenderPassOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 8 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 8 fields")).boxed()); }
        let options_color_texture_inner_inner = decode_uint64(slots[0], "options_color_texture_inner_inner", "colorTexture")?;
        ...
        let options_depth_texture_inner_inner = decode_uint64(slots[2], "options_depth_texture_inner_inner", "depthTexture")?;
        ...
        let options_clear_color_r = decode_float64(slots[3], "options_clear_color_r", "clearColorR")?;
        let options_clear_color_g = decode_float64(slots[4], "options_clear_color_g", "clearColorG")?;
        let options_clear_color_b = decode_float64(slots[5], "options_clear_color_b", "clearColorB")?;
        let options_clear_color_a = decode_float64(slots[6], "options_clear_color_a", "clearColorA")?;
...
fn encode_destack_gpu_device_info_result(
    ...
    result.map(|value| { let field_0 = vm::Value::uint(value.enabled_feature_mask, 64); let field_1 = { let field_0 = vm::Value::uint(value.effective_limits.max_bind_groups as u64, 32); let field_1 = vm::Value::uint(value.effective_limits.max_texture_dimension2_d as u64, 32); let field_2 = vm::Value::uint(value.effective_limits.max_uniform_buffer_binding_size, 64); let field_3 = vm::Value::uint(value.effective_limits.max_storage_buffer_binding_size, 64); context.allocate_aggregate(vec![field_0, field_1, field_2, field_3]) }; context.allocate_aggregate(vec![field_0, field_1]) })
}
...
fn decode_destack_gpu_device_open_args(
    ...
    let adapter_inner_inner = decode_uint64(adapter_value, "adapter_inner_inner", "GpuAdapterHandle")?;
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuDeviceOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 3 fields")).boxed()); }
        let options_required_feature_mask = decode_uint64(slots[0], "options_required_feature_mask", "requiredFeatureMask")?;
        let options_required_limits = {
            if slots[1].tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options_required_limits", "requiredLimits")).boxed()); }
            let slots = context.aggregate_slots(slots[1]).map_err(|error| RuntimeError::from(error).boxed())?;
            if slots.len() != 4 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options_required_limits", "expected 4 fields")).boxed()); }
            let options_required_limits_max_bind_groups = decode_uint32(slots[0], "options_required_limits_max_bind_groups", "maxBindGroups")?;
            let options_required_limits_max_texture_dimension2_d = decode_uint32(slots[1], "options_required_limits_max_texture_dimension2_d", "maxTextureDimension2D")?;
            let options_required_limits_max_uniform_buffer_binding_size = decode_uint64(slots[2], "options_required_limits_max_uniform_buffer_binding_size", "maxUniformBufferBindingSize")?;
            let options_required_limits_max_storage_buffer_binding_size = decode_uint64(slots[3], "options_required_limits_max_storage_buffer_binding_size", "maxStorageBufferBindingSize")?;
            GpuDeviceLimitsVm {
                ...
                max_uniform_buffer_binding_size: options_required_limits_max_uniform_buffer_binding_size,
                max_storage_buffer_binding_size: options_required_limits_max_storage_buffer_binding_size,
            }
        };
...
fn decode_destack_gpu_pipeline_compute_create_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuComputePipelineOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 3 fields")).boxed()); }
        let options_shader_inner_inner = decode_uint64(slots[0], "options_shader_inner_inner", "shader")?;
...
fn decode_destack_gpu_pipeline_destroy_args(
    ...
    let handle_inner_inner = decode_uint64(handle_value, "handle_inner_inner", "GpuPipelineHandle")?;
    let handle_inner = resource::ResourceId(handle_inner_inner);
...
fn decode_destack_gpu_pipeline_render_create_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuRenderPipelineOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 8 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 8 fields")).boxed()); }
        let options_vertex_shader_inner_inner = decode_uint64(slots[0], "options_vertex_shader_inner_inner", "vertexShader")?;
        ...
        let options_fragment_shader_inner_inner = decode_uint64(slots[2], "options_fragment_shader_inner_inner", "fragmentShader")?;
        let options_fragment_shader_inner = resource::ResourceId(options_fragment_shader_inner_inner);
        let options_fragment_shader = resource::GpuShaderHandle(options_fragment_shader_inner);
        let options_fragment_entry = decode_string(slots[3], "options_fragment_entry", "fragmentEntry")?;
        let options_color_format = decode_uint32(slots[4], "options_color_format", "colorFormat")?;
        let options_depth_stencil_format = decode_uint32(slots[5], "options_depth_stencil_format", "depthStencilFormat")?;
...
fn decode_destack_gpu_pipeline_shader_create_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuShaderOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 3 fields")).boxed()); }
        let options_stage_raw = decode_uint8(slots[0], "options_stage_raw", "stage")?;
        let options_stage = match options_stage_raw { 1u8 => GpuShaderStage::Vertex, 2u8 => GpuShaderStage::Fragment, 3u8 => GpuShaderStage::Compute , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("options_stage", "unknown GpuShaderStage value")).boxed()), };
...
fn decode_destack_gpu_present_queue_present_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuQueueHandle, resource::WindowHandle, GpuPresentOptionsVm)> {
    let queue_value = arg_value(args, 0, "queue", "GpuQueueHandle")?;
    let queue_inner_inner = decode_uint64(queue_value, "queue_inner_inner", "GpuQueueHandle")?;
    let queue_inner = resource::ResourceId(queue_inner_inner);
    let queue = resource::GpuQueueHandle(queue_inner);
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuPresentOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 2 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 2 fields")).boxed()); }
        let options_flags = decode_uint32(slots[0], "options_flags", "flags")?;
...
fn decode_destack_gpu_present_surface_configure_args(
    context: &mut vm::ExternalCallContext<'_>,
    args: &[vm::Value],
) -> RuntimeResult<(resource::GpuDeviceHandle, resource::WindowHandle, GpuSurfaceOptionsVm)> {
    let device_value = arg_value(args, 0, "device", "GpuDeviceHandle")?;
    let device_inner_inner = decode_uint64(device_value, "device_inner_inner", "GpuDeviceHandle")?;
    let device_inner = resource::ResourceId(device_inner_inner);
    let device = resource::GpuDeviceHandle(device_inner);
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuSurfaceOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
...
fn decode_destack_gpu_resource_buffer_create_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuBufferOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 3 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 3 fields")).boxed()); }
...
fn decode_destack_gpu_resource_sampler_create_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuSamplerOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 5 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 5 fields")).boxed()); }
...
fn decode_destack_gpu_resource_texture_create_args(
    ...
    let options = {
        if options_value.tag() != vm::ValueTag::Aggregate { return Err(RuntimeError::from(PlatformError::invalid_argument_type("options", "GpuTextureOptions")).boxed()); }
        let slots = context.aggregate_slots(options_value).map_err(|error| RuntimeError::from(error).boxed())?;
        if slots.len() != 7 { return Err(RuntimeError::from(PlatformError::invalid_argument_value("options", "expected 7 fields")).boxed()); }
        let options_width = decode_uint32(slots[0], "options_width", "width")?;
        let options_height = decode_uint32(slots[1], "options_height", "height")?;
        let options_depth_or_layers = decode_uint32(slots[2], "options_depth_or_layers", "depthOrLayers")?;
...
pub const GPU_ADAPTER_CLOSE: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_ADAPTER_OPEN: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_DEVICE_CLOSE: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_DEVICE_INFO: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_PIPELINE_DESTROY: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_PIPELINE_SHADER_DESTROY: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_RESOURCE_BUFFER_DESTROY: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_RESOURCE_MEMORY_UNMAP: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_RESOURCE_SAMPLER_DESTROY: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_RESOURCE_TEXTURE_DESTROY: BindingDescriptor = BindingDescriptor::external_with_requires_and_behavior(
...
pub const GPU_NATIVE_BINDINGS: NativeBindingSet = NativeBindingSet {
    name: "gpu",
    bindings: &[
        NativeBinding::new(GPU_ADAPTER_CLOSE, "destack.gpu.adapter.close", destack_gpu_adapter_close as *const ()),
        NativeBinding::new(GPU_ADAPTER_INFO, "destack.gpu.adapter.info", destack_gpu_adapter_info as *const ()),
        NativeBinding::new(GPU_ADAPTER_LIST, "destack.gpu.adapter.list", destack_gpu_adapter_list as *const ()),
        NativeBinding::new(GPU_ADAPTER_OPEN, "destack.gpu.adapter.open", destack_gpu_adapter_open as *const ()),
        NativeBinding::new(GPU_COMMAND_BIND_COMPUTE_PIPELINE, "destack.gpu.command.bindComputePipeline", destack_gpu_command_bind_compute_pipeline as *const ()),
        NativeBinding::new(GPU_COMMAND_BIND_RENDER_PIPELINE, "destack.gpu.command.bindRenderPipeline", destack_gpu_command_bind_render_pipeline as *const ()),
        NativeBinding::new(GPU_COMMAND_COMPUTE_PASS_BEGIN, "destack.gpu.command.computePassBegin", destack_gpu_command_compute_pass_begin as *const ()),
        NativeBinding::new(GPU_COMMAND_COMPUTE_PASS_END, "destack.gpu.command.computePassEnd", destack_gpu_command_compute_pass_end as *const ()),
        NativeBinding::new(GPU_COMMAND_COPY_BUFFER, "destack.gpu.command.copyBuffer", destack_gpu_command_copy_buffer as *const ()),
        NativeBinding::new(GPU_COMMAND_COPY_BUFFER_TO_TEXTURE, "destack.gpu.command.copyBufferToTexture", destack_gpu_command_copy_buffer_to_texture as *const ()),
        NativeBinding::new(GPU_COMMAND_COPY_TEXTURE_TO_BUFFER, "destack.gpu.command.copyTextureToBuffer", destack_gpu_command_copy_texture_to_buffer as *const ()),
        NativeBinding::new(GPU_COMMAND_COPY_TEXTURE_TO_TEXTURE, "destack.gpu.command.copyTextureToTexture", destack_gpu_command_copy_texture_to_texture as *const ()),
        NativeBinding::new(GPU_COMMAND_DISPATCH, "destack.gpu.command.dispatch", destack_gpu_command_dispatch as *const ()),
        NativeBinding::new(GPU_COMMAND_DRAW, "destack.gpu.command.draw", destack_gpu_command_draw as *const ()),
        NativeBinding::new(GPU_COMMAND_DRAW_INDEXED, "destack.gpu.command.drawIndexed", destack_gpu_command_draw_indexed as *const ()),
        NativeBinding::new(GPU_COMMAND_ENCODER_CLOSE, "destack.gpu.command.encoderClose", destack_gpu_command_encoder_close as *const ()),
        NativeBinding::new(GPU_COMMAND_ENCODER_FINISH, "destack.gpu.command.encoderFinish", destack_gpu_command_encoder_finish as *const ()),
        NativeBinding::new(GPU_COMMAND_ENCODER_OPEN, "destack.gpu.command.encoderOpen", destack_gpu_command_encoder_open as *const ()),
        NativeBinding::new(GPU_COMMAND_QUEUE_SUBMIT, "destack.gpu.command.queueSubmit", destack_gpu_command_queue_submit as *const ()),
        NativeBinding::new(GPU_COMMAND_QUEUE_WAIT_IDLE, "destack.gpu.command.queueWaitIdle", destack_gpu_command_queue_wait_idle as *const ()),
        NativeBinding::new(GPU_COMMAND_QUEUE_WRITE_BUFFER, "destack.gpu.command.queueWriteBuffer", destack_gpu_command_queue_write_buffer as *const ()),
        NativeBinding::new(GPU_COMMAND_QUEUE_WRITE_TEXTURE, "destack.gpu.command.queueWriteTexture", destack_gpu_command_queue_write_texture as *const ()),
        NativeBinding::new(GPU_COMMAND_RENDER_PASS_BEGIN, "destack.gpu.command.renderPassBegin", destack_gpu_command_render_pass_begin as *const ()),
        NativeBinding::new(GPU_COMMAND_RENDER_PASS_END, "destack.gpu.command.renderPassEnd", destack_gpu_command_render_pass_end as *const ()),
        NativeBinding::new(GPU_DEVICE_CLOSE, "destack.gpu.device.close", destack_gpu_device_close as *const ()),
        NativeBinding::new(GPU_DEVICE_INFO, "destack.gpu.device.info", destack_gpu_device_info as *const ()),
        NativeBinding::new(GPU_DEVICE_OPEN, "destack.gpu.device.open", destack_gpu_device_open as *const ()),
        NativeBinding::new(GPU_DEVICE_POLL, "destack.gpu.device.poll", destack_gpu_device_poll as *const ()),
        NativeBinding::new(GPU_DEVICE_QUEUE, "destack.gpu.device.queue", destack_gpu_device_queue as *const ()),
        NativeBinding::new(GPU_PIPELINE_COMPUTE_CREATE, "destack.gpu.pipeline.computeCreate", destack_gpu_pipeline_compute_create as *const ()),
        NativeBinding::new(GPU_PIPELINE_DESTROY, "destack.gpu.pipeline.destroy", destack_gpu_pipeline_destroy as *const ()),
        NativeBinding::new(GPU_PIPELINE_RENDER_CREATE, "destack.gpu.pipeline.renderCreate", destack_gpu_pipeline_render_create as *const ()),
        NativeBinding::new(GPU_PIPELINE_SHADER_CREATE, "destack.gpu.pipeline.shaderCreate", destack_gpu_pipeline_shader_create as *const ()),
        NativeBinding::new(GPU_PIPELINE_SHADER_DESTROY, "destack.gpu.pipeline.shaderDestroy", destack_gpu_pipeline_shader_destroy as *const ()),
        NativeBinding::new(GPU_PRESENT_QUEUE_PRESENT, "destack.gpu.present.queuePresent", destack_gpu_present_queue_present as *const ()),
        NativeBinding::new(GPU_PRESENT_SURFACE_CONFIGURE, "destack.gpu.present.surfaceConfigure", destack_gpu_present_surface_configure as *const ()),
        NativeBinding::new(GPU_RESOURCE_BUFFER_CREATE, "destack.gpu.resource.bufferCreate", destack_gpu_resource_buffer_create as *const ()),
        NativeBinding::new(GPU_RESOURCE_BUFFER_DESTROY, "destack.gpu.resource.bufferDestroy", destack_gpu_resource_buffer_destroy as *const ()),
        NativeBinding::new(GPU_RESOURCE_BUFFER_READ, "destack.gpu.resource.bufferRead", destack_gpu_resource_buffer_read as *const ()),
        NativeBinding::new(GPU_RESOURCE_BUFFER_WRITE, "destack.gpu.resource.bufferWrite", destack_gpu_resource_buffer_write as *const ()),
        NativeBinding::new(GPU_RESOURCE_MEMORY_MAP, "destack.gpu.resource.memoryMap", destack_gpu_resource_memory_map as *const ()),
        NativeBinding::new(GPU_RESOURCE_MEMORY_UNMAP, "destack.gpu.resource.memoryUnmap", destack_gpu_resource_memory_unmap as *const ()),
        NativeBinding::new(GPU_RESOURCE_SAMPLER_CREATE, "destack.gpu.resource.samplerCreate", destack_gpu_resource_sampler_create as *const ()),
        NativeBinding::new(GPU_RESOURCE_SAMPLER_DESTROY, "destack.gpu.resource.samplerDestroy", destack_gpu_resource_sampler_destroy as *const ()),
        NativeBinding::new(GPU_RESOURCE_TEXTURE_CREATE, "destack.gpu.resource.textureCreate", destack_gpu_resource_texture_create as *const ()),
        NativeBinding::new(GPU_RESOURCE_TEXTURE_DESTROY, "destack.gpu.resource.textureDestroy", destack_gpu_resource_texture_destroy as *const ()),
    ],
};
...
fn destack_gpu_adapter_close_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_adapter_close(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_adapter_close(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_adapter_info_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_adapter_info(context, out, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_adapter_info(context, out, handle) },
        },
        |result| {
            if let Ok(()) = result {
                ...
                let result_recorded_max_texture_dimension2_d = result_value.max_texture_dimension2_d;
                let result_recorded_max_storage_buffer_binding_size = result_value.max_storage_buffer_binding_size;
                let result_recorded = GpuAdapterInfoReplayRecord {
                    ...
                    max_storage_buffer_binding_size: result_recorded_max_storage_buffer_binding_size,
                };
        ...
        |payload| {
            ...
            match payload.result {
                Ok(value) => {
                    ...
                    let value_native_max_storage_buffer_binding_size = value.max_storage_buffer_binding_size;
                    let value_native = GpuAdapterInfo {
                        ...
                        max_storage_buffer_binding_size: value_native_max_storage_buffer_binding_size,
                    };
        ...
        |result| {
            if let Ok(()) = result {
                ...
                for result_recorded_item_value in result_recorded_raw {
                    ...
                    let result_recorded_item_recorded_adapter_type = result_recorded_item.adapter_type;
                    ...
                    let result_recorded_item_recorded_feature_mask = result_recorded_item.feature_mask;
                    let result_recorded_item_recorded_max_bind_groups = result_recorded_item.max_bind_groups;
                    let result_recorded_item_recorded_max_texture_dimension2_d = result_recorded_item.max_texture_dimension2_d;
                    let result_recorded_item_recorded_max_storage_buffer_binding_size = result_recorded_item.max_storage_buffer_binding_size;
                    let result_recorded_item_recorded = GpuAdapterInfoReplayRecord {
                        ...
                        max_texture_dimension2_d: result_recorded_item_recorded_max_texture_dimension2_d,
                        max_storage_buffer_binding_size: result_recorded_item_recorded_max_storage_buffer_binding_size,
                    };
            ...
            match payload.result {
                Ok(value) => {
                    ...
                    for value_native_item in value {
                        ...
                        let value_native_item_native_max_bind_groups = value_native_item.max_bind_groups;
                        let value_native_item_native_max_texture_dimension2_d = value_native_item.max_texture_dimension2_d;
                        let value_native_item_native_max_storage_buffer_binding_size = value_native_item.max_storage_buffer_binding_size;
                        let value_native_item_native = GpuAdapterInfo {
                            ...
                            max_texture_dimension2_d: value_native_item_native_max_texture_dimension2_d,
                            max_storage_buffer_binding_size: value_native_item_native_max_storage_buffer_binding_size,
                        };
...
fn destack_gpu_adapter_open_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_adapter_open(context, out, id) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_adapter_open(context, out, id) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_command_bind_compute_pipeline_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_bind_compute_pipeline(context, handle, pipeline) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_bind_compute_pipeline(context, handle, pipeline) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandBindComputePipelineReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_bind_render_pipeline_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_bind_render_pipeline(context, handle, pipeline) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_bind_render_pipeline(context, handle, pipeline) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandBindRenderPipelineReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_compute_pass_begin_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_compute_pass_begin(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_compute_pass_begin(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandComputePassBeginReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_compute_pass_end_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_compute_pass_end(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_compute_pass_end(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandComputePassEndReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_copy_buffer_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_copy_buffer(context, handle, src, srcoffset, dst, dstoffset, argument_bytes) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_copy_buffer(context, handle, src, srcoffset, dst, dstoffset, argument_bytes) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_copy_buffer_to_texture_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_copy_buffer_to_texture(context, handle, source, destination, size) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_copy_buffer_to_texture(context, handle, source, destination, size) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandCopyBufferToTextureReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_copy_texture_to_buffer_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_copy_texture_to_buffer(context, handle, source, destination, size) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_copy_texture_to_buffer(context, handle, source, destination, size) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandCopyTextureToBufferReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_copy_texture_to_texture_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_copy_texture_to_texture(context, handle, source, destination, size) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_copy_texture_to_texture(context, handle, source, destination, size) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandCopyTextureToTextureReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_dispatch_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_dispatch(context, handle, groupx, groupy, groupz) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_dispatch(context, handle, groupx, groupy, groupz) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_draw_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_draw(context, handle, vertexcount, instancecount, firstvertex, firstinstance) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_draw(context, handle, vertexcount, instancecount, firstvertex, firstinstance) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_draw_indexed_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_draw_indexed(context, handle, indexcount, instancecount, firstindex, basevertex, firstinstance) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_draw_indexed(context, handle, indexcount, instancecount, firstindex, basevertex, firstinstance) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_encoder_close_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_encoder_close(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_encoder_close(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_encoder_finish_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_encoder_finish(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_encoder_finish(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_encoder_open_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_encoder_open(context, out, device, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_encoder_open(context, out, device, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_command_queue_submit_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_queue_submit(context, queue, commandlists, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_queue_submit(context, queue, commandlists, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_queue_wait_idle_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_queue_wait_idle(context, queue, timeoutns) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_queue_wait_idle(context, queue, timeoutns) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_command_queue_write_buffer_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_queue_write_buffer(context, queue, buffer, bufferoffset, data, dataoffset, size) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_queue_write_buffer(context, queue, buffer, bufferoffset, data, dataoffset, size) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandQueueWriteBufferReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_queue_write_texture_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_queue_write_texture(context, queue, destination, data, layout, size) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_queue_write_texture(context, queue, destination, data, layout, size) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandQueueWriteTextureReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_render_pass_begin_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_render_pass_begin(context, handle, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_render_pass_begin(context, handle, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuCommandRenderPassBeginReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_command_render_pass_end_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_command_render_pass_end(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_command_render_pass_end(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_device_close_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_device_close(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_device_close(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_device_open_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_device_open(context, out, adapter, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_device_open(context, out, adapter, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_device_poll_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_device_poll(context, out, device, wait, timeoutns) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_device_poll(context, out, device, wait, timeoutns) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_device_queue_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_device_queue(context, out, device) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_device_queue(context, out, device) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_pipeline_compute_create_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_compute_pipeline_create(context, out, device, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_compute_pipeline_create(context, out, device, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuPipelineComputeCreateReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_pipeline_destroy_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_pipeline_destroy(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_pipeline_destroy(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_pipeline_render_create_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_render_pipeline_create(context, out, device, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_render_pipeline_create(context, out, device, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_pipeline_shader_create_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_shader_create(context, out, device, options, argument_bytes) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_shader_create(context, out, device, options, argument_bytes) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_pipeline_shader_destroy_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_shader_destroy(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_shader_destroy(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuPipelineShaderDestroyReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_resource_buffer_create_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_buffer_create(context, out, device, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_buffer_create(context, out, device, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_resource_buffer_destroy_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_buffer_destroy(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_buffer_destroy(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuResourceBufferDestroyReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_resource_buffer_read_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_buffer_read(context, out, handle, offset, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_buffer_read(context, out, handle, offset, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_resource_buffer_write_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_buffer_write(context, handle, offset, argument_bytes) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_buffer_write(context, handle, offset, argument_bytes) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_resource_memory_map_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_memory_map(context, out, handle, offset, length) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_memory_map(context, out, handle, offset, length) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
...
fn destack_gpu_resource_memory_unmap_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_memory_unmap(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_memory_unmap(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
...
fn destack_gpu_resource_sampler_create_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_sampler_create(context, out, device, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_sampler_create(context, out, device, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuResourceSamplerCreateReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_resource_sampler_destroy_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_sampler_destroy(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_sampler_destroy(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuResourceSamplerDestroyReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_resource_texture_create_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_texture_create(context, out, device, options) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_texture_create(context, out, device, options) },
        },
        |result| {
            if let Ok(()) = result {
                let result_value = unsafe {
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuResourceTextureCreateReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
fn destack_gpu_resource_texture_destroy_replay(
    ...
    context.replay().run_binding_with_payload_policy(
        ...
        || match world {
            RuntimeWorld::Host => unsafe { platform_native::destack_gpu_texture_destroy(context, handle) },
            RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_texture_destroy(context, handle) },
        },
        |result| {
            if let Ok(()) = result {
                let result_recorded = ();
            ...
            if let Err(error) = result {
                let payload = {
                    let result = Err(PlatformError::from(error.as_ref()));
                    GpuResourceTextureDestroyReplay {
                        result,
                    }
                };
                return Ok(Some(payload));
...
pub unsafe extern "C" fn destack_gpu_command_copy_buffer(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_copy_buffer_replay(context, world, handle, src, srcoffset, dst, dstoffset, argument_bytes)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_copy_buffer_to_texture(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_copy_buffer_to_texture_replay(context, world, handle, source, destination, size)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_copy_texture_to_buffer(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_copy_texture_to_buffer_replay(context, world, handle, source, destination, size)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_copy_texture_to_texture(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_copy_texture_to_texture_replay(context, world, handle, source, destination, size)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_draw(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_draw_replay(context, world, handle, vertexcount, instancecount, firstvertex, firstinstance)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_draw_indexed(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_draw_indexed_replay(context, world, handle, indexcount, instancecount, firstindex, basevertex, firstinstance)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_queue_write_buffer(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_queue_write_buffer_replay(context, world, queue, buffer, bufferoffset, data, dataoffset, size)
    })
}
...
pub unsafe extern "C" fn destack_gpu_command_queue_write_texture(
    ...
    native_call(|context| {
        ...
        destack_gpu_command_queue_write_texture_replay(context, world, queue, destination, data, layout, size)
    })
}
...
pub unsafe extern "C" fn destack_gpu_pipeline_shader_create(
    ...
    native_call(|context| {
        ...
        destack_gpu_pipeline_shader_create_replay(context, world, out, device, options, argument_bytes)
    })
}
...
pub unsafe extern "C" fn destack_gpu_present_queue_present(
    ...
    native_call(|context| {
...

        {
            ...
            match world {
                RuntimeWorld::Host => unsafe { platform_native::destack_gpu_queue_present(context, queue, window, options) },
                RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_queue_present(context, queue, window, options) },
            }
        }
    })
}
...
pub unsafe extern "C" fn destack_gpu_present_surface_configure(
    ...
    native_call(|context| {
...

        {
            ...
            match world {
                RuntimeWorld::Host => unsafe { platform_native::destack_gpu_surface_configure(context, device, window, options) },
                RuntimeWorld::Simulated => unsafe { platform_simulated_native::destack_gpu_surface_configure(context, device, window, options) },
            }
        }
    })
}
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_adapter_close(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_adapter_info(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
        ...
        |context, payload| {
            ...
            match payload.result {
                Ok(value) => {
                    ...
                    let vm_result_max_storage_buffer_binding_size = value.max_storage_buffer_binding_size;
            ...
            if let Ok(value) = result {
                ...
                for result_recorded_item_value in result_recorded_raw {
                    let result_recorded_item = {
                        ...
                        let result_recorded_item_backend_raw = decode_uint8(slots[2], "result_recorded_item_backend_raw", "backend")?;
                        let result_recorded_item_backend = match result_recorded_item_backend_raw { 1u8 => GpuBackend::Vulkan, 2u8 => GpuBackend::Metal, 3u8 => GpuBackend::D3D12, 4u8 => GpuBackend::OpenGL, 5u8 => GpuBackend::WebGpu, 255u8 => GpuBackend::Null , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("result_recorded_item_backend", "unknown GpuBackend value")).boxed()), };
                        let result_recorded_item_adapter_type_raw = decode_uint8(slots[3], "result_recorded_item_adapter_type_raw", "adapterType")?;
                        let result_recorded_item_adapter_type = match result_recorded_item_adapter_type_raw { 1u8 => GpuAdapterType::Integrated, 2u8 => GpuAdapterType::Discrete, 3u8 => GpuAdapterType::Cpu, 255u8 => GpuAdapterType::Unknown , _ => return Err(RuntimeError::from(PlatformError::invalid_argument_value("result_recorded_item_adapter_type", "unknown GpuAdapterType value")).boxed()), };
                        let result_recorded_item_vendor_id = decode_uint32(slots[4], "result_recorded_item_vendor_id", "vendorId")?;
                        let result_recorded_item_device_id = decode_uint32(slots[5], "result_recorded_item_device_id", "deviceId")?;
                        let result_recorded_item_feature_mask = decode_uint64(slots[6], "result_recorded_item_feature_mask", "featureMask")?;
                        let result_recorded_item_max_bind_groups = decode_uint32(slots[7], "result_recorded_item_max_bind_groups", "maxBindGroups")?;
                        let result_recorded_item_max_texture_dimension2_d = decode_uint32(slots[8], "result_recorded_item_max_texture_dimension2_d", "maxTextureDimension2D")?;
                        let result_recorded_item_max_storage_buffer_binding_size = decode_uint64(slots[9], "result_recorded_item_max_storage_buffer_binding_size", "maxStorageBufferBindingSize")?;
                        GpuAdapterInfoVm {
                            ...
                            max_texture_dimension2_d: result_recorded_item_max_texture_dimension2_d,
                            max_storage_buffer_binding_size: result_recorded_item_max_storage_buffer_binding_size,
                        }
                    };
            ...
            match payload.result {
                Ok(value) => {
                    ...
                    for vm_result_item in value.iter() {
                        ...
                        let vm_result_item_value_max_bind_groups = vm_result_item.max_bind_groups;
                        let vm_result_item_value_max_texture_dimension2_d = vm_result_item.max_texture_dimension2_d;
                        let vm_result_item_value_max_storage_buffer_binding_size = vm_result_item.max_storage_buffer_binding_size;
                        let vm_result_item_value = GpuAdapterInfoVm {
                            ...
                            max_texture_dimension2_d: vm_result_item_value_max_texture_dimension2_d,
                            max_storage_buffer_binding_size: vm_result_item_value_max_storage_buffer_binding_size,
                        };
                        let vm_result_item_value_encoded = { let field_0 = vm_result_item_value.id.value(); let field_1 = vm_result_item_value.name.value(); let field_2 = vm::Value::uint(vm_result_item_value.backend as u8 as u64, 8); let field_3 = vm::Value::uint(vm_result_item_value.adapter_type as u8 as u64, 8); let field_4 = vm::Value::uint(vm_result_item_value.vendor_id as u64, 32); let field_5 = vm::Value::uint(vm_result_item_value.device_id as u64, 32); let field_6 = vm::Value::uint(vm_result_item_value.feature_mask, 64); let field_7 = vm::Value::uint(vm_result_item_value.max_bind_groups as u64, 32); let field_8 = vm::Value::uint(vm_result_item_value.max_texture_dimension2_d as u64, 32); let field_9 = vm::Value::uint(vm_result_item_value.max_storage_buffer_binding_size, 64); context.allocate_aggregate(vec![field_0, field_1, field_2, field_3, field_4, field_5, field_6, field_7, field_8, field_9]) };
                        vm_result_values.push(vm_result_item_value_encoded);
                    }
                    let vm_result_data = context.allocate_raw_values(vm_result_values);
                    let vm_result: VmArray<GpuAdapterInfoVm> = VmArray { data: vm_result_data, len: value.len() as u32, capacity: value.len() as u32, _marker: std::marker::PhantomData };
                    Ok(vm_result)
                }
                Err(error) => Err(RuntimeError::from(error).boxed()),
            }
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_adapter_open(runtime, context, id),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_bind_compute_pipeline_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_bind_compute_pipeline(runtime, context, handle, pipeline),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_bind_compute_pipeline(runtime, context, handle, pipeline),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_bind_render_pipeline_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_bind_render_pipeline(runtime, context, handle, pipeline),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_bind_render_pipeline(runtime, context, handle, pipeline),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_compute_pass_begin_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_compute_pass_begin(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_compute_pass_begin(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_compute_pass_end_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_compute_pass_end(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_compute_pass_end(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_copy_buffer_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_copy_buffer(runtime, context, handle, src, srcoffset, dst, dstoffset, argument_bytes),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_copy_buffer(runtime, context, handle, src, srcoffset, dst, dstoffset, argument_bytes),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_copy_buffer_to_texture_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_copy_buffer_to_texture(runtime, context, handle, source, destination, size),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_copy_buffer_to_texture(runtime, context, handle, source, destination, size),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_copy_texture_to_buffer_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_copy_texture_to_buffer(runtime, context, handle, source, destination, size),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_copy_texture_to_buffer(runtime, context, handle, source, destination, size),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_copy_texture_to_texture_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_copy_texture_to_texture(runtime, context, handle, source, destination, size),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_copy_texture_to_texture(runtime, context, handle, source, destination, size),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_dispatch_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_dispatch(runtime, context, handle, groupx, groupy, groupz),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_dispatch(runtime, context, handle, groupx, groupy, groupz),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_draw_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_draw(runtime, context, handle, vertexcount, instancecount, firstvertex, firstinstance),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_draw(runtime, context, handle, vertexcount, instancecount, firstvertex, firstinstance),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_draw_indexed_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_draw_indexed(runtime, context, handle, indexcount, instancecount, firstindex, basevertex, firstinstance),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_draw_indexed(runtime, context, handle, indexcount, instancecount, firstindex, basevertex, firstinstance),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_encoder_close_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_encoder_close(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_encoder_close(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_encoder_finish_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_encoder_finish(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_encoder_finish(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_encoder_open_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_encoder_open(runtime, context, device, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_encoder_open(runtime, context, device, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_queue_submit_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_queue_submit(runtime, context, queue, commandlists, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_queue_submit(runtime, context, queue, commandlists, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_queue_wait_idle_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_queue_wait_idle(runtime, context, queue, timeoutns),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_queue_wait_idle(runtime, context, queue, timeoutns),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_queue_write_buffer_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_queue_write_buffer(runtime, context, queue, buffer, bufferoffset, data, dataoffset, size),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_queue_write_buffer(runtime, context, queue, buffer, bufferoffset, data, dataoffset, size),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_queue_write_texture_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_queue_write_texture(runtime, context, queue, destination, data, layout, size),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_queue_write_texture(runtime, context, queue, destination, data, layout, size),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_render_pass_begin_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_render_pass_begin(runtime, context, handle, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_render_pass_begin(runtime, context, handle, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_command_render_pass_end_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_command_render_pass_end(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_command_render_pass_end(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_device_close(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_device_open_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_device_open(runtime, context, adapter, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_device_open(runtime, context, adapter, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_device_poll_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_device_poll(runtime, context, device, wait, timeoutns),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_device_poll(runtime, context, device, wait, timeoutns),
            }
        },
        |context, result| {
            let _ = &context;
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_device_queue(runtime, context, device),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_pipeline_compute_create_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_compute_pipeline_create(runtime, context, device, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_compute_pipeline_create(runtime, context, device, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_pipeline_destroy_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_pipeline_destroy(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_pipeline_destroy(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_pipeline_render_create_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_render_pipeline_create(runtime, context, device, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_render_pipeline_create(runtime, context, device, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_pipeline_shader_create_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_shader_create(runtime, context, device, options, argument_bytes),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_shader_create(runtime, context, device, options, argument_bytes),
            }
        },
        |context, result| {
            let _ = &context;
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_shader_destroy(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_buffer_create_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_buffer_create(runtime, context, device, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_buffer_create(runtime, context, device, options),
            }
        },
        |context, result| {
            let _ = &context;
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                ...
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_buffer_destroy(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_buffer_read_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_buffer_read(runtime, context, handle, offset, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_buffer_read(runtime, context, handle, offset, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_buffer_write_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_buffer_write(runtime, context, handle, offset, argument_bytes),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_buffer_write(runtime, context, handle, offset, argument_bytes),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_memory_map_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_memory_map(runtime, context, handle, offset, length),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_memory_map(runtime, context, handle, offset, length),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_memory_unmap_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_memory_unmap(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_memory_unmap(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_sampler_create_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_sampler_create(runtime, context, device, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_sampler_create(runtime, context, device, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_sampler_destroy_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_sampler_destroy(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_sampler_destroy(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_texture_create_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_texture_create(runtime, context, device, options),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_texture_create(runtime, context, device, options),
            }
        },
        |context, result| {
            let _ = &context;
...
fn destack_gpu_resource_texture_destroy_vm_replay(
    ...
    let result = runtime.replay().run_binding_with_context_and_payload_policy(
        ...
        |context| {
            match world {
                RuntimeWorld::Host => platform_vm::destack_gpu_texture_destroy(runtime, context, handle),
                RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_texture_destroy(runtime, context, handle),
            }
        },
        |context, result| {
            let _ = &context;
...
pub fn register_gpu_vm_bindings(
    registry: &mut BindingRegistry,
    isolate: &mut Isolate,
) {
    {
        binding!(registry, isolate, GPU_ADAPTER_CLOSE, move |context, args| {
    ...
    {
        binding!(registry, isolate, GPU_COMMAND_BIND_COMPUTE_PIPELINE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, pipeline) = decode_destack_gpu_command_bind_compute_pipeline_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_BIND_COMPUTE_PIPELINE)?;
                destack_gpu_command_bind_compute_pipeline_vm_replay(runtime, context, world, handle, pipeline)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_BIND_RENDER_PIPELINE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, pipeline) = decode_destack_gpu_command_bind_render_pipeline_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_BIND_RENDER_PIPELINE)?;
                destack_gpu_command_bind_render_pipeline_vm_replay(runtime, context, world, handle, pipeline)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_COMPUTE_PASS_BEGIN, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle,) = decode_destack_gpu_command_compute_pass_begin_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_COMPUTE_PASS_BEGIN)?;
                destack_gpu_command_compute_pass_begin_vm_replay(runtime, context, world, handle)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_COMPUTE_PASS_END, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle,) = decode_destack_gpu_command_compute_pass_end_args(context, args)?;
    ...
    {
        binding!(registry, isolate, GPU_COMMAND_COPY_BUFFER, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, src, srcoffset, dst, dstoffset, argument_bytes) = decode_destack_gpu_command_copy_buffer_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_COPY_BUFFER)?;
                destack_gpu_command_copy_buffer_vm_replay(runtime, context, world, handle, src, srcoffset, dst, dstoffset, argument_bytes)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_COPY_BUFFER_TO_TEXTURE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, source, destination, size) = decode_destack_gpu_command_copy_buffer_to_texture_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_COPY_BUFFER_TO_TEXTURE)?;
                destack_gpu_command_copy_buffer_to_texture_vm_replay(runtime, context, world, handle, source, destination, size)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_COPY_TEXTURE_TO_BUFFER, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, source, destination, size) = decode_destack_gpu_command_copy_texture_to_buffer_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_COPY_TEXTURE_TO_BUFFER)?;
                destack_gpu_command_copy_texture_to_buffer_vm_replay(runtime, context, world, handle, source, destination, size)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_COPY_TEXTURE_TO_TEXTURE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, source, destination, size) = decode_destack_gpu_command_copy_texture_to_texture_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_COPY_TEXTURE_TO_TEXTURE)?;
                destack_gpu_command_copy_texture_to_texture_vm_replay(runtime, context, world, handle, source, destination, size)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_DISPATCH, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, groupx, groupy, groupz) = decode_destack_gpu_command_dispatch_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_DISPATCH)?;
                destack_gpu_command_dispatch_vm_replay(runtime, context, world, handle, groupx, groupy, groupz)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_DRAW, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, vertexcount, instancecount, firstvertex, firstinstance) = decode_destack_gpu_command_draw_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_DRAW)?;
                destack_gpu_command_draw_vm_replay(runtime, context, world, handle, vertexcount, instancecount, firstvertex, firstinstance)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_DRAW_INDEXED, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, indexcount, instancecount, firstindex, basevertex, firstinstance) = decode_destack_gpu_command_draw_indexed_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_DRAW_INDEXED)?;
                destack_gpu_command_draw_indexed_vm_replay(runtime, context, world, handle, indexcount, instancecount, firstindex, basevertex, firstinstance)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_ENCODER_CLOSE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_COMMAND_ENCODER_FINISH, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_COMMAND_ENCODER_OPEN, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options) = decode_destack_gpu_command_encoder_open_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_ENCODER_OPEN)?;
                destack_gpu_command_encoder_open_vm_replay(runtime, context, world, device, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_QUEUE_SUBMIT, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (queue, commandlists, options) = decode_destack_gpu_command_queue_submit_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_QUEUE_SUBMIT)?;
                destack_gpu_command_queue_submit_vm_replay(runtime, context, world, queue, commandlists, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_QUEUE_WAIT_IDLE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (queue, timeoutns) = decode_destack_gpu_command_queue_wait_idle_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_QUEUE_WAIT_IDLE)?;
                destack_gpu_command_queue_wait_idle_vm_replay(runtime, context, world, queue, timeoutns)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_QUEUE_WRITE_BUFFER, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (queue, buffer, bufferoffset, data, dataoffset, size) = decode_destack_gpu_command_queue_write_buffer_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_QUEUE_WRITE_BUFFER)?;
                destack_gpu_command_queue_write_buffer_vm_replay(runtime, context, world, queue, buffer, bufferoffset, data, dataoffset, size)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_QUEUE_WRITE_TEXTURE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (queue, destination, data, layout, size) = decode_destack_gpu_command_queue_write_texture_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_QUEUE_WRITE_TEXTURE)?;
                destack_gpu_command_queue_write_texture_vm_replay(runtime, context, world, queue, destination, data, layout, size)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_RENDER_PASS_BEGIN, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, options) = decode_destack_gpu_command_render_pass_begin_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_COMMAND_RENDER_PASS_BEGIN)?;
                destack_gpu_command_render_pass_begin_vm_replay(runtime, context, world, handle, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_COMMAND_RENDER_PASS_END, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_PIPELINE_COMPUTE_CREATE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options) = decode_destack_gpu_pipeline_compute_create_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_PIPELINE_COMPUTE_CREATE)?;
                destack_gpu_pipeline_compute_create_vm_replay(runtime, context, world, device, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_PIPELINE_DESTROY, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_PIPELINE_RENDER_CREATE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options) = decode_destack_gpu_pipeline_render_create_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_PIPELINE_RENDER_CREATE)?;
                destack_gpu_pipeline_render_create_vm_replay(runtime, context, world, device, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_PIPELINE_SHADER_CREATE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options, argument_bytes) = decode_destack_gpu_pipeline_shader_create_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_PIPELINE_SHADER_CREATE)?;
                destack_gpu_pipeline_shader_create_vm_replay(runtime, context, world, device, options, argument_bytes)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_PIPELINE_SHADER_DESTROY, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_PRESENT_QUEUE_PRESENT, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (queue, window, options) = decode_destack_gpu_present_queue_present_args(context, args)?;

                // execute binding
                let result = {
                        let world = runtime.check_and_resolve_world(GPU_PRESENT_QUEUE_PRESENT)?;
                        match world {
                            RuntimeWorld::Host => platform_vm::destack_gpu_queue_present(runtime, context, queue, window, options),
                            RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_queue_present(runtime, context, queue, window, options),
                        }
                    };
    ...
    {
        binding!(registry, isolate, GPU_PRESENT_SURFACE_CONFIGURE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, window, options) = decode_destack_gpu_present_surface_configure_args(context, args)?;

                // execute binding
                let result = {
                        let world = runtime.check_and_resolve_world(GPU_PRESENT_SURFACE_CONFIGURE)?;
                        match world {
                            RuntimeWorld::Host => platform_vm::destack_gpu_surface_configure(runtime, context, device, window, options),
                            RuntimeWorld::Simulated => platform_simulated_vm::destack_gpu_surface_configure(runtime, context, device, window, options),
                        }
                    };
    ...
    {
        binding!(registry, isolate, GPU_RESOURCE_BUFFER_CREATE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options) = decode_destack_gpu_resource_buffer_create_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_RESOURCE_BUFFER_CREATE)?;
                destack_gpu_resource_buffer_create_vm_replay(runtime, context, world, device, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_RESOURCE_BUFFER_DESTROY, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_RESOURCE_BUFFER_READ, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, offset, length) = decode_destack_gpu_resource_buffer_read_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_RESOURCE_BUFFER_READ)?;
                destack_gpu_resource_buffer_read_vm_replay(runtime, context, world, handle, offset, length)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_RESOURCE_BUFFER_WRITE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, offset, argument_bytes) = decode_destack_gpu_resource_buffer_write_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_RESOURCE_BUFFER_WRITE)?;
                destack_gpu_resource_buffer_write_vm_replay(runtime, context, world, handle, offset, argument_bytes)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_RESOURCE_MEMORY_MAP, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle, offset, length) = decode_destack_gpu_resource_memory_map_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_RESOURCE_MEMORY_MAP)?;
                destack_gpu_resource_memory_map_vm_replay(runtime, context, world, handle, offset, length)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_RESOURCE_MEMORY_UNMAP, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
    ...
    {
        binding!(registry, isolate, GPU_RESOURCE_SAMPLER_CREATE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options) = decode_destack_gpu_resource_sampler_create_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_RESOURCE_SAMPLER_CREATE)?;
                destack_gpu_resource_sampler_create_vm_replay(runtime, context, world, device, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_RESOURCE_SAMPLER_DESTROY, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle,) = decode_destack_gpu_resource_sampler_destroy_args(context, args)?;
    ...
    {
        binding!(registry, isolate, GPU_RESOURCE_TEXTURE_CREATE, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (device, options) = decode_destack_gpu_resource_texture_create_args(context, args)?;

                // execute binding
                let world = runtime.check_and_resolve_world(GPU_RESOURCE_TEXTURE_CREATE)?;
                destack_gpu_resource_texture_create_vm_replay(runtime, context, world, device, options)
            })
            .map_err(Into::into)
        });
    }
    {
        binding!(registry, isolate, GPU_RESOURCE_TEXTURE_DESTROY, move |context, args| {
            with_runtime_call_context(|runtime| {
                // decode args
                let (handle,) = decode_destack_gpu_resource_texture_destroy_args(context, args)?;
...
pub fn install_gpu_vm_bindings(
    registry: &mut BindingRegistry,
    isolate: &mut Isolate,
) {
