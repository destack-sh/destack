...
/// One committed-history query root over shared lineage.
#[derive(Debug, Clone, Copy)]
...
impl<'a> LineageView<'a> {
    ...
    pub fn view(self, moment: Moment) -> RuntimeResult<WorldView> {
        ...
        let image = self.world.image_at_committed_moment(moment)?;
    ...
    fn replay_trace_for_branch(self, branch_id: BranchId) -> RuntimeResult<Trace> {
        let trace_image = {
            ...
            lineage
                .trace_images
                .get(&head_revision.id)
                .cloned()
                .ok_or_else(|| RuntimeError::RevisionTraceImageMissing {
                    revision_id: head_revision.id.get(),
                })?
        };
...
impl World {
    ...
    /// Return one committed-history query root over shared lineage.
    pub fn lineage(&self) -> LineageView<'_> {
        LineageView { world: self }
    }
