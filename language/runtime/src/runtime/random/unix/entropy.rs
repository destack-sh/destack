...
pub(crate) fn host_fill_bytes(buffer: &mut [u8]) -> RuntimeResult<()> {
    // linux and android: use getrandom syscall directly
    ...
    // bsd and apple families: use getentropy with chunking
    #[cfg(any(
        target_os = "macos",
        target_os = "ios",
        target_os = "freebsd",
...
pub(crate) fn host_try_fill_bytes(buffer: &mut [u8]) -> RuntimeResult<()> {
    // linux and android: use nonblocking getrandom syscall
    #[cfg(any(target_os = "linux", target_os = "android"))]
    {
    ...
    // bsd and apple families: no nonblocking api, use getentropy directly
    #[cfg(any(
        target_os = "macos",
        target_os = "ios",
        target_os = "freebsd",
