...
use crate::runtime::host::core::{
    HostBridge, HostPlatform, HostWindowEvent, host_bridge_for_runtime,
};
...
/// Return the active macOS host bridge for this process.
fn macos_host_bridge(runtime_id: u64) -> RuntimeResult<Arc<HostBridge>> {
    host_bridge_for_runtime(runtime_id, HostPlatform::MacOS)
}
...
mod tests {
    ...
    fn test_map_application_lifecycle_to_running() {
        let state = host_lifecycle_state_for_application_lifecycle(
            MacosApplicationLifecycle::DidBecomeActive,
        );
    ...
    fn test_map_application_lifecycle_to_paused() {
        let state = host_lifecycle_state_for_application_lifecycle(
            MacosApplicationLifecycle::WillResignActive,
        );
    ...
    fn test_map_application_lifecycle_to_destroyed() {
        let state = host_lifecycle_state_for_application_lifecycle(
            MacosApplicationLifecycle::WillTerminate,
        );
