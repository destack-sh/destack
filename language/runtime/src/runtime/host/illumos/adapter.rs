...
/// illumos host adapter implementation.
#[derive(Debug)]
...
impl IllumosHostAdapter {
    /// Create one illumos host adapter.
    pub(crate) fn new() -> Self {
        ...
        let registration = register_host_bridge(HostPlatform::Illumos, &bridge);
        let services = build_illumos_services(bridge.state_store());
...
/// Build illumos host service trait surfaces from one shared state object.
fn build_illumos_services(state_store: &Arc<HostStateStore>) -> HostServices {
