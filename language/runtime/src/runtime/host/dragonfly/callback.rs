...
pub fn dragonfly_notify_window_available(runtime_id: u64) -> RuntimeResult<()> {
    unix_notify_window_available(runtime_id, HostPlatform::DragonFly)
}

/// Submit one dragonfly window-terminated callback.
pub fn dragonfly_notify_window_terminated(runtime_id: u64) -> RuntimeResult<()> {
    unix_notify_window_terminated(runtime_id, HostPlatform::DragonFly)
}
...
pub fn dragonfly_notify_window_resized(
    ...
    unix_notify_window_resized(runtime_id, HostPlatform::DragonFly, width_px, height_px)
}
...
pub fn dragonfly_notify_window_focus_changed(
    ...
    unix_notify_window_focus_changed(runtime_id, HostPlatform::DragonFly, is_focused)
}
