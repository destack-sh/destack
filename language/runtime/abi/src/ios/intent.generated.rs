// generated by generate-bindings: do not edit

use crate::diagnostic::RuntimeStatus;
use crate::host::abi::intent::HostIntentEvent;
use crate::host::apple::abi::ingress::destack_host_ios_notify_intent_event as runtime_notify_intent_event;
use crate::host::apple::abi::intent::destack_host_ios_intent_can_open_url as runtime_intent_can_open_url;
use crate::host::apple::abi::intent::destack_host_ios_intent_open_path as runtime_intent_open_path;
use crate::host::apple::abi::intent::destack_host_ios_intent_open_url as runtime_intent_open_url;
use crate::host::apple::abi::intent::destack_host_ios_intent_share_paths as runtime_intent_share_paths;
use crate::host::apple::abi::intent::destack_host_ios_intent_share_text as runtime_intent_share_text;
use crate::platform::abi::NativeStringRef;
use crate::platform::abi::NativeStringSlice;

/// Forward the `can_open_url` intent request through the ABI crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destack_host_ios_intent_can_open_url(
    session_handle: u64,
    url: NativeStringRef,
    is_supported: *mut bool,
) -> u32 {
    unsafe {
        runtime_intent_can_open_url(
            session_handle,
            url,
            is_supported,
        )
    }
}

/// Forward the `open_url` intent request through the ABI crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destack_host_ios_intent_open_url(
    session_handle: u64,
    url: NativeStringRef,
) -> u32 {
    unsafe {
        runtime_intent_open_url(
            session_handle,
            url,
        )
    }
}

/// Forward the `open_path` intent request through the ABI crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destack_host_ios_intent_open_path(
    session_handle: u64,
    path: NativeStringRef,
) -> u32 {
    unsafe {
        runtime_intent_open_path(
            session_handle,
            path,
        )
    }
}

/// Forward the `share_text` intent request through the ABI crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destack_host_ios_intent_share_text(
    session_handle: u64,
    text: NativeStringRef,
    mime_type: crate::host::abi::core::HostAbiOptional<NativeStringRef>,
) -> u32 {
    unsafe {
        runtime_intent_share_text(
            session_handle,
            text,
            mime_type,
        )
    }
}

/// Forward the `share_paths` intent request through the ABI crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destack_host_ios_intent_share_paths(
    session_handle: u64,
    paths: NativeStringSlice,
    mime_type: crate::host::abi::core::HostAbiOptional<NativeStringRef>,
) -> u32 {
    unsafe {
        runtime_intent_share_paths(
            session_handle,
            paths,
            mime_type,
        )
    }
}

/// Forward the `notify_intent_event` ingress through the ABI crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destack_host_ios_notify_intent_event(
    runtime_id: u64,
    event: HostIntentEvent,
) -> RuntimeStatus {
    unsafe { runtime_notify_intent_event(
        runtime_id,
        event,
    ) }
}
