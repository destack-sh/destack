...
static enum WhitespaceResult scan_whitespace_and_comments(
    TSLexer *lexer,
        ...
        if (lexer->lookahead == '/') {
            ...
            if (lexer->lookahead == '/') {
                ...
                *scanned_comment = true;
                        ...
                        if (lexer->lookahead == '/') {
                            ...
                            *scanned_comment = true;
                            ...
                            if (lexer->lookahead != '/' && !consume) {
                                return saw_block_newline ? WHITESPACE_ACCEPT : WHITESPACE_NO_NEWLINE;
                            }
                ...
                return WHITESPACE_REJECT;
            }
        } else {
            return WHITESPACE_ACCEPT;
        }
    }
}
...
static bool scan_automatic_semicolon(TSLexer *lexer, const bool *valid_symbols, bool *scanned_comment) {
    ...
    for (;;) {
        ...
        if (lexer->lookahead == '/') {
            ...
            if (result == WHITESPACE_REJECT) {
                return false;
            }

            if (result == WHITESPACE_ACCEPT && !valid_symbols[LOGICAL_OR] &&
    ...
    if (scan_whitespace_and_comments(lexer, scanned_comment, true) == WHITESPACE_REJECT) {
        return false;
    }
    ...
    switch (lexer->lookahead) {
        ...
        // insert a semicolon before decimal literals but not otherwise
        case '.':
            skip(lexer);
