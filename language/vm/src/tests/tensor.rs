...
fn run_tensor_slots<F>(mir_text: &str, function: &str, setup: F) -> Vec<Value>
where
    F: FnOnce(&mut TestIsolate) -> Vec<Value>,
{
    let mut isolate = create_isolate(mir_text);
    let arguments = setup(&mut isolate);
    let output = isolate
...
block0(v0: tensor<i32, [2, 2]>):
    ...
    assert_eq!(
        slots,
        vec![2, 4].into_iter().map(Value::int32).collect::<Vec<_>>()
    );
}
...
block0(v0: tensor<i32, [2, 2]>):
    ...
    assert_eq!(
        slots,
        vec![3, 7].into_iter().map(Value::int32).collect::<Vec<_>>()
    );
}
