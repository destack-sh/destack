...
pub struct Continuation {
    ...
    /// The stack arena bytes captured with this continuation.
...
impl Continuation {
    ...
    pub(crate) fn from_image(
        ...
        for frame_image in &image.frames {
            ...
            let base = super::interpreter::align_stack_bytes(stack.len(), Word::BYTE_LEN)
                .ok_or_else(|| RuntimeError::new(Error::StackOverflow))?;
            let end = base
                .checked_add(frame_image.bytes.len())
                .ok_or_else(|| RuntimeError::new(Error::StackOverflow))?;
            stack.resize(end, 0);
            stack[base..end].copy_from_slice(&frame_image.bytes);

            let frame_base = unsafe { stack.as_mut_ptr().add(base) };
...
fn restore_frame_image(
    ...
    let function_ptr = functions.get_ptr_for(function_id).ok_or_else(|| {
