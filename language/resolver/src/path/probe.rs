...
use crate::{EnforceExtension, Resolution, ResolveError, ResolveFrame, ResolveOrigin, Resolver};
...
impl Resolver {
    ...
    pub(crate) fn probe_file(
        ...
        // try extension alias
        if let Some(resolved) = self.rewrite_extension_alias(path, ctx)? {
            Ok(Some(resolved))
        }
        // if the path is a file, load it as its file extension format
            ...
            && let Some(resolved) = self.probe_alias_or_file(path, ctx)?
        {
            Ok(Some(resolved))
        }
        // try appending extensions (skip if path already has a known extension)
        else if !self.has_known_extension(path)
            && let Some(resolved) = self.probe_extensions(path, &self.options.extensions, ctx)?
        {
            Ok(Some(resolved))
        }
        // not found
    ...
    pub(crate) fn probe_directory(
        ...
        if let Some(resolved) = self.resolve_package_directory(path, ctx)? {
            return Ok(Some(resolved));
        }

        // try to load index file
        self.probe_directory_index(path, ctx)
    }
    ...
    pub(crate) fn probe_path(
        ...
        if !specifier.ends_with('/')
            && let Some(resolved) = self.probe_file(path, ctx)?
        {
            Ok(Some(resolved))
        }
        // try as directory
        else if self.is_directory(path, ctx)
            && let Some(resolved) = self.probe_directory(path, ctx)?
        {
            Ok(Some(resolved))
        }
        // not found
    ...
    pub(crate) fn probe_extensions(
        ...
        for extension in extensions {
            ...
            if let Some(resolved) = self.probe_alias_or_file(&extended_path, ctx)? {
                return Ok(Some(resolved));
            }
        }
        Ok(None)
    ...
    pub(crate) fn probe_directory_index(
        ...
        for main_file in &self.options.main_files {
            ...
            if self.options.enforce_extension == EnforceExtension::Disabled
                && let Some(resolved) = self.rewrite_path(&resolved_path, ctx)?
            ...
            if let Some(resolved) =
                self.probe_extensions(&resolved_path, &self.options.extensions, ctx)?
            {
                return Ok(Some(resolved));
            }
        }
    ...
    pub(crate) fn rewrite_path(
        ...
        if !self.options.alias.is_empty() {
            ...
            if let Some(resolved) = self.rewrite_primary_alias(
    ...
    pub(crate) fn probe_alias_or_file(
        ...
        if let Some(resolved) = self.rewrite_path(path, ctx)? {
            Ok(Some(resolved))
        }
        // try as direct file
