...
use super::store::ArtifactCache;

/// One retained exact artifact version.
#[derive(Debug)]
pub struct ArtifactPin {
    /// The shared artifact cache that owns this version.
...
impl ArtifactPin {
    ...
    pub(crate) fn new(store: Arc<ArtifactCache>, version: ArtifactVersion) -> Self {
        Self { store, version }
    }
...
pub struct ArtifactPinSet {
    /// The shared artifact cache that owns these pins.
    store: Arc<ArtifactCache>,
...
impl ArtifactPinSet {
    /// Build one empty pin set for one artifact cache.
    pub fn new(store: Arc<ArtifactCache>) -> Self {
        Self {
            store,
    ...
    /// Return the shared artifact cache.
