...
use destack_workspace::workspace::{Module, ModuleSource, Package, PackageKind};
use im::OrdMap;
...
impl Repository {
    ...
    fn package_root_for_path(
        ...
        for ancestor in directory.ancestors() {
            if !ancestor.starts_with(&self.workspace.root) {
                break;
            }
    ...
    fn seed_workspace_file(
        ...
        let file_type = FileType::from_path_or_unknown(path);
        let file_content = if file_type.is_binary() {
            let content =
                self.fs
                    .read(path)
            ...
            let content = self.fs.read_to_string(path).map_err(|error| {
                RepositoryError::ImportFileSystem {
    ...
    /// Validate one workspace logical path and return its file id.
    fn workspace_file_id_for_logical_path(
        &self,
        logical_path: &str,
        ...
        if logical_path.contains("://") {
            return Err(RepositoryError::InvalidEditPath {
                path: logical_path.to_string(),
                message: "non workspace source requires explicit registration".to_string(),
            });
        }

        Ok(self.intern_workspace_logical_file_id(logical_path))
    }
        ...
        for edit in change.edits {
            match edit {
                ...
                Edit::WriteFile {
                    ...
                    let file_id = self.workspace_file_id_for_logical_path(&logical_path)?;
                ...
                Edit::MoveFile { from, to } => {
                    ...
                    let from_file_id = self.workspace_file_id_for_logical_path(&from)?;
                    if !source.contains_key(&from_file_id) {
                        return Err(RepositoryError::MissingRenameSource { path: from });
                    }
                    let Some(from_file) = source.get(&from_file_id).cloned() else {
                        return Err(RepositoryError::MissingRenameSource { path: from });
                    };
                    let to_file_id = self.workspace_file_id_for_logical_path(&to)?;

                    source.remove(&from_file_id);
                    self.logical_path_by_file_id.remove(&from_file_id);
                    self.file_origin_by_file_id.remove(&from_file_id);
                    self.logical_path_by_file_id
                        .insert(to_file_id, Arc::<str>::from(to.clone()));
                    self.file_origin_by_file_id.insert(
                        to_file_id,
                        FileOrigin::Workspace {
                            path: self.workspace.root.join(&to),
                        },
                    );
