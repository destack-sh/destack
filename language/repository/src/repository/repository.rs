...
use destack_workspace::config::{Destack, FormatterOptions, LinterOptions, TsConfigOptions};
use destack_workspace::workspace::Workspace;
use parking_lot::RwLock;

use crate::repository::{
    Builtins, ContentId, ContentStore, FileOrigin, QueryIndex, RepositoryError,
    RepositoryImage, RepositoryImageHeader, RepositoryImageKey, RepositoryImageStore,
    RepositoryOptions, RepositorySnapshot,
};
...
pub struct Repository {
    ...
    /// The attached workspace descriptor.
...
impl Repository {
    /// Open one repository over one workspace with default options and disk cache.
    pub fn open(workspace: Workspace) -> Self {
        ...
        let root = repository.workspace.root.clone();
    ...
    /// Open one repository over one workspace and import the workspace root from one file system.
    pub fn open_from_fs(
        workspace: Workspace,
    ...
    pub fn new(
        ...
        let workspace_reference = Ref::for_workspace_root(&workspace.root);
    ...
    /// Return the attached workspace descriptor.
    ...
    pub fn cache_dir(&self) -> PathBuf {
        if let Some(cache_dir) = self.options.cache_dir_override.as_ref() {
            ...
            return self.workspace.root.join(cache_dir);
        }

        self.workspace.root.join(".destack")
    }
