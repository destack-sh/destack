...
use super::{
    ArtifactAttemptSnapshot, TraceSnapshot, TraceSpanKind, TraceTimelineOptions,
    ...
    fn render(&self, output: &mut String) {
        ...
        for row in &self.rows {
            let line = row
                .iter()
                .enumerate()
                .map(|(index, cell)| cell.render(widths[index], index == 0, self.use_color))
                .collect::<Vec<_>>()
                .join("  ");
