use crate::{
    ...
    Enum, EnumField, Expression, For, Function, If, Implement, Index, Let, Loop, Match, MatchCase,
    Module, NodeTree, NodeVisitor, Parameter, Pattern, PatternField, RangeLiteral, Return,
...
pub fn walk_block<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, block: &Block) {
    for statement_id in &block.statements {
        ...
        visitor.visit_statement(tree, statement);
    }
}
...
pub fn walk_statement<V: NodeVisitor + ?Sized>(
    ...
    match statement {
        Statement::Expression(node) => visitor.visit_expression(tree, tree.get(*node)),
        Statement::Module(node) => visitor.visit_module(tree, tree.get(*node)),
        Statement::Struct(node) => visitor.visit_struct(tree, tree.get(*node)),
        Statement::Enum(node) => visitor.visit_enum(tree, tree.get(*node)),
        Statement::Union(node) => visitor.visit_union(tree, tree.get(*node)),
        Statement::Trait(node) => visitor.visit_trait(tree, tree.get(*node)),
        Statement::Implement(node) => visitor.visit_implement(tree, tree.get(*node)),
        Statement::Function(node) => visitor.visit_function(tree, tree.get(*node)),
        Statement::With(node) => visitor.visit_with(tree, tree.get(*node)),
        Statement::Use(node) => visitor.visit_use(tree, tree.get(*node)),
    }
}
...
pub fn walk_expression<V: NodeVisitor + ?Sized>(
    ...
    match expression {
        Expression::Module(node) => visitor.visit_module(tree, tree.get(*node)),
        Expression::Struct(node) => visitor.visit_struct(tree, tree.get(*node)),
        Expression::Enum(node) => visitor.visit_enum(tree, tree.get(*node)),
        Expression::Union(node) => visitor.visit_union(tree, tree.get(*node)),
        Expression::Trait(node) => visitor.visit_trait(tree, tree.get(*node)),
        Expression::Implement(node) => visitor.visit_implement(tree, tree.get(*node)),
        Expression::Function(node) => visitor.visit_function(tree, tree.get(*node)),

        Expression::Let(node) => visitor.visit_let(tree, tree.get(*node)),
        Expression::Block(node) => visitor.visit_block(tree, tree.get(*node)),
        Expression::If(node) => visitor.visit_if(tree, tree.get(*node)),
        Expression::While(node) => visitor.visit_while(tree, tree.get(*node)),
        Expression::For(node) => visitor.visit_for(tree, tree.get(*node)),
        Expression::Loop(node) => visitor.visit_loop(tree, tree.get(*node)),
        Expression::Break(node) => visitor.visit_break(tree, tree.get(*node)),
        Expression::Continue(node) => visitor.visit_continue(tree, tree.get(*node)),
        Expression::Defer(node) => visitor.visit_defer(tree, tree.get(*node)),
        Expression::Return(node) => visitor.visit_return(tree, tree.get(*node)),
        Expression::Try(node) => visitor.visit_try(tree, tree.get(*node)),
        Expression::Match(node) => visitor.visit_match(tree, tree.get(*node)),

        Expression::Path(_) => {}
        Expression::ScalarLiteral(node) => visitor.visit_scalar_literal(tree, tree.get(*node)),
        Expression::RangeLiteral(node) => visitor.visit_range_literal(tree, tree.get(*node)),
        Expression::ArrayLiteral(node) => visitor.visit_array_literal(tree, tree.get(*node)),
        Expression::TupleLiteral(node) => visitor.visit_tuple_literal(tree, tree.get(*node)),
        Expression::StructLiteral(node) => visitor.visit_struct_literal(tree, tree.get(*node)),

        Expression::Unary { operator: _, right } => {
            visitor.visit_expression(tree, tree.get(*right));
        }
        Expression::Binary {
            left,
            operator: _,
            ...
            visitor.visit_expression(tree, tree.get(*left));
            visitor.visit_expression(tree, tree.get(*right));
        }
        Expression::Assign {
            left,
        ...
        Expression::Member { receiver, path: _ } => {
            visitor.visit_expression(tree, tree.get(*receiver));
        }
        Expression::Index(node) => visitor.visit_index(tree, tree.get(*node)),
        Expression::Unwrap(node) => visitor.visit_expression(tree, tree.get(*node)),
        Expression::Call(node) => visitor.visit_call(tree, tree.get(*node)),
        Expression::Cast(node) => visitor.visit_cast(tree, tree.get(*node)),
        Expression::Coalesce(node) => visitor.visit_coalesce(tree, tree.get(*node)),
...
pub fn walk_struct<V: NodeVisitor + ?Sized>(
    ...
    if let Some(super_types) = &struct_node.super_types {
        for type_id in super_types {
            ...
            visitor.visit_type(tree, type_node);
        }
    }
    ...
    if let Some(static_parameters) = &struct_node.static_parameters {
        for parameter_id in static_parameters {
            ...
            visitor.visit_parameter(tree, parameter);
        }
    }

    for field_id in &struct_node.fields {
        let field = tree.get(*field_id);
        visitor.visit_struct_field(tree, field);
    }
...
pub fn walk_enum<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, enum_node: &Enum) {
    if let Some(type_node) = &enum_node.r#type {
        ...
        visitor.visit_type(tree, type_ref);
    }
    ...
    for field_id in &enum_node.fields {
        let field = tree.get(*field_id);
        visitor.visit_enum_field(tree, field);
    }
...
pub fn walk_union<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, union_node: &Union) {
    ...
    for field_id in &union_node.fields {
        let field = tree.get(*field_id);
        visitor.visit_union_field(tree, field);
    }
...
pub fn walk_trait<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, trait_node: &Trait) {
    ...
    for with_id in &trait_node.withs {
        ...
        visitor.visit_with(tree, with_node);
    }
...
pub fn walk_function<V: NodeVisitor + ?Sized>(
    ...
    if let Some(body) = &function.body {
        ...
        visitor.visit_block(tree, block);
    }
}
...
pub fn walk_let<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, let_node: &Let) {
    ...
    visitor.visit_pattern(tree, pattern);
...
pub fn walk_tuple<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, tuple: &Tuple) {
    for element_id in &tuple.elements {
        ...
        visitor.visit_tuple_field(tree, element);
    }
}
...
pub fn walk_type<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, type_node: &Type) {
    match type_node {
        ...
        Type::Maybe(inner) => {
            ...
            visitor.visit_type(tree, inner_type);
        }
        Type::Not(inner) => {
            let inner_type = tree.get(*inner);
            visitor.visit_type(tree, inner_type);
        ...
        Type::Pointer {
            ...
            visitor.visit_type(tree, target_type);
        }
        Type::Virtual(inner) => {
            let inner_type = tree.get(*inner);
            visitor.visit_type(tree, inner_type);
        ...
        Type::Array { element, count } => {
            ...
            visitor.visit_type(tree, element_type);
            let count_expr = tree.get(*count);
            visitor.visit_expression(tree, count_expr);
        }
        Type::Slice { element } => {
            let element_type = tree.get(*element);
            visitor.visit_type(tree, element_type);
        ...
        Type::Tuple(tuple_id) => {
            ...
            visitor.visit_tuple(tree, tuple);
        }
        Type::Struct(struct_id) => {
            let struct_node = tree.get(*struct_id);
            visitor.visit_struct(tree, struct_node);
        }
        Type::Enum(enum_id) => {
            let enum_node = tree.get(*enum_id);
            visitor.visit_enum(tree, enum_node);
        }
        Type::Union(union_id) => {
            let union_node = tree.get(*union_id);
            visitor.visit_union(tree, union_node);
        }
        Type::Function(function_id) => {
            let function = tree.get(*function_id);
            visitor.visit_function(tree, function);
        }
    }
}
...
pub fn walk_with<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, with: &With) {
    for clause_id in &with.clauses {
        ...
        visitor.visit_with_clause(tree, clause);
    }
}
...
pub fn walk_with_clause<V: NodeVisitor + ?Sized>(
    ...
    match with_clause {
        ...
        WithClause::Assertion { target, assertion } => {
            ...
            visitor.visit_type(tree, assertion_type);
        }
    }
}
...
pub fn walk_use<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, use_node: &Use) {
    for clause_id in &use_node.clauses {
        let clause = tree.get(*clause_id);
        visitor.visit_use_clause(tree, clause);
    }
...
pub fn walk_use_clause<V: NodeVisitor + ?Sized>(
    ...
    visitor.visit_expression(tree, target);

    if let Some(items) = &use_clause.items {
        for item_id in items {
            let item = tree.get(*item_id);
            visitor.visit_use_item(tree, item);
        }
    }
}
...
pub fn walk_if<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, if_node: &If) {
    match if_node {
        If::If {
            ...
            visitor.visit_expression(tree, condition_expr);
            let then_block_node = tree.get(*then_block);
            visitor.visit_block(tree, then_block_node);
        }
        If::IfElse {
            condition,
            then_block,
            ...
            visitor.visit_block(tree, else_block_node);
        }
        If::IfElseIf {
            condition,
            then_block,
            ...
            visitor.visit_if(tree, else_if_node);
        }
    }
}
...
pub fn walk_for<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, for_node: &For) {
    ...
    visitor.visit_expression(tree, iterator);
...
pub fn walk_break<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, break_node: &Break) {
    if let Some(value) = &break_node.value {
        ...
        visitor.visit_expression(tree, value_expr);
    }
}
...
pub fn walk_match<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, match_node: &Match) {
    ...
    visitor.visit_expression(tree, value);

    for case_id in &match_node.cases {
        let case = tree.get(*case_id);
        visitor.visit_match_case(tree, case);
    }
}
...
pub fn walk_try<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, try_node: &Try) {
    match try_node {
        ...
        Try::BlockWithCatch {
            ...
            visitor.visit_match(tree, match_node);
        }
    }
}
...
pub fn walk_parameter<V: NodeVisitor + ?Sized>(
    ...
    if let Some(default) = &parameter.default {
        ...
        visitor.visit_expression(tree, default_expr);
    }
}
...
pub fn walk_range_literal<V: NodeVisitor + ?Sized>(
    ...
    if let Some(start) = &range_literal.start {
        ...
        visitor.visit_expression(tree, start_expr);
    }
    if let Some(end) = &range_literal.end {
        let end_expr = tree.get(*end);
        visitor.visit_expression(tree, end_expr);
    }
}
    ...
    match array_literal {
        ArrayLiteral::Fixed { elements } => {
            for element_id in elements {
                let element = tree.get(*element_id);
                visitor.visit_expression(tree, element);
            }
        }
    }
}
...
pub fn walk_index<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, index: &Index) {
    ...
    visitor.visit_expression(tree, receiver);
    let index_expr = tree.get(index.index);
    visitor.visit_expression(tree, index_expr);
}
...
pub fn walk_call<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, call: &Call) {
    ...
    if let Some(static_arguments) = &call.static_arguments {
        for arg_id in static_arguments {
            ...
            visitor.visit_argument(tree, arg);
        }
    }
...
pub fn walk_coalesce<V: NodeVisitor + ?Sized>(
    ...
    visitor.visit_expression(tree, default);
}
...
pub fn walk_pattern<V: NodeVisitor + ?Sized>(visitor: &mut V, tree: &NodeTree, pattern: &Pattern) {
    match pattern {
        ...
        Pattern::Pointer {
            ...
            visitor.visit_pattern(tree, target_pattern);
        }
        Pattern::Literal(literal_id) => {
            let literal = tree.get(*literal_id);
            visitor.visit_scalar_literal(tree, literal);
        }
        ...
        Pattern::Range {
            ...
            if let Some(start_pattern) = start {
                ...
                visitor.visit_pattern(tree, start_node);
            }
            if let Some(end_pattern) = end {
                let end_node = tree.get(*end_pattern);
                visitor.visit_pattern(tree, end_node);
            }
        }
        Pattern::Tuple { path: _, fields } => {
            for field_id in fields {
                let field = tree.get(*field_id);
                visitor.visit_pattern_field(tree, field);
            }
        }
        ...
        Pattern::Union { fields } => {
            for field_id in fields {
                let field = tree.get(*field_id);
                visitor.visit_pattern(tree, field);
            }
        }
    }
}
    ...
    match pattern_field {
        PatternField::Named { name: _, pattern } => {
            if let Some(pattern_id) = pattern {
                ...
                visitor.visit_pattern(tree, pattern_node);
            }
        }
...
pub fn walk_match_case<V: NodeVisitor + ?Sized>(
    ...
    match match_case {
        MatchCase::Expression {
            ...
            visitor.visit_expression(tree, body_expr);
            if let Some(guard_expr) = guard {
                let guard_node = tree.get(*guard_expr);
                visitor.visit_expression(tree, guard_node);
            }
        }
        MatchCase::Block {
            pattern,
            ...
            visitor.visit_block(tree, body_block);
            if let Some(guard_expr) = guard {
                let guard_node = tree.get(*guard_expr);
                visitor.visit_expression(tree, guard_node);
            }
...
/// Walk the Doc's children.
pub fn walk_doc<V: NodeVisitor + ?Sized>(_visitor: &mut V, _tree: &NodeTree, _doc: &Doc) {
    // Doc has no child nodes to visit (only StringId)
}

/// Walk the Comment's children.
pub fn walk_comment<V: NodeVisitor + ?Sized>(
    _visitor: &mut V,
    _tree: &NodeTree,
    _comment: &Comment,
) {
    // Comment has no child nodes to visit (only StringId)
}
