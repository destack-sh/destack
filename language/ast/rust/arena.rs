...
pub struct IdentifierId(NonZeroU32);
...
///
/// Stores each unique string once and returns IdentifierId handles for fast comparison.
...
pub struct StringPool {
    ...
    index: HashMap<u64, Vec<IdentifierId>>,
}
...
impl StringPool {
    ...
    /// Get the string associated with the given IdentifierId.
    #[inline]
    pub fn get(&self, id: IdentifierId) -> &str {
        &self.storage[id.as_usize()]
    }

    /// Intern a string, storing only one owned copy of bytes.
    ///
    /// Returns the same IdentifierId for identical strings.
    pub fn intern<S: AsRef<str>>(&mut self, some_str: S) -> IdentifierId {
        ...
        let id = IdentifierId::from_zero_based_index(next_index);
