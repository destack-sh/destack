...
struct TestIpcDaemon {
    ...
    /// Stable daemon endpoint metadata.
...
impl TestIpcDaemon {
    ...
    fn new(prefix: &str) -> Self {
        // build a root and daemon endpoint
    ...
    fn start_with_options(&mut self, options: DaemonServerOptions) {
        ...
        let server =
            DaemonServer::with_options(self.repository.clone(), self.endpoint.clone(), options)
    ...
    fn connect(&self) -> crate::daemon::DaemonConnection {
        ...
        wait_for_daemon(&self.endpoint, server_result_rx)
    }
...
fn test_daemon_ipc_restart_resubscribe() {
    ...
    let response = connection
        ...
        .send_request(DaemonRequest::OpenRoot(OpenRootRequest {
            workspace: daemon.root_path().to_path_buf(),
...
fn wait_for_daemon(
    ...
    while std::time::Instant::now() < deadline {
        ...
        if let Ok(Some(_)) = endpoint.read_metadata()
            && let Ok(connection) = connect_ipc_daemon(endpoint, options.clone(), None)
        {
            return connection;
        }
