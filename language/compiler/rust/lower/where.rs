    ...
    pub fn lower_where_clause(
        ...
        match where_clause {
            ast::WhereClause::Assertion { left, right } => {
                let left = self.intern_string(file_id, *left);
            ...
            ast::WhereClause::Guard { guard } => {
                let guard = self.lower_expression(file_id, ast, *guard);
                self.tree
                    .insert_from_ast(WhereClause::Guard { guard }, file_id, where_clause_id)
            }
        }
    }
}
