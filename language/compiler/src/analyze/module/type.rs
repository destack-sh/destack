...
impl Compiler {
    ...
    ) -> Result<Option<Lineage>, RequirementError> {
    ...
    > {
        ...
        let snapshot =
            self.require_artifact_dir_declared(context.revision(), symbol.module_id, profile)?;
        ...
        if matches!(
            ...
            let options = context.analyze_context_options_for_module(remote_module.id);
