    ...
    pub(crate) fn update_value_shape_with_field(
        ...
        let object_ty_id = if merged_shape.is_empty() {
            ...
            Some(types.insert_type_from_any(
                merged_shape.into_object_type(),
                source_id,
            ))
        };

        // always include the nominal type descriptor
        let nominal_reference_id = types.insert_type_from_any(
            Type::Reference {
                symbol,
                static_arguments: None,
            },
        ...
        let descriptor_ty_id = types.insert_type_from_any(
            Type::Value {
                value: nominal_reference_id,
            },
            source_id,
        ...
        let value_ty_id = if elements.len() == 1 {
            ...
            types.insert_type_from_any(Type::Intersection { elements }, source_id)
        };
        types.set_value_type(symbol, value_ty_id);
        value_ty_id
    }
