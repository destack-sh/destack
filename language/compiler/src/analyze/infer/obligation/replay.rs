...
impl Compiler {
    ...
    pub(crate) fn replay_post_solve_obligations(
        ...
        self.report_missing_member_obligation_errors_after_infer_convergence(
            module, profile, types, infer, options,
        )?;

        // report deferred relation diagnostics last
        self.report_type_relation_obligation_errors_after_infer_convergence(
            module, profile, symbols, types, infer, options,
        )?;
