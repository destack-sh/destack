...
impl Compiler {
    ...
    pub(super) fn import_instance_type_for_symbol(
        ...
        // avoid re-importing from the same module table
        ...
        self.with_module_tree_symbols_by_id_for_stage(
            ...
            |remote_module, remote_tree, remote_symbols| {
                ...
                // prefer writable access so remote unevaluated types can be materialized
                if let Some(mut remote_types) = remote_dir.types.try_write() {
                    ...
                    return Ok(Some(local_instance_id));
                }

                // when remote write lock is already held in this thread, avoid deadlock:
                // import from a read snapshot without forcing materialization
