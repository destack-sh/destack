...
const values: Array<float64> = identity<Array<float64>>([1, 2]);
...
/// @generic.instance id="identity<Array<float64>>" symbol=identity arguments=[Array<float64>]
"#,
    );
}
...
/// @type.node source=1 type=float64
/// @type.node source=2 type=float64
/// @generic.instance id=first<float64> symbol=first arguments=[float64]
"#,
    );
}
...
"#,
        r#"
/// @diagnostic.error code=EC200 message="type '\"x\"' is not assignable to type 'int32'"
...
const overridden: (1, "x") = pair<1, "x">(1, "x");
