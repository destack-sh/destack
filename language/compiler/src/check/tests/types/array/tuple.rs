...
/// @resolution.member source=tuple[0] receiver=readonly ["id", 42] kind=element index=0

const count = tuple[1];
/// @type.symbol symbol=count source=count type=42
/// @resolution.name source=tuple target=tuple
/// @resolution.member source=tuple[1] receiver=readonly ["id", 42] kind=element index=1
"#,
    );
}
