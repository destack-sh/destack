...
/// @resolution.subscript source=bytes[1..3] type=Slice<uint8> kind=call target="collections.fixed-array.index#2(parameters=(Range<float64>), arguments=(provided(1..3) as Range<float64>), return=memory.borrow.Borrowed<Slice<uint8>, \"static\", \"exclusive\">)"
/// @generic.instance source=bytes[1..3] id="FixedArray<uint8, 4>.<extension#5>.index#2"
/// @type.node source=1 type=1
/// @type.node source=1..3 type=Range<float64>
/// @generic.instance source=1..3 id=Range<float64>
/// @type.node source=3 type=3

/// @generic.instance id="FixedArray<uint8, 4>.<extension#5>.index#2" template=collections.fixed-array.index#2 arguments=(uint8, 4, Range<float64>, "exclusive")
/// @generic.instance id=Range<float64> template=range.range.Range arguments=(float64)
"#,
    );
}
