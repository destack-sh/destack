...
/// @resolution.construct source="Config({ debug: true })" parameters=(Managed<{ debug: boolean }>) return=Config kind=newtype target=Config
/// @type.node source="{ debug: true }" type=Managed<{ debug: boolean }>
