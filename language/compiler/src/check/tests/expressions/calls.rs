...
fn test_check_records_callable_value_resolution() {
    let session = TestSession::single(
        r#"
declare const transform: (int32) => string;
...
/// @resolution.call source="transform(1)" parameters=[int32] return=string kind=direct target=transform
...
/// @resolution.call source="parse(\"id\")" parameters=[string] return="string" kind=direct target=parse#1
...
/// @diagnostic.label line=5 column=6 source="parse(true);"
"#,
    );
}
