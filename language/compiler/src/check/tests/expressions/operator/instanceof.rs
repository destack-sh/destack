...
/// @resolution.name source=User target=User
    ...
    /// @resolution.name source=value target=value
    /// @resolution.member source=value.name receiver=User kind=field key=name
...
/// @resolution.name source=User target=User
    ...
    /// @resolution.name source=value target=value
    /// @resolution.member source=value.title receiver=Team kind=field key=title
...
declare const value: unknown;
/// @type.symbol symbol=value type=unknown
...
"#,
        r#"
/// @diagnostic.error code=EC200 message="right-hand side of instanceof must be a class"
/// @diagnostic.label line=8 column=29 source=Named
"#,
    );
}
...
"#,
        r#"
/// @diagnostic.error code=EC200 message="type 'string' can never be an instance of 'User'"
/// @diagnostic.label line=5 column=12 source=value
"#,
    );
}
