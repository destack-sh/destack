...
impl CheckState<'_> {
    /// Apply selected decorators in authored order.
    pub(in crate::check) fn apply_decorators(
        &mut self,
        selections: Vec<SelectedDecorator>,
    ) -> CompilerResult<()> {
        let mut representation_sources = FxIndexMap::default();

        // evaluate and apply selections in authored order
        for selection in selections {
            ...
            if let dir::DecoratorTarget::LanguageItem { item, .. } = resolution.target {
                match DiagnosticControlLevel::try_from(item) {
                    Ok(level) => {
                        ...
                        if let Some(control) = control {
                            ...
                            if let Some(forbidden) = forbidden {
                                self.report_forbidden_diagnostic_override(
                                    module, &control, &forbidden,
                                );
                            } else {
                    ...
                    Err(dir::LanguageItem::ReprDecorator) => self.apply_representation_decorator(
                        module,
                        &application,
                        &value,
