...
use crate::check::{
    ConditionBranch, ControlTargetForm, ExpectedType, FlowBranch, FlowPredicate, Obligation,
    PatternArm, PatternCoverage, PatternCoverageObligation, WalkState,
};
...
impl WalkState<'_, '_> {
    ...
    pub(in crate::check) fn walk_match_expression(
        ...
        let value_expectation = ExpectedType::Node(value.into_global_any(self.module));
        ...
        for (index, case) in cases.iter().enumerate() {
            ...
            match selector {
                dir::SwitchSelector::Case(selector) => {
                    ...
                    self.narrow_by_equality(
                        case.into_global_any(self.module),
                        value,
                        selector,
                        true,
                    );
    ...
    fn walk_match_arms(
        &mut self,
        arms: &[dir::LocalNodeId<dir::MatchArm>],
        value_path: &Option<dir::AccessPath>,
