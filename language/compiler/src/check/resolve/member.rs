...
impl CheckState<'_> {
    ...
    pub(in crate::check) fn member_type_candidates_matching(
        ...
        match term {
            ...
            TypeTerm::Shape(shape) => self
                .shape_member_candidate(&self.term(*shape).members, key, protocol)
                .map(|candidate| candidate.map(|candidate| vec![candidate])),
    ...
    /// Collect component extension candidates declared in one module.
    fn collect_component_extension_candidates(
        ...
        for symbol in symbols {
            self.collect_extension_candidate(module, symbol, key, seen, candidates)?;
        }

        Ok(())
    }

    /// Collect receiver-module extension candidates.
