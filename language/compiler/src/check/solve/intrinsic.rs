...
impl CheckComponentState<'_> {
    pub(super) fn reduce_memory_type(
        &mut self,
        module: ModuleId,
        ...
        let Some(term) = (match item {
            dir::LanguageItem::PayloadOf => self.memory_payload_type(module, target)?,
    ...
    pub(super) fn type_argument(arguments: &[ArgumentTerm], index: usize) -> Option<VariableId> {
    ...
    pub(super) fn static_argument(arguments: &[ArgumentTerm], index: usize) -> Option<VariableId> {
        match arguments.get(index) {
    ...
    fn memory_payload_type(
        ...
        let term = match target {
            ...
            TypeTerm::Literal(dir::Type::Form(form)) => {
                TypeTerm::Literal(self.module(module)?.get_type(form.value))
            }
            _ => target,
        };
    ...
    fn memory_base_type(
        ...
        let term = match target {
            ...
            TypeTerm::Literal(dir::Type::Form(form)) => {
                let value = self.type_id_variable(module, form.value)?;
                let Some(term) = self.memory_base_type(module, value)? else {
                    return Ok(None);
                };
    ...
    fn memory_with_base_type(
        ...
        let term = match target {
            ...
            TypeTerm::Literal(dir::Type::Form(form)) => {
                let value = self.type_id_variable(module, form.value)?;
                let form = self.form_term(module, form.form)?;
                let Some(payload) = self.memory_with_base_type(module, value, base)? else {
                    return Ok(None);
                };
                let payload = self.push_solved_type_variable(module, payload)?;
    ...
    fn memory_with_ownership_type(
        ...
        let term = match target {
            TypeTerm::Literal(dir::Type::Form(form)) => {
                let value = self.type_id_variable(module, form.value)?;
                let form = self.form_term(module, form.form)?;
                let target = self.push_solved_type_variable(
                    module,
                    TypeTerm::Form {
                        form,
                        payload: value,
    ...
    fn memory_with_place_type(
        ...
        let payload = match target {
            TypeTerm::Literal(dir::Type::Form(form)) => {
                ...
                let Some(term) = self.memory_with_place_type(module, target, place)? else {
                    return Ok(None);
                };
    ...
    fn memory_with_lifetime_type(
        ...
        let term = match target {
            TypeTerm::Literal(dir::Type::Form(form)) => {
                ...
                let Some(term) = self.memory_with_lifetime_type(module, target, lifetime)? else {
                    return Ok(None);
                };
    ...
    fn memory_with_access_type(
        ...
        let term = match target {
            TypeTerm::Literal(dir::Type::Form(form)) => {
                ...
                let Some(term) = self.memory_with_access_type(module, target, access)? else {
                    return Ok(None);
                };
    ...
    /// Return one check memory form from a committed DIR form.
    fn form_term(&mut self, module: ModuleId, form: dir::Form) -> CompilerResult<FormTerm> {
        let form = match form {
            dir::Form::Managed => FormTerm::Managed,
            dir::Form::Owned => FormTerm::Owned,
            dir::Form::Borrowed { lifetime, access } => FormTerm::Borrowed {
                lifetime: self.static_id_variable(module, lifetime)?,
                access: self.static_id_variable(module, access)?,
            },
            dir::Form::Raw => FormTerm::Raw,
            dir::Form::Placed { place } => FormTerm::Placed {
                place: self.static_id_variable(module, place)?,
            },
    ...
    pub(super) fn memory_static_value(
        &self,
        module: ModuleId,
        item: dir::LanguageItem,
        arguments: &[ArgumentTerm],
    ...
    fn memory_ownership_value(
        ...
        let value = match target {
            ...
            TypeTerm::Literal(ty) => self.memory_ownership_type(module, &ty)?,
            _ => None,
        };
    ...
    /// Return the outer ownership value from one DIR type.
    fn memory_ownership_type(
        &self,
        module: ModuleId,
        ty: &dir::Type,
    ) -> CompilerResult<Option<dir::StaticTerm>> {
        let value = match ty {
            dir::Type::Form(form) => match form.form {
                dir::Form::Managed => Some(self.ownership_static(module, "managed")?),
                dir::Form::Owned => Some(self.ownership_static(module, "owned")?),
                dir::Form::Borrowed { .. } => Some(self.ownership_static(module, "borrowed")?),
                dir::Form::Raw => Some(self.ownership_static(module, "raw")?),
                dir::Form::Placed { .. } | dir::Form::Readonly => {
                    let value = self.module(module)?.get_type(form.value);

                    self.memory_ownership_type(module, &value)?
                }
            },
            _ => None,
        };
    ...
    fn memory_place_value(
        ...
        let value = match target {
            ...
            TypeTerm::Literal(ty) => self.memory_place_type(module, &ty)?,
            _ => None,
        };
    ...
    /// Return the outer placement value from one DIR type.
    fn memory_place_type(
        &self,
        module: ModuleId,
        ty: &dir::Type,
    ) -> CompilerResult<Option<dir::StaticTerm>> {
        let value = match ty {
            dir::Type::Form(form) => match form.form {
                dir::Form::Placed { place } => Some(self.module(module)?.get_static(place)),
                dir::Form::Readonly => {
                    let value = self.module(module)?.get_type(form.value);

                    self.memory_place_type(module, &value)?
                }
                _ => None,
            },
            _ => None,
    ...
    fn memory_lifetime_value(
        ...
        let value = match target {
            ...
            TypeTerm::Literal(ty) => self.memory_lifetime_type(module, &ty)?,
            _ => None,
        };
    ...
    /// Return the borrow lifetime value from one DIR type.
    fn memory_lifetime_type(
        &self,
        module: ModuleId,
        ty: &dir::Type,
    ) -> CompilerResult<Option<dir::StaticTerm>> {
        let value = match ty {
            dir::Type::Form(form) => match form.form {
                dir::Form::Borrowed { lifetime, .. } => {
                    Some(self.module(module)?.get_static(lifetime))
                }
                dir::Form::Placed { .. } | dir::Form::Readonly => {
                    let value = self.module(module)?.get_type(form.value);

                    self.memory_lifetime_type(module, &value)?
                }
                _ => None,
            },
            _ => None,
    ...
    fn memory_access_value(
        ...
        let value = match target {
            ...
            TypeTerm::Literal(ty) => self.memory_access_type(module, &ty)?,
            _ => None,
        };
    ...
    /// Return the borrow access value from one DIR type.
    fn memory_access_type(
        &self,
        module: ModuleId,
        ty: &dir::Type,
    ) -> CompilerResult<Option<dir::StaticTerm>> {
        let value = match ty {
            dir::Type::Form(form) => match form.form {
                dir::Form::Borrowed { access, .. } => Some(self.module(module)?.get_static(access)),
                dir::Form::Placed { .. } | dir::Form::Readonly => {
                    let value = self.module(module)?.get_type(form.value);

                    self.memory_access_type(module, &value)?
                }
                _ => None,
            },
            _ => None,
    ...
    /// Create a solved type variable from one stored type id.
    pub(super) fn type_id_variable(
        &mut self,
        module: ModuleId,
        ty: dir::LocalTypeId,
    ) -> CompilerResult<VariableId> {
        let term = TypeTerm::Literal(self.module(module)?.get_type(ty));

        self.push_solved_type_variable(module, term)
    }

    /// Create a solved static variable from one stored static id.
    fn static_id_variable(
        &mut self,
        module: ModuleId,
        value: dir::LocalStaticId,
    ) -> CompilerResult<VariableId> {
        let term = StaticTerm::Literal(self.module(module)?.get_static(value));

        self.push_solved_static_term_variable(module, term)
    }
}
