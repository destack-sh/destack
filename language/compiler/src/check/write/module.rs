...
use crate::check::{
    Answer, CheckState, ConstraintId, ConstraintRole, ConstraintState, Decision, Origin, Relation,
};
...
impl CheckState<'_> {
    ...
    pub(in crate::check) fn write_module(&mut self, module: ModuleId) -> CompilerResult<()> {
        ...
        let patches = self.variable_type_patches(module)?;
        ...
        for (local, patched) in patches {
            state.types.update_type(local, patched);
        }

        // write alias definition values as evaluated answers
    ...
    fn constraint_coercion(
        ...
        let (relation, role, left, right, origin) = {
        ...
        if !matches!(
            role,
            ConstraintRole::Value | ConstraintRole::Argument | ConstraintRole::Output
        ) {
            return Ok(None);
        }
        ...
        let Some(coercion) = self.implicit_coercion(source, target)? else {
            return Ok(None);
        };
    ...
    fn implicit_coercion(
        ...
        if !self.requires_implicit_coercion(source, target)? {
            return Ok(None);
        }
    ...
    /// Resolve one written type to its evaluated canonical form.
    ...
    fn resolved_symbol_types(
        ...
        for (symbol, ty) in symbol_types {
            // write alias values as evaluated answers, every other
    ...
    ///
    /// Alias values write their evaluated answer like alias symbol types.
