...
impl<'a> JsLinker<'a> {
    ...
    pub(crate) fn build_chunked_js_output_graph(
        &self,
        ...
        self.finalize_js_output_graph(&mut output_graph, module_set, dynamic_entry_modules)?;
    ...
    fn build_preserve_js_output_graph(
        &self,
        module_set: &ModuleSet,
        dynamic_entry_modules: &IndexSet<ModuleId>,
    ) -> LinkResult<OutputGraph> {
    ...
    fn preserve_output(
        ...
        let external_imports = self.retained_static_js_imports(
            module_id,
            self.target,
            self.target_id,
            self.package_id,
        )?;
        let external_dynamic_imports = self.retained_dynamic_js_imports(
            module_id,
            self.target,
            self.target_id,
            self.package_id,
        ...
        let static_output_dependencies =
            self.map_js_output_dependencies(static_dependency_modules, output_ids_by_module);
        let dynamic_output_dependencies =
            self.map_js_output_dependencies(dynamic_dependency_modules, output_ids_by_module);
    ...
    fn finalize_js_output_graph(
        ...
        for output_index in 0..output_graph.outputs.len() {
            ...
            for module_id in output_graph.outputs[output_index].modules() {
                ...
                let static_imports = self.retained_static_js_imports(
                    *module_id,
                    self.target,
                    self.target_id,
                    self.package_id,
                )?;
                let dynamic_imports = self.retained_dynamic_js_imports(
                    *module_id,
                    self.target,
                    self.target_id,
                    self.package_id,
