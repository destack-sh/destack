...
impl OutputLayout {
    ...
    pub(crate) fn render_entry_file_name(
        ...
        output_layout.render_output_file_name_with_values(
            ...
            OutputFileNameValues {
                ...
                name,
                hash,
                format: Some(Self::script_output_format_name(target)),
                extension: "js",
            },
        )
    }
    ...
    /// Return the naming token for the configured script output format.
...
impl<'a> ScriptLinker<'a> {
    ...
    pub(crate) fn build_chunked_output_layout(
        ...
        for output in output_graph.outputs() {
            ...
            let output_hash = self.script_output_hash(output)?;
            ...
            if !used_output_paths.insert(output_location.path().to_path_buf()) {
                return Err(LinkError::InvalidTarget {
                    ...
                    message: format!(
                        "multiple script outputs resolve to the same emitted path '{}'",
    ...
    fn build_preserve_output_layout(&self, output_graph: &OutputGraph) -> LinkResult<OutputLayout> {
        ...
        for output in output_graph.outputs() {
            ...
            let file_type = ScriptLinker::script_output_file_type(self)?;
    ...
    fn automatic_script_output_name(&self, output: &super::Output) -> LinkResult<String> {
        ...
        let module_id = output
            .facade_module()
            .or_else(|| output.modules().first().copied())
            .unwrap_or_else(|| unreachable!("script output should contain at least one module"));

        self.base_script_output_name(module_id, self.package_dir)
    }
