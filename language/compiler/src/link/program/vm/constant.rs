...
impl<'a> BlockLowerer<'a> {
    ...
    fn null_cell(&self, value_type: mir::LocalNodeId<mir::Type>) -> Cell {
        let reference = self.function.operand_lowerer().reference_meta(value_type);
    ...
    fn constant_bytes(&self, value: &mir::Constant, byte_len: usize) -> LinkResult<Box<[u8]>> {
        ...
        match value {
            ...
            _ => {
                return Err(self.type_mismatch("frame-backed constant", format!("{value:?}")));
            }
        }
