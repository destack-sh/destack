use crate::sema::{CheckState, FlowBranch, FlowCheckpoint};

impl CheckState<'_> {
    /// Mark the current flow position for branch rollback.
    pub(in crate::sema) fn fork_flow(&self) -> FlowCheckpoint {
        self.flow.fork()
    }

    /// Collect the flow changes since one checkpoint.
    pub(in crate::sema) fn collect_flow_branch(&self, checkpoint: FlowCheckpoint) -> FlowBranch {
        self.flow.branch(checkpoint)
    }

    /// Restore current flow state to one checkpoint.
    pub(in crate::sema) fn restore_flow(&mut self, checkpoint: FlowCheckpoint) {
        self.flow.restore(checkpoint);
    }

    /// Restore one completed branch.
    pub(in crate::sema) fn restore_flow_branch(
        &mut self,
    ...
    pub(in crate::sema) fn merge_flow_branches(
        &mut self,
        checkpoint: FlowCheckpoint,
        left: &FlowBranch,
        right: &FlowBranch,
    ...
    pub(in crate::sema) fn merge_flow_branches_from(
        &mut self,
        checkpoint: FlowCheckpoint,
        branches: &[FlowBranch],
    ) {
