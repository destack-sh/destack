...
impl CheckState<'_> {
    ...
    fn collect_symbol_dependents(
        ...
        // collect over the evaluated spelling lowering reads
        ...
        for ty in self.declared_types(symbol)? {
            ...
            self.collect_dependents(origin, receiver_is_open, ty, &mut dependents, collecting)?;
        }
    ...
    fn is_dependent(&mut self, origin: Origin, id: dir::GlobalTypeId) -> CompilerResult<bool> {
        let kind = self.ty(id)?;
        Ok(match &kind {
            // an unqualified projection stays open for lowering to close at an instance
    ...
    fn collect_dependents(
        ...
        while let Some(id) = pending.pop() {
            ...
            let kind = self.ty(id)?;
            if self.is_dependent(origin, id)? {
