...
pub(in crate::sema) struct ConstraintId(u32);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub(in crate::sema) struct Constraint {
...
impl Constraint {
    ...
    pub(in crate::sema) fn relation(&self) -> Relation {
        self.relation
    }

    /// Return why this constraint exists.
    pub(in crate::sema) fn cause(&self) -> CauseId {
        self.cause
    }
}
...
pub(in crate::sema) struct ConstraintTable {
...
impl ConstraintTable {
    ...
    pub(in crate::sema) fn lookup(&self, constraint: &Constraint) -> Option<ConstraintId> {
        self.interned.get(constraint).copied()
    }

    /// Append one constraint at the next id.
    pub(in crate::sema) fn insert(&mut self, id: ConstraintId, constraint: Constraint) {
    ...
    pub(in crate::sema) fn get(&self, id: ConstraintId) -> CompilerResult<&Constraint> {
        self.constraints
            .get(id.index())
    ...
    pub(in crate::sema) fn iter(&self) -> impl Iterator<Item = (ConstraintId, &Constraint)> {
        self.constraints
            .iter()
            .enumerate()
    ...
    pub(in crate::sema) fn failures_from(
        &self,
        start: usize,
    ...
    pub(in crate::sema) fn is_complete(&self, id: ConstraintId) -> bool {
...
pub(in crate::sema) struct ConstraintResult {
...
impl CheckOutcome {
    ...
    pub(in crate::sema) fn is_holds(self) -> bool {
