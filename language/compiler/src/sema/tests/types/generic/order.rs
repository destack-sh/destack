...
/// @resolution.name source=Box target=Box
    ...
    /// @resolution.call source="box.swap(\"x\")" parameters=("x") arguments=(provided("x") as "x") return="x" kind=symbol target=swap receiver=Box<int32> adjustments=(borrow(&'frame readonly Box<int32>)) instance="Box<int32>.<extension#1>.swap<\"x\">"
    /// @resolution.place source=box placement="local" lifetime="frame" access="exclusive"
    /// @resolution.access source=box root=use.box
    /// @generic.instantiation id="swap<int32, \"x\">" template=swap arguments=(int32, "x")
    /// @generic.instantiation id=swap<int32> template=swap arguments=(int32)
    /// @generic.instance id="swap<int32, \"x\">" template=swap arguments=(int32, "x")
