...
/// @type.symbol symbol=mesh type=<T#2, ...Axes#2: Axis[], mesh.'a, mesh.P3: Place>(&mesh.'a readonly Grid<T#2, Sharding<Axes#2>>) => int32
...
/// @definition.method symbol=mesh#1 slot=mesh role=getter type=<mesh#1.'a, mesh#1.P1: Place>(this: &mesh#1.'a readonly this) => int32
...
/// @resolution.name source=Axes target=Axes
    ...
    /// @type.symbol symbol=mesh#1 type=<mesh#1.'a, mesh#1.P1: Place>(this: &mesh#1.'a readonly this) => int32
        ...
        /// @resolution.call source="mesh<T, ...Axes>(this)" parameters=(&mesh#1.'a readonly Grid<T#3, Sharding<Axes#3>>) arguments=(provided(this) as &mesh#1.'a readonly Grid<T#3, Sharding<Axes#3>>) return=int32 kind=symbol target=mesh instance="mesh<T#3, Axes#3, mesh#1.P1>"
...
/// @type.symbol symbol=mesh type=<T#1, ...Xs#1: sharding.Axis[], mesh.'a, mesh.P3: Place>(&mesh.'a readonly sharding.Grid<T#1, sharding.Wrap<Xs#1>>) => int32
