...
/// @resolution.access source=values root=values
/// @resolution.access source=values[0] root=values keys=[0]
/// @resolution.subscript source=values[0] type=1 | 2 kind=call target="index#1(parameters=(isize), arguments=(provided(0) as isize), return=WithAccess<Borrowed<1 | 2, \"managed\" & \"local\", \"mutable\">, \"exclusive\">)"
/// @generic.instantiation id="index#1<1 | 2, \"exclusive\">" template=index#1 arguments=(1 | 2, "exclusive")
/// @generic.instance id="Cast.truncate<isize, usize>" template=Cast.truncate arguments=(isize, usize)
/// @generic.instance id="assumeInitReference<1 | 2, \"exclusive\">" template=assumeInitReference arguments=(1 | 2, "exclusive") evaluated=(<assumeInitReference.T, const assumeInitReference.A: Access = "mutable", assumeInitReference.'a>(WithAccess<&assumeInitReference.'a MaybeUninit<assumeInitReference.T>, assumeInitReference.A>) => WithAccess<&assumeInitReference.'a assumeInitReference.T, assumeInitReference.A> => <assumeInitReference.T, const assumeInitReference.A: Access = "mutable", assumeInitReference.'a>(&assumeInitReference.'a exclusive MaybeUninit<1 | 2>) => &assumeInitReference.'a exclusive 1 | 2)
...
/// @generic.instance id="index#1<1 | 2, \"exclusive\">" template=index#1 arguments=(1 | 2, "exclusive") evaluated=(<const index.A: Access = "readonly", index#1.'a>(this: WithAccess<&index#1.'a T#6[], index.A>, isize) => WithAccess<&index#1.'a T#6, index.A> => <const index.A: Access = "readonly", index#1.'a>(this: &index#1.'a exclusive 1 | 2[], isize) => &index#1.'a exclusive 1 | 2, WithAccess<&index#1.'a T#6, index.A> => &index#1.'a exclusive 1 | 2, WithAccess<&index#1.'a T#6[], index.A> => &index#1.'a exclusive 1 | 2[], (WithAccess<&index#1.'a MaybeUninit<T#6>, index.A>) => WithAccess<&index#1.'a T#6, index.A> => (&index#1.'a exclusive MaybeUninit<1 | 2>) => &index#1.'a exclusive 1 | 2, WithAccess<&index#1.'a MaybeUninit<T#6>, index.A> => &index#1.'a exclusive MaybeUninit<1 | 2>, WithAccess<&index#1.'a T#6, index.A> => &index#1.'a exclusive 1 | 2, (WithAccess<&index#1.'a MaybeUninit<T#6>, index.A>) => WithAccess<&index#1.'a T#6, index.A> => (&index#1.'a exclusive MaybeUninit<1 | 2>) => &index#1.'a exclusive 1 | 2, WithAccess<&index#1.'a MaybeUninit<T#6>, index.A> => &index#1.'a exclusive MaybeUninit<1 | 2>, (WithAccess<&index#1.'a T#6[], index.A>, usize) => WithAccess<&index#1.'a MaybeUninit<T#6>, index.A> => (&index#1.'a exclusive 1 | 2[], usize) => &index#1.'a exclusive MaybeUninit<1 | 2>, WithAccess<&index#1.'a T#6[], index.A> => &index#1.'a exclusive 1 | 2[])
