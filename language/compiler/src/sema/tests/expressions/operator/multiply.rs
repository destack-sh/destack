...
/// @resolution.name source=force target=force
/// @resolution.operator source="force * 2.0" type=Force operator="*" kind=call parameters=(float64) arguments=(provided(2.0) as float64) return=Force kind=symbol target=multiply receiver=Force adjustments=(borrow(&'static readonly Force))
...
/// @resolution.operator source="width * height" type=float64 operator="*" kind=call parameters=(Meters) arguments=(provided(height) as Meters) return=float64 kind=symbol target=multiply receiver=Meters adjustments=(borrow(&'static readonly Meters))
/// @resolution.place source=width placement="local" lifetime="static" access="readonly"
/// @resolution.access source=width root=width
/// @resolution.name source=height target=height
/// @resolution.place source=height placement="local" lifetime="static" access="readonly"
