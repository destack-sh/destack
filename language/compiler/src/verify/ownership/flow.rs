use destack_mir::{self as mir, Place, PlaceMap};
...
impl FlowState {
    ...
    pub(super) fn merge_predecessors(predecessors: &[FlowState]) -> Self {
        ...
        let mut places = predecessors[0].places.clone();
        ...
        // retain only address derivations shared by every predecessor
        for predecessor in &predecessors[1..] {
            places.intersect(&predecessor.places);
        }
    ...
    pub(super) fn place_for_value(&self, value: mir::Value) -> Place {
        self.places.resolve(value)
    }
