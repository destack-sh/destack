...
fn test_reuses_language_environment_across_fresh_scenario_sessions() {
    ...
    let loaded = fresh
        .compiler()
        .load_language_environment_image(fresh_profile_id)
...
"#,
    ...
    let loaded = fresh
        .compiler()
        .load_module_graph_image(fresh_profile_id)
    ...
    assert_eq!(loaded.module_versions, expected.module_versions);
}
...
fn test_language_environment_image_tracks_builtin_source_content() {
    ...
    let builtin_file_id = run.program().modules.get(builtin_module_id).file_id;
    append_file_text(
        run.program().as_ref(),
        builtin_file_id,
...
"#,
    ...
    assert!(
        run.compiler()
            .load_module_graph_image(profile_id)
            .unwrap_or_else(|error| panic!("failed to load module graph image: {error}"))
            .is_some()
    );
...
fn test_library_environment_image_tracks_library_source_content() {
    ...
    let library_file_id = run.program().modules.get(library_module_id).file_id;
    append_file_text(
        run.program().as_ref(),
        library_file_id,
        "\n// changed for cache invalidation\n",
