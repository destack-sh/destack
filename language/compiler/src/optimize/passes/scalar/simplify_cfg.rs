...
/// Thread edges through empty or condition only blocks using edge specific state.
...
/// Resolve a single edge to a threaded target using edge specific state.
fn resolve_edge_target(
    ...
    let block = tree.get(target_block).clone();
...
fn assume_truth_value(block: &mir::Block, tree: &mir::Tree, condition: mir::Value) -> Option<bool> {
...
fn fold_switch(
    ...
    for case in cases {
        let case_value = if *is_signed {
            ...
            let Ok(case_value) = u128::try_from(case.value) else {
                continue;
            };
    ...
    let filtered_cases = tree.add_switch_cases(&filtered_cases);
...
fn tail_duplicate_blocks(
    ...
    for block_id in block_ids {
        ...
        let candidates =
            select_tail_dup_predecessors(block_id, jump_preds, execution_counts.edges());
