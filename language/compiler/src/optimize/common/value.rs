...
pub struct ValueTypeMap {
    ...
    values: Vec<mir::LocalNodeId<mir::Type>>,
    /// Local types keyed by local id.
    locals: HashMap<mir::LocalNodeId<mir::Local>, mir::LocalNodeId<mir::Type>>,
}
...
impl ValueTypeMap {
    ...
    pub fn new(function: &mir::Function, tree: &mir::NodeTree) -> Self {
        ...
        for &local_id in &function.locals {
            ...
            locals.insert(local_id, local.ty);
        }
    ...
    pub fn value_type(&self, value: mir::Value) -> mir::LocalNodeId<mir::Type> {
        match self.values.get(value.0 as usize).copied() {
    ...
    /// Return the type of a value.
    pub fn require_value_type(&self, value: mir::Value) -> mir::LocalNodeId<mir::Type> {
    ...
    pub fn values(&self) -> &[mir::LocalNodeId<mir::Type>] {
        &self.values
    }
}
