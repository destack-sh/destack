...
use super::{FunctionContext, LocalBinding};
...
impl FunctionContext<'_> {
    ...
    pub(crate) fn start_constructor(
        &mut self,
        ...
        let instance_mir_type = self.builder.tree().get(instance_type).clone();
        let this_value = match instance_mir_type {
            mir::Type::Reference { kind, pointee, .. } => match kind {
                mir::ReferenceKind::Managed => {
                    let pointer = self.builder.managed_alloc(pointee);
                    let zero = self.zero_value_for_type(pointee, node)?;
                    self.builder.store(pointer, zero);
                    pointer
                }
                _ => {
        ...
        let this_variable = self.builder.create_variable(instance_type);
        self.builder.define_variable(this_variable, this_value);
        self.this_binding = Some(LocalBinding {
            variable: this_variable,
            ty: instance_type,
        });

        // track constructor initialization state
        self.constructor_state = Some(ConstructorState {
            layout,
    ...
    pub(crate) fn mark_constructor_field_initialized(&mut self, field_index: u32) {
        ...
        let Some(state) = self.constructor_state.as_mut() else {
            return;
        };
    ...
    pub(crate) fn ensure_constructor_field_initialized(
        ...
        // skip when not in a constructor body
        let Some(state) = self.constructor_state.as_ref() else {
            return Ok(());
        };
        ...
        let field_name = self.strings.get(field_name).to_string();
    ...
    pub(crate) fn ensure_constructor_complete(
        ...
        let message = if let Some(name) = missing_field {
            let field_name = self.strings.get(name).to_string();
