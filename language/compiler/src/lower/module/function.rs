...
use crate::lower::{
    FunctionDeclaration, FunctionDefinition, GenericInstanceKey, LifetimeParameters, LowerState,
...
impl LowerState<'_> {
    ...
    pub(in crate::lower) fn declare_function(
        ...
        // lower the signature and check its arity against the parameters
    ...
    pub(in crate::lower) fn instance_bindings(
        ...
        // select each argument the specialization binds
        let mut bindings = Vec::new();
        for binding in generic_arguments {
            // read the parameter the binding names
            ...
            // skip region parameters and keep every other binding
    ...
    fn template_has_instance_parameters(
        ...
        // scan the template parameters for one demanding an instance
        let generics = &self.state(template.module_id)?.generics;
        let template = generics.get_template(template.local_id);
        let declares_instance_parameter = template
            .parameters
            .iter()
            .any(|parameter| generics.get_parameter(*parameter).is_instance_parameter());

        Ok(declares_instance_parameter)
    }
    ...
    pub(in crate::lower) fn declare_method(
        ...
        // read the declared type and the lifetime generics its signature induces
        ...
        let result = match role {
            Some(dir::FunctionRole::Constructor) => tree.intern_type(mir::Type::Void),
            _ => result,
        };
