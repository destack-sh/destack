...
impl LowerState<'_> {
    ...
    fn decorated_language_item(
        ...
        // read the declaration behind the symbol
        ...
        for application in state.decorators.applications_for_owner(node) {
            ...
            let Some(key) = argument.as_string() else {
                return Err(CompilerError::Internal {
                    message: "a non-string language item key".to_string(),
                });
            };
    ...
    pub(in crate::lower) fn index_language_items(&mut self) -> CompilerResult<()> {
        ...
        for module in modules {
            ...
            for symbol in symbols {
                ...
                // keep the first loaded module's declaration for each item
        ...
        T: mir::Node,
    {
        ...
        if let Some(item) = item {
            ...
            {
                return Err(CompilerError::Internal {
                    message: format!("two language items on the MIR declaration {node:?}"),
                });
            }
        }
        ...
        if let Some(member) = declared {
            ...
            {
                return Err(CompilerError::Internal {
                    message: format!("two language members on the MIR declaration {node:?}"),
                });
            }
        }
    ...
    fn implemented_language_members(
        ...
        // follow every declaration reachable through conformances and overrides
        ...
        while let Some(implementation) = pending.pop() {
            ...
            // record the members each reached declaration declares
