...
impl ModuleLowerer<'_> {
    ...
    /// Ensure the takePlatformError binding is declared for ABI lowering.
    pub(crate) fn ensure_take_platform_error_function(
        &mut self,
        expression_id: LocalNodeId<Expression>,
        ...
        if let Some(function_id) = self.functions_by_symbol.get(&symbol) {
    ...
    fn take_platform_error_symbol(
        ...
        let symbol = self
            .compiler
            .get_declared_lib_symbol_for_space_order(
                self.profile,
                name,
