...
entry:
    ...
    v8: Vector = call test.main.Vector.add(v6, v7): (ref<Vector, borrowed, lifetime(0), exclusive>, Vector) => Vector
    local.set l2, v8
...
entry:
    ...
    v3: Charge = call test.main.Charge.negate(v2): (ref<Charge, borrowed, lifetime(0), exclusive>) => Charge
    local.set l1, v3
