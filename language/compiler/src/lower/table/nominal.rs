...
impl ModuleLowerer<'_> {
    ...
    pub(crate) fn lower_type(
        ...
        self.predeclare_nominal_layouts_for_type(type_id, &mut visited)?;
    ...
    fn predeclare_nominal_layouts_for_type(
        &mut self,
        type_id: dir::LocalTypeId,
        ...
        match self.types.get_type(type_id) {
            ...
            dir::Type::Value { value } => {
                self.predeclare_nominal_layouts_for_type(*value, visited)?;
            }
            dir::Type::Conditional {
                ...
                self.predeclare_nominal_layouts_for_type(*left, visited)?;
                self.predeclare_nominal_layouts_for_type(*right, visited)?;
                self.predeclare_nominal_layouts_for_type(*then_type, visited)?;
                self.predeclare_nominal_layouts_for_type(*else_type, visited)?;
            }
            dir::Type::Mapped {
                parameter,
                modifiers,
                ...
                let _ = modifiers;
                self.predeclare_nominal_layouts_for_type(parameter.constraint, visited)?;
                if let Some(key_remap) = parameter.key_remap {
                    self.predeclare_nominal_layouts_for_type(key_remap, visited)?;
                }
                self.predeclare_nominal_layouts_for_type(*value, visited)?;
            }
            dir::Type::Index { left, index } => {
                self.predeclare_nominal_layouts_for_type(*left, visited)?;
                self.predeclare_nominal_layouts_for_type(*index, visited)?;
            }
            dir::Type::TemplateLiteral { spans, .. } => {
                for span in spans {
                    self.predeclare_nominal_layouts_for_type(*span, visited)?;
                }
            }
            dir::Type::Import { .. } => {}
            dir::Type::Infer { constraint, .. } => {
                if let Some(constraint) = constraint {
                    self.predeclare_nominal_layouts_for_type(*constraint, visited)?;
                }
            }
            dir::Type::Predicate { target, .. } => {
                if let Some(target) = target {
                    self.predeclare_nominal_layouts_for_type(*target, visited)?;
                }
            }
            ...
            dir::Type::ArraySized { element, .. } => {
                self.predeclare_nominal_layouts_for_type(*element, visited)?;
            }
            ...
            dir::Type::Tuple { elements, .. } => {
                for element in elements {
                    self.predeclare_nominal_layouts_for_type(element.ty, visited)?;
                }
            }
            dir::Type::Object {
                fields,
                ...
                for field in fields {
                    self.predeclare_nominal_layouts_for_type(field.ty, visited)?;
                }
                for signature in call_signatures {
                    self.predeclare_nominal_layouts_for_type(*signature, visited)?;
                }
                for signature in construct_signatures {
                    self.predeclare_nominal_layouts_for_type(*signature, visited)?;
                }
                for signature in index_signatures {
                    self.predeclare_nominal_layouts_for_type(signature.key_type, visited)?;
                    self.predeclare_nominal_layouts_for_type(signature.value_type, visited)?;
                }
            }
            dir::Type::Function {
                static_parameters,
                ...
                for parameter in static_parameters {
                    self.predeclare_nominal_layouts_for_type(*parameter, visited)?;
                }
                if let Some(this_parameter) = this_parameter {
                    self.predeclare_nominal_layouts_for_type(*this_parameter, visited)?;
                }
                for parameter in dynamic_parameters {
                    self.predeclare_nominal_layouts_for_type(*parameter, visited)?;
                }
                if let Some(return_type) = return_type {
                    self.predeclare_nominal_layouts_for_type(*return_type, visited)?;
                }
            }
