...
pub(crate) fn compute_type_layout(
    ...
    match mir_type {
        ...
        // recovered error types do not have executable layouts
        mir::Type::Error => Err(CodegenCraneliftError::unsupported_type(
            "error types do not have a runtime layout",
            type_id.into_any(),
        )),
        ...
        mir::Type::Slice {
            ...
            let (data, _length) =
                mir::slice_header_types(*kind, element.clone(), *access, space.clone());
            let data = tree
                .iter_nodes::<mir::Type>()
