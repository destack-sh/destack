...
resource "aws_iam_user" "host" {
  name = "destack-${var.env}-host"
}
resource "aws_iam_policy" "host_s3" {
  name        = "destack-${var.env}-${var.region}-host-s3"
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["python", "destack.py", "serve", "host", "0.0.0.0", "60061"]
