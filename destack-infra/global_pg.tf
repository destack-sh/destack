...
resource "aws_security_group" "global_pg_security_group" {
  name        = "destack-${var.env}-global-pg-security-group"
...
resource "aws_db_subnet_group" "global_pg_subnet_group" {
  name       = "destack-${var.env}-global-pg-subnet-group"
...
resource "aws_rds_cluster" "global_pg_primary" {
  cluster_identifier        = "destack-${var.env}-global-pg-primary"
  ...
  final_snapshot_identifier = "destack-${var.env}-global-pg-final-snapshot"
...
resource "aws_rds_cluster_instance" "global_pg_primary_instance" {
  count                      = 1
  identifier                 = "destack-${var.env}-global-pg-${count.index}"
