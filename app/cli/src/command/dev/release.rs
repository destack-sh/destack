...
pub struct ReleaseArgs {
    ...
    /// Artifact directory passed to app preflight recipes.
...
pub fn run(args: &ReleaseArgs) -> i32 {
    ...
    let app_publish_arguments = vec![OsString::from("app/publish"), publish_mode_argument.clone()];
    let app_publish_exit_code = run_just_command(&app_publish_arguments);
    if app_publish_exit_code != 0 {
        return app_publish_exit_code;
    }
