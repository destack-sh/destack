...
/**
 * Encodes the given object into a 'robust' proto-based JSON serialization where field names and enums are proto ids.
 * Fortunately, this is a lot simpler than the Python version because we're in JSON-land.
 *  */
export function toRobustJson(object: AnyStructData): any {
...
/** Decodes 'robust' proto JSON. See encode. */
export function fromRobustJson(value: any): AnyStructData {
      ...
      if (field.kind == "message") {
        if (field.repeat) {
          if (fieldValue.length == 0) {
            ...
            object[field.jsonName] = fieldValue.map(fromRobustJson);
          }
        } else {
          if (typeof fieldValue != "object" || !MESSAGE_TYPE_BY_OBJECT_TYPE[fieldValue["1"] as ObjectType]) {
            object[field.jsonName] = fieldValue; // not one of our objects
          } else {
            object[field.jsonName] = fromRobustJson(fieldValue);
          }
        }
      } else {
  ...
  return object as AnyStructData;
}
