...
<template>
  <div v-if="task" class="group/task flex flex-row items-start gap-x-2">
    ...
    <div class="w-full">
      ...
      <!-- Owner -->
      <button
        class="inline-flex flex-row items-center gap-x-1.5 rounded bg-white px-1.5 py-0.5 transition-colors duration-150 hover:bg-gray-100 data-[popover=true]:bg-gray-100"
        :class="[
          ...
          (e: MouseEvent) => {
            ...
            pushPopover({
              ...
              props: {
                valueType: makeType({
                  kind: TypeKind.NODE,
                  constraint: makeTypeConstraint({ nodeTypes: [NodeType.USER, NodeType.FLOW, NodeType.ROLE] }),
                }),
