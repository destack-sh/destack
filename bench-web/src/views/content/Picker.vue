...
const indices: Ref<Record<string, SearchIndex<any>>> = computed(() => {
  ...
  if (isEnumType(props.valueType?.benchType)) {
    ...
    indices["graph"] = graphIndex({ graph: pkgGraph, metatypes: [props.valueType.benchType] });
  } else {
...
<template>
  ...
  <div>
    ...
    <div :style="{ width: DEFAULT_WIDTH + 'px' }">
      <!-- Header -->
      <div class="flex w-full flex-row items-center border-b border-gray-200 px-3.5 py-2">
      ...
      >
        ...
        <ul v-if="results.length > 0" class="flex flex-col py-1">
          <template v-for="item in results" :key="item.id">
            ...
            <li
              ...
              role="menuitem"
              class="mx-1 mb-[1px] mr-1.5 mt-[2px] flex h-[28px] max-w-full flex-row items-center rounded-md border border-transparent px-2 hover:border-gray-400 hover:bg-primary-300 data-[active=true]:border-gray-400 data-[active=true]:bg-primary-300"
        ...
        <div v-if="results.length < resultsTotal" class="my-1 px-4 pb-2 text-gray-500">
