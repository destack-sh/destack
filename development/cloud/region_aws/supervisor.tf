# 
# Destack
# 

locals {
  destack_env_vars = {
    SERVICE_NAME  = "destack-destack"
    ENVIRONMENT   = var.env
    CLOUD         = var.cloud
    REGION        = var.region
    VERSION       = var.destack_version
    OTLP_ENDPOINT = local.otlp_endpoint
    TRACING       = 1
    LOG_LEVEL     = "DEBUG"
    LOG_MODE      = "JSON"
    USE_WAITLIST  = 1

    DESTACK_URL = local.destack_url
    HOST_MAP = join(",", flatten([
      for k, v in var.cell_provider : [
        format("%s=%s", k, v)
      ]
    ]))

    MACHINE_RUNTIME_IMAGE         = "ghcr.io/symbolx/destack-machine-runtime"
    MACHINE_UBUNTU_DESKTOP_IMAGE  = "ghcr.io/symbolx/destack-machine-ubuntu-desktop"
    MACHINE_UBUNTU_TERMINAL_IMAGE = "ghcr.io/symbolx/destack-machine-ubuntu-terminal"
    MACHINE_GRPC_PORT             = 5432
    MACHINE_VNC_PORT              = 6080
  }
  destack_secret_env_vars = {
    "${kubernetes_secret.db_secret.metadata[0].name}" = [
      "GLOBAL_DATABASE_URL",
      "REGIONAL_PG_MAP"
    ]
    "${kubernetes_secret.external_secret.metadata[0].name}" = [
      "NEON_API_KEY",
      "NEON_BASE_URL",
      "POSTHOG_TOKEN",
      "POSTHOG_HOST",
    ]
    "${kubernetes_secret.destack_s3_secret.metadata[0].name}" = [
      "S3_REGION",
      "S3_ENDPOINT",
      "S3_ACCESS_KEY",
      "S3_SECRET_KEY"
    ]
  }
}

# destack s3 access
resource "kubernetes_secret" "destack_s3_secret" {
  metadata {
    name = "${local.prefix}-destack-s3-credentials"
  }

  data = {
    S3_REGION     = aws_s3_bucket.destack_files.region
    S3_ENDPOINT   = "https://s3.${aws_s3_bucket.destack_files.region}.amazonaws.com"
    S3_ACCESS_KEY = aws_iam_access_key.destack.id
    S3_SECRET_KEY = aws_iam_access_key.destack.secret
  }
}

# s3 access (to everything)
resource "aws_iam_user" "destack" {
  name = "destack-${var.env}-${var.region}-destack"
}
resource "aws_iam_policy" "destack_s3" {
  name        = "destack-${var.env}-${var.region}-destack-s3"
  description = "Allow access to S3 buckets for destack"

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Effect = "Allow"
        Action = [
          "s3:GetObject",
          "s3:ListBucket",
          "s3:PutObject",
          "s3:DeleteObject",
        ]
        Resource = [
          "arn:aws:s3:::destack-${var.env}*",
          "arn:aws:s3:::destack-${var.env}*/*"
        ]
      }
    ]
  })
}
resource "aws_iam_user_policy_attachment" "destack_s3" {
  user       = aws_iam_user.destack.name
  policy_arn = aws_iam_policy.destack_s3.arn
}
resource "aws_iam_access_key" "destack" {
  user = aws_iam_user.destack.name
}

# Destack deployment
resource "kubernetes_deployment" "destack" {
  metadata {
    name      = "${local.prefix}-destack"
    namespace = "default"
    labels = {
      app     = "destack-destack"
      env     = var.env
      cloud   = var.cloud
      region  = var.region
      version = var.destack_version
    }
  }

  spec {
    replicas = 1

    selector {
      match_labels = {
        app    = "destack-destack"
        env    = var.env
        cloud  = var.cloud
        region = var.region
      }
    }

    template {
      metadata {
        labels = {
          app     = "destack-destack"
          env     = var.env
          cloud   = var.cloud
          region  = var.region
          version = var.destack_version
        }
        annotations = {
          "prometheus.io/scrape" = "true"
        }
      }

      spec {
        # init container
        init_container {
          name    = "destack-init"
          image   = "ghcr.io/symbolx/destack-system:${var.destack_version}"
          command = ["/bin/sh", "-c"]
          # NOTE: migrating & bootstrapping in destack is scary if :MultiRegion
          args = [
            <<-EOT
              set -e
              python destack.py migrate apply --area global
              python destack.py system bootstrap --region ${var.region} --upsert
              python destack.py migrate apply --area regional --region ${var.region}
            EOT
          ]

          dynamic "env" {
            for_each = local.destack_env_vars
            content {
              name  = env.key
              value = env.value
            }
          }
          dynamic "env" {
            for_each = flatten([
              for secret_name, env_vars in local.destack_secret_env_vars : [
                for env_var in env_vars : {
                  name   = env_var
                  secret = secret_name
                }
              ]
            ])
            content {
              name = env.value.name
              value_from {
                secret_key_ref {
                  name = env.value.secret
                  key  = env.value.name
                }
              }
            }
          }
        }

        # main container
        container {
          name  = "destack"
          image = "ghcr.io/symbolx/destack-system:${var.destack_version}"

          port {
            container_port = 60061
            name           = "grpc"
          }

          dynamic "env" {
            for_each = local.destack_env_vars
            content {
              name  = env.key
              value = env.value
            }
          }
          dynamic "env" {
            for_each = flatten([
              for secret_name, env_vars in local.destack_secret_env_vars : [
                for env_var in env_vars : {
                  name   = env_var
                  secret = secret_name
                }
              ]
            ])
            content {
              name = env.value.name
              value_from {
                secret_key_ref {
                  name = env.value.secret
                  key  = env.value.name
                }
              }
            }
          }

          command = ["python", "destack.py", "serve", "destack", "0.0.0.0", "60061"]

          resources {
            requests = {
              cpu    = "500m"
              memory = "500Mi"
            }
          }
        }

        image_pull_secrets {
          name = kubernetes_secret.image_pull_secret.metadata[0].name
        }
      }
    }
  }
}

# Destack service
resource "kubernetes_service" "destack" {
  metadata {
    name = "${local.prefix}-destack"
  }

  spec {
    selector = {
      app = "destack-destack"
    }

    port {
      port        = 60061
      target_port = 60061
      name        = "grpc"
    }

    type = "NodePort"
  }
}

# 
# Envoy proxy
#


# Envoy ConfigMap
resource "kubernetes_config_map" "destack_envoy_config" {
  metadata {
    name = "${local.prefix}-destack-envoy-config"
  }

  data = {
    "envoy.yaml" = <<-EOT
      static_resources:
        listeners:
        - name: grpc_web_listener
          address:
            socket_address: { address: 0.0.0.0, port_value: 8080 }
          filter_chains:
            - filters:
              - name: envoy.filters.network.http_connection_manager
                typed_config:
                  "@type": type.googleapis.com/envoy.extensions.filters.network.http_connection_manager.v3.HttpConnectionManager
                  codec_type: auto
                  stat_prefix: ingress_http
                  stream_idle_timeout: 0s
                  route_config:
                    name: local_route
                    virtual_hosts:
                    - name: local_service
                      domains: ["*"]
                      routes:
                      - match: { prefix: "/" }
                        route:
                          cluster: universe_service
                          timeout: 0s
                          max_stream_duration:
                            grpc_timeout_header_max: 0s
                      cors:
                        allow_origin_string_match:
                          - prefix: "*"
                        allow_methods: GET, PUT, DELETE, POST, OPTIONS
                        allow_headers: keep-alive,user-agent,cache-control,content-type,content-transfer-encoding,x-accept-content-transfer-encoding,x-accept-response-streaming,x-user-agent,grpc-web,x-grpc-web,grpc-timeout,grpc-timeout,x-destack-1,x-destack-2,x-destack-3,x-destack-4,x-destack-5,x-destack-6,x-destack-7,x-destack-8,x-destack-9
                        max_age: "1728000"
                        expose_headers: content-type,grpc-status,grpc-message,grpc-web,x-grpc-web,x-destack-1,x-destack-2,x-destack-3,x-destack-4,x-destack-5,x-destack-6,x-destack-7,x-destack-8,x-destack-9
                  http_filters:
                    - name: envoy.filters.http.grpc_web
                      typed_config:
                        "@type": type.googleapis.com/envoy.extensions.filters.http.grpc_web.v3.GrpcWeb
                    - name: envoy.filters.http.cors
                      typed_config:
                        "@type": type.googleapis.com/envoy.extensions.filters.http.cors.v3.Cors
                    - name: envoy.filters.http.router
                      typed_config:
                        "@type": type.googleapis.com/envoy.extensions.filters.http.router.v3.Router
                  access_log:
                    - name: envoy.access_loggers.stdout
                      typed_config:
                        "@type": type.googleapis.com/envoy.extensions.access_loggers.stream.v3.StdoutAccessLog
                        log_format:
                          text_format: "[%START_TIME%] \"%REQ(:METHOD)% %REQ(X-ENVOY-ORIGINAL-PATH?:PATH)% %PROTOCOL%\" %RESPONSE_CODE% %RESPONSE_FLAGS% %BYTES_RECEIVED% %BYTES_SENT% %DURATION% %RESP(X-ENVOY-UPSTREAM-SERVICE-TIME)% \"%REQ(X-FORWARDED-FOR)%\" \"%REQ(USER-AGENT)%\" \"%REQ(X-REQUEST-ID)%\" \"%REQ(:AUTHORITY)%\" \"%UPSTREAM_HOST%\"\n"
              # tls to enable h2
              transport_socket:
                name: envoy.tls_context.http3
                typed_config:
                  "@type": type.googleapis.com/envoy.extensions.transport_sockets.tls.v3.DownstreamTlsContext
                  common_tls_context:
                    alpn_protocols: ["h2"]
                    tls_certificates:
                      - certificate_chain:
                          filename: /etc/envoy/tls/tls.crt
                        private_key:
                          filename: /etc/envoy/tls/tls.key
        - name: grpc_listener
          address:
            socket_address: { address: 0.0.0.0, port_value: 60061 }
          filter_chains:
          - filters:
            - name: envoy.filters.network.http_connection_manager
              typed_config:
                "@type": type.googleapis.com/envoy.extensions.filters.network.http_connection_manager.v3.HttpConnectionManager
                codec_type: auto
                stat_prefix: grpc_json
                route_config:
                  name: local_route
                  virtual_hosts:
                  - name: local_service
                    domains: ["*"]
                    routes:
                    - match: { prefix: "/" }
                      route:
                        cluster: universe_service
                        timeout: 0s
                http_filters:
                - name: envoy.filters.http.router
                  typed_config:
                    "@type": type.googleapis.com/envoy.extensions.filters.http.router.v3.Router
                access_log:
                - name: envoy.access_loggers.stdout
                  typed_config:
                    "@type": type.googleapis.com/envoy.extensions.access_loggers.stream.v3.StdoutAccessLog
                    log_format:
                      text_format: "[%START_TIME%] \"%REQ(:METHOD)% %REQ(X-ENVOY-ORIGIN)% %REQ(:AUTHORITY)% %UPSTREAM_HOST%\" %RESP(STATUS)% %BYTES_RECEIVED% %BYTES_SENT% %DURATION% %RESP(X-ENVOY-UPSTREAM-SERVICE-TIME)% \"%REQ(X-FORWARDED-FOR)%\" \"%REQ(USER-AGENT)%\" \"%REQ(X-REQUEST-ID)%\" \"%REQ(:AUTHORITY)%\" \"%UPSTREAM_HOST%\"\n"
            transport_socket:
              name: envoy.transport_sockets.tls
              typed_config:
                "@type": type.googleapis.com/envoy.extensions.transport_sockets.tls.v3.DownstreamTlsContext
                common_tls_context:
                  alpn_protocols: ["h2"]
                  tls_certificates:
                    - certificate_chain:
                        filename: /etc/envoy/tls/tls.crt
                      private_key:
                        filename: /etc/envoy/tls/tls.key
        clusters:
          - name: universe_service
            connect_timeout: 0.25s
            type: logical_dns
            http2_protocol_options: {}
            lb_policy: round_robin
            load_assignment:
              cluster_name: cluster_0
              endpoints:
                - lb_endpoints:
                    - endpoint:
                        address:
                          socket_address:
                            address: ${local.prefix}-destack
                            port_value: 60061
    EOT
  }
}

# Envoy Deployment
resource "kubernetes_deployment" "destack_proxy" {
  metadata {
    name = "${local.prefix}-destack-proxy"
    labels = {
      app     = "destack-destack-proxy"
      env     = var.env
      cloud   = var.cloud
      region  = var.region
      version = var.destack_version
    }
  }

  spec {
    replicas = 1

    selector {
      match_labels = {
        app    = "destack-destack-proxy"
        env    = var.env
        cloud  = var.cloud
        region = var.region
      }
    }

    template {
      metadata {
        labels = {
          app     = "destack-destack-proxy"
          env     = var.env
          cloud   = var.cloud
          region  = var.region
          version = var.destack_version
        }
      }

      spec {
        container {
          image = "envoyproxy/envoy:v1.31.0"
          name  = "envoy"

          port {
            container_port = 8080
          }
          port {
            container_port = 60061
          }

          volume_mount {
            name       = "envoy-config"
            mount_path = "/etc/envoy"
            read_only  = true
          }

          volume_mount {
            name       = "envoy-cert"
            mount_path = "/etc/envoy/tls"
            read_only  = true
          }
        }

        volume {
          name = "envoy-config"
          config_map {
            name = kubernetes_config_map.destack_envoy_config.metadata[0].name
          }
        }

        volume {
          name = "envoy-cert"
          secret {
            secret_name = kubernetes_secret.web_certificate_secret.metadata[0].name
            items {
              key  = "tls.crt"
              path = "tls.crt"
            }
            items {
              key  = "tls.key"
              path = "tls.key"
            }
          }
        }
      }
    }
  }
}

# Envoy Service
resource "kubernetes_service" "destack_proxy" {
  metadata {
    name = "${local.prefix}-destack-proxy"
    annotations = {
      "service.beta.kubernetes.io/aws-load-balancer-type"                            = "nlb"
      "service.beta.kubernetes.io/aws-load-balancer-nlb-target-type"                 = "ip"
      "service.beta.kubernetes.io/aws-load-balancer-scheme"                          = "internet-facing"
      "service.beta.kubernetes.io/aws-load-balancer-backend-protocol"                = "ssl"
      "service.beta.kubernetes.io/aws-load-balancer-name"                            = "${local.prefix}-nlb"
      "service.beta.kubernetes.io/aws-load-balancer-healthcheck-protocol"            = "TCP"
      "service.beta.kubernetes.io/aws-load-balancer-healthcheck-healthy-threshold"   = "2"
      "service.beta.kubernetes.io/aws-load-balancer-healthcheck-unhealthy-threshold" = "2"
      "service.beta.kubernetes.io/aws-load-balancer-healthcheck-interval"            = "10"
      "service.beta.kubernetes.io/aws-load-balancer-healthcheck-timeout"             = "5"
    }
  }

  spec {
    selector = {
      app = "destack-destack-proxy"
    }

    port {
      name        = "grpc-web"
      port        = 443
      target_port = 8080
    }

    port {
      name        = "grpc"
      port        = 60061
      target_port = 60061
    }

    type = "LoadBalancer"
  }
}

data "kubernetes_service" "destack_proxy" {
  metadata {
    name = kubernetes_service.destack_proxy.metadata[0].name
  }

  depends_on = [kubernetes_service.destack_proxy]
}

output "destack_hostname" {
  value       = data.kubernetes_service.destack_proxy.status.0.load_balancer.0.ingress.0.hostname
  description = "The public hostname of the Destack service (ingress)"
}

# point 'destack' for this region to the destack ingress
resource "cloudflare_record" "destack" {
  zone_id         = var.web_zone_id
  name            = "${var.cloud}-${var.region}.destack"
  type            = "CNAME"
  content         = data.kubernetes_service.destack_proxy.status.0.load_balancer.0.ingress.0.hostname
  ttl             = 300
  proxied         = false
  allow_overwrite = true
}
